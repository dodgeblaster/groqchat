import { createRequire } from 'node:module'
const require = createRequire(import.meta.url)

var vl = Object.create
var Rd = Object.defineProperty
var bl = Object.getOwnPropertyDescriptor
var wl = Object.getOwnPropertyNames
var _l = Object.getPrototypeOf,
    yl = Object.prototype.hasOwnProperty
var Ae = ((e) =>
    typeof require < 'u'
        ? require
        : typeof Proxy < 'u'
        ? new Proxy(e, {
              get: (a, t) => (typeof require < 'u' ? require : a)[t]
          })
        : e)(function (e) {
    if (typeof require < 'u') return require.apply(this, arguments)
    throw Error('Dynamic require of "' + e + '" is not supported')
})
var x1 = (e, a) => () => (e && (a = e((e = 0))), a)
var se = (e, a) => () => (a || e((a = { exports: {} }).exports, a), a.exports),
    n0 = (e, a) => {
        for (var t in a) Rd(e, t, { get: a[t], enumerable: !0 })
    },
    gl = (e, a, t, r) => {
        if ((a && typeof a == 'object') || typeof a == 'function')
            for (let i of wl(a))
                !yl.call(e, i) &&
                    i !== t &&
                    Rd(e, i, {
                        get: () => a[i],
                        enumerable: !(r = bl(a, i)) || r.enumerable
                    })
        return e
    }
var Et = (e, a, t) => (
    (t = e != null ? vl(_l(e)) : {}),
    gl(
        a || !e || !e.__esModule
            ? Rd(t, 'default', { value: e, enumerable: !0 })
            : t,
        e
    )
)
var u0 = se((s3, m0) => {
    'use strict'
    var ae = {}
    m0.exports = ae
    function s0(e) {
        return e < 0 ? -1 : 1
    }
    function Pl(e) {
        return e % 1 === 0.5 && !(e & 1) ? Math.floor(e) : Math.round(e)
    }
    function L1(e, a) {
        a.unsigned || --e
        let t = a.unsigned ? 0 : -Math.pow(2, e),
            r = Math.pow(2, e) - 1,
            i = a.moduloBitLength
                ? Math.pow(2, a.moduloBitLength)
                : Math.pow(2, e),
            o = a.moduloBitLength
                ? Math.pow(2, a.moduloBitLength - 1)
                : Math.pow(2, e - 1)
        return function (m, u) {
            u || (u = {})
            let s = +m
            if (u.enforceRange) {
                if (!Number.isFinite(s))
                    throw new TypeError('Argument is not a finite number')
                if (((s = s0(s) * Math.floor(Math.abs(s))), s < t || s > r))
                    throw new TypeError('Argument is not in byte range')
                return s
            }
            if (!isNaN(s) && u.clamp)
                return (s = Pl(s)), s < t && (s = t), s > r && (s = r), s
            if (!Number.isFinite(s) || s === 0) return 0
            if (
                ((s = s0(s) * Math.floor(Math.abs(s))),
                (s = s % i),
                !a.unsigned && s >= o)
            )
                return s - i
            if (a.unsigned) {
                if (s < 0) s += i
                else if (s === -0) return 0
            }
            return s
        }
    }
    ae.void = function () {}
    ae.boolean = function (e) {
        return !!e
    }
    ae.byte = L1(8, { unsigned: !1 })
    ae.octet = L1(8, { unsigned: !0 })
    ae.short = L1(16, { unsigned: !1 })
    ae['unsigned short'] = L1(16, { unsigned: !0 })
    ae.long = L1(32, { unsigned: !1 })
    ae['unsigned long'] = L1(32, { unsigned: !0 })
    ae['long long'] = L1(32, { unsigned: !1, moduloBitLength: 64 })
    ae['unsigned long long'] = L1(32, { unsigned: !0, moduloBitLength: 64 })
    ae.double = function (e) {
        let a = +e
        if (!Number.isFinite(a))
            throw new TypeError('Argument is not a finite floating-point value')
        return a
    }
    ae['unrestricted double'] = function (e) {
        let a = +e
        if (isNaN(a)) throw new TypeError('Argument is NaN')
        return a
    }
    ae.float = ae.double
    ae['unrestricted float'] = ae['unrestricted double']
    ae.DOMString = function (e, a) {
        return (
            a || (a = {}),
            a.treatNullAsEmptyString && e === null ? '' : String(e)
        )
    }
    ae.ByteString = function (e, a) {
        let t = String(e),
            r
        for (let i = 0; (r = t.codePointAt(i)) !== void 0; ++i)
            if (r > 255)
                throw new TypeError('Argument is not a valid bytestring')
        return t
    }
    ae.USVString = function (e) {
        let a = String(e),
            t = a.length,
            r = []
        for (let i = 0; i < t; ++i) {
            let o = a.charCodeAt(i)
            if (o < 55296 || o > 57343) r.push(String.fromCodePoint(o))
            else if (56320 <= o && o <= 57343)
                r.push(String.fromCodePoint(65533))
            else if (i === t - 1) r.push(String.fromCodePoint(65533))
            else {
                let m = a.charCodeAt(i + 1)
                if (56320 <= m && m <= 57343) {
                    let u = o & 1023,
                        s = m & 1023
                    r.push(String.fromCodePoint(65536 + 1024 * u + s)), ++i
                } else r.push(String.fromCodePoint(65533))
            }
        }
        return r.join('')
    }
    ae.Date = function (e, a) {
        if (!(e instanceof Date))
            throw new TypeError('Argument is not a Date object')
        if (!isNaN(e)) return e
    }
    ae.RegExp = function (e, a) {
        return e instanceof RegExp || (e = new RegExp(e)), e
    }
})
var c0 = se((m3, I1) => {
    'use strict'
    I1.exports.mixin = function (a, t) {
        let r = Object.getOwnPropertyNames(t)
        for (let i = 0; i < r.length; ++i)
            Object.defineProperty(
                a,
                r[i],
                Object.getOwnPropertyDescriptor(t, r[i])
            )
    }
    I1.exports.wrapperSymbol = Symbol('wrapper')
    I1.exports.implSymbol = Symbol('impl')
    I1.exports.wrapperForImpl = function (e) {
        return e[I1.exports.wrapperSymbol]
    }
    I1.exports.implForWrapper = function (e) {
        return e[I1.exports.implSymbol]
    }
})
var f0 = se((u3, Nl) => {
    Nl.exports = [
        [[0, 44], 'disallowed_STD3_valid'],
        [[45, 46], 'valid'],
        [[47, 47], 'disallowed_STD3_valid'],
        [[48, 57], 'valid'],
        [[58, 64], 'disallowed_STD3_valid'],
        [[65, 65], 'mapped', [97]],
        [[66, 66], 'mapped', [98]],
        [[67, 67], 'mapped', [99]],
        [[68, 68], 'mapped', [100]],
        [[69, 69], 'mapped', [101]],
        [[70, 70], 'mapped', [102]],
        [[71, 71], 'mapped', [103]],
        [[72, 72], 'mapped', [104]],
        [[73, 73], 'mapped', [105]],
        [[74, 74], 'mapped', [106]],
        [[75, 75], 'mapped', [107]],
        [[76, 76], 'mapped', [108]],
        [[77, 77], 'mapped', [109]],
        [[78, 78], 'mapped', [110]],
        [[79, 79], 'mapped', [111]],
        [[80, 80], 'mapped', [112]],
        [[81, 81], 'mapped', [113]],
        [[82, 82], 'mapped', [114]],
        [[83, 83], 'mapped', [115]],
        [[84, 84], 'mapped', [116]],
        [[85, 85], 'mapped', [117]],
        [[86, 86], 'mapped', [118]],
        [[87, 87], 'mapped', [119]],
        [[88, 88], 'mapped', [120]],
        [[89, 89], 'mapped', [121]],
        [[90, 90], 'mapped', [122]],
        [[91, 96], 'disallowed_STD3_valid'],
        [[97, 122], 'valid'],
        [[123, 127], 'disallowed_STD3_valid'],
        [[128, 159], 'disallowed'],
        [[160, 160], 'disallowed_STD3_mapped', [32]],
        [[161, 167], 'valid', [], 'NV8'],
        [[168, 168], 'disallowed_STD3_mapped', [32, 776]],
        [[169, 169], 'valid', [], 'NV8'],
        [[170, 170], 'mapped', [97]],
        [[171, 172], 'valid', [], 'NV8'],
        [[173, 173], 'ignored'],
        [[174, 174], 'valid', [], 'NV8'],
        [[175, 175], 'disallowed_STD3_mapped', [32, 772]],
        [[176, 177], 'valid', [], 'NV8'],
        [[178, 178], 'mapped', [50]],
        [[179, 179], 'mapped', [51]],
        [[180, 180], 'disallowed_STD3_mapped', [32, 769]],
        [[181, 181], 'mapped', [956]],
        [[182, 182], 'valid', [], 'NV8'],
        [[183, 183], 'valid'],
        [[184, 184], 'disallowed_STD3_mapped', [32, 807]],
        [[185, 185], 'mapped', [49]],
        [[186, 186], 'mapped', [111]],
        [[187, 187], 'valid', [], 'NV8'],
        [[188, 188], 'mapped', [49, 8260, 52]],
        [[189, 189], 'mapped', [49, 8260, 50]],
        [[190, 190], 'mapped', [51, 8260, 52]],
        [[191, 191], 'valid', [], 'NV8'],
        [[192, 192], 'mapped', [224]],
        [[193, 193], 'mapped', [225]],
        [[194, 194], 'mapped', [226]],
        [[195, 195], 'mapped', [227]],
        [[196, 196], 'mapped', [228]],
        [[197, 197], 'mapped', [229]],
        [[198, 198], 'mapped', [230]],
        [[199, 199], 'mapped', [231]],
        [[200, 200], 'mapped', [232]],
        [[201, 201], 'mapped', [233]],
        [[202, 202], 'mapped', [234]],
        [[203, 203], 'mapped', [235]],
        [[204, 204], 'mapped', [236]],
        [[205, 205], 'mapped', [237]],
        [[206, 206], 'mapped', [238]],
        [[207, 207], 'mapped', [239]],
        [[208, 208], 'mapped', [240]],
        [[209, 209], 'mapped', [241]],
        [[210, 210], 'mapped', [242]],
        [[211, 211], 'mapped', [243]],
        [[212, 212], 'mapped', [244]],
        [[213, 213], 'mapped', [245]],
        [[214, 214], 'mapped', [246]],
        [[215, 215], 'valid', [], 'NV8'],
        [[216, 216], 'mapped', [248]],
        [[217, 217], 'mapped', [249]],
        [[218, 218], 'mapped', [250]],
        [[219, 219], 'mapped', [251]],
        [[220, 220], 'mapped', [252]],
        [[221, 221], 'mapped', [253]],
        [[222, 222], 'mapped', [254]],
        [[223, 223], 'deviation', [115, 115]],
        [[224, 246], 'valid'],
        [[247, 247], 'valid', [], 'NV8'],
        [[248, 255], 'valid'],
        [[256, 256], 'mapped', [257]],
        [[257, 257], 'valid'],
        [[258, 258], 'mapped', [259]],
        [[259, 259], 'valid'],
        [[260, 260], 'mapped', [261]],
        [[261, 261], 'valid'],
        [[262, 262], 'mapped', [263]],
        [[263, 263], 'valid'],
        [[264, 264], 'mapped', [265]],
        [[265, 265], 'valid'],
        [[266, 266], 'mapped', [267]],
        [[267, 267], 'valid'],
        [[268, 268], 'mapped', [269]],
        [[269, 269], 'valid'],
        [[270, 270], 'mapped', [271]],
        [[271, 271], 'valid'],
        [[272, 272], 'mapped', [273]],
        [[273, 273], 'valid'],
        [[274, 274], 'mapped', [275]],
        [[275, 275], 'valid'],
        [[276, 276], 'mapped', [277]],
        [[277, 277], 'valid'],
        [[278, 278], 'mapped', [279]],
        [[279, 279], 'valid'],
        [[280, 280], 'mapped', [281]],
        [[281, 281], 'valid'],
        [[282, 282], 'mapped', [283]],
        [[283, 283], 'valid'],
        [[284, 284], 'mapped', [285]],
        [[285, 285], 'valid'],
        [[286, 286], 'mapped', [287]],
        [[287, 287], 'valid'],
        [[288, 288], 'mapped', [289]],
        [[289, 289], 'valid'],
        [[290, 290], 'mapped', [291]],
        [[291, 291], 'valid'],
        [[292, 292], 'mapped', [293]],
        [[293, 293], 'valid'],
        [[294, 294], 'mapped', [295]],
        [[295, 295], 'valid'],
        [[296, 296], 'mapped', [297]],
        [[297, 297], 'valid'],
        [[298, 298], 'mapped', [299]],
        [[299, 299], 'valid'],
        [[300, 300], 'mapped', [301]],
        [[301, 301], 'valid'],
        [[302, 302], 'mapped', [303]],
        [[303, 303], 'valid'],
        [[304, 304], 'mapped', [105, 775]],
        [[305, 305], 'valid'],
        [[306, 307], 'mapped', [105, 106]],
        [[308, 308], 'mapped', [309]],
        [[309, 309], 'valid'],
        [[310, 310], 'mapped', [311]],
        [[311, 312], 'valid'],
        [[313, 313], 'mapped', [314]],
        [[314, 314], 'valid'],
        [[315, 315], 'mapped', [316]],
        [[316, 316], 'valid'],
        [[317, 317], 'mapped', [318]],
        [[318, 318], 'valid'],
        [[319, 320], 'mapped', [108, 183]],
        [[321, 321], 'mapped', [322]],
        [[322, 322], 'valid'],
        [[323, 323], 'mapped', [324]],
        [[324, 324], 'valid'],
        [[325, 325], 'mapped', [326]],
        [[326, 326], 'valid'],
        [[327, 327], 'mapped', [328]],
        [[328, 328], 'valid'],
        [[329, 329], 'mapped', [700, 110]],
        [[330, 330], 'mapped', [331]],
        [[331, 331], 'valid'],
        [[332, 332], 'mapped', [333]],
        [[333, 333], 'valid'],
        [[334, 334], 'mapped', [335]],
        [[335, 335], 'valid'],
        [[336, 336], 'mapped', [337]],
        [[337, 337], 'valid'],
        [[338, 338], 'mapped', [339]],
        [[339, 339], 'valid'],
        [[340, 340], 'mapped', [341]],
        [[341, 341], 'valid'],
        [[342, 342], 'mapped', [343]],
        [[343, 343], 'valid'],
        [[344, 344], 'mapped', [345]],
        [[345, 345], 'valid'],
        [[346, 346], 'mapped', [347]],
        [[347, 347], 'valid'],
        [[348, 348], 'mapped', [349]],
        [[349, 349], 'valid'],
        [[350, 350], 'mapped', [351]],
        [[351, 351], 'valid'],
        [[352, 352], 'mapped', [353]],
        [[353, 353], 'valid'],
        [[354, 354], 'mapped', [355]],
        [[355, 355], 'valid'],
        [[356, 356], 'mapped', [357]],
        [[357, 357], 'valid'],
        [[358, 358], 'mapped', [359]],
        [[359, 359], 'valid'],
        [[360, 360], 'mapped', [361]],
        [[361, 361], 'valid'],
        [[362, 362], 'mapped', [363]],
        [[363, 363], 'valid'],
        [[364, 364], 'mapped', [365]],
        [[365, 365], 'valid'],
        [[366, 366], 'mapped', [367]],
        [[367, 367], 'valid'],
        [[368, 368], 'mapped', [369]],
        [[369, 369], 'valid'],
        [[370, 370], 'mapped', [371]],
        [[371, 371], 'valid'],
        [[372, 372], 'mapped', [373]],
        [[373, 373], 'valid'],
        [[374, 374], 'mapped', [375]],
        [[375, 375], 'valid'],
        [[376, 376], 'mapped', [255]],
        [[377, 377], 'mapped', [378]],
        [[378, 378], 'valid'],
        [[379, 379], 'mapped', [380]],
        [[380, 380], 'valid'],
        [[381, 381], 'mapped', [382]],
        [[382, 382], 'valid'],
        [[383, 383], 'mapped', [115]],
        [[384, 384], 'valid'],
        [[385, 385], 'mapped', [595]],
        [[386, 386], 'mapped', [387]],
        [[387, 387], 'valid'],
        [[388, 388], 'mapped', [389]],
        [[389, 389], 'valid'],
        [[390, 390], 'mapped', [596]],
        [[391, 391], 'mapped', [392]],
        [[392, 392], 'valid'],
        [[393, 393], 'mapped', [598]],
        [[394, 394], 'mapped', [599]],
        [[395, 395], 'mapped', [396]],
        [[396, 397], 'valid'],
        [[398, 398], 'mapped', [477]],
        [[399, 399], 'mapped', [601]],
        [[400, 400], 'mapped', [603]],
        [[401, 401], 'mapped', [402]],
        [[402, 402], 'valid'],
        [[403, 403], 'mapped', [608]],
        [[404, 404], 'mapped', [611]],
        [[405, 405], 'valid'],
        [[406, 406], 'mapped', [617]],
        [[407, 407], 'mapped', [616]],
        [[408, 408], 'mapped', [409]],
        [[409, 411], 'valid'],
        [[412, 412], 'mapped', [623]],
        [[413, 413], 'mapped', [626]],
        [[414, 414], 'valid'],
        [[415, 415], 'mapped', [629]],
        [[416, 416], 'mapped', [417]],
        [[417, 417], 'valid'],
        [[418, 418], 'mapped', [419]],
        [[419, 419], 'valid'],
        [[420, 420], 'mapped', [421]],
        [[421, 421], 'valid'],
        [[422, 422], 'mapped', [640]],
        [[423, 423], 'mapped', [424]],
        [[424, 424], 'valid'],
        [[425, 425], 'mapped', [643]],
        [[426, 427], 'valid'],
        [[428, 428], 'mapped', [429]],
        [[429, 429], 'valid'],
        [[430, 430], 'mapped', [648]],
        [[431, 431], 'mapped', [432]],
        [[432, 432], 'valid'],
        [[433, 433], 'mapped', [650]],
        [[434, 434], 'mapped', [651]],
        [[435, 435], 'mapped', [436]],
        [[436, 436], 'valid'],
        [[437, 437], 'mapped', [438]],
        [[438, 438], 'valid'],
        [[439, 439], 'mapped', [658]],
        [[440, 440], 'mapped', [441]],
        [[441, 443], 'valid'],
        [[444, 444], 'mapped', [445]],
        [[445, 451], 'valid'],
        [[452, 454], 'mapped', [100, 382]],
        [[455, 457], 'mapped', [108, 106]],
        [[458, 460], 'mapped', [110, 106]],
        [[461, 461], 'mapped', [462]],
        [[462, 462], 'valid'],
        [[463, 463], 'mapped', [464]],
        [[464, 464], 'valid'],
        [[465, 465], 'mapped', [466]],
        [[466, 466], 'valid'],
        [[467, 467], 'mapped', [468]],
        [[468, 468], 'valid'],
        [[469, 469], 'mapped', [470]],
        [[470, 470], 'valid'],
        [[471, 471], 'mapped', [472]],
        [[472, 472], 'valid'],
        [[473, 473], 'mapped', [474]],
        [[474, 474], 'valid'],
        [[475, 475], 'mapped', [476]],
        [[476, 477], 'valid'],
        [[478, 478], 'mapped', [479]],
        [[479, 479], 'valid'],
        [[480, 480], 'mapped', [481]],
        [[481, 481], 'valid'],
        [[482, 482], 'mapped', [483]],
        [[483, 483], 'valid'],
        [[484, 484], 'mapped', [485]],
        [[485, 485], 'valid'],
        [[486, 486], 'mapped', [487]],
        [[487, 487], 'valid'],
        [[488, 488], 'mapped', [489]],
        [[489, 489], 'valid'],
        [[490, 490], 'mapped', [491]],
        [[491, 491], 'valid'],
        [[492, 492], 'mapped', [493]],
        [[493, 493], 'valid'],
        [[494, 494], 'mapped', [495]],
        [[495, 496], 'valid'],
        [[497, 499], 'mapped', [100, 122]],
        [[500, 500], 'mapped', [501]],
        [[501, 501], 'valid'],
        [[502, 502], 'mapped', [405]],
        [[503, 503], 'mapped', [447]],
        [[504, 504], 'mapped', [505]],
        [[505, 505], 'valid'],
        [[506, 506], 'mapped', [507]],
        [[507, 507], 'valid'],
        [[508, 508], 'mapped', [509]],
        [[509, 509], 'valid'],
        [[510, 510], 'mapped', [511]],
        [[511, 511], 'valid'],
        [[512, 512], 'mapped', [513]],
        [[513, 513], 'valid'],
        [[514, 514], 'mapped', [515]],
        [[515, 515], 'valid'],
        [[516, 516], 'mapped', [517]],
        [[517, 517], 'valid'],
        [[518, 518], 'mapped', [519]],
        [[519, 519], 'valid'],
        [[520, 520], 'mapped', [521]],
        [[521, 521], 'valid'],
        [[522, 522], 'mapped', [523]],
        [[523, 523], 'valid'],
        [[524, 524], 'mapped', [525]],
        [[525, 525], 'valid'],
        [[526, 526], 'mapped', [527]],
        [[527, 527], 'valid'],
        [[528, 528], 'mapped', [529]],
        [[529, 529], 'valid'],
        [[530, 530], 'mapped', [531]],
        [[531, 531], 'valid'],
        [[532, 532], 'mapped', [533]],
        [[533, 533], 'valid'],
        [[534, 534], 'mapped', [535]],
        [[535, 535], 'valid'],
        [[536, 536], 'mapped', [537]],
        [[537, 537], 'valid'],
        [[538, 538], 'mapped', [539]],
        [[539, 539], 'valid'],
        [[540, 540], 'mapped', [541]],
        [[541, 541], 'valid'],
        [[542, 542], 'mapped', [543]],
        [[543, 543], 'valid'],
        [[544, 544], 'mapped', [414]],
        [[545, 545], 'valid'],
        [[546, 546], 'mapped', [547]],
        [[547, 547], 'valid'],
        [[548, 548], 'mapped', [549]],
        [[549, 549], 'valid'],
        [[550, 550], 'mapped', [551]],
        [[551, 551], 'valid'],
        [[552, 552], 'mapped', [553]],
        [[553, 553], 'valid'],
        [[554, 554], 'mapped', [555]],
        [[555, 555], 'valid'],
        [[556, 556], 'mapped', [557]],
        [[557, 557], 'valid'],
        [[558, 558], 'mapped', [559]],
        [[559, 559], 'valid'],
        [[560, 560], 'mapped', [561]],
        [[561, 561], 'valid'],
        [[562, 562], 'mapped', [563]],
        [[563, 563], 'valid'],
        [[564, 566], 'valid'],
        [[567, 569], 'valid'],
        [[570, 570], 'mapped', [11365]],
        [[571, 571], 'mapped', [572]],
        [[572, 572], 'valid'],
        [[573, 573], 'mapped', [410]],
        [[574, 574], 'mapped', [11366]],
        [[575, 576], 'valid'],
        [[577, 577], 'mapped', [578]],
        [[578, 578], 'valid'],
        [[579, 579], 'mapped', [384]],
        [[580, 580], 'mapped', [649]],
        [[581, 581], 'mapped', [652]],
        [[582, 582], 'mapped', [583]],
        [[583, 583], 'valid'],
        [[584, 584], 'mapped', [585]],
        [[585, 585], 'valid'],
        [[586, 586], 'mapped', [587]],
        [[587, 587], 'valid'],
        [[588, 588], 'mapped', [589]],
        [[589, 589], 'valid'],
        [[590, 590], 'mapped', [591]],
        [[591, 591], 'valid'],
        [[592, 680], 'valid'],
        [[681, 685], 'valid'],
        [[686, 687], 'valid'],
        [[688, 688], 'mapped', [104]],
        [[689, 689], 'mapped', [614]],
        [[690, 690], 'mapped', [106]],
        [[691, 691], 'mapped', [114]],
        [[692, 692], 'mapped', [633]],
        [[693, 693], 'mapped', [635]],
        [[694, 694], 'mapped', [641]],
        [[695, 695], 'mapped', [119]],
        [[696, 696], 'mapped', [121]],
        [[697, 705], 'valid'],
        [[706, 709], 'valid', [], 'NV8'],
        [[710, 721], 'valid'],
        [[722, 727], 'valid', [], 'NV8'],
        [[728, 728], 'disallowed_STD3_mapped', [32, 774]],
        [[729, 729], 'disallowed_STD3_mapped', [32, 775]],
        [[730, 730], 'disallowed_STD3_mapped', [32, 778]],
        [[731, 731], 'disallowed_STD3_mapped', [32, 808]],
        [[732, 732], 'disallowed_STD3_mapped', [32, 771]],
        [[733, 733], 'disallowed_STD3_mapped', [32, 779]],
        [[734, 734], 'valid', [], 'NV8'],
        [[735, 735], 'valid', [], 'NV8'],
        [[736, 736], 'mapped', [611]],
        [[737, 737], 'mapped', [108]],
        [[738, 738], 'mapped', [115]],
        [[739, 739], 'mapped', [120]],
        [[740, 740], 'mapped', [661]],
        [[741, 745], 'valid', [], 'NV8'],
        [[746, 747], 'valid', [], 'NV8'],
        [[748, 748], 'valid'],
        [[749, 749], 'valid', [], 'NV8'],
        [[750, 750], 'valid'],
        [[751, 767], 'valid', [], 'NV8'],
        [[768, 831], 'valid'],
        [[832, 832], 'mapped', [768]],
        [[833, 833], 'mapped', [769]],
        [[834, 834], 'valid'],
        [[835, 835], 'mapped', [787]],
        [[836, 836], 'mapped', [776, 769]],
        [[837, 837], 'mapped', [953]],
        [[838, 846], 'valid'],
        [[847, 847], 'ignored'],
        [[848, 855], 'valid'],
        [[856, 860], 'valid'],
        [[861, 863], 'valid'],
        [[864, 865], 'valid'],
        [[866, 866], 'valid'],
        [[867, 879], 'valid'],
        [[880, 880], 'mapped', [881]],
        [[881, 881], 'valid'],
        [[882, 882], 'mapped', [883]],
        [[883, 883], 'valid'],
        [[884, 884], 'mapped', [697]],
        [[885, 885], 'valid'],
        [[886, 886], 'mapped', [887]],
        [[887, 887], 'valid'],
        [[888, 889], 'disallowed'],
        [[890, 890], 'disallowed_STD3_mapped', [32, 953]],
        [[891, 893], 'valid'],
        [[894, 894], 'disallowed_STD3_mapped', [59]],
        [[895, 895], 'mapped', [1011]],
        [[896, 899], 'disallowed'],
        [[900, 900], 'disallowed_STD3_mapped', [32, 769]],
        [[901, 901], 'disallowed_STD3_mapped', [32, 776, 769]],
        [[902, 902], 'mapped', [940]],
        [[903, 903], 'mapped', [183]],
        [[904, 904], 'mapped', [941]],
        [[905, 905], 'mapped', [942]],
        [[906, 906], 'mapped', [943]],
        [[907, 907], 'disallowed'],
        [[908, 908], 'mapped', [972]],
        [[909, 909], 'disallowed'],
        [[910, 910], 'mapped', [973]],
        [[911, 911], 'mapped', [974]],
        [[912, 912], 'valid'],
        [[913, 913], 'mapped', [945]],
        [[914, 914], 'mapped', [946]],
        [[915, 915], 'mapped', [947]],
        [[916, 916], 'mapped', [948]],
        [[917, 917], 'mapped', [949]],
        [[918, 918], 'mapped', [950]],
        [[919, 919], 'mapped', [951]],
        [[920, 920], 'mapped', [952]],
        [[921, 921], 'mapped', [953]],
        [[922, 922], 'mapped', [954]],
        [[923, 923], 'mapped', [955]],
        [[924, 924], 'mapped', [956]],
        [[925, 925], 'mapped', [957]],
        [[926, 926], 'mapped', [958]],
        [[927, 927], 'mapped', [959]],
        [[928, 928], 'mapped', [960]],
        [[929, 929], 'mapped', [961]],
        [[930, 930], 'disallowed'],
        [[931, 931], 'mapped', [963]],
        [[932, 932], 'mapped', [964]],
        [[933, 933], 'mapped', [965]],
        [[934, 934], 'mapped', [966]],
        [[935, 935], 'mapped', [967]],
        [[936, 936], 'mapped', [968]],
        [[937, 937], 'mapped', [969]],
        [[938, 938], 'mapped', [970]],
        [[939, 939], 'mapped', [971]],
        [[940, 961], 'valid'],
        [[962, 962], 'deviation', [963]],
        [[963, 974], 'valid'],
        [[975, 975], 'mapped', [983]],
        [[976, 976], 'mapped', [946]],
        [[977, 977], 'mapped', [952]],
        [[978, 978], 'mapped', [965]],
        [[979, 979], 'mapped', [973]],
        [[980, 980], 'mapped', [971]],
        [[981, 981], 'mapped', [966]],
        [[982, 982], 'mapped', [960]],
        [[983, 983], 'valid'],
        [[984, 984], 'mapped', [985]],
        [[985, 985], 'valid'],
        [[986, 986], 'mapped', [987]],
        [[987, 987], 'valid'],
        [[988, 988], 'mapped', [989]],
        [[989, 989], 'valid'],
        [[990, 990], 'mapped', [991]],
        [[991, 991], 'valid'],
        [[992, 992], 'mapped', [993]],
        [[993, 993], 'valid'],
        [[994, 994], 'mapped', [995]],
        [[995, 995], 'valid'],
        [[996, 996], 'mapped', [997]],
        [[997, 997], 'valid'],
        [[998, 998], 'mapped', [999]],
        [[999, 999], 'valid'],
        [[1e3, 1e3], 'mapped', [1001]],
        [[1001, 1001], 'valid'],
        [[1002, 1002], 'mapped', [1003]],
        [[1003, 1003], 'valid'],
        [[1004, 1004], 'mapped', [1005]],
        [[1005, 1005], 'valid'],
        [[1006, 1006], 'mapped', [1007]],
        [[1007, 1007], 'valid'],
        [[1008, 1008], 'mapped', [954]],
        [[1009, 1009], 'mapped', [961]],
        [[1010, 1010], 'mapped', [963]],
        [[1011, 1011], 'valid'],
        [[1012, 1012], 'mapped', [952]],
        [[1013, 1013], 'mapped', [949]],
        [[1014, 1014], 'valid', [], 'NV8'],
        [[1015, 1015], 'mapped', [1016]],
        [[1016, 1016], 'valid'],
        [[1017, 1017], 'mapped', [963]],
        [[1018, 1018], 'mapped', [1019]],
        [[1019, 1019], 'valid'],
        [[1020, 1020], 'valid'],
        [[1021, 1021], 'mapped', [891]],
        [[1022, 1022], 'mapped', [892]],
        [[1023, 1023], 'mapped', [893]],
        [[1024, 1024], 'mapped', [1104]],
        [[1025, 1025], 'mapped', [1105]],
        [[1026, 1026], 'mapped', [1106]],
        [[1027, 1027], 'mapped', [1107]],
        [[1028, 1028], 'mapped', [1108]],
        [[1029, 1029], 'mapped', [1109]],
        [[1030, 1030], 'mapped', [1110]],
        [[1031, 1031], 'mapped', [1111]],
        [[1032, 1032], 'mapped', [1112]],
        [[1033, 1033], 'mapped', [1113]],
        [[1034, 1034], 'mapped', [1114]],
        [[1035, 1035], 'mapped', [1115]],
        [[1036, 1036], 'mapped', [1116]],
        [[1037, 1037], 'mapped', [1117]],
        [[1038, 1038], 'mapped', [1118]],
        [[1039, 1039], 'mapped', [1119]],
        [[1040, 1040], 'mapped', [1072]],
        [[1041, 1041], 'mapped', [1073]],
        [[1042, 1042], 'mapped', [1074]],
        [[1043, 1043], 'mapped', [1075]],
        [[1044, 1044], 'mapped', [1076]],
        [[1045, 1045], 'mapped', [1077]],
        [[1046, 1046], 'mapped', [1078]],
        [[1047, 1047], 'mapped', [1079]],
        [[1048, 1048], 'mapped', [1080]],
        [[1049, 1049], 'mapped', [1081]],
        [[1050, 1050], 'mapped', [1082]],
        [[1051, 1051], 'mapped', [1083]],
        [[1052, 1052], 'mapped', [1084]],
        [[1053, 1053], 'mapped', [1085]],
        [[1054, 1054], 'mapped', [1086]],
        [[1055, 1055], 'mapped', [1087]],
        [[1056, 1056], 'mapped', [1088]],
        [[1057, 1057], 'mapped', [1089]],
        [[1058, 1058], 'mapped', [1090]],
        [[1059, 1059], 'mapped', [1091]],
        [[1060, 1060], 'mapped', [1092]],
        [[1061, 1061], 'mapped', [1093]],
        [[1062, 1062], 'mapped', [1094]],
        [[1063, 1063], 'mapped', [1095]],
        [[1064, 1064], 'mapped', [1096]],
        [[1065, 1065], 'mapped', [1097]],
        [[1066, 1066], 'mapped', [1098]],
        [[1067, 1067], 'mapped', [1099]],
        [[1068, 1068], 'mapped', [1100]],
        [[1069, 1069], 'mapped', [1101]],
        [[1070, 1070], 'mapped', [1102]],
        [[1071, 1071], 'mapped', [1103]],
        [[1072, 1103], 'valid'],
        [[1104, 1104], 'valid'],
        [[1105, 1116], 'valid'],
        [[1117, 1117], 'valid'],
        [[1118, 1119], 'valid'],
        [[1120, 1120], 'mapped', [1121]],
        [[1121, 1121], 'valid'],
        [[1122, 1122], 'mapped', [1123]],
        [[1123, 1123], 'valid'],
        [[1124, 1124], 'mapped', [1125]],
        [[1125, 1125], 'valid'],
        [[1126, 1126], 'mapped', [1127]],
        [[1127, 1127], 'valid'],
        [[1128, 1128], 'mapped', [1129]],
        [[1129, 1129], 'valid'],
        [[1130, 1130], 'mapped', [1131]],
        [[1131, 1131], 'valid'],
        [[1132, 1132], 'mapped', [1133]],
        [[1133, 1133], 'valid'],
        [[1134, 1134], 'mapped', [1135]],
        [[1135, 1135], 'valid'],
        [[1136, 1136], 'mapped', [1137]],
        [[1137, 1137], 'valid'],
        [[1138, 1138], 'mapped', [1139]],
        [[1139, 1139], 'valid'],
        [[1140, 1140], 'mapped', [1141]],
        [[1141, 1141], 'valid'],
        [[1142, 1142], 'mapped', [1143]],
        [[1143, 1143], 'valid'],
        [[1144, 1144], 'mapped', [1145]],
        [[1145, 1145], 'valid'],
        [[1146, 1146], 'mapped', [1147]],
        [[1147, 1147], 'valid'],
        [[1148, 1148], 'mapped', [1149]],
        [[1149, 1149], 'valid'],
        [[1150, 1150], 'mapped', [1151]],
        [[1151, 1151], 'valid'],
        [[1152, 1152], 'mapped', [1153]],
        [[1153, 1153], 'valid'],
        [[1154, 1154], 'valid', [], 'NV8'],
        [[1155, 1158], 'valid'],
        [[1159, 1159], 'valid'],
        [[1160, 1161], 'valid', [], 'NV8'],
        [[1162, 1162], 'mapped', [1163]],
        [[1163, 1163], 'valid'],
        [[1164, 1164], 'mapped', [1165]],
        [[1165, 1165], 'valid'],
        [[1166, 1166], 'mapped', [1167]],
        [[1167, 1167], 'valid'],
        [[1168, 1168], 'mapped', [1169]],
        [[1169, 1169], 'valid'],
        [[1170, 1170], 'mapped', [1171]],
        [[1171, 1171], 'valid'],
        [[1172, 1172], 'mapped', [1173]],
        [[1173, 1173], 'valid'],
        [[1174, 1174], 'mapped', [1175]],
        [[1175, 1175], 'valid'],
        [[1176, 1176], 'mapped', [1177]],
        [[1177, 1177], 'valid'],
        [[1178, 1178], 'mapped', [1179]],
        [[1179, 1179], 'valid'],
        [[1180, 1180], 'mapped', [1181]],
        [[1181, 1181], 'valid'],
        [[1182, 1182], 'mapped', [1183]],
        [[1183, 1183], 'valid'],
        [[1184, 1184], 'mapped', [1185]],
        [[1185, 1185], 'valid'],
        [[1186, 1186], 'mapped', [1187]],
        [[1187, 1187], 'valid'],
        [[1188, 1188], 'mapped', [1189]],
        [[1189, 1189], 'valid'],
        [[1190, 1190], 'mapped', [1191]],
        [[1191, 1191], 'valid'],
        [[1192, 1192], 'mapped', [1193]],
        [[1193, 1193], 'valid'],
        [[1194, 1194], 'mapped', [1195]],
        [[1195, 1195], 'valid'],
        [[1196, 1196], 'mapped', [1197]],
        [[1197, 1197], 'valid'],
        [[1198, 1198], 'mapped', [1199]],
        [[1199, 1199], 'valid'],
        [[1200, 1200], 'mapped', [1201]],
        [[1201, 1201], 'valid'],
        [[1202, 1202], 'mapped', [1203]],
        [[1203, 1203], 'valid'],
        [[1204, 1204], 'mapped', [1205]],
        [[1205, 1205], 'valid'],
        [[1206, 1206], 'mapped', [1207]],
        [[1207, 1207], 'valid'],
        [[1208, 1208], 'mapped', [1209]],
        [[1209, 1209], 'valid'],
        [[1210, 1210], 'mapped', [1211]],
        [[1211, 1211], 'valid'],
        [[1212, 1212], 'mapped', [1213]],
        [[1213, 1213], 'valid'],
        [[1214, 1214], 'mapped', [1215]],
        [[1215, 1215], 'valid'],
        [[1216, 1216], 'disallowed'],
        [[1217, 1217], 'mapped', [1218]],
        [[1218, 1218], 'valid'],
        [[1219, 1219], 'mapped', [1220]],
        [[1220, 1220], 'valid'],
        [[1221, 1221], 'mapped', [1222]],
        [[1222, 1222], 'valid'],
        [[1223, 1223], 'mapped', [1224]],
        [[1224, 1224], 'valid'],
        [[1225, 1225], 'mapped', [1226]],
        [[1226, 1226], 'valid'],
        [[1227, 1227], 'mapped', [1228]],
        [[1228, 1228], 'valid'],
        [[1229, 1229], 'mapped', [1230]],
        [[1230, 1230], 'valid'],
        [[1231, 1231], 'valid'],
        [[1232, 1232], 'mapped', [1233]],
        [[1233, 1233], 'valid'],
        [[1234, 1234], 'mapped', [1235]],
        [[1235, 1235], 'valid'],
        [[1236, 1236], 'mapped', [1237]],
        [[1237, 1237], 'valid'],
        [[1238, 1238], 'mapped', [1239]],
        [[1239, 1239], 'valid'],
        [[1240, 1240], 'mapped', [1241]],
        [[1241, 1241], 'valid'],
        [[1242, 1242], 'mapped', [1243]],
        [[1243, 1243], 'valid'],
        [[1244, 1244], 'mapped', [1245]],
        [[1245, 1245], 'valid'],
        [[1246, 1246], 'mapped', [1247]],
        [[1247, 1247], 'valid'],
        [[1248, 1248], 'mapped', [1249]],
        [[1249, 1249], 'valid'],
        [[1250, 1250], 'mapped', [1251]],
        [[1251, 1251], 'valid'],
        [[1252, 1252], 'mapped', [1253]],
        [[1253, 1253], 'valid'],
        [[1254, 1254], 'mapped', [1255]],
        [[1255, 1255], 'valid'],
        [[1256, 1256], 'mapped', [1257]],
        [[1257, 1257], 'valid'],
        [[1258, 1258], 'mapped', [1259]],
        [[1259, 1259], 'valid'],
        [[1260, 1260], 'mapped', [1261]],
        [[1261, 1261], 'valid'],
        [[1262, 1262], 'mapped', [1263]],
        [[1263, 1263], 'valid'],
        [[1264, 1264], 'mapped', [1265]],
        [[1265, 1265], 'valid'],
        [[1266, 1266], 'mapped', [1267]],
        [[1267, 1267], 'valid'],
        [[1268, 1268], 'mapped', [1269]],
        [[1269, 1269], 'valid'],
        [[1270, 1270], 'mapped', [1271]],
        [[1271, 1271], 'valid'],
        [[1272, 1272], 'mapped', [1273]],
        [[1273, 1273], 'valid'],
        [[1274, 1274], 'mapped', [1275]],
        [[1275, 1275], 'valid'],
        [[1276, 1276], 'mapped', [1277]],
        [[1277, 1277], 'valid'],
        [[1278, 1278], 'mapped', [1279]],
        [[1279, 1279], 'valid'],
        [[1280, 1280], 'mapped', [1281]],
        [[1281, 1281], 'valid'],
        [[1282, 1282], 'mapped', [1283]],
        [[1283, 1283], 'valid'],
        [[1284, 1284], 'mapped', [1285]],
        [[1285, 1285], 'valid'],
        [[1286, 1286], 'mapped', [1287]],
        [[1287, 1287], 'valid'],
        [[1288, 1288], 'mapped', [1289]],
        [[1289, 1289], 'valid'],
        [[1290, 1290], 'mapped', [1291]],
        [[1291, 1291], 'valid'],
        [[1292, 1292], 'mapped', [1293]],
        [[1293, 1293], 'valid'],
        [[1294, 1294], 'mapped', [1295]],
        [[1295, 1295], 'valid'],
        [[1296, 1296], 'mapped', [1297]],
        [[1297, 1297], 'valid'],
        [[1298, 1298], 'mapped', [1299]],
        [[1299, 1299], 'valid'],
        [[1300, 1300], 'mapped', [1301]],
        [[1301, 1301], 'valid'],
        [[1302, 1302], 'mapped', [1303]],
        [[1303, 1303], 'valid'],
        [[1304, 1304], 'mapped', [1305]],
        [[1305, 1305], 'valid'],
        [[1306, 1306], 'mapped', [1307]],
        [[1307, 1307], 'valid'],
        [[1308, 1308], 'mapped', [1309]],
        [[1309, 1309], 'valid'],
        [[1310, 1310], 'mapped', [1311]],
        [[1311, 1311], 'valid'],
        [[1312, 1312], 'mapped', [1313]],
        [[1313, 1313], 'valid'],
        [[1314, 1314], 'mapped', [1315]],
        [[1315, 1315], 'valid'],
        [[1316, 1316], 'mapped', [1317]],
        [[1317, 1317], 'valid'],
        [[1318, 1318], 'mapped', [1319]],
        [[1319, 1319], 'valid'],
        [[1320, 1320], 'mapped', [1321]],
        [[1321, 1321], 'valid'],
        [[1322, 1322], 'mapped', [1323]],
        [[1323, 1323], 'valid'],
        [[1324, 1324], 'mapped', [1325]],
        [[1325, 1325], 'valid'],
        [[1326, 1326], 'mapped', [1327]],
        [[1327, 1327], 'valid'],
        [[1328, 1328], 'disallowed'],
        [[1329, 1329], 'mapped', [1377]],
        [[1330, 1330], 'mapped', [1378]],
        [[1331, 1331], 'mapped', [1379]],
        [[1332, 1332], 'mapped', [1380]],
        [[1333, 1333], 'mapped', [1381]],
        [[1334, 1334], 'mapped', [1382]],
        [[1335, 1335], 'mapped', [1383]],
        [[1336, 1336], 'mapped', [1384]],
        [[1337, 1337], 'mapped', [1385]],
        [[1338, 1338], 'mapped', [1386]],
        [[1339, 1339], 'mapped', [1387]],
        [[1340, 1340], 'mapped', [1388]],
        [[1341, 1341], 'mapped', [1389]],
        [[1342, 1342], 'mapped', [1390]],
        [[1343, 1343], 'mapped', [1391]],
        [[1344, 1344], 'mapped', [1392]],
        [[1345, 1345], 'mapped', [1393]],
        [[1346, 1346], 'mapped', [1394]],
        [[1347, 1347], 'mapped', [1395]],
        [[1348, 1348], 'mapped', [1396]],
        [[1349, 1349], 'mapped', [1397]],
        [[1350, 1350], 'mapped', [1398]],
        [[1351, 1351], 'mapped', [1399]],
        [[1352, 1352], 'mapped', [1400]],
        [[1353, 1353], 'mapped', [1401]],
        [[1354, 1354], 'mapped', [1402]],
        [[1355, 1355], 'mapped', [1403]],
        [[1356, 1356], 'mapped', [1404]],
        [[1357, 1357], 'mapped', [1405]],
        [[1358, 1358], 'mapped', [1406]],
        [[1359, 1359], 'mapped', [1407]],
        [[1360, 1360], 'mapped', [1408]],
        [[1361, 1361], 'mapped', [1409]],
        [[1362, 1362], 'mapped', [1410]],
        [[1363, 1363], 'mapped', [1411]],
        [[1364, 1364], 'mapped', [1412]],
        [[1365, 1365], 'mapped', [1413]],
        [[1366, 1366], 'mapped', [1414]],
        [[1367, 1368], 'disallowed'],
        [[1369, 1369], 'valid'],
        [[1370, 1375], 'valid', [], 'NV8'],
        [[1376, 1376], 'disallowed'],
        [[1377, 1414], 'valid'],
        [[1415, 1415], 'mapped', [1381, 1410]],
        [[1416, 1416], 'disallowed'],
        [[1417, 1417], 'valid', [], 'NV8'],
        [[1418, 1418], 'valid', [], 'NV8'],
        [[1419, 1420], 'disallowed'],
        [[1421, 1422], 'valid', [], 'NV8'],
        [[1423, 1423], 'valid', [], 'NV8'],
        [[1424, 1424], 'disallowed'],
        [[1425, 1441], 'valid'],
        [[1442, 1442], 'valid'],
        [[1443, 1455], 'valid'],
        [[1456, 1465], 'valid'],
        [[1466, 1466], 'valid'],
        [[1467, 1469], 'valid'],
        [[1470, 1470], 'valid', [], 'NV8'],
        [[1471, 1471], 'valid'],
        [[1472, 1472], 'valid', [], 'NV8'],
        [[1473, 1474], 'valid'],
        [[1475, 1475], 'valid', [], 'NV8'],
        [[1476, 1476], 'valid'],
        [[1477, 1477], 'valid'],
        [[1478, 1478], 'valid', [], 'NV8'],
        [[1479, 1479], 'valid'],
        [[1480, 1487], 'disallowed'],
        [[1488, 1514], 'valid'],
        [[1515, 1519], 'disallowed'],
        [[1520, 1524], 'valid'],
        [[1525, 1535], 'disallowed'],
        [[1536, 1539], 'disallowed'],
        [[1540, 1540], 'disallowed'],
        [[1541, 1541], 'disallowed'],
        [[1542, 1546], 'valid', [], 'NV8'],
        [[1547, 1547], 'valid', [], 'NV8'],
        [[1548, 1548], 'valid', [], 'NV8'],
        [[1549, 1551], 'valid', [], 'NV8'],
        [[1552, 1557], 'valid'],
        [[1558, 1562], 'valid'],
        [[1563, 1563], 'valid', [], 'NV8'],
        [[1564, 1564], 'disallowed'],
        [[1565, 1565], 'disallowed'],
        [[1566, 1566], 'valid', [], 'NV8'],
        [[1567, 1567], 'valid', [], 'NV8'],
        [[1568, 1568], 'valid'],
        [[1569, 1594], 'valid'],
        [[1595, 1599], 'valid'],
        [[1600, 1600], 'valid', [], 'NV8'],
        [[1601, 1618], 'valid'],
        [[1619, 1621], 'valid'],
        [[1622, 1624], 'valid'],
        [[1625, 1630], 'valid'],
        [[1631, 1631], 'valid'],
        [[1632, 1641], 'valid'],
        [[1642, 1645], 'valid', [], 'NV8'],
        [[1646, 1647], 'valid'],
        [[1648, 1652], 'valid'],
        [[1653, 1653], 'mapped', [1575, 1652]],
        [[1654, 1654], 'mapped', [1608, 1652]],
        [[1655, 1655], 'mapped', [1735, 1652]],
        [[1656, 1656], 'mapped', [1610, 1652]],
        [[1657, 1719], 'valid'],
        [[1720, 1721], 'valid'],
        [[1722, 1726], 'valid'],
        [[1727, 1727], 'valid'],
        [[1728, 1742], 'valid'],
        [[1743, 1743], 'valid'],
        [[1744, 1747], 'valid'],
        [[1748, 1748], 'valid', [], 'NV8'],
        [[1749, 1756], 'valid'],
        [[1757, 1757], 'disallowed'],
        [[1758, 1758], 'valid', [], 'NV8'],
        [[1759, 1768], 'valid'],
        [[1769, 1769], 'valid', [], 'NV8'],
        [[1770, 1773], 'valid'],
        [[1774, 1775], 'valid'],
        [[1776, 1785], 'valid'],
        [[1786, 1790], 'valid'],
        [[1791, 1791], 'valid'],
        [[1792, 1805], 'valid', [], 'NV8'],
        [[1806, 1806], 'disallowed'],
        [[1807, 1807], 'disallowed'],
        [[1808, 1836], 'valid'],
        [[1837, 1839], 'valid'],
        [[1840, 1866], 'valid'],
        [[1867, 1868], 'disallowed'],
        [[1869, 1871], 'valid'],
        [[1872, 1901], 'valid'],
        [[1902, 1919], 'valid'],
        [[1920, 1968], 'valid'],
        [[1969, 1969], 'valid'],
        [[1970, 1983], 'disallowed'],
        [[1984, 2037], 'valid'],
        [[2038, 2042], 'valid', [], 'NV8'],
        [[2043, 2047], 'disallowed'],
        [[2048, 2093], 'valid'],
        [[2094, 2095], 'disallowed'],
        [[2096, 2110], 'valid', [], 'NV8'],
        [[2111, 2111], 'disallowed'],
        [[2112, 2139], 'valid'],
        [[2140, 2141], 'disallowed'],
        [[2142, 2142], 'valid', [], 'NV8'],
        [[2143, 2207], 'disallowed'],
        [[2208, 2208], 'valid'],
        [[2209, 2209], 'valid'],
        [[2210, 2220], 'valid'],
        [[2221, 2226], 'valid'],
        [[2227, 2228], 'valid'],
        [[2229, 2274], 'disallowed'],
        [[2275, 2275], 'valid'],
        [[2276, 2302], 'valid'],
        [[2303, 2303], 'valid'],
        [[2304, 2304], 'valid'],
        [[2305, 2307], 'valid'],
        [[2308, 2308], 'valid'],
        [[2309, 2361], 'valid'],
        [[2362, 2363], 'valid'],
        [[2364, 2381], 'valid'],
        [[2382, 2382], 'valid'],
        [[2383, 2383], 'valid'],
        [[2384, 2388], 'valid'],
        [[2389, 2389], 'valid'],
        [[2390, 2391], 'valid'],
        [[2392, 2392], 'mapped', [2325, 2364]],
        [[2393, 2393], 'mapped', [2326, 2364]],
        [[2394, 2394], 'mapped', [2327, 2364]],
        [[2395, 2395], 'mapped', [2332, 2364]],
        [[2396, 2396], 'mapped', [2337, 2364]],
        [[2397, 2397], 'mapped', [2338, 2364]],
        [[2398, 2398], 'mapped', [2347, 2364]],
        [[2399, 2399], 'mapped', [2351, 2364]],
        [[2400, 2403], 'valid'],
        [[2404, 2405], 'valid', [], 'NV8'],
        [[2406, 2415], 'valid'],
        [[2416, 2416], 'valid', [], 'NV8'],
        [[2417, 2418], 'valid'],
        [[2419, 2423], 'valid'],
        [[2424, 2424], 'valid'],
        [[2425, 2426], 'valid'],
        [[2427, 2428], 'valid'],
        [[2429, 2429], 'valid'],
        [[2430, 2431], 'valid'],
        [[2432, 2432], 'valid'],
        [[2433, 2435], 'valid'],
        [[2436, 2436], 'disallowed'],
        [[2437, 2444], 'valid'],
        [[2445, 2446], 'disallowed'],
        [[2447, 2448], 'valid'],
        [[2449, 2450], 'disallowed'],
        [[2451, 2472], 'valid'],
        [[2473, 2473], 'disallowed'],
        [[2474, 2480], 'valid'],
        [[2481, 2481], 'disallowed'],
        [[2482, 2482], 'valid'],
        [[2483, 2485], 'disallowed'],
        [[2486, 2489], 'valid'],
        [[2490, 2491], 'disallowed'],
        [[2492, 2492], 'valid'],
        [[2493, 2493], 'valid'],
        [[2494, 2500], 'valid'],
        [[2501, 2502], 'disallowed'],
        [[2503, 2504], 'valid'],
        [[2505, 2506], 'disallowed'],
        [[2507, 2509], 'valid'],
        [[2510, 2510], 'valid'],
        [[2511, 2518], 'disallowed'],
        [[2519, 2519], 'valid'],
        [[2520, 2523], 'disallowed'],
        [[2524, 2524], 'mapped', [2465, 2492]],
        [[2525, 2525], 'mapped', [2466, 2492]],
        [[2526, 2526], 'disallowed'],
        [[2527, 2527], 'mapped', [2479, 2492]],
        [[2528, 2531], 'valid'],
        [[2532, 2533], 'disallowed'],
        [[2534, 2545], 'valid'],
        [[2546, 2554], 'valid', [], 'NV8'],
        [[2555, 2555], 'valid', [], 'NV8'],
        [[2556, 2560], 'disallowed'],
        [[2561, 2561], 'valid'],
        [[2562, 2562], 'valid'],
        [[2563, 2563], 'valid'],
        [[2564, 2564], 'disallowed'],
        [[2565, 2570], 'valid'],
        [[2571, 2574], 'disallowed'],
        [[2575, 2576], 'valid'],
        [[2577, 2578], 'disallowed'],
        [[2579, 2600], 'valid'],
        [[2601, 2601], 'disallowed'],
        [[2602, 2608], 'valid'],
        [[2609, 2609], 'disallowed'],
        [[2610, 2610], 'valid'],
        [[2611, 2611], 'mapped', [2610, 2620]],
        [[2612, 2612], 'disallowed'],
        [[2613, 2613], 'valid'],
        [[2614, 2614], 'mapped', [2616, 2620]],
        [[2615, 2615], 'disallowed'],
        [[2616, 2617], 'valid'],
        [[2618, 2619], 'disallowed'],
        [[2620, 2620], 'valid'],
        [[2621, 2621], 'disallowed'],
        [[2622, 2626], 'valid'],
        [[2627, 2630], 'disallowed'],
        [[2631, 2632], 'valid'],
        [[2633, 2634], 'disallowed'],
        [[2635, 2637], 'valid'],
        [[2638, 2640], 'disallowed'],
        [[2641, 2641], 'valid'],
        [[2642, 2648], 'disallowed'],
        [[2649, 2649], 'mapped', [2582, 2620]],
        [[2650, 2650], 'mapped', [2583, 2620]],
        [[2651, 2651], 'mapped', [2588, 2620]],
        [[2652, 2652], 'valid'],
        [[2653, 2653], 'disallowed'],
        [[2654, 2654], 'mapped', [2603, 2620]],
        [[2655, 2661], 'disallowed'],
        [[2662, 2676], 'valid'],
        [[2677, 2677], 'valid'],
        [[2678, 2688], 'disallowed'],
        [[2689, 2691], 'valid'],
        [[2692, 2692], 'disallowed'],
        [[2693, 2699], 'valid'],
        [[2700, 2700], 'valid'],
        [[2701, 2701], 'valid'],
        [[2702, 2702], 'disallowed'],
        [[2703, 2705], 'valid'],
        [[2706, 2706], 'disallowed'],
        [[2707, 2728], 'valid'],
        [[2729, 2729], 'disallowed'],
        [[2730, 2736], 'valid'],
        [[2737, 2737], 'disallowed'],
        [[2738, 2739], 'valid'],
        [[2740, 2740], 'disallowed'],
        [[2741, 2745], 'valid'],
        [[2746, 2747], 'disallowed'],
        [[2748, 2757], 'valid'],
        [[2758, 2758], 'disallowed'],
        [[2759, 2761], 'valid'],
        [[2762, 2762], 'disallowed'],
        [[2763, 2765], 'valid'],
        [[2766, 2767], 'disallowed'],
        [[2768, 2768], 'valid'],
        [[2769, 2783], 'disallowed'],
        [[2784, 2784], 'valid'],
        [[2785, 2787], 'valid'],
        [[2788, 2789], 'disallowed'],
        [[2790, 2799], 'valid'],
        [[2800, 2800], 'valid', [], 'NV8'],
        [[2801, 2801], 'valid', [], 'NV8'],
        [[2802, 2808], 'disallowed'],
        [[2809, 2809], 'valid'],
        [[2810, 2816], 'disallowed'],
        [[2817, 2819], 'valid'],
        [[2820, 2820], 'disallowed'],
        [[2821, 2828], 'valid'],
        [[2829, 2830], 'disallowed'],
        [[2831, 2832], 'valid'],
        [[2833, 2834], 'disallowed'],
        [[2835, 2856], 'valid'],
        [[2857, 2857], 'disallowed'],
        [[2858, 2864], 'valid'],
        [[2865, 2865], 'disallowed'],
        [[2866, 2867], 'valid'],
        [[2868, 2868], 'disallowed'],
        [[2869, 2869], 'valid'],
        [[2870, 2873], 'valid'],
        [[2874, 2875], 'disallowed'],
        [[2876, 2883], 'valid'],
        [[2884, 2884], 'valid'],
        [[2885, 2886], 'disallowed'],
        [[2887, 2888], 'valid'],
        [[2889, 2890], 'disallowed'],
        [[2891, 2893], 'valid'],
        [[2894, 2901], 'disallowed'],
        [[2902, 2903], 'valid'],
        [[2904, 2907], 'disallowed'],
        [[2908, 2908], 'mapped', [2849, 2876]],
        [[2909, 2909], 'mapped', [2850, 2876]],
        [[2910, 2910], 'disallowed'],
        [[2911, 2913], 'valid'],
        [[2914, 2915], 'valid'],
        [[2916, 2917], 'disallowed'],
        [[2918, 2927], 'valid'],
        [[2928, 2928], 'valid', [], 'NV8'],
        [[2929, 2929], 'valid'],
        [[2930, 2935], 'valid', [], 'NV8'],
        [[2936, 2945], 'disallowed'],
        [[2946, 2947], 'valid'],
        [[2948, 2948], 'disallowed'],
        [[2949, 2954], 'valid'],
        [[2955, 2957], 'disallowed'],
        [[2958, 2960], 'valid'],
        [[2961, 2961], 'disallowed'],
        [[2962, 2965], 'valid'],
        [[2966, 2968], 'disallowed'],
        [[2969, 2970], 'valid'],
        [[2971, 2971], 'disallowed'],
        [[2972, 2972], 'valid'],
        [[2973, 2973], 'disallowed'],
        [[2974, 2975], 'valid'],
        [[2976, 2978], 'disallowed'],
        [[2979, 2980], 'valid'],
        [[2981, 2983], 'disallowed'],
        [[2984, 2986], 'valid'],
        [[2987, 2989], 'disallowed'],
        [[2990, 2997], 'valid'],
        [[2998, 2998], 'valid'],
        [[2999, 3001], 'valid'],
        [[3002, 3005], 'disallowed'],
        [[3006, 3010], 'valid'],
        [[3011, 3013], 'disallowed'],
        [[3014, 3016], 'valid'],
        [[3017, 3017], 'disallowed'],
        [[3018, 3021], 'valid'],
        [[3022, 3023], 'disallowed'],
        [[3024, 3024], 'valid'],
        [[3025, 3030], 'disallowed'],
        [[3031, 3031], 'valid'],
        [[3032, 3045], 'disallowed'],
        [[3046, 3046], 'valid'],
        [[3047, 3055], 'valid'],
        [[3056, 3058], 'valid', [], 'NV8'],
        [[3059, 3066], 'valid', [], 'NV8'],
        [[3067, 3071], 'disallowed'],
        [[3072, 3072], 'valid'],
        [[3073, 3075], 'valid'],
        [[3076, 3076], 'disallowed'],
        [[3077, 3084], 'valid'],
        [[3085, 3085], 'disallowed'],
        [[3086, 3088], 'valid'],
        [[3089, 3089], 'disallowed'],
        [[3090, 3112], 'valid'],
        [[3113, 3113], 'disallowed'],
        [[3114, 3123], 'valid'],
        [[3124, 3124], 'valid'],
        [[3125, 3129], 'valid'],
        [[3130, 3132], 'disallowed'],
        [[3133, 3133], 'valid'],
        [[3134, 3140], 'valid'],
        [[3141, 3141], 'disallowed'],
        [[3142, 3144], 'valid'],
        [[3145, 3145], 'disallowed'],
        [[3146, 3149], 'valid'],
        [[3150, 3156], 'disallowed'],
        [[3157, 3158], 'valid'],
        [[3159, 3159], 'disallowed'],
        [[3160, 3161], 'valid'],
        [[3162, 3162], 'valid'],
        [[3163, 3167], 'disallowed'],
        [[3168, 3169], 'valid'],
        [[3170, 3171], 'valid'],
        [[3172, 3173], 'disallowed'],
        [[3174, 3183], 'valid'],
        [[3184, 3191], 'disallowed'],
        [[3192, 3199], 'valid', [], 'NV8'],
        [[3200, 3200], 'disallowed'],
        [[3201, 3201], 'valid'],
        [[3202, 3203], 'valid'],
        [[3204, 3204], 'disallowed'],
        [[3205, 3212], 'valid'],
        [[3213, 3213], 'disallowed'],
        [[3214, 3216], 'valid'],
        [[3217, 3217], 'disallowed'],
        [[3218, 3240], 'valid'],
        [[3241, 3241], 'disallowed'],
        [[3242, 3251], 'valid'],
        [[3252, 3252], 'disallowed'],
        [[3253, 3257], 'valid'],
        [[3258, 3259], 'disallowed'],
        [[3260, 3261], 'valid'],
        [[3262, 3268], 'valid'],
        [[3269, 3269], 'disallowed'],
        [[3270, 3272], 'valid'],
        [[3273, 3273], 'disallowed'],
        [[3274, 3277], 'valid'],
        [[3278, 3284], 'disallowed'],
        [[3285, 3286], 'valid'],
        [[3287, 3293], 'disallowed'],
        [[3294, 3294], 'valid'],
        [[3295, 3295], 'disallowed'],
        [[3296, 3297], 'valid'],
        [[3298, 3299], 'valid'],
        [[3300, 3301], 'disallowed'],
        [[3302, 3311], 'valid'],
        [[3312, 3312], 'disallowed'],
        [[3313, 3314], 'valid'],
        [[3315, 3328], 'disallowed'],
        [[3329, 3329], 'valid'],
        [[3330, 3331], 'valid'],
        [[3332, 3332], 'disallowed'],
        [[3333, 3340], 'valid'],
        [[3341, 3341], 'disallowed'],
        [[3342, 3344], 'valid'],
        [[3345, 3345], 'disallowed'],
        [[3346, 3368], 'valid'],
        [[3369, 3369], 'valid'],
        [[3370, 3385], 'valid'],
        [[3386, 3386], 'valid'],
        [[3387, 3388], 'disallowed'],
        [[3389, 3389], 'valid'],
        [[3390, 3395], 'valid'],
        [[3396, 3396], 'valid'],
        [[3397, 3397], 'disallowed'],
        [[3398, 3400], 'valid'],
        [[3401, 3401], 'disallowed'],
        [[3402, 3405], 'valid'],
        [[3406, 3406], 'valid'],
        [[3407, 3414], 'disallowed'],
        [[3415, 3415], 'valid'],
        [[3416, 3422], 'disallowed'],
        [[3423, 3423], 'valid'],
        [[3424, 3425], 'valid'],
        [[3426, 3427], 'valid'],
        [[3428, 3429], 'disallowed'],
        [[3430, 3439], 'valid'],
        [[3440, 3445], 'valid', [], 'NV8'],
        [[3446, 3448], 'disallowed'],
        [[3449, 3449], 'valid', [], 'NV8'],
        [[3450, 3455], 'valid'],
        [[3456, 3457], 'disallowed'],
        [[3458, 3459], 'valid'],
        [[3460, 3460], 'disallowed'],
        [[3461, 3478], 'valid'],
        [[3479, 3481], 'disallowed'],
        [[3482, 3505], 'valid'],
        [[3506, 3506], 'disallowed'],
        [[3507, 3515], 'valid'],
        [[3516, 3516], 'disallowed'],
        [[3517, 3517], 'valid'],
        [[3518, 3519], 'disallowed'],
        [[3520, 3526], 'valid'],
        [[3527, 3529], 'disallowed'],
        [[3530, 3530], 'valid'],
        [[3531, 3534], 'disallowed'],
        [[3535, 3540], 'valid'],
        [[3541, 3541], 'disallowed'],
        [[3542, 3542], 'valid'],
        [[3543, 3543], 'disallowed'],
        [[3544, 3551], 'valid'],
        [[3552, 3557], 'disallowed'],
        [[3558, 3567], 'valid'],
        [[3568, 3569], 'disallowed'],
        [[3570, 3571], 'valid'],
        [[3572, 3572], 'valid', [], 'NV8'],
        [[3573, 3584], 'disallowed'],
        [[3585, 3634], 'valid'],
        [[3635, 3635], 'mapped', [3661, 3634]],
        [[3636, 3642], 'valid'],
        [[3643, 3646], 'disallowed'],
        [[3647, 3647], 'valid', [], 'NV8'],
        [[3648, 3662], 'valid'],
        [[3663, 3663], 'valid', [], 'NV8'],
        [[3664, 3673], 'valid'],
        [[3674, 3675], 'valid', [], 'NV8'],
        [[3676, 3712], 'disallowed'],
        [[3713, 3714], 'valid'],
        [[3715, 3715], 'disallowed'],
        [[3716, 3716], 'valid'],
        [[3717, 3718], 'disallowed'],
        [[3719, 3720], 'valid'],
        [[3721, 3721], 'disallowed'],
        [[3722, 3722], 'valid'],
        [[3723, 3724], 'disallowed'],
        [[3725, 3725], 'valid'],
        [[3726, 3731], 'disallowed'],
        [[3732, 3735], 'valid'],
        [[3736, 3736], 'disallowed'],
        [[3737, 3743], 'valid'],
        [[3744, 3744], 'disallowed'],
        [[3745, 3747], 'valid'],
        [[3748, 3748], 'disallowed'],
        [[3749, 3749], 'valid'],
        [[3750, 3750], 'disallowed'],
        [[3751, 3751], 'valid'],
        [[3752, 3753], 'disallowed'],
        [[3754, 3755], 'valid'],
        [[3756, 3756], 'disallowed'],
        [[3757, 3762], 'valid'],
        [[3763, 3763], 'mapped', [3789, 3762]],
        [[3764, 3769], 'valid'],
        [[3770, 3770], 'disallowed'],
        [[3771, 3773], 'valid'],
        [[3774, 3775], 'disallowed'],
        [[3776, 3780], 'valid'],
        [[3781, 3781], 'disallowed'],
        [[3782, 3782], 'valid'],
        [[3783, 3783], 'disallowed'],
        [[3784, 3789], 'valid'],
        [[3790, 3791], 'disallowed'],
        [[3792, 3801], 'valid'],
        [[3802, 3803], 'disallowed'],
        [[3804, 3804], 'mapped', [3755, 3737]],
        [[3805, 3805], 'mapped', [3755, 3745]],
        [[3806, 3807], 'valid'],
        [[3808, 3839], 'disallowed'],
        [[3840, 3840], 'valid'],
        [[3841, 3850], 'valid', [], 'NV8'],
        [[3851, 3851], 'valid'],
        [[3852, 3852], 'mapped', [3851]],
        [[3853, 3863], 'valid', [], 'NV8'],
        [[3864, 3865], 'valid'],
        [[3866, 3871], 'valid', [], 'NV8'],
        [[3872, 3881], 'valid'],
        [[3882, 3892], 'valid', [], 'NV8'],
        [[3893, 3893], 'valid'],
        [[3894, 3894], 'valid', [], 'NV8'],
        [[3895, 3895], 'valid'],
        [[3896, 3896], 'valid', [], 'NV8'],
        [[3897, 3897], 'valid'],
        [[3898, 3901], 'valid', [], 'NV8'],
        [[3902, 3906], 'valid'],
        [[3907, 3907], 'mapped', [3906, 4023]],
        [[3908, 3911], 'valid'],
        [[3912, 3912], 'disallowed'],
        [[3913, 3916], 'valid'],
        [[3917, 3917], 'mapped', [3916, 4023]],
        [[3918, 3921], 'valid'],
        [[3922, 3922], 'mapped', [3921, 4023]],
        [[3923, 3926], 'valid'],
        [[3927, 3927], 'mapped', [3926, 4023]],
        [[3928, 3931], 'valid'],
        [[3932, 3932], 'mapped', [3931, 4023]],
        [[3933, 3944], 'valid'],
        [[3945, 3945], 'mapped', [3904, 4021]],
        [[3946, 3946], 'valid'],
        [[3947, 3948], 'valid'],
        [[3949, 3952], 'disallowed'],
        [[3953, 3954], 'valid'],
        [[3955, 3955], 'mapped', [3953, 3954]],
        [[3956, 3956], 'valid'],
        [[3957, 3957], 'mapped', [3953, 3956]],
        [[3958, 3958], 'mapped', [4018, 3968]],
        [[3959, 3959], 'mapped', [4018, 3953, 3968]],
        [[3960, 3960], 'mapped', [4019, 3968]],
        [[3961, 3961], 'mapped', [4019, 3953, 3968]],
        [[3962, 3968], 'valid'],
        [[3969, 3969], 'mapped', [3953, 3968]],
        [[3970, 3972], 'valid'],
        [[3973, 3973], 'valid', [], 'NV8'],
        [[3974, 3979], 'valid'],
        [[3980, 3983], 'valid'],
        [[3984, 3986], 'valid'],
        [[3987, 3987], 'mapped', [3986, 4023]],
        [[3988, 3989], 'valid'],
        [[3990, 3990], 'valid'],
        [[3991, 3991], 'valid'],
        [[3992, 3992], 'disallowed'],
        [[3993, 3996], 'valid'],
        [[3997, 3997], 'mapped', [3996, 4023]],
        [[3998, 4001], 'valid'],
        [[4002, 4002], 'mapped', [4001, 4023]],
        [[4003, 4006], 'valid'],
        [[4007, 4007], 'mapped', [4006, 4023]],
        [[4008, 4011], 'valid'],
        [[4012, 4012], 'mapped', [4011, 4023]],
        [[4013, 4013], 'valid'],
        [[4014, 4016], 'valid'],
        [[4017, 4023], 'valid'],
        [[4024, 4024], 'valid'],
        [[4025, 4025], 'mapped', [3984, 4021]],
        [[4026, 4028], 'valid'],
        [[4029, 4029], 'disallowed'],
        [[4030, 4037], 'valid', [], 'NV8'],
        [[4038, 4038], 'valid'],
        [[4039, 4044], 'valid', [], 'NV8'],
        [[4045, 4045], 'disallowed'],
        [[4046, 4046], 'valid', [], 'NV8'],
        [[4047, 4047], 'valid', [], 'NV8'],
        [[4048, 4049], 'valid', [], 'NV8'],
        [[4050, 4052], 'valid', [], 'NV8'],
        [[4053, 4056], 'valid', [], 'NV8'],
        [[4057, 4058], 'valid', [], 'NV8'],
        [[4059, 4095], 'disallowed'],
        [[4096, 4129], 'valid'],
        [[4130, 4130], 'valid'],
        [[4131, 4135], 'valid'],
        [[4136, 4136], 'valid'],
        [[4137, 4138], 'valid'],
        [[4139, 4139], 'valid'],
        [[4140, 4146], 'valid'],
        [[4147, 4149], 'valid'],
        [[4150, 4153], 'valid'],
        [[4154, 4159], 'valid'],
        [[4160, 4169], 'valid'],
        [[4170, 4175], 'valid', [], 'NV8'],
        [[4176, 4185], 'valid'],
        [[4186, 4249], 'valid'],
        [[4250, 4253], 'valid'],
        [[4254, 4255], 'valid', [], 'NV8'],
        [[4256, 4293], 'disallowed'],
        [[4294, 4294], 'disallowed'],
        [[4295, 4295], 'mapped', [11559]],
        [[4296, 4300], 'disallowed'],
        [[4301, 4301], 'mapped', [11565]],
        [[4302, 4303], 'disallowed'],
        [[4304, 4342], 'valid'],
        [[4343, 4344], 'valid'],
        [[4345, 4346], 'valid'],
        [[4347, 4347], 'valid', [], 'NV8'],
        [[4348, 4348], 'mapped', [4316]],
        [[4349, 4351], 'valid'],
        [[4352, 4441], 'valid', [], 'NV8'],
        [[4442, 4446], 'valid', [], 'NV8'],
        [[4447, 4448], 'disallowed'],
        [[4449, 4514], 'valid', [], 'NV8'],
        [[4515, 4519], 'valid', [], 'NV8'],
        [[4520, 4601], 'valid', [], 'NV8'],
        [[4602, 4607], 'valid', [], 'NV8'],
        [[4608, 4614], 'valid'],
        [[4615, 4615], 'valid'],
        [[4616, 4678], 'valid'],
        [[4679, 4679], 'valid'],
        [[4680, 4680], 'valid'],
        [[4681, 4681], 'disallowed'],
        [[4682, 4685], 'valid'],
        [[4686, 4687], 'disallowed'],
        [[4688, 4694], 'valid'],
        [[4695, 4695], 'disallowed'],
        [[4696, 4696], 'valid'],
        [[4697, 4697], 'disallowed'],
        [[4698, 4701], 'valid'],
        [[4702, 4703], 'disallowed'],
        [[4704, 4742], 'valid'],
        [[4743, 4743], 'valid'],
        [[4744, 4744], 'valid'],
        [[4745, 4745], 'disallowed'],
        [[4746, 4749], 'valid'],
        [[4750, 4751], 'disallowed'],
        [[4752, 4782], 'valid'],
        [[4783, 4783], 'valid'],
        [[4784, 4784], 'valid'],
        [[4785, 4785], 'disallowed'],
        [[4786, 4789], 'valid'],
        [[4790, 4791], 'disallowed'],
        [[4792, 4798], 'valid'],
        [[4799, 4799], 'disallowed'],
        [[4800, 4800], 'valid'],
        [[4801, 4801], 'disallowed'],
        [[4802, 4805], 'valid'],
        [[4806, 4807], 'disallowed'],
        [[4808, 4814], 'valid'],
        [[4815, 4815], 'valid'],
        [[4816, 4822], 'valid'],
        [[4823, 4823], 'disallowed'],
        [[4824, 4846], 'valid'],
        [[4847, 4847], 'valid'],
        [[4848, 4878], 'valid'],
        [[4879, 4879], 'valid'],
        [[4880, 4880], 'valid'],
        [[4881, 4881], 'disallowed'],
        [[4882, 4885], 'valid'],
        [[4886, 4887], 'disallowed'],
        [[4888, 4894], 'valid'],
        [[4895, 4895], 'valid'],
        [[4896, 4934], 'valid'],
        [[4935, 4935], 'valid'],
        [[4936, 4954], 'valid'],
        [[4955, 4956], 'disallowed'],
        [[4957, 4958], 'valid'],
        [[4959, 4959], 'valid'],
        [[4960, 4960], 'valid', [], 'NV8'],
        [[4961, 4988], 'valid', [], 'NV8'],
        [[4989, 4991], 'disallowed'],
        [[4992, 5007], 'valid'],
        [[5008, 5017], 'valid', [], 'NV8'],
        [[5018, 5023], 'disallowed'],
        [[5024, 5108], 'valid'],
        [[5109, 5109], 'valid'],
        [[5110, 5111], 'disallowed'],
        [[5112, 5112], 'mapped', [5104]],
        [[5113, 5113], 'mapped', [5105]],
        [[5114, 5114], 'mapped', [5106]],
        [[5115, 5115], 'mapped', [5107]],
        [[5116, 5116], 'mapped', [5108]],
        [[5117, 5117], 'mapped', [5109]],
        [[5118, 5119], 'disallowed'],
        [[5120, 5120], 'valid', [], 'NV8'],
        [[5121, 5740], 'valid'],
        [[5741, 5742], 'valid', [], 'NV8'],
        [[5743, 5750], 'valid'],
        [[5751, 5759], 'valid'],
        [[5760, 5760], 'disallowed'],
        [[5761, 5786], 'valid'],
        [[5787, 5788], 'valid', [], 'NV8'],
        [[5789, 5791], 'disallowed'],
        [[5792, 5866], 'valid'],
        [[5867, 5872], 'valid', [], 'NV8'],
        [[5873, 5880], 'valid'],
        [[5881, 5887], 'disallowed'],
        [[5888, 5900], 'valid'],
        [[5901, 5901], 'disallowed'],
        [[5902, 5908], 'valid'],
        [[5909, 5919], 'disallowed'],
        [[5920, 5940], 'valid'],
        [[5941, 5942], 'valid', [], 'NV8'],
        [[5943, 5951], 'disallowed'],
        [[5952, 5971], 'valid'],
        [[5972, 5983], 'disallowed'],
        [[5984, 5996], 'valid'],
        [[5997, 5997], 'disallowed'],
        [[5998, 6e3], 'valid'],
        [[6001, 6001], 'disallowed'],
        [[6002, 6003], 'valid'],
        [[6004, 6015], 'disallowed'],
        [[6016, 6067], 'valid'],
        [[6068, 6069], 'disallowed'],
        [[6070, 6099], 'valid'],
        [[6100, 6102], 'valid', [], 'NV8'],
        [[6103, 6103], 'valid'],
        [[6104, 6107], 'valid', [], 'NV8'],
        [[6108, 6108], 'valid'],
        [[6109, 6109], 'valid'],
        [[6110, 6111], 'disallowed'],
        [[6112, 6121], 'valid'],
        [[6122, 6127], 'disallowed'],
        [[6128, 6137], 'valid', [], 'NV8'],
        [[6138, 6143], 'disallowed'],
        [[6144, 6149], 'valid', [], 'NV8'],
        [[6150, 6150], 'disallowed'],
        [[6151, 6154], 'valid', [], 'NV8'],
        [[6155, 6157], 'ignored'],
        [[6158, 6158], 'disallowed'],
        [[6159, 6159], 'disallowed'],
        [[6160, 6169], 'valid'],
        [[6170, 6175], 'disallowed'],
        [[6176, 6263], 'valid'],
        [[6264, 6271], 'disallowed'],
        [[6272, 6313], 'valid'],
        [[6314, 6314], 'valid'],
        [[6315, 6319], 'disallowed'],
        [[6320, 6389], 'valid'],
        [[6390, 6399], 'disallowed'],
        [[6400, 6428], 'valid'],
        [[6429, 6430], 'valid'],
        [[6431, 6431], 'disallowed'],
        [[6432, 6443], 'valid'],
        [[6444, 6447], 'disallowed'],
        [[6448, 6459], 'valid'],
        [[6460, 6463], 'disallowed'],
        [[6464, 6464], 'valid', [], 'NV8'],
        [[6465, 6467], 'disallowed'],
        [[6468, 6469], 'valid', [], 'NV8'],
        [[6470, 6509], 'valid'],
        [[6510, 6511], 'disallowed'],
        [[6512, 6516], 'valid'],
        [[6517, 6527], 'disallowed'],
        [[6528, 6569], 'valid'],
        [[6570, 6571], 'valid'],
        [[6572, 6575], 'disallowed'],
        [[6576, 6601], 'valid'],
        [[6602, 6607], 'disallowed'],
        [[6608, 6617], 'valid'],
        [[6618, 6618], 'valid', [], 'XV8'],
        [[6619, 6621], 'disallowed'],
        [[6622, 6623], 'valid', [], 'NV8'],
        [[6624, 6655], 'valid', [], 'NV8'],
        [[6656, 6683], 'valid'],
        [[6684, 6685], 'disallowed'],
        [[6686, 6687], 'valid', [], 'NV8'],
        [[6688, 6750], 'valid'],
        [[6751, 6751], 'disallowed'],
        [[6752, 6780], 'valid'],
        [[6781, 6782], 'disallowed'],
        [[6783, 6793], 'valid'],
        [[6794, 6799], 'disallowed'],
        [[6800, 6809], 'valid'],
        [[6810, 6815], 'disallowed'],
        [[6816, 6822], 'valid', [], 'NV8'],
        [[6823, 6823], 'valid'],
        [[6824, 6829], 'valid', [], 'NV8'],
        [[6830, 6831], 'disallowed'],
        [[6832, 6845], 'valid'],
        [[6846, 6846], 'valid', [], 'NV8'],
        [[6847, 6911], 'disallowed'],
        [[6912, 6987], 'valid'],
        [[6988, 6991], 'disallowed'],
        [[6992, 7001], 'valid'],
        [[7002, 7018], 'valid', [], 'NV8'],
        [[7019, 7027], 'valid'],
        [[7028, 7036], 'valid', [], 'NV8'],
        [[7037, 7039], 'disallowed'],
        [[7040, 7082], 'valid'],
        [[7083, 7085], 'valid'],
        [[7086, 7097], 'valid'],
        [[7098, 7103], 'valid'],
        [[7104, 7155], 'valid'],
        [[7156, 7163], 'disallowed'],
        [[7164, 7167], 'valid', [], 'NV8'],
        [[7168, 7223], 'valid'],
        [[7224, 7226], 'disallowed'],
        [[7227, 7231], 'valid', [], 'NV8'],
        [[7232, 7241], 'valid'],
        [[7242, 7244], 'disallowed'],
        [[7245, 7293], 'valid'],
        [[7294, 7295], 'valid', [], 'NV8'],
        [[7296, 7359], 'disallowed'],
        [[7360, 7367], 'valid', [], 'NV8'],
        [[7368, 7375], 'disallowed'],
        [[7376, 7378], 'valid'],
        [[7379, 7379], 'valid', [], 'NV8'],
        [[7380, 7410], 'valid'],
        [[7411, 7414], 'valid'],
        [[7415, 7415], 'disallowed'],
        [[7416, 7417], 'valid'],
        [[7418, 7423], 'disallowed'],
        [[7424, 7467], 'valid'],
        [[7468, 7468], 'mapped', [97]],
        [[7469, 7469], 'mapped', [230]],
        [[7470, 7470], 'mapped', [98]],
        [[7471, 7471], 'valid'],
        [[7472, 7472], 'mapped', [100]],
        [[7473, 7473], 'mapped', [101]],
        [[7474, 7474], 'mapped', [477]],
        [[7475, 7475], 'mapped', [103]],
        [[7476, 7476], 'mapped', [104]],
        [[7477, 7477], 'mapped', [105]],
        [[7478, 7478], 'mapped', [106]],
        [[7479, 7479], 'mapped', [107]],
        [[7480, 7480], 'mapped', [108]],
        [[7481, 7481], 'mapped', [109]],
        [[7482, 7482], 'mapped', [110]],
        [[7483, 7483], 'valid'],
        [[7484, 7484], 'mapped', [111]],
        [[7485, 7485], 'mapped', [547]],
        [[7486, 7486], 'mapped', [112]],
        [[7487, 7487], 'mapped', [114]],
        [[7488, 7488], 'mapped', [116]],
        [[7489, 7489], 'mapped', [117]],
        [[7490, 7490], 'mapped', [119]],
        [[7491, 7491], 'mapped', [97]],
        [[7492, 7492], 'mapped', [592]],
        [[7493, 7493], 'mapped', [593]],
        [[7494, 7494], 'mapped', [7426]],
        [[7495, 7495], 'mapped', [98]],
        [[7496, 7496], 'mapped', [100]],
        [[7497, 7497], 'mapped', [101]],
        [[7498, 7498], 'mapped', [601]],
        [[7499, 7499], 'mapped', [603]],
        [[7500, 7500], 'mapped', [604]],
        [[7501, 7501], 'mapped', [103]],
        [[7502, 7502], 'valid'],
        [[7503, 7503], 'mapped', [107]],
        [[7504, 7504], 'mapped', [109]],
        [[7505, 7505], 'mapped', [331]],
        [[7506, 7506], 'mapped', [111]],
        [[7507, 7507], 'mapped', [596]],
        [[7508, 7508], 'mapped', [7446]],
        [[7509, 7509], 'mapped', [7447]],
        [[7510, 7510], 'mapped', [112]],
        [[7511, 7511], 'mapped', [116]],
        [[7512, 7512], 'mapped', [117]],
        [[7513, 7513], 'mapped', [7453]],
        [[7514, 7514], 'mapped', [623]],
        [[7515, 7515], 'mapped', [118]],
        [[7516, 7516], 'mapped', [7461]],
        [[7517, 7517], 'mapped', [946]],
        [[7518, 7518], 'mapped', [947]],
        [[7519, 7519], 'mapped', [948]],
        [[7520, 7520], 'mapped', [966]],
        [[7521, 7521], 'mapped', [967]],
        [[7522, 7522], 'mapped', [105]],
        [[7523, 7523], 'mapped', [114]],
        [[7524, 7524], 'mapped', [117]],
        [[7525, 7525], 'mapped', [118]],
        [[7526, 7526], 'mapped', [946]],
        [[7527, 7527], 'mapped', [947]],
        [[7528, 7528], 'mapped', [961]],
        [[7529, 7529], 'mapped', [966]],
        [[7530, 7530], 'mapped', [967]],
        [[7531, 7531], 'valid'],
        [[7532, 7543], 'valid'],
        [[7544, 7544], 'mapped', [1085]],
        [[7545, 7578], 'valid'],
        [[7579, 7579], 'mapped', [594]],
        [[7580, 7580], 'mapped', [99]],
        [[7581, 7581], 'mapped', [597]],
        [[7582, 7582], 'mapped', [240]],
        [[7583, 7583], 'mapped', [604]],
        [[7584, 7584], 'mapped', [102]],
        [[7585, 7585], 'mapped', [607]],
        [[7586, 7586], 'mapped', [609]],
        [[7587, 7587], 'mapped', [613]],
        [[7588, 7588], 'mapped', [616]],
        [[7589, 7589], 'mapped', [617]],
        [[7590, 7590], 'mapped', [618]],
        [[7591, 7591], 'mapped', [7547]],
        [[7592, 7592], 'mapped', [669]],
        [[7593, 7593], 'mapped', [621]],
        [[7594, 7594], 'mapped', [7557]],
        [[7595, 7595], 'mapped', [671]],
        [[7596, 7596], 'mapped', [625]],
        [[7597, 7597], 'mapped', [624]],
        [[7598, 7598], 'mapped', [626]],
        [[7599, 7599], 'mapped', [627]],
        [[7600, 7600], 'mapped', [628]],
        [[7601, 7601], 'mapped', [629]],
        [[7602, 7602], 'mapped', [632]],
        [[7603, 7603], 'mapped', [642]],
        [[7604, 7604], 'mapped', [643]],
        [[7605, 7605], 'mapped', [427]],
        [[7606, 7606], 'mapped', [649]],
        [[7607, 7607], 'mapped', [650]],
        [[7608, 7608], 'mapped', [7452]],
        [[7609, 7609], 'mapped', [651]],
        [[7610, 7610], 'mapped', [652]],
        [[7611, 7611], 'mapped', [122]],
        [[7612, 7612], 'mapped', [656]],
        [[7613, 7613], 'mapped', [657]],
        [[7614, 7614], 'mapped', [658]],
        [[7615, 7615], 'mapped', [952]],
        [[7616, 7619], 'valid'],
        [[7620, 7626], 'valid'],
        [[7627, 7654], 'valid'],
        [[7655, 7669], 'valid'],
        [[7670, 7675], 'disallowed'],
        [[7676, 7676], 'valid'],
        [[7677, 7677], 'valid'],
        [[7678, 7679], 'valid'],
        [[7680, 7680], 'mapped', [7681]],
        [[7681, 7681], 'valid'],
        [[7682, 7682], 'mapped', [7683]],
        [[7683, 7683], 'valid'],
        [[7684, 7684], 'mapped', [7685]],
        [[7685, 7685], 'valid'],
        [[7686, 7686], 'mapped', [7687]],
        [[7687, 7687], 'valid'],
        [[7688, 7688], 'mapped', [7689]],
        [[7689, 7689], 'valid'],
        [[7690, 7690], 'mapped', [7691]],
        [[7691, 7691], 'valid'],
        [[7692, 7692], 'mapped', [7693]],
        [[7693, 7693], 'valid'],
        [[7694, 7694], 'mapped', [7695]],
        [[7695, 7695], 'valid'],
        [[7696, 7696], 'mapped', [7697]],
        [[7697, 7697], 'valid'],
        [[7698, 7698], 'mapped', [7699]],
        [[7699, 7699], 'valid'],
        [[7700, 7700], 'mapped', [7701]],
        [[7701, 7701], 'valid'],
        [[7702, 7702], 'mapped', [7703]],
        [[7703, 7703], 'valid'],
        [[7704, 7704], 'mapped', [7705]],
        [[7705, 7705], 'valid'],
        [[7706, 7706], 'mapped', [7707]],
        [[7707, 7707], 'valid'],
        [[7708, 7708], 'mapped', [7709]],
        [[7709, 7709], 'valid'],
        [[7710, 7710], 'mapped', [7711]],
        [[7711, 7711], 'valid'],
        [[7712, 7712], 'mapped', [7713]],
        [[7713, 7713], 'valid'],
        [[7714, 7714], 'mapped', [7715]],
        [[7715, 7715], 'valid'],
        [[7716, 7716], 'mapped', [7717]],
        [[7717, 7717], 'valid'],
        [[7718, 7718], 'mapped', [7719]],
        [[7719, 7719], 'valid'],
        [[7720, 7720], 'mapped', [7721]],
        [[7721, 7721], 'valid'],
        [[7722, 7722], 'mapped', [7723]],
        [[7723, 7723], 'valid'],
        [[7724, 7724], 'mapped', [7725]],
        [[7725, 7725], 'valid'],
        [[7726, 7726], 'mapped', [7727]],
        [[7727, 7727], 'valid'],
        [[7728, 7728], 'mapped', [7729]],
        [[7729, 7729], 'valid'],
        [[7730, 7730], 'mapped', [7731]],
        [[7731, 7731], 'valid'],
        [[7732, 7732], 'mapped', [7733]],
        [[7733, 7733], 'valid'],
        [[7734, 7734], 'mapped', [7735]],
        [[7735, 7735], 'valid'],
        [[7736, 7736], 'mapped', [7737]],
        [[7737, 7737], 'valid'],
        [[7738, 7738], 'mapped', [7739]],
        [[7739, 7739], 'valid'],
        [[7740, 7740], 'mapped', [7741]],
        [[7741, 7741], 'valid'],
        [[7742, 7742], 'mapped', [7743]],
        [[7743, 7743], 'valid'],
        [[7744, 7744], 'mapped', [7745]],
        [[7745, 7745], 'valid'],
        [[7746, 7746], 'mapped', [7747]],
        [[7747, 7747], 'valid'],
        [[7748, 7748], 'mapped', [7749]],
        [[7749, 7749], 'valid'],
        [[7750, 7750], 'mapped', [7751]],
        [[7751, 7751], 'valid'],
        [[7752, 7752], 'mapped', [7753]],
        [[7753, 7753], 'valid'],
        [[7754, 7754], 'mapped', [7755]],
        [[7755, 7755], 'valid'],
        [[7756, 7756], 'mapped', [7757]],
        [[7757, 7757], 'valid'],
        [[7758, 7758], 'mapped', [7759]],
        [[7759, 7759], 'valid'],
        [[7760, 7760], 'mapped', [7761]],
        [[7761, 7761], 'valid'],
        [[7762, 7762], 'mapped', [7763]],
        [[7763, 7763], 'valid'],
        [[7764, 7764], 'mapped', [7765]],
        [[7765, 7765], 'valid'],
        [[7766, 7766], 'mapped', [7767]],
        [[7767, 7767], 'valid'],
        [[7768, 7768], 'mapped', [7769]],
        [[7769, 7769], 'valid'],
        [[7770, 7770], 'mapped', [7771]],
        [[7771, 7771], 'valid'],
        [[7772, 7772], 'mapped', [7773]],
        [[7773, 7773], 'valid'],
        [[7774, 7774], 'mapped', [7775]],
        [[7775, 7775], 'valid'],
        [[7776, 7776], 'mapped', [7777]],
        [[7777, 7777], 'valid'],
        [[7778, 7778], 'mapped', [7779]],
        [[7779, 7779], 'valid'],
        [[7780, 7780], 'mapped', [7781]],
        [[7781, 7781], 'valid'],
        [[7782, 7782], 'mapped', [7783]],
        [[7783, 7783], 'valid'],
        [[7784, 7784], 'mapped', [7785]],
        [[7785, 7785], 'valid'],
        [[7786, 7786], 'mapped', [7787]],
        [[7787, 7787], 'valid'],
        [[7788, 7788], 'mapped', [7789]],
        [[7789, 7789], 'valid'],
        [[7790, 7790], 'mapped', [7791]],
        [[7791, 7791], 'valid'],
        [[7792, 7792], 'mapped', [7793]],
        [[7793, 7793], 'valid'],
        [[7794, 7794], 'mapped', [7795]],
        [[7795, 7795], 'valid'],
        [[7796, 7796], 'mapped', [7797]],
        [[7797, 7797], 'valid'],
        [[7798, 7798], 'mapped', [7799]],
        [[7799, 7799], 'valid'],
        [[7800, 7800], 'mapped', [7801]],
        [[7801, 7801], 'valid'],
        [[7802, 7802], 'mapped', [7803]],
        [[7803, 7803], 'valid'],
        [[7804, 7804], 'mapped', [7805]],
        [[7805, 7805], 'valid'],
        [[7806, 7806], 'mapped', [7807]],
        [[7807, 7807], 'valid'],
        [[7808, 7808], 'mapped', [7809]],
        [[7809, 7809], 'valid'],
        [[7810, 7810], 'mapped', [7811]],
        [[7811, 7811], 'valid'],
        [[7812, 7812], 'mapped', [7813]],
        [[7813, 7813], 'valid'],
        [[7814, 7814], 'mapped', [7815]],
        [[7815, 7815], 'valid'],
        [[7816, 7816], 'mapped', [7817]],
        [[7817, 7817], 'valid'],
        [[7818, 7818], 'mapped', [7819]],
        [[7819, 7819], 'valid'],
        [[7820, 7820], 'mapped', [7821]],
        [[7821, 7821], 'valid'],
        [[7822, 7822], 'mapped', [7823]],
        [[7823, 7823], 'valid'],
        [[7824, 7824], 'mapped', [7825]],
        [[7825, 7825], 'valid'],
        [[7826, 7826], 'mapped', [7827]],
        [[7827, 7827], 'valid'],
        [[7828, 7828], 'mapped', [7829]],
        [[7829, 7833], 'valid'],
        [[7834, 7834], 'mapped', [97, 702]],
        [[7835, 7835], 'mapped', [7777]],
        [[7836, 7837], 'valid'],
        [[7838, 7838], 'mapped', [115, 115]],
        [[7839, 7839], 'valid'],
        [[7840, 7840], 'mapped', [7841]],
        [[7841, 7841], 'valid'],
        [[7842, 7842], 'mapped', [7843]],
        [[7843, 7843], 'valid'],
        [[7844, 7844], 'mapped', [7845]],
        [[7845, 7845], 'valid'],
        [[7846, 7846], 'mapped', [7847]],
        [[7847, 7847], 'valid'],
        [[7848, 7848], 'mapped', [7849]],
        [[7849, 7849], 'valid'],
        [[7850, 7850], 'mapped', [7851]],
        [[7851, 7851], 'valid'],
        [[7852, 7852], 'mapped', [7853]],
        [[7853, 7853], 'valid'],
        [[7854, 7854], 'mapped', [7855]],
        [[7855, 7855], 'valid'],
        [[7856, 7856], 'mapped', [7857]],
        [[7857, 7857], 'valid'],
        [[7858, 7858], 'mapped', [7859]],
        [[7859, 7859], 'valid'],
        [[7860, 7860], 'mapped', [7861]],
        [[7861, 7861], 'valid'],
        [[7862, 7862], 'mapped', [7863]],
        [[7863, 7863], 'valid'],
        [[7864, 7864], 'mapped', [7865]],
        [[7865, 7865], 'valid'],
        [[7866, 7866], 'mapped', [7867]],
        [[7867, 7867], 'valid'],
        [[7868, 7868], 'mapped', [7869]],
        [[7869, 7869], 'valid'],
        [[7870, 7870], 'mapped', [7871]],
        [[7871, 7871], 'valid'],
        [[7872, 7872], 'mapped', [7873]],
        [[7873, 7873], 'valid'],
        [[7874, 7874], 'mapped', [7875]],
        [[7875, 7875], 'valid'],
        [[7876, 7876], 'mapped', [7877]],
        [[7877, 7877], 'valid'],
        [[7878, 7878], 'mapped', [7879]],
        [[7879, 7879], 'valid'],
        [[7880, 7880], 'mapped', [7881]],
        [[7881, 7881], 'valid'],
        [[7882, 7882], 'mapped', [7883]],
        [[7883, 7883], 'valid'],
        [[7884, 7884], 'mapped', [7885]],
        [[7885, 7885], 'valid'],
        [[7886, 7886], 'mapped', [7887]],
        [[7887, 7887], 'valid'],
        [[7888, 7888], 'mapped', [7889]],
        [[7889, 7889], 'valid'],
        [[7890, 7890], 'mapped', [7891]],
        [[7891, 7891], 'valid'],
        [[7892, 7892], 'mapped', [7893]],
        [[7893, 7893], 'valid'],
        [[7894, 7894], 'mapped', [7895]],
        [[7895, 7895], 'valid'],
        [[7896, 7896], 'mapped', [7897]],
        [[7897, 7897], 'valid'],
        [[7898, 7898], 'mapped', [7899]],
        [[7899, 7899], 'valid'],
        [[7900, 7900], 'mapped', [7901]],
        [[7901, 7901], 'valid'],
        [[7902, 7902], 'mapped', [7903]],
        [[7903, 7903], 'valid'],
        [[7904, 7904], 'mapped', [7905]],
        [[7905, 7905], 'valid'],
        [[7906, 7906], 'mapped', [7907]],
        [[7907, 7907], 'valid'],
        [[7908, 7908], 'mapped', [7909]],
        [[7909, 7909], 'valid'],
        [[7910, 7910], 'mapped', [7911]],
        [[7911, 7911], 'valid'],
        [[7912, 7912], 'mapped', [7913]],
        [[7913, 7913], 'valid'],
        [[7914, 7914], 'mapped', [7915]],
        [[7915, 7915], 'valid'],
        [[7916, 7916], 'mapped', [7917]],
        [[7917, 7917], 'valid'],
        [[7918, 7918], 'mapped', [7919]],
        [[7919, 7919], 'valid'],
        [[7920, 7920], 'mapped', [7921]],
        [[7921, 7921], 'valid'],
        [[7922, 7922], 'mapped', [7923]],
        [[7923, 7923], 'valid'],
        [[7924, 7924], 'mapped', [7925]],
        [[7925, 7925], 'valid'],
        [[7926, 7926], 'mapped', [7927]],
        [[7927, 7927], 'valid'],
        [[7928, 7928], 'mapped', [7929]],
        [[7929, 7929], 'valid'],
        [[7930, 7930], 'mapped', [7931]],
        [[7931, 7931], 'valid'],
        [[7932, 7932], 'mapped', [7933]],
        [[7933, 7933], 'valid'],
        [[7934, 7934], 'mapped', [7935]],
        [[7935, 7935], 'valid'],
        [[7936, 7943], 'valid'],
        [[7944, 7944], 'mapped', [7936]],
        [[7945, 7945], 'mapped', [7937]],
        [[7946, 7946], 'mapped', [7938]],
        [[7947, 7947], 'mapped', [7939]],
        [[7948, 7948], 'mapped', [7940]],
        [[7949, 7949], 'mapped', [7941]],
        [[7950, 7950], 'mapped', [7942]],
        [[7951, 7951], 'mapped', [7943]],
        [[7952, 7957], 'valid'],
        [[7958, 7959], 'disallowed'],
        [[7960, 7960], 'mapped', [7952]],
        [[7961, 7961], 'mapped', [7953]],
        [[7962, 7962], 'mapped', [7954]],
        [[7963, 7963], 'mapped', [7955]],
        [[7964, 7964], 'mapped', [7956]],
        [[7965, 7965], 'mapped', [7957]],
        [[7966, 7967], 'disallowed'],
        [[7968, 7975], 'valid'],
        [[7976, 7976], 'mapped', [7968]],
        [[7977, 7977], 'mapped', [7969]],
        [[7978, 7978], 'mapped', [7970]],
        [[7979, 7979], 'mapped', [7971]],
        [[7980, 7980], 'mapped', [7972]],
        [[7981, 7981], 'mapped', [7973]],
        [[7982, 7982], 'mapped', [7974]],
        [[7983, 7983], 'mapped', [7975]],
        [[7984, 7991], 'valid'],
        [[7992, 7992], 'mapped', [7984]],
        [[7993, 7993], 'mapped', [7985]],
        [[7994, 7994], 'mapped', [7986]],
        [[7995, 7995], 'mapped', [7987]],
        [[7996, 7996], 'mapped', [7988]],
        [[7997, 7997], 'mapped', [7989]],
        [[7998, 7998], 'mapped', [7990]],
        [[7999, 7999], 'mapped', [7991]],
        [[8e3, 8005], 'valid'],
        [[8006, 8007], 'disallowed'],
        [[8008, 8008], 'mapped', [8e3]],
        [[8009, 8009], 'mapped', [8001]],
        [[8010, 8010], 'mapped', [8002]],
        [[8011, 8011], 'mapped', [8003]],
        [[8012, 8012], 'mapped', [8004]],
        [[8013, 8013], 'mapped', [8005]],
        [[8014, 8015], 'disallowed'],
        [[8016, 8023], 'valid'],
        [[8024, 8024], 'disallowed'],
        [[8025, 8025], 'mapped', [8017]],
        [[8026, 8026], 'disallowed'],
        [[8027, 8027], 'mapped', [8019]],
        [[8028, 8028], 'disallowed'],
        [[8029, 8029], 'mapped', [8021]],
        [[8030, 8030], 'disallowed'],
        [[8031, 8031], 'mapped', [8023]],
        [[8032, 8039], 'valid'],
        [[8040, 8040], 'mapped', [8032]],
        [[8041, 8041], 'mapped', [8033]],
        [[8042, 8042], 'mapped', [8034]],
        [[8043, 8043], 'mapped', [8035]],
        [[8044, 8044], 'mapped', [8036]],
        [[8045, 8045], 'mapped', [8037]],
        [[8046, 8046], 'mapped', [8038]],
        [[8047, 8047], 'mapped', [8039]],
        [[8048, 8048], 'valid'],
        [[8049, 8049], 'mapped', [940]],
        [[8050, 8050], 'valid'],
        [[8051, 8051], 'mapped', [941]],
        [[8052, 8052], 'valid'],
        [[8053, 8053], 'mapped', [942]],
        [[8054, 8054], 'valid'],
        [[8055, 8055], 'mapped', [943]],
        [[8056, 8056], 'valid'],
        [[8057, 8057], 'mapped', [972]],
        [[8058, 8058], 'valid'],
        [[8059, 8059], 'mapped', [973]],
        [[8060, 8060], 'valid'],
        [[8061, 8061], 'mapped', [974]],
        [[8062, 8063], 'disallowed'],
        [[8064, 8064], 'mapped', [7936, 953]],
        [[8065, 8065], 'mapped', [7937, 953]],
        [[8066, 8066], 'mapped', [7938, 953]],
        [[8067, 8067], 'mapped', [7939, 953]],
        [[8068, 8068], 'mapped', [7940, 953]],
        [[8069, 8069], 'mapped', [7941, 953]],
        [[8070, 8070], 'mapped', [7942, 953]],
        [[8071, 8071], 'mapped', [7943, 953]],
        [[8072, 8072], 'mapped', [7936, 953]],
        [[8073, 8073], 'mapped', [7937, 953]],
        [[8074, 8074], 'mapped', [7938, 953]],
        [[8075, 8075], 'mapped', [7939, 953]],
        [[8076, 8076], 'mapped', [7940, 953]],
        [[8077, 8077], 'mapped', [7941, 953]],
        [[8078, 8078], 'mapped', [7942, 953]],
        [[8079, 8079], 'mapped', [7943, 953]],
        [[8080, 8080], 'mapped', [7968, 953]],
        [[8081, 8081], 'mapped', [7969, 953]],
        [[8082, 8082], 'mapped', [7970, 953]],
        [[8083, 8083], 'mapped', [7971, 953]],
        [[8084, 8084], 'mapped', [7972, 953]],
        [[8085, 8085], 'mapped', [7973, 953]],
        [[8086, 8086], 'mapped', [7974, 953]],
        [[8087, 8087], 'mapped', [7975, 953]],
        [[8088, 8088], 'mapped', [7968, 953]],
        [[8089, 8089], 'mapped', [7969, 953]],
        [[8090, 8090], 'mapped', [7970, 953]],
        [[8091, 8091], 'mapped', [7971, 953]],
        [[8092, 8092], 'mapped', [7972, 953]],
        [[8093, 8093], 'mapped', [7973, 953]],
        [[8094, 8094], 'mapped', [7974, 953]],
        [[8095, 8095], 'mapped', [7975, 953]],
        [[8096, 8096], 'mapped', [8032, 953]],
        [[8097, 8097], 'mapped', [8033, 953]],
        [[8098, 8098], 'mapped', [8034, 953]],
        [[8099, 8099], 'mapped', [8035, 953]],
        [[8100, 8100], 'mapped', [8036, 953]],
        [[8101, 8101], 'mapped', [8037, 953]],
        [[8102, 8102], 'mapped', [8038, 953]],
        [[8103, 8103], 'mapped', [8039, 953]],
        [[8104, 8104], 'mapped', [8032, 953]],
        [[8105, 8105], 'mapped', [8033, 953]],
        [[8106, 8106], 'mapped', [8034, 953]],
        [[8107, 8107], 'mapped', [8035, 953]],
        [[8108, 8108], 'mapped', [8036, 953]],
        [[8109, 8109], 'mapped', [8037, 953]],
        [[8110, 8110], 'mapped', [8038, 953]],
        [[8111, 8111], 'mapped', [8039, 953]],
        [[8112, 8113], 'valid'],
        [[8114, 8114], 'mapped', [8048, 953]],
        [[8115, 8115], 'mapped', [945, 953]],
        [[8116, 8116], 'mapped', [940, 953]],
        [[8117, 8117], 'disallowed'],
        [[8118, 8118], 'valid'],
        [[8119, 8119], 'mapped', [8118, 953]],
        [[8120, 8120], 'mapped', [8112]],
        [[8121, 8121], 'mapped', [8113]],
        [[8122, 8122], 'mapped', [8048]],
        [[8123, 8123], 'mapped', [940]],
        [[8124, 8124], 'mapped', [945, 953]],
        [[8125, 8125], 'disallowed_STD3_mapped', [32, 787]],
        [[8126, 8126], 'mapped', [953]],
        [[8127, 8127], 'disallowed_STD3_mapped', [32, 787]],
        [[8128, 8128], 'disallowed_STD3_mapped', [32, 834]],
        [[8129, 8129], 'disallowed_STD3_mapped', [32, 776, 834]],
        [[8130, 8130], 'mapped', [8052, 953]],
        [[8131, 8131], 'mapped', [951, 953]],
        [[8132, 8132], 'mapped', [942, 953]],
        [[8133, 8133], 'disallowed'],
        [[8134, 8134], 'valid'],
        [[8135, 8135], 'mapped', [8134, 953]],
        [[8136, 8136], 'mapped', [8050]],
        [[8137, 8137], 'mapped', [941]],
        [[8138, 8138], 'mapped', [8052]],
        [[8139, 8139], 'mapped', [942]],
        [[8140, 8140], 'mapped', [951, 953]],
        [[8141, 8141], 'disallowed_STD3_mapped', [32, 787, 768]],
        [[8142, 8142], 'disallowed_STD3_mapped', [32, 787, 769]],
        [[8143, 8143], 'disallowed_STD3_mapped', [32, 787, 834]],
        [[8144, 8146], 'valid'],
        [[8147, 8147], 'mapped', [912]],
        [[8148, 8149], 'disallowed'],
        [[8150, 8151], 'valid'],
        [[8152, 8152], 'mapped', [8144]],
        [[8153, 8153], 'mapped', [8145]],
        [[8154, 8154], 'mapped', [8054]],
        [[8155, 8155], 'mapped', [943]],
        [[8156, 8156], 'disallowed'],
        [[8157, 8157], 'disallowed_STD3_mapped', [32, 788, 768]],
        [[8158, 8158], 'disallowed_STD3_mapped', [32, 788, 769]],
        [[8159, 8159], 'disallowed_STD3_mapped', [32, 788, 834]],
        [[8160, 8162], 'valid'],
        [[8163, 8163], 'mapped', [944]],
        [[8164, 8167], 'valid'],
        [[8168, 8168], 'mapped', [8160]],
        [[8169, 8169], 'mapped', [8161]],
        [[8170, 8170], 'mapped', [8058]],
        [[8171, 8171], 'mapped', [973]],
        [[8172, 8172], 'mapped', [8165]],
        [[8173, 8173], 'disallowed_STD3_mapped', [32, 776, 768]],
        [[8174, 8174], 'disallowed_STD3_mapped', [32, 776, 769]],
        [[8175, 8175], 'disallowed_STD3_mapped', [96]],
        [[8176, 8177], 'disallowed'],
        [[8178, 8178], 'mapped', [8060, 953]],
        [[8179, 8179], 'mapped', [969, 953]],
        [[8180, 8180], 'mapped', [974, 953]],
        [[8181, 8181], 'disallowed'],
        [[8182, 8182], 'valid'],
        [[8183, 8183], 'mapped', [8182, 953]],
        [[8184, 8184], 'mapped', [8056]],
        [[8185, 8185], 'mapped', [972]],
        [[8186, 8186], 'mapped', [8060]],
        [[8187, 8187], 'mapped', [974]],
        [[8188, 8188], 'mapped', [969, 953]],
        [[8189, 8189], 'disallowed_STD3_mapped', [32, 769]],
        [[8190, 8190], 'disallowed_STD3_mapped', [32, 788]],
        [[8191, 8191], 'disallowed'],
        [[8192, 8202], 'disallowed_STD3_mapped', [32]],
        [[8203, 8203], 'ignored'],
        [[8204, 8205], 'deviation', []],
        [[8206, 8207], 'disallowed'],
        [[8208, 8208], 'valid', [], 'NV8'],
        [[8209, 8209], 'mapped', [8208]],
        [[8210, 8214], 'valid', [], 'NV8'],
        [[8215, 8215], 'disallowed_STD3_mapped', [32, 819]],
        [[8216, 8227], 'valid', [], 'NV8'],
        [[8228, 8230], 'disallowed'],
        [[8231, 8231], 'valid', [], 'NV8'],
        [[8232, 8238], 'disallowed'],
        [[8239, 8239], 'disallowed_STD3_mapped', [32]],
        [[8240, 8242], 'valid', [], 'NV8'],
        [[8243, 8243], 'mapped', [8242, 8242]],
        [[8244, 8244], 'mapped', [8242, 8242, 8242]],
        [[8245, 8245], 'valid', [], 'NV8'],
        [[8246, 8246], 'mapped', [8245, 8245]],
        [[8247, 8247], 'mapped', [8245, 8245, 8245]],
        [[8248, 8251], 'valid', [], 'NV8'],
        [[8252, 8252], 'disallowed_STD3_mapped', [33, 33]],
        [[8253, 8253], 'valid', [], 'NV8'],
        [[8254, 8254], 'disallowed_STD3_mapped', [32, 773]],
        [[8255, 8262], 'valid', [], 'NV8'],
        [[8263, 8263], 'disallowed_STD3_mapped', [63, 63]],
        [[8264, 8264], 'disallowed_STD3_mapped', [63, 33]],
        [[8265, 8265], 'disallowed_STD3_mapped', [33, 63]],
        [[8266, 8269], 'valid', [], 'NV8'],
        [[8270, 8274], 'valid', [], 'NV8'],
        [[8275, 8276], 'valid', [], 'NV8'],
        [[8277, 8278], 'valid', [], 'NV8'],
        [[8279, 8279], 'mapped', [8242, 8242, 8242, 8242]],
        [[8280, 8286], 'valid', [], 'NV8'],
        [[8287, 8287], 'disallowed_STD3_mapped', [32]],
        [[8288, 8288], 'ignored'],
        [[8289, 8291], 'disallowed'],
        [[8292, 8292], 'ignored'],
        [[8293, 8293], 'disallowed'],
        [[8294, 8297], 'disallowed'],
        [[8298, 8303], 'disallowed'],
        [[8304, 8304], 'mapped', [48]],
        [[8305, 8305], 'mapped', [105]],
        [[8306, 8307], 'disallowed'],
        [[8308, 8308], 'mapped', [52]],
        [[8309, 8309], 'mapped', [53]],
        [[8310, 8310], 'mapped', [54]],
        [[8311, 8311], 'mapped', [55]],
        [[8312, 8312], 'mapped', [56]],
        [[8313, 8313], 'mapped', [57]],
        [[8314, 8314], 'disallowed_STD3_mapped', [43]],
        [[8315, 8315], 'mapped', [8722]],
        [[8316, 8316], 'disallowed_STD3_mapped', [61]],
        [[8317, 8317], 'disallowed_STD3_mapped', [40]],
        [[8318, 8318], 'disallowed_STD3_mapped', [41]],
        [[8319, 8319], 'mapped', [110]],
        [[8320, 8320], 'mapped', [48]],
        [[8321, 8321], 'mapped', [49]],
        [[8322, 8322], 'mapped', [50]],
        [[8323, 8323], 'mapped', [51]],
        [[8324, 8324], 'mapped', [52]],
        [[8325, 8325], 'mapped', [53]],
        [[8326, 8326], 'mapped', [54]],
        [[8327, 8327], 'mapped', [55]],
        [[8328, 8328], 'mapped', [56]],
        [[8329, 8329], 'mapped', [57]],
        [[8330, 8330], 'disallowed_STD3_mapped', [43]],
        [[8331, 8331], 'mapped', [8722]],
        [[8332, 8332], 'disallowed_STD3_mapped', [61]],
        [[8333, 8333], 'disallowed_STD3_mapped', [40]],
        [[8334, 8334], 'disallowed_STD3_mapped', [41]],
        [[8335, 8335], 'disallowed'],
        [[8336, 8336], 'mapped', [97]],
        [[8337, 8337], 'mapped', [101]],
        [[8338, 8338], 'mapped', [111]],
        [[8339, 8339], 'mapped', [120]],
        [[8340, 8340], 'mapped', [601]],
        [[8341, 8341], 'mapped', [104]],
        [[8342, 8342], 'mapped', [107]],
        [[8343, 8343], 'mapped', [108]],
        [[8344, 8344], 'mapped', [109]],
        [[8345, 8345], 'mapped', [110]],
        [[8346, 8346], 'mapped', [112]],
        [[8347, 8347], 'mapped', [115]],
        [[8348, 8348], 'mapped', [116]],
        [[8349, 8351], 'disallowed'],
        [[8352, 8359], 'valid', [], 'NV8'],
        [[8360, 8360], 'mapped', [114, 115]],
        [[8361, 8362], 'valid', [], 'NV8'],
        [[8363, 8363], 'valid', [], 'NV8'],
        [[8364, 8364], 'valid', [], 'NV8'],
        [[8365, 8367], 'valid', [], 'NV8'],
        [[8368, 8369], 'valid', [], 'NV8'],
        [[8370, 8373], 'valid', [], 'NV8'],
        [[8374, 8376], 'valid', [], 'NV8'],
        [[8377, 8377], 'valid', [], 'NV8'],
        [[8378, 8378], 'valid', [], 'NV8'],
        [[8379, 8381], 'valid', [], 'NV8'],
        [[8382, 8382], 'valid', [], 'NV8'],
        [[8383, 8399], 'disallowed'],
        [[8400, 8417], 'valid', [], 'NV8'],
        [[8418, 8419], 'valid', [], 'NV8'],
        [[8420, 8426], 'valid', [], 'NV8'],
        [[8427, 8427], 'valid', [], 'NV8'],
        [[8428, 8431], 'valid', [], 'NV8'],
        [[8432, 8432], 'valid', [], 'NV8'],
        [[8433, 8447], 'disallowed'],
        [[8448, 8448], 'disallowed_STD3_mapped', [97, 47, 99]],
        [[8449, 8449], 'disallowed_STD3_mapped', [97, 47, 115]],
        [[8450, 8450], 'mapped', [99]],
        [[8451, 8451], 'mapped', [176, 99]],
        [[8452, 8452], 'valid', [], 'NV8'],
        [[8453, 8453], 'disallowed_STD3_mapped', [99, 47, 111]],
        [[8454, 8454], 'disallowed_STD3_mapped', [99, 47, 117]],
        [[8455, 8455], 'mapped', [603]],
        [[8456, 8456], 'valid', [], 'NV8'],
        [[8457, 8457], 'mapped', [176, 102]],
        [[8458, 8458], 'mapped', [103]],
        [[8459, 8462], 'mapped', [104]],
        [[8463, 8463], 'mapped', [295]],
        [[8464, 8465], 'mapped', [105]],
        [[8466, 8467], 'mapped', [108]],
        [[8468, 8468], 'valid', [], 'NV8'],
        [[8469, 8469], 'mapped', [110]],
        [[8470, 8470], 'mapped', [110, 111]],
        [[8471, 8472], 'valid', [], 'NV8'],
        [[8473, 8473], 'mapped', [112]],
        [[8474, 8474], 'mapped', [113]],
        [[8475, 8477], 'mapped', [114]],
        [[8478, 8479], 'valid', [], 'NV8'],
        [[8480, 8480], 'mapped', [115, 109]],
        [[8481, 8481], 'mapped', [116, 101, 108]],
        [[8482, 8482], 'mapped', [116, 109]],
        [[8483, 8483], 'valid', [], 'NV8'],
        [[8484, 8484], 'mapped', [122]],
        [[8485, 8485], 'valid', [], 'NV8'],
        [[8486, 8486], 'mapped', [969]],
        [[8487, 8487], 'valid', [], 'NV8'],
        [[8488, 8488], 'mapped', [122]],
        [[8489, 8489], 'valid', [], 'NV8'],
        [[8490, 8490], 'mapped', [107]],
        [[8491, 8491], 'mapped', [229]],
        [[8492, 8492], 'mapped', [98]],
        [[8493, 8493], 'mapped', [99]],
        [[8494, 8494], 'valid', [], 'NV8'],
        [[8495, 8496], 'mapped', [101]],
        [[8497, 8497], 'mapped', [102]],
        [[8498, 8498], 'disallowed'],
        [[8499, 8499], 'mapped', [109]],
        [[8500, 8500], 'mapped', [111]],
        [[8501, 8501], 'mapped', [1488]],
        [[8502, 8502], 'mapped', [1489]],
        [[8503, 8503], 'mapped', [1490]],
        [[8504, 8504], 'mapped', [1491]],
        [[8505, 8505], 'mapped', [105]],
        [[8506, 8506], 'valid', [], 'NV8'],
        [[8507, 8507], 'mapped', [102, 97, 120]],
        [[8508, 8508], 'mapped', [960]],
        [[8509, 8510], 'mapped', [947]],
        [[8511, 8511], 'mapped', [960]],
        [[8512, 8512], 'mapped', [8721]],
        [[8513, 8516], 'valid', [], 'NV8'],
        [[8517, 8518], 'mapped', [100]],
        [[8519, 8519], 'mapped', [101]],
        [[8520, 8520], 'mapped', [105]],
        [[8521, 8521], 'mapped', [106]],
        [[8522, 8523], 'valid', [], 'NV8'],
        [[8524, 8524], 'valid', [], 'NV8'],
        [[8525, 8525], 'valid', [], 'NV8'],
        [[8526, 8526], 'valid'],
        [[8527, 8527], 'valid', [], 'NV8'],
        [[8528, 8528], 'mapped', [49, 8260, 55]],
        [[8529, 8529], 'mapped', [49, 8260, 57]],
        [[8530, 8530], 'mapped', [49, 8260, 49, 48]],
        [[8531, 8531], 'mapped', [49, 8260, 51]],
        [[8532, 8532], 'mapped', [50, 8260, 51]],
        [[8533, 8533], 'mapped', [49, 8260, 53]],
        [[8534, 8534], 'mapped', [50, 8260, 53]],
        [[8535, 8535], 'mapped', [51, 8260, 53]],
        [[8536, 8536], 'mapped', [52, 8260, 53]],
        [[8537, 8537], 'mapped', [49, 8260, 54]],
        [[8538, 8538], 'mapped', [53, 8260, 54]],
        [[8539, 8539], 'mapped', [49, 8260, 56]],
        [[8540, 8540], 'mapped', [51, 8260, 56]],
        [[8541, 8541], 'mapped', [53, 8260, 56]],
        [[8542, 8542], 'mapped', [55, 8260, 56]],
        [[8543, 8543], 'mapped', [49, 8260]],
        [[8544, 8544], 'mapped', [105]],
        [[8545, 8545], 'mapped', [105, 105]],
        [[8546, 8546], 'mapped', [105, 105, 105]],
        [[8547, 8547], 'mapped', [105, 118]],
        [[8548, 8548], 'mapped', [118]],
        [[8549, 8549], 'mapped', [118, 105]],
        [[8550, 8550], 'mapped', [118, 105, 105]],
        [[8551, 8551], 'mapped', [118, 105, 105, 105]],
        [[8552, 8552], 'mapped', [105, 120]],
        [[8553, 8553], 'mapped', [120]],
        [[8554, 8554], 'mapped', [120, 105]],
        [[8555, 8555], 'mapped', [120, 105, 105]],
        [[8556, 8556], 'mapped', [108]],
        [[8557, 8557], 'mapped', [99]],
        [[8558, 8558], 'mapped', [100]],
        [[8559, 8559], 'mapped', [109]],
        [[8560, 8560], 'mapped', [105]],
        [[8561, 8561], 'mapped', [105, 105]],
        [[8562, 8562], 'mapped', [105, 105, 105]],
        [[8563, 8563], 'mapped', [105, 118]],
        [[8564, 8564], 'mapped', [118]],
        [[8565, 8565], 'mapped', [118, 105]],
        [[8566, 8566], 'mapped', [118, 105, 105]],
        [[8567, 8567], 'mapped', [118, 105, 105, 105]],
        [[8568, 8568], 'mapped', [105, 120]],
        [[8569, 8569], 'mapped', [120]],
        [[8570, 8570], 'mapped', [120, 105]],
        [[8571, 8571], 'mapped', [120, 105, 105]],
        [[8572, 8572], 'mapped', [108]],
        [[8573, 8573], 'mapped', [99]],
        [[8574, 8574], 'mapped', [100]],
        [[8575, 8575], 'mapped', [109]],
        [[8576, 8578], 'valid', [], 'NV8'],
        [[8579, 8579], 'disallowed'],
        [[8580, 8580], 'valid'],
        [[8581, 8584], 'valid', [], 'NV8'],
        [[8585, 8585], 'mapped', [48, 8260, 51]],
        [[8586, 8587], 'valid', [], 'NV8'],
        [[8588, 8591], 'disallowed'],
        [[8592, 8682], 'valid', [], 'NV8'],
        [[8683, 8691], 'valid', [], 'NV8'],
        [[8692, 8703], 'valid', [], 'NV8'],
        [[8704, 8747], 'valid', [], 'NV8'],
        [[8748, 8748], 'mapped', [8747, 8747]],
        [[8749, 8749], 'mapped', [8747, 8747, 8747]],
        [[8750, 8750], 'valid', [], 'NV8'],
        [[8751, 8751], 'mapped', [8750, 8750]],
        [[8752, 8752], 'mapped', [8750, 8750, 8750]],
        [[8753, 8799], 'valid', [], 'NV8'],
        [[8800, 8800], 'disallowed_STD3_valid'],
        [[8801, 8813], 'valid', [], 'NV8'],
        [[8814, 8815], 'disallowed_STD3_valid'],
        [[8816, 8945], 'valid', [], 'NV8'],
        [[8946, 8959], 'valid', [], 'NV8'],
        [[8960, 8960], 'valid', [], 'NV8'],
        [[8961, 8961], 'valid', [], 'NV8'],
        [[8962, 9e3], 'valid', [], 'NV8'],
        [[9001, 9001], 'mapped', [12296]],
        [[9002, 9002], 'mapped', [12297]],
        [[9003, 9082], 'valid', [], 'NV8'],
        [[9083, 9083], 'valid', [], 'NV8'],
        [[9084, 9084], 'valid', [], 'NV8'],
        [[9085, 9114], 'valid', [], 'NV8'],
        [[9115, 9166], 'valid', [], 'NV8'],
        [[9167, 9168], 'valid', [], 'NV8'],
        [[9169, 9179], 'valid', [], 'NV8'],
        [[9180, 9191], 'valid', [], 'NV8'],
        [[9192, 9192], 'valid', [], 'NV8'],
        [[9193, 9203], 'valid', [], 'NV8'],
        [[9204, 9210], 'valid', [], 'NV8'],
        [[9211, 9215], 'disallowed'],
        [[9216, 9252], 'valid', [], 'NV8'],
        [[9253, 9254], 'valid', [], 'NV8'],
        [[9255, 9279], 'disallowed'],
        [[9280, 9290], 'valid', [], 'NV8'],
        [[9291, 9311], 'disallowed'],
        [[9312, 9312], 'mapped', [49]],
        [[9313, 9313], 'mapped', [50]],
        [[9314, 9314], 'mapped', [51]],
        [[9315, 9315], 'mapped', [52]],
        [[9316, 9316], 'mapped', [53]],
        [[9317, 9317], 'mapped', [54]],
        [[9318, 9318], 'mapped', [55]],
        [[9319, 9319], 'mapped', [56]],
        [[9320, 9320], 'mapped', [57]],
        [[9321, 9321], 'mapped', [49, 48]],
        [[9322, 9322], 'mapped', [49, 49]],
        [[9323, 9323], 'mapped', [49, 50]],
        [[9324, 9324], 'mapped', [49, 51]],
        [[9325, 9325], 'mapped', [49, 52]],
        [[9326, 9326], 'mapped', [49, 53]],
        [[9327, 9327], 'mapped', [49, 54]],
        [[9328, 9328], 'mapped', [49, 55]],
        [[9329, 9329], 'mapped', [49, 56]],
        [[9330, 9330], 'mapped', [49, 57]],
        [[9331, 9331], 'mapped', [50, 48]],
        [[9332, 9332], 'disallowed_STD3_mapped', [40, 49, 41]],
        [[9333, 9333], 'disallowed_STD3_mapped', [40, 50, 41]],
        [[9334, 9334], 'disallowed_STD3_mapped', [40, 51, 41]],
        [[9335, 9335], 'disallowed_STD3_mapped', [40, 52, 41]],
        [[9336, 9336], 'disallowed_STD3_mapped', [40, 53, 41]],
        [[9337, 9337], 'disallowed_STD3_mapped', [40, 54, 41]],
        [[9338, 9338], 'disallowed_STD3_mapped', [40, 55, 41]],
        [[9339, 9339], 'disallowed_STD3_mapped', [40, 56, 41]],
        [[9340, 9340], 'disallowed_STD3_mapped', [40, 57, 41]],
        [[9341, 9341], 'disallowed_STD3_mapped', [40, 49, 48, 41]],
        [[9342, 9342], 'disallowed_STD3_mapped', [40, 49, 49, 41]],
        [[9343, 9343], 'disallowed_STD3_mapped', [40, 49, 50, 41]],
        [[9344, 9344], 'disallowed_STD3_mapped', [40, 49, 51, 41]],
        [[9345, 9345], 'disallowed_STD3_mapped', [40, 49, 52, 41]],
        [[9346, 9346], 'disallowed_STD3_mapped', [40, 49, 53, 41]],
        [[9347, 9347], 'disallowed_STD3_mapped', [40, 49, 54, 41]],
        [[9348, 9348], 'disallowed_STD3_mapped', [40, 49, 55, 41]],
        [[9349, 9349], 'disallowed_STD3_mapped', [40, 49, 56, 41]],
        [[9350, 9350], 'disallowed_STD3_mapped', [40, 49, 57, 41]],
        [[9351, 9351], 'disallowed_STD3_mapped', [40, 50, 48, 41]],
        [[9352, 9371], 'disallowed'],
        [[9372, 9372], 'disallowed_STD3_mapped', [40, 97, 41]],
        [[9373, 9373], 'disallowed_STD3_mapped', [40, 98, 41]],
        [[9374, 9374], 'disallowed_STD3_mapped', [40, 99, 41]],
        [[9375, 9375], 'disallowed_STD3_mapped', [40, 100, 41]],
        [[9376, 9376], 'disallowed_STD3_mapped', [40, 101, 41]],
        [[9377, 9377], 'disallowed_STD3_mapped', [40, 102, 41]],
        [[9378, 9378], 'disallowed_STD3_mapped', [40, 103, 41]],
        [[9379, 9379], 'disallowed_STD3_mapped', [40, 104, 41]],
        [[9380, 9380], 'disallowed_STD3_mapped', [40, 105, 41]],
        [[9381, 9381], 'disallowed_STD3_mapped', [40, 106, 41]],
        [[9382, 9382], 'disallowed_STD3_mapped', [40, 107, 41]],
        [[9383, 9383], 'disallowed_STD3_mapped', [40, 108, 41]],
        [[9384, 9384], 'disallowed_STD3_mapped', [40, 109, 41]],
        [[9385, 9385], 'disallowed_STD3_mapped', [40, 110, 41]],
        [[9386, 9386], 'disallowed_STD3_mapped', [40, 111, 41]],
        [[9387, 9387], 'disallowed_STD3_mapped', [40, 112, 41]],
        [[9388, 9388], 'disallowed_STD3_mapped', [40, 113, 41]],
        [[9389, 9389], 'disallowed_STD3_mapped', [40, 114, 41]],
        [[9390, 9390], 'disallowed_STD3_mapped', [40, 115, 41]],
        [[9391, 9391], 'disallowed_STD3_mapped', [40, 116, 41]],
        [[9392, 9392], 'disallowed_STD3_mapped', [40, 117, 41]],
        [[9393, 9393], 'disallowed_STD3_mapped', [40, 118, 41]],
        [[9394, 9394], 'disallowed_STD3_mapped', [40, 119, 41]],
        [[9395, 9395], 'disallowed_STD3_mapped', [40, 120, 41]],
        [[9396, 9396], 'disallowed_STD3_mapped', [40, 121, 41]],
        [[9397, 9397], 'disallowed_STD3_mapped', [40, 122, 41]],
        [[9398, 9398], 'mapped', [97]],
        [[9399, 9399], 'mapped', [98]],
        [[9400, 9400], 'mapped', [99]],
        [[9401, 9401], 'mapped', [100]],
        [[9402, 9402], 'mapped', [101]],
        [[9403, 9403], 'mapped', [102]],
        [[9404, 9404], 'mapped', [103]],
        [[9405, 9405], 'mapped', [104]],
        [[9406, 9406], 'mapped', [105]],
        [[9407, 9407], 'mapped', [106]],
        [[9408, 9408], 'mapped', [107]],
        [[9409, 9409], 'mapped', [108]],
        [[9410, 9410], 'mapped', [109]],
        [[9411, 9411], 'mapped', [110]],
        [[9412, 9412], 'mapped', [111]],
        [[9413, 9413], 'mapped', [112]],
        [[9414, 9414], 'mapped', [113]],
        [[9415, 9415], 'mapped', [114]],
        [[9416, 9416], 'mapped', [115]],
        [[9417, 9417], 'mapped', [116]],
        [[9418, 9418], 'mapped', [117]],
        [[9419, 9419], 'mapped', [118]],
        [[9420, 9420], 'mapped', [119]],
        [[9421, 9421], 'mapped', [120]],
        [[9422, 9422], 'mapped', [121]],
        [[9423, 9423], 'mapped', [122]],
        [[9424, 9424], 'mapped', [97]],
        [[9425, 9425], 'mapped', [98]],
        [[9426, 9426], 'mapped', [99]],
        [[9427, 9427], 'mapped', [100]],
        [[9428, 9428], 'mapped', [101]],
        [[9429, 9429], 'mapped', [102]],
        [[9430, 9430], 'mapped', [103]],
        [[9431, 9431], 'mapped', [104]],
        [[9432, 9432], 'mapped', [105]],
        [[9433, 9433], 'mapped', [106]],
        [[9434, 9434], 'mapped', [107]],
        [[9435, 9435], 'mapped', [108]],
        [[9436, 9436], 'mapped', [109]],
        [[9437, 9437], 'mapped', [110]],
        [[9438, 9438], 'mapped', [111]],
        [[9439, 9439], 'mapped', [112]],
        [[9440, 9440], 'mapped', [113]],
        [[9441, 9441], 'mapped', [114]],
        [[9442, 9442], 'mapped', [115]],
        [[9443, 9443], 'mapped', [116]],
        [[9444, 9444], 'mapped', [117]],
        [[9445, 9445], 'mapped', [118]],
        [[9446, 9446], 'mapped', [119]],
        [[9447, 9447], 'mapped', [120]],
        [[9448, 9448], 'mapped', [121]],
        [[9449, 9449], 'mapped', [122]],
        [[9450, 9450], 'mapped', [48]],
        [[9451, 9470], 'valid', [], 'NV8'],
        [[9471, 9471], 'valid', [], 'NV8'],
        [[9472, 9621], 'valid', [], 'NV8'],
        [[9622, 9631], 'valid', [], 'NV8'],
        [[9632, 9711], 'valid', [], 'NV8'],
        [[9712, 9719], 'valid', [], 'NV8'],
        [[9720, 9727], 'valid', [], 'NV8'],
        [[9728, 9747], 'valid', [], 'NV8'],
        [[9748, 9749], 'valid', [], 'NV8'],
        [[9750, 9751], 'valid', [], 'NV8'],
        [[9752, 9752], 'valid', [], 'NV8'],
        [[9753, 9753], 'valid', [], 'NV8'],
        [[9754, 9839], 'valid', [], 'NV8'],
        [[9840, 9841], 'valid', [], 'NV8'],
        [[9842, 9853], 'valid', [], 'NV8'],
        [[9854, 9855], 'valid', [], 'NV8'],
        [[9856, 9865], 'valid', [], 'NV8'],
        [[9866, 9873], 'valid', [], 'NV8'],
        [[9874, 9884], 'valid', [], 'NV8'],
        [[9885, 9885], 'valid', [], 'NV8'],
        [[9886, 9887], 'valid', [], 'NV8'],
        [[9888, 9889], 'valid', [], 'NV8'],
        [[9890, 9905], 'valid', [], 'NV8'],
        [[9906, 9906], 'valid', [], 'NV8'],
        [[9907, 9916], 'valid', [], 'NV8'],
        [[9917, 9919], 'valid', [], 'NV8'],
        [[9920, 9923], 'valid', [], 'NV8'],
        [[9924, 9933], 'valid', [], 'NV8'],
        [[9934, 9934], 'valid', [], 'NV8'],
        [[9935, 9953], 'valid', [], 'NV8'],
        [[9954, 9954], 'valid', [], 'NV8'],
        [[9955, 9955], 'valid', [], 'NV8'],
        [[9956, 9959], 'valid', [], 'NV8'],
        [[9960, 9983], 'valid', [], 'NV8'],
        [[9984, 9984], 'valid', [], 'NV8'],
        [[9985, 9988], 'valid', [], 'NV8'],
        [[9989, 9989], 'valid', [], 'NV8'],
        [[9990, 9993], 'valid', [], 'NV8'],
        [[9994, 9995], 'valid', [], 'NV8'],
        [[9996, 10023], 'valid', [], 'NV8'],
        [[10024, 10024], 'valid', [], 'NV8'],
        [[10025, 10059], 'valid', [], 'NV8'],
        [[10060, 10060], 'valid', [], 'NV8'],
        [[10061, 10061], 'valid', [], 'NV8'],
        [[10062, 10062], 'valid', [], 'NV8'],
        [[10063, 10066], 'valid', [], 'NV8'],
        [[10067, 10069], 'valid', [], 'NV8'],
        [[10070, 10070], 'valid', [], 'NV8'],
        [[10071, 10071], 'valid', [], 'NV8'],
        [[10072, 10078], 'valid', [], 'NV8'],
        [[10079, 10080], 'valid', [], 'NV8'],
        [[10081, 10087], 'valid', [], 'NV8'],
        [[10088, 10101], 'valid', [], 'NV8'],
        [[10102, 10132], 'valid', [], 'NV8'],
        [[10133, 10135], 'valid', [], 'NV8'],
        [[10136, 10159], 'valid', [], 'NV8'],
        [[10160, 10160], 'valid', [], 'NV8'],
        [[10161, 10174], 'valid', [], 'NV8'],
        [[10175, 10175], 'valid', [], 'NV8'],
        [[10176, 10182], 'valid', [], 'NV8'],
        [[10183, 10186], 'valid', [], 'NV8'],
        [[10187, 10187], 'valid', [], 'NV8'],
        [[10188, 10188], 'valid', [], 'NV8'],
        [[10189, 10189], 'valid', [], 'NV8'],
        [[10190, 10191], 'valid', [], 'NV8'],
        [[10192, 10219], 'valid', [], 'NV8'],
        [[10220, 10223], 'valid', [], 'NV8'],
        [[10224, 10239], 'valid', [], 'NV8'],
        [[10240, 10495], 'valid', [], 'NV8'],
        [[10496, 10763], 'valid', [], 'NV8'],
        [[10764, 10764], 'mapped', [8747, 8747, 8747, 8747]],
        [[10765, 10867], 'valid', [], 'NV8'],
        [[10868, 10868], 'disallowed_STD3_mapped', [58, 58, 61]],
        [[10869, 10869], 'disallowed_STD3_mapped', [61, 61]],
        [[10870, 10870], 'disallowed_STD3_mapped', [61, 61, 61]],
        [[10871, 10971], 'valid', [], 'NV8'],
        [[10972, 10972], 'mapped', [10973, 824]],
        [[10973, 11007], 'valid', [], 'NV8'],
        [[11008, 11021], 'valid', [], 'NV8'],
        [[11022, 11027], 'valid', [], 'NV8'],
        [[11028, 11034], 'valid', [], 'NV8'],
        [[11035, 11039], 'valid', [], 'NV8'],
        [[11040, 11043], 'valid', [], 'NV8'],
        [[11044, 11084], 'valid', [], 'NV8'],
        [[11085, 11087], 'valid', [], 'NV8'],
        [[11088, 11092], 'valid', [], 'NV8'],
        [[11093, 11097], 'valid', [], 'NV8'],
        [[11098, 11123], 'valid', [], 'NV8'],
        [[11124, 11125], 'disallowed'],
        [[11126, 11157], 'valid', [], 'NV8'],
        [[11158, 11159], 'disallowed'],
        [[11160, 11193], 'valid', [], 'NV8'],
        [[11194, 11196], 'disallowed'],
        [[11197, 11208], 'valid', [], 'NV8'],
        [[11209, 11209], 'disallowed'],
        [[11210, 11217], 'valid', [], 'NV8'],
        [[11218, 11243], 'disallowed'],
        [[11244, 11247], 'valid', [], 'NV8'],
        [[11248, 11263], 'disallowed'],
        [[11264, 11264], 'mapped', [11312]],
        [[11265, 11265], 'mapped', [11313]],
        [[11266, 11266], 'mapped', [11314]],
        [[11267, 11267], 'mapped', [11315]],
        [[11268, 11268], 'mapped', [11316]],
        [[11269, 11269], 'mapped', [11317]],
        [[11270, 11270], 'mapped', [11318]],
        [[11271, 11271], 'mapped', [11319]],
        [[11272, 11272], 'mapped', [11320]],
        [[11273, 11273], 'mapped', [11321]],
        [[11274, 11274], 'mapped', [11322]],
        [[11275, 11275], 'mapped', [11323]],
        [[11276, 11276], 'mapped', [11324]],
        [[11277, 11277], 'mapped', [11325]],
        [[11278, 11278], 'mapped', [11326]],
        [[11279, 11279], 'mapped', [11327]],
        [[11280, 11280], 'mapped', [11328]],
        [[11281, 11281], 'mapped', [11329]],
        [[11282, 11282], 'mapped', [11330]],
        [[11283, 11283], 'mapped', [11331]],
        [[11284, 11284], 'mapped', [11332]],
        [[11285, 11285], 'mapped', [11333]],
        [[11286, 11286], 'mapped', [11334]],
        [[11287, 11287], 'mapped', [11335]],
        [[11288, 11288], 'mapped', [11336]],
        [[11289, 11289], 'mapped', [11337]],
        [[11290, 11290], 'mapped', [11338]],
        [[11291, 11291], 'mapped', [11339]],
        [[11292, 11292], 'mapped', [11340]],
        [[11293, 11293], 'mapped', [11341]],
        [[11294, 11294], 'mapped', [11342]],
        [[11295, 11295], 'mapped', [11343]],
        [[11296, 11296], 'mapped', [11344]],
        [[11297, 11297], 'mapped', [11345]],
        [[11298, 11298], 'mapped', [11346]],
        [[11299, 11299], 'mapped', [11347]],
        [[11300, 11300], 'mapped', [11348]],
        [[11301, 11301], 'mapped', [11349]],
        [[11302, 11302], 'mapped', [11350]],
        [[11303, 11303], 'mapped', [11351]],
        [[11304, 11304], 'mapped', [11352]],
        [[11305, 11305], 'mapped', [11353]],
        [[11306, 11306], 'mapped', [11354]],
        [[11307, 11307], 'mapped', [11355]],
        [[11308, 11308], 'mapped', [11356]],
        [[11309, 11309], 'mapped', [11357]],
        [[11310, 11310], 'mapped', [11358]],
        [[11311, 11311], 'disallowed'],
        [[11312, 11358], 'valid'],
        [[11359, 11359], 'disallowed'],
        [[11360, 11360], 'mapped', [11361]],
        [[11361, 11361], 'valid'],
        [[11362, 11362], 'mapped', [619]],
        [[11363, 11363], 'mapped', [7549]],
        [[11364, 11364], 'mapped', [637]],
        [[11365, 11366], 'valid'],
        [[11367, 11367], 'mapped', [11368]],
        [[11368, 11368], 'valid'],
        [[11369, 11369], 'mapped', [11370]],
        [[11370, 11370], 'valid'],
        [[11371, 11371], 'mapped', [11372]],
        [[11372, 11372], 'valid'],
        [[11373, 11373], 'mapped', [593]],
        [[11374, 11374], 'mapped', [625]],
        [[11375, 11375], 'mapped', [592]],
        [[11376, 11376], 'mapped', [594]],
        [[11377, 11377], 'valid'],
        [[11378, 11378], 'mapped', [11379]],
        [[11379, 11379], 'valid'],
        [[11380, 11380], 'valid'],
        [[11381, 11381], 'mapped', [11382]],
        [[11382, 11383], 'valid'],
        [[11384, 11387], 'valid'],
        [[11388, 11388], 'mapped', [106]],
        [[11389, 11389], 'mapped', [118]],
        [[11390, 11390], 'mapped', [575]],
        [[11391, 11391], 'mapped', [576]],
        [[11392, 11392], 'mapped', [11393]],
        [[11393, 11393], 'valid'],
        [[11394, 11394], 'mapped', [11395]],
        [[11395, 11395], 'valid'],
        [[11396, 11396], 'mapped', [11397]],
        [[11397, 11397], 'valid'],
        [[11398, 11398], 'mapped', [11399]],
        [[11399, 11399], 'valid'],
        [[11400, 11400], 'mapped', [11401]],
        [[11401, 11401], 'valid'],
        [[11402, 11402], 'mapped', [11403]],
        [[11403, 11403], 'valid'],
        [[11404, 11404], 'mapped', [11405]],
        [[11405, 11405], 'valid'],
        [[11406, 11406], 'mapped', [11407]],
        [[11407, 11407], 'valid'],
        [[11408, 11408], 'mapped', [11409]],
        [[11409, 11409], 'valid'],
        [[11410, 11410], 'mapped', [11411]],
        [[11411, 11411], 'valid'],
        [[11412, 11412], 'mapped', [11413]],
        [[11413, 11413], 'valid'],
        [[11414, 11414], 'mapped', [11415]],
        [[11415, 11415], 'valid'],
        [[11416, 11416], 'mapped', [11417]],
        [[11417, 11417], 'valid'],
        [[11418, 11418], 'mapped', [11419]],
        [[11419, 11419], 'valid'],
        [[11420, 11420], 'mapped', [11421]],
        [[11421, 11421], 'valid'],
        [[11422, 11422], 'mapped', [11423]],
        [[11423, 11423], 'valid'],
        [[11424, 11424], 'mapped', [11425]],
        [[11425, 11425], 'valid'],
        [[11426, 11426], 'mapped', [11427]],
        [[11427, 11427], 'valid'],
        [[11428, 11428], 'mapped', [11429]],
        [[11429, 11429], 'valid'],
        [[11430, 11430], 'mapped', [11431]],
        [[11431, 11431], 'valid'],
        [[11432, 11432], 'mapped', [11433]],
        [[11433, 11433], 'valid'],
        [[11434, 11434], 'mapped', [11435]],
        [[11435, 11435], 'valid'],
        [[11436, 11436], 'mapped', [11437]],
        [[11437, 11437], 'valid'],
        [[11438, 11438], 'mapped', [11439]],
        [[11439, 11439], 'valid'],
        [[11440, 11440], 'mapped', [11441]],
        [[11441, 11441], 'valid'],
        [[11442, 11442], 'mapped', [11443]],
        [[11443, 11443], 'valid'],
        [[11444, 11444], 'mapped', [11445]],
        [[11445, 11445], 'valid'],
        [[11446, 11446], 'mapped', [11447]],
        [[11447, 11447], 'valid'],
        [[11448, 11448], 'mapped', [11449]],
        [[11449, 11449], 'valid'],
        [[11450, 11450], 'mapped', [11451]],
        [[11451, 11451], 'valid'],
        [[11452, 11452], 'mapped', [11453]],
        [[11453, 11453], 'valid'],
        [[11454, 11454], 'mapped', [11455]],
        [[11455, 11455], 'valid'],
        [[11456, 11456], 'mapped', [11457]],
        [[11457, 11457], 'valid'],
        [[11458, 11458], 'mapped', [11459]],
        [[11459, 11459], 'valid'],
        [[11460, 11460], 'mapped', [11461]],
        [[11461, 11461], 'valid'],
        [[11462, 11462], 'mapped', [11463]],
        [[11463, 11463], 'valid'],
        [[11464, 11464], 'mapped', [11465]],
        [[11465, 11465], 'valid'],
        [[11466, 11466], 'mapped', [11467]],
        [[11467, 11467], 'valid'],
        [[11468, 11468], 'mapped', [11469]],
        [[11469, 11469], 'valid'],
        [[11470, 11470], 'mapped', [11471]],
        [[11471, 11471], 'valid'],
        [[11472, 11472], 'mapped', [11473]],
        [[11473, 11473], 'valid'],
        [[11474, 11474], 'mapped', [11475]],
        [[11475, 11475], 'valid'],
        [[11476, 11476], 'mapped', [11477]],
        [[11477, 11477], 'valid'],
        [[11478, 11478], 'mapped', [11479]],
        [[11479, 11479], 'valid'],
        [[11480, 11480], 'mapped', [11481]],
        [[11481, 11481], 'valid'],
        [[11482, 11482], 'mapped', [11483]],
        [[11483, 11483], 'valid'],
        [[11484, 11484], 'mapped', [11485]],
        [[11485, 11485], 'valid'],
        [[11486, 11486], 'mapped', [11487]],
        [[11487, 11487], 'valid'],
        [[11488, 11488], 'mapped', [11489]],
        [[11489, 11489], 'valid'],
        [[11490, 11490], 'mapped', [11491]],
        [[11491, 11492], 'valid'],
        [[11493, 11498], 'valid', [], 'NV8'],
        [[11499, 11499], 'mapped', [11500]],
        [[11500, 11500], 'valid'],
        [[11501, 11501], 'mapped', [11502]],
        [[11502, 11505], 'valid'],
        [[11506, 11506], 'mapped', [11507]],
        [[11507, 11507], 'valid'],
        [[11508, 11512], 'disallowed'],
        [[11513, 11519], 'valid', [], 'NV8'],
        [[11520, 11557], 'valid'],
        [[11558, 11558], 'disallowed'],
        [[11559, 11559], 'valid'],
        [[11560, 11564], 'disallowed'],
        [[11565, 11565], 'valid'],
        [[11566, 11567], 'disallowed'],
        [[11568, 11621], 'valid'],
        [[11622, 11623], 'valid'],
        [[11624, 11630], 'disallowed'],
        [[11631, 11631], 'mapped', [11617]],
        [[11632, 11632], 'valid', [], 'NV8'],
        [[11633, 11646], 'disallowed'],
        [[11647, 11647], 'valid'],
        [[11648, 11670], 'valid'],
        [[11671, 11679], 'disallowed'],
        [[11680, 11686], 'valid'],
        [[11687, 11687], 'disallowed'],
        [[11688, 11694], 'valid'],
        [[11695, 11695], 'disallowed'],
        [[11696, 11702], 'valid'],
        [[11703, 11703], 'disallowed'],
        [[11704, 11710], 'valid'],
        [[11711, 11711], 'disallowed'],
        [[11712, 11718], 'valid'],
        [[11719, 11719], 'disallowed'],
        [[11720, 11726], 'valid'],
        [[11727, 11727], 'disallowed'],
        [[11728, 11734], 'valid'],
        [[11735, 11735], 'disallowed'],
        [[11736, 11742], 'valid'],
        [[11743, 11743], 'disallowed'],
        [[11744, 11775], 'valid'],
        [[11776, 11799], 'valid', [], 'NV8'],
        [[11800, 11803], 'valid', [], 'NV8'],
        [[11804, 11805], 'valid', [], 'NV8'],
        [[11806, 11822], 'valid', [], 'NV8'],
        [[11823, 11823], 'valid'],
        [[11824, 11824], 'valid', [], 'NV8'],
        [[11825, 11825], 'valid', [], 'NV8'],
        [[11826, 11835], 'valid', [], 'NV8'],
        [[11836, 11842], 'valid', [], 'NV8'],
        [[11843, 11903], 'disallowed'],
        [[11904, 11929], 'valid', [], 'NV8'],
        [[11930, 11930], 'disallowed'],
        [[11931, 11934], 'valid', [], 'NV8'],
        [[11935, 11935], 'mapped', [27597]],
        [[11936, 12018], 'valid', [], 'NV8'],
        [[12019, 12019], 'mapped', [40863]],
        [[12020, 12031], 'disallowed'],
        [[12032, 12032], 'mapped', [19968]],
        [[12033, 12033], 'mapped', [20008]],
        [[12034, 12034], 'mapped', [20022]],
        [[12035, 12035], 'mapped', [20031]],
        [[12036, 12036], 'mapped', [20057]],
        [[12037, 12037], 'mapped', [20101]],
        [[12038, 12038], 'mapped', [20108]],
        [[12039, 12039], 'mapped', [20128]],
        [[12040, 12040], 'mapped', [20154]],
        [[12041, 12041], 'mapped', [20799]],
        [[12042, 12042], 'mapped', [20837]],
        [[12043, 12043], 'mapped', [20843]],
        [[12044, 12044], 'mapped', [20866]],
        [[12045, 12045], 'mapped', [20886]],
        [[12046, 12046], 'mapped', [20907]],
        [[12047, 12047], 'mapped', [20960]],
        [[12048, 12048], 'mapped', [20981]],
        [[12049, 12049], 'mapped', [20992]],
        [[12050, 12050], 'mapped', [21147]],
        [[12051, 12051], 'mapped', [21241]],
        [[12052, 12052], 'mapped', [21269]],
        [[12053, 12053], 'mapped', [21274]],
        [[12054, 12054], 'mapped', [21304]],
        [[12055, 12055], 'mapped', [21313]],
        [[12056, 12056], 'mapped', [21340]],
        [[12057, 12057], 'mapped', [21353]],
        [[12058, 12058], 'mapped', [21378]],
        [[12059, 12059], 'mapped', [21430]],
        [[12060, 12060], 'mapped', [21448]],
        [[12061, 12061], 'mapped', [21475]],
        [[12062, 12062], 'mapped', [22231]],
        [[12063, 12063], 'mapped', [22303]],
        [[12064, 12064], 'mapped', [22763]],
        [[12065, 12065], 'mapped', [22786]],
        [[12066, 12066], 'mapped', [22794]],
        [[12067, 12067], 'mapped', [22805]],
        [[12068, 12068], 'mapped', [22823]],
        [[12069, 12069], 'mapped', [22899]],
        [[12070, 12070], 'mapped', [23376]],
        [[12071, 12071], 'mapped', [23424]],
        [[12072, 12072], 'mapped', [23544]],
        [[12073, 12073], 'mapped', [23567]],
        [[12074, 12074], 'mapped', [23586]],
        [[12075, 12075], 'mapped', [23608]],
        [[12076, 12076], 'mapped', [23662]],
        [[12077, 12077], 'mapped', [23665]],
        [[12078, 12078], 'mapped', [24027]],
        [[12079, 12079], 'mapped', [24037]],
        [[12080, 12080], 'mapped', [24049]],
        [[12081, 12081], 'mapped', [24062]],
        [[12082, 12082], 'mapped', [24178]],
        [[12083, 12083], 'mapped', [24186]],
        [[12084, 12084], 'mapped', [24191]],
        [[12085, 12085], 'mapped', [24308]],
        [[12086, 12086], 'mapped', [24318]],
        [[12087, 12087], 'mapped', [24331]],
        [[12088, 12088], 'mapped', [24339]],
        [[12089, 12089], 'mapped', [24400]],
        [[12090, 12090], 'mapped', [24417]],
        [[12091, 12091], 'mapped', [24435]],
        [[12092, 12092], 'mapped', [24515]],
        [[12093, 12093], 'mapped', [25096]],
        [[12094, 12094], 'mapped', [25142]],
        [[12095, 12095], 'mapped', [25163]],
        [[12096, 12096], 'mapped', [25903]],
        [[12097, 12097], 'mapped', [25908]],
        [[12098, 12098], 'mapped', [25991]],
        [[12099, 12099], 'mapped', [26007]],
        [[12100, 12100], 'mapped', [26020]],
        [[12101, 12101], 'mapped', [26041]],
        [[12102, 12102], 'mapped', [26080]],
        [[12103, 12103], 'mapped', [26085]],
        [[12104, 12104], 'mapped', [26352]],
        [[12105, 12105], 'mapped', [26376]],
        [[12106, 12106], 'mapped', [26408]],
        [[12107, 12107], 'mapped', [27424]],
        [[12108, 12108], 'mapped', [27490]],
        [[12109, 12109], 'mapped', [27513]],
        [[12110, 12110], 'mapped', [27571]],
        [[12111, 12111], 'mapped', [27595]],
        [[12112, 12112], 'mapped', [27604]],
        [[12113, 12113], 'mapped', [27611]],
        [[12114, 12114], 'mapped', [27663]],
        [[12115, 12115], 'mapped', [27668]],
        [[12116, 12116], 'mapped', [27700]],
        [[12117, 12117], 'mapped', [28779]],
        [[12118, 12118], 'mapped', [29226]],
        [[12119, 12119], 'mapped', [29238]],
        [[12120, 12120], 'mapped', [29243]],
        [[12121, 12121], 'mapped', [29247]],
        [[12122, 12122], 'mapped', [29255]],
        [[12123, 12123], 'mapped', [29273]],
        [[12124, 12124], 'mapped', [29275]],
        [[12125, 12125], 'mapped', [29356]],
        [[12126, 12126], 'mapped', [29572]],
        [[12127, 12127], 'mapped', [29577]],
        [[12128, 12128], 'mapped', [29916]],
        [[12129, 12129], 'mapped', [29926]],
        [[12130, 12130], 'mapped', [29976]],
        [[12131, 12131], 'mapped', [29983]],
        [[12132, 12132], 'mapped', [29992]],
        [[12133, 12133], 'mapped', [3e4]],
        [[12134, 12134], 'mapped', [30091]],
        [[12135, 12135], 'mapped', [30098]],
        [[12136, 12136], 'mapped', [30326]],
        [[12137, 12137], 'mapped', [30333]],
        [[12138, 12138], 'mapped', [30382]],
        [[12139, 12139], 'mapped', [30399]],
        [[12140, 12140], 'mapped', [30446]],
        [[12141, 12141], 'mapped', [30683]],
        [[12142, 12142], 'mapped', [30690]],
        [[12143, 12143], 'mapped', [30707]],
        [[12144, 12144], 'mapped', [31034]],
        [[12145, 12145], 'mapped', [31160]],
        [[12146, 12146], 'mapped', [31166]],
        [[12147, 12147], 'mapped', [31348]],
        [[12148, 12148], 'mapped', [31435]],
        [[12149, 12149], 'mapped', [31481]],
        [[12150, 12150], 'mapped', [31859]],
        [[12151, 12151], 'mapped', [31992]],
        [[12152, 12152], 'mapped', [32566]],
        [[12153, 12153], 'mapped', [32593]],
        [[12154, 12154], 'mapped', [32650]],
        [[12155, 12155], 'mapped', [32701]],
        [[12156, 12156], 'mapped', [32769]],
        [[12157, 12157], 'mapped', [32780]],
        [[12158, 12158], 'mapped', [32786]],
        [[12159, 12159], 'mapped', [32819]],
        [[12160, 12160], 'mapped', [32895]],
        [[12161, 12161], 'mapped', [32905]],
        [[12162, 12162], 'mapped', [33251]],
        [[12163, 12163], 'mapped', [33258]],
        [[12164, 12164], 'mapped', [33267]],
        [[12165, 12165], 'mapped', [33276]],
        [[12166, 12166], 'mapped', [33292]],
        [[12167, 12167], 'mapped', [33307]],
        [[12168, 12168], 'mapped', [33311]],
        [[12169, 12169], 'mapped', [33390]],
        [[12170, 12170], 'mapped', [33394]],
        [[12171, 12171], 'mapped', [33400]],
        [[12172, 12172], 'mapped', [34381]],
        [[12173, 12173], 'mapped', [34411]],
        [[12174, 12174], 'mapped', [34880]],
        [[12175, 12175], 'mapped', [34892]],
        [[12176, 12176], 'mapped', [34915]],
        [[12177, 12177], 'mapped', [35198]],
        [[12178, 12178], 'mapped', [35211]],
        [[12179, 12179], 'mapped', [35282]],
        [[12180, 12180], 'mapped', [35328]],
        [[12181, 12181], 'mapped', [35895]],
        [[12182, 12182], 'mapped', [35910]],
        [[12183, 12183], 'mapped', [35925]],
        [[12184, 12184], 'mapped', [35960]],
        [[12185, 12185], 'mapped', [35997]],
        [[12186, 12186], 'mapped', [36196]],
        [[12187, 12187], 'mapped', [36208]],
        [[12188, 12188], 'mapped', [36275]],
        [[12189, 12189], 'mapped', [36523]],
        [[12190, 12190], 'mapped', [36554]],
        [[12191, 12191], 'mapped', [36763]],
        [[12192, 12192], 'mapped', [36784]],
        [[12193, 12193], 'mapped', [36789]],
        [[12194, 12194], 'mapped', [37009]],
        [[12195, 12195], 'mapped', [37193]],
        [[12196, 12196], 'mapped', [37318]],
        [[12197, 12197], 'mapped', [37324]],
        [[12198, 12198], 'mapped', [37329]],
        [[12199, 12199], 'mapped', [38263]],
        [[12200, 12200], 'mapped', [38272]],
        [[12201, 12201], 'mapped', [38428]],
        [[12202, 12202], 'mapped', [38582]],
        [[12203, 12203], 'mapped', [38585]],
        [[12204, 12204], 'mapped', [38632]],
        [[12205, 12205], 'mapped', [38737]],
        [[12206, 12206], 'mapped', [38750]],
        [[12207, 12207], 'mapped', [38754]],
        [[12208, 12208], 'mapped', [38761]],
        [[12209, 12209], 'mapped', [38859]],
        [[12210, 12210], 'mapped', [38893]],
        [[12211, 12211], 'mapped', [38899]],
        [[12212, 12212], 'mapped', [38913]],
        [[12213, 12213], 'mapped', [39080]],
        [[12214, 12214], 'mapped', [39131]],
        [[12215, 12215], 'mapped', [39135]],
        [[12216, 12216], 'mapped', [39318]],
        [[12217, 12217], 'mapped', [39321]],
        [[12218, 12218], 'mapped', [39340]],
        [[12219, 12219], 'mapped', [39592]],
        [[12220, 12220], 'mapped', [39640]],
        [[12221, 12221], 'mapped', [39647]],
        [[12222, 12222], 'mapped', [39717]],
        [[12223, 12223], 'mapped', [39727]],
        [[12224, 12224], 'mapped', [39730]],
        [[12225, 12225], 'mapped', [39740]],
        [[12226, 12226], 'mapped', [39770]],
        [[12227, 12227], 'mapped', [40165]],
        [[12228, 12228], 'mapped', [40565]],
        [[12229, 12229], 'mapped', [40575]],
        [[12230, 12230], 'mapped', [40613]],
        [[12231, 12231], 'mapped', [40635]],
        [[12232, 12232], 'mapped', [40643]],
        [[12233, 12233], 'mapped', [40653]],
        [[12234, 12234], 'mapped', [40657]],
        [[12235, 12235], 'mapped', [40697]],
        [[12236, 12236], 'mapped', [40701]],
        [[12237, 12237], 'mapped', [40718]],
        [[12238, 12238], 'mapped', [40723]],
        [[12239, 12239], 'mapped', [40736]],
        [[12240, 12240], 'mapped', [40763]],
        [[12241, 12241], 'mapped', [40778]],
        [[12242, 12242], 'mapped', [40786]],
        [[12243, 12243], 'mapped', [40845]],
        [[12244, 12244], 'mapped', [40860]],
        [[12245, 12245], 'mapped', [40864]],
        [[12246, 12271], 'disallowed'],
        [[12272, 12283], 'disallowed'],
        [[12284, 12287], 'disallowed'],
        [[12288, 12288], 'disallowed_STD3_mapped', [32]],
        [[12289, 12289], 'valid', [], 'NV8'],
        [[12290, 12290], 'mapped', [46]],
        [[12291, 12292], 'valid', [], 'NV8'],
        [[12293, 12295], 'valid'],
        [[12296, 12329], 'valid', [], 'NV8'],
        [[12330, 12333], 'valid'],
        [[12334, 12341], 'valid', [], 'NV8'],
        [[12342, 12342], 'mapped', [12306]],
        [[12343, 12343], 'valid', [], 'NV8'],
        [[12344, 12344], 'mapped', [21313]],
        [[12345, 12345], 'mapped', [21316]],
        [[12346, 12346], 'mapped', [21317]],
        [[12347, 12347], 'valid', [], 'NV8'],
        [[12348, 12348], 'valid'],
        [[12349, 12349], 'valid', [], 'NV8'],
        [[12350, 12350], 'valid', [], 'NV8'],
        [[12351, 12351], 'valid', [], 'NV8'],
        [[12352, 12352], 'disallowed'],
        [[12353, 12436], 'valid'],
        [[12437, 12438], 'valid'],
        [[12439, 12440], 'disallowed'],
        [[12441, 12442], 'valid'],
        [[12443, 12443], 'disallowed_STD3_mapped', [32, 12441]],
        [[12444, 12444], 'disallowed_STD3_mapped', [32, 12442]],
        [[12445, 12446], 'valid'],
        [[12447, 12447], 'mapped', [12424, 12426]],
        [[12448, 12448], 'valid', [], 'NV8'],
        [[12449, 12542], 'valid'],
        [[12543, 12543], 'mapped', [12467, 12488]],
        [[12544, 12548], 'disallowed'],
        [[12549, 12588], 'valid'],
        [[12589, 12589], 'valid'],
        [[12590, 12592], 'disallowed'],
        [[12593, 12593], 'mapped', [4352]],
        [[12594, 12594], 'mapped', [4353]],
        [[12595, 12595], 'mapped', [4522]],
        [[12596, 12596], 'mapped', [4354]],
        [[12597, 12597], 'mapped', [4524]],
        [[12598, 12598], 'mapped', [4525]],
        [[12599, 12599], 'mapped', [4355]],
        [[12600, 12600], 'mapped', [4356]],
        [[12601, 12601], 'mapped', [4357]],
        [[12602, 12602], 'mapped', [4528]],
        [[12603, 12603], 'mapped', [4529]],
        [[12604, 12604], 'mapped', [4530]],
        [[12605, 12605], 'mapped', [4531]],
        [[12606, 12606], 'mapped', [4532]],
        [[12607, 12607], 'mapped', [4533]],
        [[12608, 12608], 'mapped', [4378]],
        [[12609, 12609], 'mapped', [4358]],
        [[12610, 12610], 'mapped', [4359]],
        [[12611, 12611], 'mapped', [4360]],
        [[12612, 12612], 'mapped', [4385]],
        [[12613, 12613], 'mapped', [4361]],
        [[12614, 12614], 'mapped', [4362]],
        [[12615, 12615], 'mapped', [4363]],
        [[12616, 12616], 'mapped', [4364]],
        [[12617, 12617], 'mapped', [4365]],
        [[12618, 12618], 'mapped', [4366]],
        [[12619, 12619], 'mapped', [4367]],
        [[12620, 12620], 'mapped', [4368]],
        [[12621, 12621], 'mapped', [4369]],
        [[12622, 12622], 'mapped', [4370]],
        [[12623, 12623], 'mapped', [4449]],
        [[12624, 12624], 'mapped', [4450]],
        [[12625, 12625], 'mapped', [4451]],
        [[12626, 12626], 'mapped', [4452]],
        [[12627, 12627], 'mapped', [4453]],
        [[12628, 12628], 'mapped', [4454]],
        [[12629, 12629], 'mapped', [4455]],
        [[12630, 12630], 'mapped', [4456]],
        [[12631, 12631], 'mapped', [4457]],
        [[12632, 12632], 'mapped', [4458]],
        [[12633, 12633], 'mapped', [4459]],
        [[12634, 12634], 'mapped', [4460]],
        [[12635, 12635], 'mapped', [4461]],
        [[12636, 12636], 'mapped', [4462]],
        [[12637, 12637], 'mapped', [4463]],
        [[12638, 12638], 'mapped', [4464]],
        [[12639, 12639], 'mapped', [4465]],
        [[12640, 12640], 'mapped', [4466]],
        [[12641, 12641], 'mapped', [4467]],
        [[12642, 12642], 'mapped', [4468]],
        [[12643, 12643], 'mapped', [4469]],
        [[12644, 12644], 'disallowed'],
        [[12645, 12645], 'mapped', [4372]],
        [[12646, 12646], 'mapped', [4373]],
        [[12647, 12647], 'mapped', [4551]],
        [[12648, 12648], 'mapped', [4552]],
        [[12649, 12649], 'mapped', [4556]],
        [[12650, 12650], 'mapped', [4558]],
        [[12651, 12651], 'mapped', [4563]],
        [[12652, 12652], 'mapped', [4567]],
        [[12653, 12653], 'mapped', [4569]],
        [[12654, 12654], 'mapped', [4380]],
        [[12655, 12655], 'mapped', [4573]],
        [[12656, 12656], 'mapped', [4575]],
        [[12657, 12657], 'mapped', [4381]],
        [[12658, 12658], 'mapped', [4382]],
        [[12659, 12659], 'mapped', [4384]],
        [[12660, 12660], 'mapped', [4386]],
        [[12661, 12661], 'mapped', [4387]],
        [[12662, 12662], 'mapped', [4391]],
        [[12663, 12663], 'mapped', [4393]],
        [[12664, 12664], 'mapped', [4395]],
        [[12665, 12665], 'mapped', [4396]],
        [[12666, 12666], 'mapped', [4397]],
        [[12667, 12667], 'mapped', [4398]],
        [[12668, 12668], 'mapped', [4399]],
        [[12669, 12669], 'mapped', [4402]],
        [[12670, 12670], 'mapped', [4406]],
        [[12671, 12671], 'mapped', [4416]],
        [[12672, 12672], 'mapped', [4423]],
        [[12673, 12673], 'mapped', [4428]],
        [[12674, 12674], 'mapped', [4593]],
        [[12675, 12675], 'mapped', [4594]],
        [[12676, 12676], 'mapped', [4439]],
        [[12677, 12677], 'mapped', [4440]],
        [[12678, 12678], 'mapped', [4441]],
        [[12679, 12679], 'mapped', [4484]],
        [[12680, 12680], 'mapped', [4485]],
        [[12681, 12681], 'mapped', [4488]],
        [[12682, 12682], 'mapped', [4497]],
        [[12683, 12683], 'mapped', [4498]],
        [[12684, 12684], 'mapped', [4500]],
        [[12685, 12685], 'mapped', [4510]],
        [[12686, 12686], 'mapped', [4513]],
        [[12687, 12687], 'disallowed'],
        [[12688, 12689], 'valid', [], 'NV8'],
        [[12690, 12690], 'mapped', [19968]],
        [[12691, 12691], 'mapped', [20108]],
        [[12692, 12692], 'mapped', [19977]],
        [[12693, 12693], 'mapped', [22235]],
        [[12694, 12694], 'mapped', [19978]],
        [[12695, 12695], 'mapped', [20013]],
        [[12696, 12696], 'mapped', [19979]],
        [[12697, 12697], 'mapped', [30002]],
        [[12698, 12698], 'mapped', [20057]],
        [[12699, 12699], 'mapped', [19993]],
        [[12700, 12700], 'mapped', [19969]],
        [[12701, 12701], 'mapped', [22825]],
        [[12702, 12702], 'mapped', [22320]],
        [[12703, 12703], 'mapped', [20154]],
        [[12704, 12727], 'valid'],
        [[12728, 12730], 'valid'],
        [[12731, 12735], 'disallowed'],
        [[12736, 12751], 'valid', [], 'NV8'],
        [[12752, 12771], 'valid', [], 'NV8'],
        [[12772, 12783], 'disallowed'],
        [[12784, 12799], 'valid'],
        [[12800, 12800], 'disallowed_STD3_mapped', [40, 4352, 41]],
        [[12801, 12801], 'disallowed_STD3_mapped', [40, 4354, 41]],
        [[12802, 12802], 'disallowed_STD3_mapped', [40, 4355, 41]],
        [[12803, 12803], 'disallowed_STD3_mapped', [40, 4357, 41]],
        [[12804, 12804], 'disallowed_STD3_mapped', [40, 4358, 41]],
        [[12805, 12805], 'disallowed_STD3_mapped', [40, 4359, 41]],
        [[12806, 12806], 'disallowed_STD3_mapped', [40, 4361, 41]],
        [[12807, 12807], 'disallowed_STD3_mapped', [40, 4363, 41]],
        [[12808, 12808], 'disallowed_STD3_mapped', [40, 4364, 41]],
        [[12809, 12809], 'disallowed_STD3_mapped', [40, 4366, 41]],
        [[12810, 12810], 'disallowed_STD3_mapped', [40, 4367, 41]],
        [[12811, 12811], 'disallowed_STD3_mapped', [40, 4368, 41]],
        [[12812, 12812], 'disallowed_STD3_mapped', [40, 4369, 41]],
        [[12813, 12813], 'disallowed_STD3_mapped', [40, 4370, 41]],
        [[12814, 12814], 'disallowed_STD3_mapped', [40, 44032, 41]],
        [[12815, 12815], 'disallowed_STD3_mapped', [40, 45208, 41]],
        [[12816, 12816], 'disallowed_STD3_mapped', [40, 45796, 41]],
        [[12817, 12817], 'disallowed_STD3_mapped', [40, 46972, 41]],
        [[12818, 12818], 'disallowed_STD3_mapped', [40, 47560, 41]],
        [[12819, 12819], 'disallowed_STD3_mapped', [40, 48148, 41]],
        [[12820, 12820], 'disallowed_STD3_mapped', [40, 49324, 41]],
        [[12821, 12821], 'disallowed_STD3_mapped', [40, 50500, 41]],
        [[12822, 12822], 'disallowed_STD3_mapped', [40, 51088, 41]],
        [[12823, 12823], 'disallowed_STD3_mapped', [40, 52264, 41]],
        [[12824, 12824], 'disallowed_STD3_mapped', [40, 52852, 41]],
        [[12825, 12825], 'disallowed_STD3_mapped', [40, 53440, 41]],
        [[12826, 12826], 'disallowed_STD3_mapped', [40, 54028, 41]],
        [[12827, 12827], 'disallowed_STD3_mapped', [40, 54616, 41]],
        [[12828, 12828], 'disallowed_STD3_mapped', [40, 51452, 41]],
        [[12829, 12829], 'disallowed_STD3_mapped', [40, 50724, 51204, 41]],
        [[12830, 12830], 'disallowed_STD3_mapped', [40, 50724, 54980, 41]],
        [[12831, 12831], 'disallowed'],
        [[12832, 12832], 'disallowed_STD3_mapped', [40, 19968, 41]],
        [[12833, 12833], 'disallowed_STD3_mapped', [40, 20108, 41]],
        [[12834, 12834], 'disallowed_STD3_mapped', [40, 19977, 41]],
        [[12835, 12835], 'disallowed_STD3_mapped', [40, 22235, 41]],
        [[12836, 12836], 'disallowed_STD3_mapped', [40, 20116, 41]],
        [[12837, 12837], 'disallowed_STD3_mapped', [40, 20845, 41]],
        [[12838, 12838], 'disallowed_STD3_mapped', [40, 19971, 41]],
        [[12839, 12839], 'disallowed_STD3_mapped', [40, 20843, 41]],
        [[12840, 12840], 'disallowed_STD3_mapped', [40, 20061, 41]],
        [[12841, 12841], 'disallowed_STD3_mapped', [40, 21313, 41]],
        [[12842, 12842], 'disallowed_STD3_mapped', [40, 26376, 41]],
        [[12843, 12843], 'disallowed_STD3_mapped', [40, 28779, 41]],
        [[12844, 12844], 'disallowed_STD3_mapped', [40, 27700, 41]],
        [[12845, 12845], 'disallowed_STD3_mapped', [40, 26408, 41]],
        [[12846, 12846], 'disallowed_STD3_mapped', [40, 37329, 41]],
        [[12847, 12847], 'disallowed_STD3_mapped', [40, 22303, 41]],
        [[12848, 12848], 'disallowed_STD3_mapped', [40, 26085, 41]],
        [[12849, 12849], 'disallowed_STD3_mapped', [40, 26666, 41]],
        [[12850, 12850], 'disallowed_STD3_mapped', [40, 26377, 41]],
        [[12851, 12851], 'disallowed_STD3_mapped', [40, 31038, 41]],
        [[12852, 12852], 'disallowed_STD3_mapped', [40, 21517, 41]],
        [[12853, 12853], 'disallowed_STD3_mapped', [40, 29305, 41]],
        [[12854, 12854], 'disallowed_STD3_mapped', [40, 36001, 41]],
        [[12855, 12855], 'disallowed_STD3_mapped', [40, 31069, 41]],
        [[12856, 12856], 'disallowed_STD3_mapped', [40, 21172, 41]],
        [[12857, 12857], 'disallowed_STD3_mapped', [40, 20195, 41]],
        [[12858, 12858], 'disallowed_STD3_mapped', [40, 21628, 41]],
        [[12859, 12859], 'disallowed_STD3_mapped', [40, 23398, 41]],
        [[12860, 12860], 'disallowed_STD3_mapped', [40, 30435, 41]],
        [[12861, 12861], 'disallowed_STD3_mapped', [40, 20225, 41]],
        [[12862, 12862], 'disallowed_STD3_mapped', [40, 36039, 41]],
        [[12863, 12863], 'disallowed_STD3_mapped', [40, 21332, 41]],
        [[12864, 12864], 'disallowed_STD3_mapped', [40, 31085, 41]],
        [[12865, 12865], 'disallowed_STD3_mapped', [40, 20241, 41]],
        [[12866, 12866], 'disallowed_STD3_mapped', [40, 33258, 41]],
        [[12867, 12867], 'disallowed_STD3_mapped', [40, 33267, 41]],
        [[12868, 12868], 'mapped', [21839]],
        [[12869, 12869], 'mapped', [24188]],
        [[12870, 12870], 'mapped', [25991]],
        [[12871, 12871], 'mapped', [31631]],
        [[12872, 12879], 'valid', [], 'NV8'],
        [[12880, 12880], 'mapped', [112, 116, 101]],
        [[12881, 12881], 'mapped', [50, 49]],
        [[12882, 12882], 'mapped', [50, 50]],
        [[12883, 12883], 'mapped', [50, 51]],
        [[12884, 12884], 'mapped', [50, 52]],
        [[12885, 12885], 'mapped', [50, 53]],
        [[12886, 12886], 'mapped', [50, 54]],
        [[12887, 12887], 'mapped', [50, 55]],
        [[12888, 12888], 'mapped', [50, 56]],
        [[12889, 12889], 'mapped', [50, 57]],
        [[12890, 12890], 'mapped', [51, 48]],
        [[12891, 12891], 'mapped', [51, 49]],
        [[12892, 12892], 'mapped', [51, 50]],
        [[12893, 12893], 'mapped', [51, 51]],
        [[12894, 12894], 'mapped', [51, 52]],
        [[12895, 12895], 'mapped', [51, 53]],
        [[12896, 12896], 'mapped', [4352]],
        [[12897, 12897], 'mapped', [4354]],
        [[12898, 12898], 'mapped', [4355]],
        [[12899, 12899], 'mapped', [4357]],
        [[12900, 12900], 'mapped', [4358]],
        [[12901, 12901], 'mapped', [4359]],
        [[12902, 12902], 'mapped', [4361]],
        [[12903, 12903], 'mapped', [4363]],
        [[12904, 12904], 'mapped', [4364]],
        [[12905, 12905], 'mapped', [4366]],
        [[12906, 12906], 'mapped', [4367]],
        [[12907, 12907], 'mapped', [4368]],
        [[12908, 12908], 'mapped', [4369]],
        [[12909, 12909], 'mapped', [4370]],
        [[12910, 12910], 'mapped', [44032]],
        [[12911, 12911], 'mapped', [45208]],
        [[12912, 12912], 'mapped', [45796]],
        [[12913, 12913], 'mapped', [46972]],
        [[12914, 12914], 'mapped', [47560]],
        [[12915, 12915], 'mapped', [48148]],
        [[12916, 12916], 'mapped', [49324]],
        [[12917, 12917], 'mapped', [50500]],
        [[12918, 12918], 'mapped', [51088]],
        [[12919, 12919], 'mapped', [52264]],
        [[12920, 12920], 'mapped', [52852]],
        [[12921, 12921], 'mapped', [53440]],
        [[12922, 12922], 'mapped', [54028]],
        [[12923, 12923], 'mapped', [54616]],
        [[12924, 12924], 'mapped', [52280, 44256]],
        [[12925, 12925], 'mapped', [51452, 51032]],
        [[12926, 12926], 'mapped', [50864]],
        [[12927, 12927], 'valid', [], 'NV8'],
        [[12928, 12928], 'mapped', [19968]],
        [[12929, 12929], 'mapped', [20108]],
        [[12930, 12930], 'mapped', [19977]],
        [[12931, 12931], 'mapped', [22235]],
        [[12932, 12932], 'mapped', [20116]],
        [[12933, 12933], 'mapped', [20845]],
        [[12934, 12934], 'mapped', [19971]],
        [[12935, 12935], 'mapped', [20843]],
        [[12936, 12936], 'mapped', [20061]],
        [[12937, 12937], 'mapped', [21313]],
        [[12938, 12938], 'mapped', [26376]],
        [[12939, 12939], 'mapped', [28779]],
        [[12940, 12940], 'mapped', [27700]],
        [[12941, 12941], 'mapped', [26408]],
        [[12942, 12942], 'mapped', [37329]],
        [[12943, 12943], 'mapped', [22303]],
        [[12944, 12944], 'mapped', [26085]],
        [[12945, 12945], 'mapped', [26666]],
        [[12946, 12946], 'mapped', [26377]],
        [[12947, 12947], 'mapped', [31038]],
        [[12948, 12948], 'mapped', [21517]],
        [[12949, 12949], 'mapped', [29305]],
        [[12950, 12950], 'mapped', [36001]],
        [[12951, 12951], 'mapped', [31069]],
        [[12952, 12952], 'mapped', [21172]],
        [[12953, 12953], 'mapped', [31192]],
        [[12954, 12954], 'mapped', [30007]],
        [[12955, 12955], 'mapped', [22899]],
        [[12956, 12956], 'mapped', [36969]],
        [[12957, 12957], 'mapped', [20778]],
        [[12958, 12958], 'mapped', [21360]],
        [[12959, 12959], 'mapped', [27880]],
        [[12960, 12960], 'mapped', [38917]],
        [[12961, 12961], 'mapped', [20241]],
        [[12962, 12962], 'mapped', [20889]],
        [[12963, 12963], 'mapped', [27491]],
        [[12964, 12964], 'mapped', [19978]],
        [[12965, 12965], 'mapped', [20013]],
        [[12966, 12966], 'mapped', [19979]],
        [[12967, 12967], 'mapped', [24038]],
        [[12968, 12968], 'mapped', [21491]],
        [[12969, 12969], 'mapped', [21307]],
        [[12970, 12970], 'mapped', [23447]],
        [[12971, 12971], 'mapped', [23398]],
        [[12972, 12972], 'mapped', [30435]],
        [[12973, 12973], 'mapped', [20225]],
        [[12974, 12974], 'mapped', [36039]],
        [[12975, 12975], 'mapped', [21332]],
        [[12976, 12976], 'mapped', [22812]],
        [[12977, 12977], 'mapped', [51, 54]],
        [[12978, 12978], 'mapped', [51, 55]],
        [[12979, 12979], 'mapped', [51, 56]],
        [[12980, 12980], 'mapped', [51, 57]],
        [[12981, 12981], 'mapped', [52, 48]],
        [[12982, 12982], 'mapped', [52, 49]],
        [[12983, 12983], 'mapped', [52, 50]],
        [[12984, 12984], 'mapped', [52, 51]],
        [[12985, 12985], 'mapped', [52, 52]],
        [[12986, 12986], 'mapped', [52, 53]],
        [[12987, 12987], 'mapped', [52, 54]],
        [[12988, 12988], 'mapped', [52, 55]],
        [[12989, 12989], 'mapped', [52, 56]],
        [[12990, 12990], 'mapped', [52, 57]],
        [[12991, 12991], 'mapped', [53, 48]],
        [[12992, 12992], 'mapped', [49, 26376]],
        [[12993, 12993], 'mapped', [50, 26376]],
        [[12994, 12994], 'mapped', [51, 26376]],
        [[12995, 12995], 'mapped', [52, 26376]],
        [[12996, 12996], 'mapped', [53, 26376]],
        [[12997, 12997], 'mapped', [54, 26376]],
        [[12998, 12998], 'mapped', [55, 26376]],
        [[12999, 12999], 'mapped', [56, 26376]],
        [[13e3, 13e3], 'mapped', [57, 26376]],
        [[13001, 13001], 'mapped', [49, 48, 26376]],
        [[13002, 13002], 'mapped', [49, 49, 26376]],
        [[13003, 13003], 'mapped', [49, 50, 26376]],
        [[13004, 13004], 'mapped', [104, 103]],
        [[13005, 13005], 'mapped', [101, 114, 103]],
        [[13006, 13006], 'mapped', [101, 118]],
        [[13007, 13007], 'mapped', [108, 116, 100]],
        [[13008, 13008], 'mapped', [12450]],
        [[13009, 13009], 'mapped', [12452]],
        [[13010, 13010], 'mapped', [12454]],
        [[13011, 13011], 'mapped', [12456]],
        [[13012, 13012], 'mapped', [12458]],
        [[13013, 13013], 'mapped', [12459]],
        [[13014, 13014], 'mapped', [12461]],
        [[13015, 13015], 'mapped', [12463]],
        [[13016, 13016], 'mapped', [12465]],
        [[13017, 13017], 'mapped', [12467]],
        [[13018, 13018], 'mapped', [12469]],
        [[13019, 13019], 'mapped', [12471]],
        [[13020, 13020], 'mapped', [12473]],
        [[13021, 13021], 'mapped', [12475]],
        [[13022, 13022], 'mapped', [12477]],
        [[13023, 13023], 'mapped', [12479]],
        [[13024, 13024], 'mapped', [12481]],
        [[13025, 13025], 'mapped', [12484]],
        [[13026, 13026], 'mapped', [12486]],
        [[13027, 13027], 'mapped', [12488]],
        [[13028, 13028], 'mapped', [12490]],
        [[13029, 13029], 'mapped', [12491]],
        [[13030, 13030], 'mapped', [12492]],
        [[13031, 13031], 'mapped', [12493]],
        [[13032, 13032], 'mapped', [12494]],
        [[13033, 13033], 'mapped', [12495]],
        [[13034, 13034], 'mapped', [12498]],
        [[13035, 13035], 'mapped', [12501]],
        [[13036, 13036], 'mapped', [12504]],
        [[13037, 13037], 'mapped', [12507]],
        [[13038, 13038], 'mapped', [12510]],
        [[13039, 13039], 'mapped', [12511]],
        [[13040, 13040], 'mapped', [12512]],
        [[13041, 13041], 'mapped', [12513]],
        [[13042, 13042], 'mapped', [12514]],
        [[13043, 13043], 'mapped', [12516]],
        [[13044, 13044], 'mapped', [12518]],
        [[13045, 13045], 'mapped', [12520]],
        [[13046, 13046], 'mapped', [12521]],
        [[13047, 13047], 'mapped', [12522]],
        [[13048, 13048], 'mapped', [12523]],
        [[13049, 13049], 'mapped', [12524]],
        [[13050, 13050], 'mapped', [12525]],
        [[13051, 13051], 'mapped', [12527]],
        [[13052, 13052], 'mapped', [12528]],
        [[13053, 13053], 'mapped', [12529]],
        [[13054, 13054], 'mapped', [12530]],
        [[13055, 13055], 'disallowed'],
        [[13056, 13056], 'mapped', [12450, 12497, 12540, 12488]],
        [[13057, 13057], 'mapped', [12450, 12523, 12501, 12449]],
        [[13058, 13058], 'mapped', [12450, 12531, 12506, 12450]],
        [[13059, 13059], 'mapped', [12450, 12540, 12523]],
        [[13060, 13060], 'mapped', [12452, 12491, 12531, 12464]],
        [[13061, 13061], 'mapped', [12452, 12531, 12481]],
        [[13062, 13062], 'mapped', [12454, 12457, 12531]],
        [[13063, 13063], 'mapped', [12456, 12473, 12463, 12540, 12489]],
        [[13064, 13064], 'mapped', [12456, 12540, 12459, 12540]],
        [[13065, 13065], 'mapped', [12458, 12531, 12473]],
        [[13066, 13066], 'mapped', [12458, 12540, 12512]],
        [[13067, 13067], 'mapped', [12459, 12452, 12522]],
        [[13068, 13068], 'mapped', [12459, 12521, 12483, 12488]],
        [[13069, 13069], 'mapped', [12459, 12525, 12522, 12540]],
        [[13070, 13070], 'mapped', [12460, 12525, 12531]],
        [[13071, 13071], 'mapped', [12460, 12531, 12510]],
        [[13072, 13072], 'mapped', [12462, 12460]],
        [[13073, 13073], 'mapped', [12462, 12491, 12540]],
        [[13074, 13074], 'mapped', [12461, 12517, 12522, 12540]],
        [[13075, 13075], 'mapped', [12462, 12523, 12480, 12540]],
        [[13076, 13076], 'mapped', [12461, 12525]],
        [[13077, 13077], 'mapped', [12461, 12525, 12464, 12521, 12512]],
        [[13078, 13078], 'mapped', [12461, 12525, 12513, 12540, 12488, 12523]],
        [[13079, 13079], 'mapped', [12461, 12525, 12527, 12483, 12488]],
        [[13080, 13080], 'mapped', [12464, 12521, 12512]],
        [[13081, 13081], 'mapped', [12464, 12521, 12512, 12488, 12531]],
        [[13082, 13082], 'mapped', [12463, 12523, 12476, 12452, 12525]],
        [[13083, 13083], 'mapped', [12463, 12525, 12540, 12493]],
        [[13084, 13084], 'mapped', [12465, 12540, 12473]],
        [[13085, 13085], 'mapped', [12467, 12523, 12490]],
        [[13086, 13086], 'mapped', [12467, 12540, 12509]],
        [[13087, 13087], 'mapped', [12469, 12452, 12463, 12523]],
        [[13088, 13088], 'mapped', [12469, 12531, 12481, 12540, 12512]],
        [[13089, 13089], 'mapped', [12471, 12522, 12531, 12464]],
        [[13090, 13090], 'mapped', [12475, 12531, 12481]],
        [[13091, 13091], 'mapped', [12475, 12531, 12488]],
        [[13092, 13092], 'mapped', [12480, 12540, 12473]],
        [[13093, 13093], 'mapped', [12487, 12471]],
        [[13094, 13094], 'mapped', [12489, 12523]],
        [[13095, 13095], 'mapped', [12488, 12531]],
        [[13096, 13096], 'mapped', [12490, 12494]],
        [[13097, 13097], 'mapped', [12494, 12483, 12488]],
        [[13098, 13098], 'mapped', [12495, 12452, 12484]],
        [[13099, 13099], 'mapped', [12497, 12540, 12475, 12531, 12488]],
        [[13100, 13100], 'mapped', [12497, 12540, 12484]],
        [[13101, 13101], 'mapped', [12496, 12540, 12524, 12523]],
        [[13102, 13102], 'mapped', [12500, 12450, 12473, 12488, 12523]],
        [[13103, 13103], 'mapped', [12500, 12463, 12523]],
        [[13104, 13104], 'mapped', [12500, 12467]],
        [[13105, 13105], 'mapped', [12499, 12523]],
        [[13106, 13106], 'mapped', [12501, 12449, 12521, 12483, 12489]],
        [[13107, 13107], 'mapped', [12501, 12451, 12540, 12488]],
        [[13108, 13108], 'mapped', [12502, 12483, 12471, 12455, 12523]],
        [[13109, 13109], 'mapped', [12501, 12521, 12531]],
        [[13110, 13110], 'mapped', [12504, 12463, 12479, 12540, 12523]],
        [[13111, 13111], 'mapped', [12506, 12477]],
        [[13112, 13112], 'mapped', [12506, 12491, 12498]],
        [[13113, 13113], 'mapped', [12504, 12523, 12484]],
        [[13114, 13114], 'mapped', [12506, 12531, 12473]],
        [[13115, 13115], 'mapped', [12506, 12540, 12472]],
        [[13116, 13116], 'mapped', [12505, 12540, 12479]],
        [[13117, 13117], 'mapped', [12509, 12452, 12531, 12488]],
        [[13118, 13118], 'mapped', [12508, 12523, 12488]],
        [[13119, 13119], 'mapped', [12507, 12531]],
        [[13120, 13120], 'mapped', [12509, 12531, 12489]],
        [[13121, 13121], 'mapped', [12507, 12540, 12523]],
        [[13122, 13122], 'mapped', [12507, 12540, 12531]],
        [[13123, 13123], 'mapped', [12510, 12452, 12463, 12525]],
        [[13124, 13124], 'mapped', [12510, 12452, 12523]],
        [[13125, 13125], 'mapped', [12510, 12483, 12495]],
        [[13126, 13126], 'mapped', [12510, 12523, 12463]],
        [[13127, 13127], 'mapped', [12510, 12531, 12471, 12519, 12531]],
        [[13128, 13128], 'mapped', [12511, 12463, 12525, 12531]],
        [[13129, 13129], 'mapped', [12511, 12522]],
        [[13130, 13130], 'mapped', [12511, 12522, 12496, 12540, 12523]],
        [[13131, 13131], 'mapped', [12513, 12460]],
        [[13132, 13132], 'mapped', [12513, 12460, 12488, 12531]],
        [[13133, 13133], 'mapped', [12513, 12540, 12488, 12523]],
        [[13134, 13134], 'mapped', [12516, 12540, 12489]],
        [[13135, 13135], 'mapped', [12516, 12540, 12523]],
        [[13136, 13136], 'mapped', [12518, 12450, 12531]],
        [[13137, 13137], 'mapped', [12522, 12483, 12488, 12523]],
        [[13138, 13138], 'mapped', [12522, 12521]],
        [[13139, 13139], 'mapped', [12523, 12500, 12540]],
        [[13140, 13140], 'mapped', [12523, 12540, 12502, 12523]],
        [[13141, 13141], 'mapped', [12524, 12512]],
        [[13142, 13142], 'mapped', [12524, 12531, 12488, 12466, 12531]],
        [[13143, 13143], 'mapped', [12527, 12483, 12488]],
        [[13144, 13144], 'mapped', [48, 28857]],
        [[13145, 13145], 'mapped', [49, 28857]],
        [[13146, 13146], 'mapped', [50, 28857]],
        [[13147, 13147], 'mapped', [51, 28857]],
        [[13148, 13148], 'mapped', [52, 28857]],
        [[13149, 13149], 'mapped', [53, 28857]],
        [[13150, 13150], 'mapped', [54, 28857]],
        [[13151, 13151], 'mapped', [55, 28857]],
        [[13152, 13152], 'mapped', [56, 28857]],
        [[13153, 13153], 'mapped', [57, 28857]],
        [[13154, 13154], 'mapped', [49, 48, 28857]],
        [[13155, 13155], 'mapped', [49, 49, 28857]],
        [[13156, 13156], 'mapped', [49, 50, 28857]],
        [[13157, 13157], 'mapped', [49, 51, 28857]],
        [[13158, 13158], 'mapped', [49, 52, 28857]],
        [[13159, 13159], 'mapped', [49, 53, 28857]],
        [[13160, 13160], 'mapped', [49, 54, 28857]],
        [[13161, 13161], 'mapped', [49, 55, 28857]],
        [[13162, 13162], 'mapped', [49, 56, 28857]],
        [[13163, 13163], 'mapped', [49, 57, 28857]],
        [[13164, 13164], 'mapped', [50, 48, 28857]],
        [[13165, 13165], 'mapped', [50, 49, 28857]],
        [[13166, 13166], 'mapped', [50, 50, 28857]],
        [[13167, 13167], 'mapped', [50, 51, 28857]],
        [[13168, 13168], 'mapped', [50, 52, 28857]],
        [[13169, 13169], 'mapped', [104, 112, 97]],
        [[13170, 13170], 'mapped', [100, 97]],
        [[13171, 13171], 'mapped', [97, 117]],
        [[13172, 13172], 'mapped', [98, 97, 114]],
        [[13173, 13173], 'mapped', [111, 118]],
        [[13174, 13174], 'mapped', [112, 99]],
        [[13175, 13175], 'mapped', [100, 109]],
        [[13176, 13176], 'mapped', [100, 109, 50]],
        [[13177, 13177], 'mapped', [100, 109, 51]],
        [[13178, 13178], 'mapped', [105, 117]],
        [[13179, 13179], 'mapped', [24179, 25104]],
        [[13180, 13180], 'mapped', [26157, 21644]],
        [[13181, 13181], 'mapped', [22823, 27491]],
        [[13182, 13182], 'mapped', [26126, 27835]],
        [[13183, 13183], 'mapped', [26666, 24335, 20250, 31038]],
        [[13184, 13184], 'mapped', [112, 97]],
        [[13185, 13185], 'mapped', [110, 97]],
        [[13186, 13186], 'mapped', [956, 97]],
        [[13187, 13187], 'mapped', [109, 97]],
        [[13188, 13188], 'mapped', [107, 97]],
        [[13189, 13189], 'mapped', [107, 98]],
        [[13190, 13190], 'mapped', [109, 98]],
        [[13191, 13191], 'mapped', [103, 98]],
        [[13192, 13192], 'mapped', [99, 97, 108]],
        [[13193, 13193], 'mapped', [107, 99, 97, 108]],
        [[13194, 13194], 'mapped', [112, 102]],
        [[13195, 13195], 'mapped', [110, 102]],
        [[13196, 13196], 'mapped', [956, 102]],
        [[13197, 13197], 'mapped', [956, 103]],
        [[13198, 13198], 'mapped', [109, 103]],
        [[13199, 13199], 'mapped', [107, 103]],
        [[13200, 13200], 'mapped', [104, 122]],
        [[13201, 13201], 'mapped', [107, 104, 122]],
        [[13202, 13202], 'mapped', [109, 104, 122]],
        [[13203, 13203], 'mapped', [103, 104, 122]],
        [[13204, 13204], 'mapped', [116, 104, 122]],
        [[13205, 13205], 'mapped', [956, 108]],
        [[13206, 13206], 'mapped', [109, 108]],
        [[13207, 13207], 'mapped', [100, 108]],
        [[13208, 13208], 'mapped', [107, 108]],
        [[13209, 13209], 'mapped', [102, 109]],
        [[13210, 13210], 'mapped', [110, 109]],
        [[13211, 13211], 'mapped', [956, 109]],
        [[13212, 13212], 'mapped', [109, 109]],
        [[13213, 13213], 'mapped', [99, 109]],
        [[13214, 13214], 'mapped', [107, 109]],
        [[13215, 13215], 'mapped', [109, 109, 50]],
        [[13216, 13216], 'mapped', [99, 109, 50]],
        [[13217, 13217], 'mapped', [109, 50]],
        [[13218, 13218], 'mapped', [107, 109, 50]],
        [[13219, 13219], 'mapped', [109, 109, 51]],
        [[13220, 13220], 'mapped', [99, 109, 51]],
        [[13221, 13221], 'mapped', [109, 51]],
        [[13222, 13222], 'mapped', [107, 109, 51]],
        [[13223, 13223], 'mapped', [109, 8725, 115]],
        [[13224, 13224], 'mapped', [109, 8725, 115, 50]],
        [[13225, 13225], 'mapped', [112, 97]],
        [[13226, 13226], 'mapped', [107, 112, 97]],
        [[13227, 13227], 'mapped', [109, 112, 97]],
        [[13228, 13228], 'mapped', [103, 112, 97]],
        [[13229, 13229], 'mapped', [114, 97, 100]],
        [[13230, 13230], 'mapped', [114, 97, 100, 8725, 115]],
        [[13231, 13231], 'mapped', [114, 97, 100, 8725, 115, 50]],
        [[13232, 13232], 'mapped', [112, 115]],
        [[13233, 13233], 'mapped', [110, 115]],
        [[13234, 13234], 'mapped', [956, 115]],
        [[13235, 13235], 'mapped', [109, 115]],
        [[13236, 13236], 'mapped', [112, 118]],
        [[13237, 13237], 'mapped', [110, 118]],
        [[13238, 13238], 'mapped', [956, 118]],
        [[13239, 13239], 'mapped', [109, 118]],
        [[13240, 13240], 'mapped', [107, 118]],
        [[13241, 13241], 'mapped', [109, 118]],
        [[13242, 13242], 'mapped', [112, 119]],
        [[13243, 13243], 'mapped', [110, 119]],
        [[13244, 13244], 'mapped', [956, 119]],
        [[13245, 13245], 'mapped', [109, 119]],
        [[13246, 13246], 'mapped', [107, 119]],
        [[13247, 13247], 'mapped', [109, 119]],
        [[13248, 13248], 'mapped', [107, 969]],
        [[13249, 13249], 'mapped', [109, 969]],
        [[13250, 13250], 'disallowed'],
        [[13251, 13251], 'mapped', [98, 113]],
        [[13252, 13252], 'mapped', [99, 99]],
        [[13253, 13253], 'mapped', [99, 100]],
        [[13254, 13254], 'mapped', [99, 8725, 107, 103]],
        [[13255, 13255], 'disallowed'],
        [[13256, 13256], 'mapped', [100, 98]],
        [[13257, 13257], 'mapped', [103, 121]],
        [[13258, 13258], 'mapped', [104, 97]],
        [[13259, 13259], 'mapped', [104, 112]],
        [[13260, 13260], 'mapped', [105, 110]],
        [[13261, 13261], 'mapped', [107, 107]],
        [[13262, 13262], 'mapped', [107, 109]],
        [[13263, 13263], 'mapped', [107, 116]],
        [[13264, 13264], 'mapped', [108, 109]],
        [[13265, 13265], 'mapped', [108, 110]],
        [[13266, 13266], 'mapped', [108, 111, 103]],
        [[13267, 13267], 'mapped', [108, 120]],
        [[13268, 13268], 'mapped', [109, 98]],
        [[13269, 13269], 'mapped', [109, 105, 108]],
        [[13270, 13270], 'mapped', [109, 111, 108]],
        [[13271, 13271], 'mapped', [112, 104]],
        [[13272, 13272], 'disallowed'],
        [[13273, 13273], 'mapped', [112, 112, 109]],
        [[13274, 13274], 'mapped', [112, 114]],
        [[13275, 13275], 'mapped', [115, 114]],
        [[13276, 13276], 'mapped', [115, 118]],
        [[13277, 13277], 'mapped', [119, 98]],
        [[13278, 13278], 'mapped', [118, 8725, 109]],
        [[13279, 13279], 'mapped', [97, 8725, 109]],
        [[13280, 13280], 'mapped', [49, 26085]],
        [[13281, 13281], 'mapped', [50, 26085]],
        [[13282, 13282], 'mapped', [51, 26085]],
        [[13283, 13283], 'mapped', [52, 26085]],
        [[13284, 13284], 'mapped', [53, 26085]],
        [[13285, 13285], 'mapped', [54, 26085]],
        [[13286, 13286], 'mapped', [55, 26085]],
        [[13287, 13287], 'mapped', [56, 26085]],
        [[13288, 13288], 'mapped', [57, 26085]],
        [[13289, 13289], 'mapped', [49, 48, 26085]],
        [[13290, 13290], 'mapped', [49, 49, 26085]],
        [[13291, 13291], 'mapped', [49, 50, 26085]],
        [[13292, 13292], 'mapped', [49, 51, 26085]],
        [[13293, 13293], 'mapped', [49, 52, 26085]],
        [[13294, 13294], 'mapped', [49, 53, 26085]],
        [[13295, 13295], 'mapped', [49, 54, 26085]],
        [[13296, 13296], 'mapped', [49, 55, 26085]],
        [[13297, 13297], 'mapped', [49, 56, 26085]],
        [[13298, 13298], 'mapped', [49, 57, 26085]],
        [[13299, 13299], 'mapped', [50, 48, 26085]],
        [[13300, 13300], 'mapped', [50, 49, 26085]],
        [[13301, 13301], 'mapped', [50, 50, 26085]],
        [[13302, 13302], 'mapped', [50, 51, 26085]],
        [[13303, 13303], 'mapped', [50, 52, 26085]],
        [[13304, 13304], 'mapped', [50, 53, 26085]],
        [[13305, 13305], 'mapped', [50, 54, 26085]],
        [[13306, 13306], 'mapped', [50, 55, 26085]],
        [[13307, 13307], 'mapped', [50, 56, 26085]],
        [[13308, 13308], 'mapped', [50, 57, 26085]],
        [[13309, 13309], 'mapped', [51, 48, 26085]],
        [[13310, 13310], 'mapped', [51, 49, 26085]],
        [[13311, 13311], 'mapped', [103, 97, 108]],
        [[13312, 19893], 'valid'],
        [[19894, 19903], 'disallowed'],
        [[19904, 19967], 'valid', [], 'NV8'],
        [[19968, 40869], 'valid'],
        [[40870, 40891], 'valid'],
        [[40892, 40899], 'valid'],
        [[40900, 40907], 'valid'],
        [[40908, 40908], 'valid'],
        [[40909, 40917], 'valid'],
        [[40918, 40959], 'disallowed'],
        [[40960, 42124], 'valid'],
        [[42125, 42127], 'disallowed'],
        [[42128, 42145], 'valid', [], 'NV8'],
        [[42146, 42147], 'valid', [], 'NV8'],
        [[42148, 42163], 'valid', [], 'NV8'],
        [[42164, 42164], 'valid', [], 'NV8'],
        [[42165, 42176], 'valid', [], 'NV8'],
        [[42177, 42177], 'valid', [], 'NV8'],
        [[42178, 42180], 'valid', [], 'NV8'],
        [[42181, 42181], 'valid', [], 'NV8'],
        [[42182, 42182], 'valid', [], 'NV8'],
        [[42183, 42191], 'disallowed'],
        [[42192, 42237], 'valid'],
        [[42238, 42239], 'valid', [], 'NV8'],
        [[42240, 42508], 'valid'],
        [[42509, 42511], 'valid', [], 'NV8'],
        [[42512, 42539], 'valid'],
        [[42540, 42559], 'disallowed'],
        [[42560, 42560], 'mapped', [42561]],
        [[42561, 42561], 'valid'],
        [[42562, 42562], 'mapped', [42563]],
        [[42563, 42563], 'valid'],
        [[42564, 42564], 'mapped', [42565]],
        [[42565, 42565], 'valid'],
        [[42566, 42566], 'mapped', [42567]],
        [[42567, 42567], 'valid'],
        [[42568, 42568], 'mapped', [42569]],
        [[42569, 42569], 'valid'],
        [[42570, 42570], 'mapped', [42571]],
        [[42571, 42571], 'valid'],
        [[42572, 42572], 'mapped', [42573]],
        [[42573, 42573], 'valid'],
        [[42574, 42574], 'mapped', [42575]],
        [[42575, 42575], 'valid'],
        [[42576, 42576], 'mapped', [42577]],
        [[42577, 42577], 'valid'],
        [[42578, 42578], 'mapped', [42579]],
        [[42579, 42579], 'valid'],
        [[42580, 42580], 'mapped', [42581]],
        [[42581, 42581], 'valid'],
        [[42582, 42582], 'mapped', [42583]],
        [[42583, 42583], 'valid'],
        [[42584, 42584], 'mapped', [42585]],
        [[42585, 42585], 'valid'],
        [[42586, 42586], 'mapped', [42587]],
        [[42587, 42587], 'valid'],
        [[42588, 42588], 'mapped', [42589]],
        [[42589, 42589], 'valid'],
        [[42590, 42590], 'mapped', [42591]],
        [[42591, 42591], 'valid'],
        [[42592, 42592], 'mapped', [42593]],
        [[42593, 42593], 'valid'],
        [[42594, 42594], 'mapped', [42595]],
        [[42595, 42595], 'valid'],
        [[42596, 42596], 'mapped', [42597]],
        [[42597, 42597], 'valid'],
        [[42598, 42598], 'mapped', [42599]],
        [[42599, 42599], 'valid'],
        [[42600, 42600], 'mapped', [42601]],
        [[42601, 42601], 'valid'],
        [[42602, 42602], 'mapped', [42603]],
        [[42603, 42603], 'valid'],
        [[42604, 42604], 'mapped', [42605]],
        [[42605, 42607], 'valid'],
        [[42608, 42611], 'valid', [], 'NV8'],
        [[42612, 42619], 'valid'],
        [[42620, 42621], 'valid'],
        [[42622, 42622], 'valid', [], 'NV8'],
        [[42623, 42623], 'valid'],
        [[42624, 42624], 'mapped', [42625]],
        [[42625, 42625], 'valid'],
        [[42626, 42626], 'mapped', [42627]],
        [[42627, 42627], 'valid'],
        [[42628, 42628], 'mapped', [42629]],
        [[42629, 42629], 'valid'],
        [[42630, 42630], 'mapped', [42631]],
        [[42631, 42631], 'valid'],
        [[42632, 42632], 'mapped', [42633]],
        [[42633, 42633], 'valid'],
        [[42634, 42634], 'mapped', [42635]],
        [[42635, 42635], 'valid'],
        [[42636, 42636], 'mapped', [42637]],
        [[42637, 42637], 'valid'],
        [[42638, 42638], 'mapped', [42639]],
        [[42639, 42639], 'valid'],
        [[42640, 42640], 'mapped', [42641]],
        [[42641, 42641], 'valid'],
        [[42642, 42642], 'mapped', [42643]],
        [[42643, 42643], 'valid'],
        [[42644, 42644], 'mapped', [42645]],
        [[42645, 42645], 'valid'],
        [[42646, 42646], 'mapped', [42647]],
        [[42647, 42647], 'valid'],
        [[42648, 42648], 'mapped', [42649]],
        [[42649, 42649], 'valid'],
        [[42650, 42650], 'mapped', [42651]],
        [[42651, 42651], 'valid'],
        [[42652, 42652], 'mapped', [1098]],
        [[42653, 42653], 'mapped', [1100]],
        [[42654, 42654], 'valid'],
        [[42655, 42655], 'valid'],
        [[42656, 42725], 'valid'],
        [[42726, 42735], 'valid', [], 'NV8'],
        [[42736, 42737], 'valid'],
        [[42738, 42743], 'valid', [], 'NV8'],
        [[42744, 42751], 'disallowed'],
        [[42752, 42774], 'valid', [], 'NV8'],
        [[42775, 42778], 'valid'],
        [[42779, 42783], 'valid'],
        [[42784, 42785], 'valid', [], 'NV8'],
        [[42786, 42786], 'mapped', [42787]],
        [[42787, 42787], 'valid'],
        [[42788, 42788], 'mapped', [42789]],
        [[42789, 42789], 'valid'],
        [[42790, 42790], 'mapped', [42791]],
        [[42791, 42791], 'valid'],
        [[42792, 42792], 'mapped', [42793]],
        [[42793, 42793], 'valid'],
        [[42794, 42794], 'mapped', [42795]],
        [[42795, 42795], 'valid'],
        [[42796, 42796], 'mapped', [42797]],
        [[42797, 42797], 'valid'],
        [[42798, 42798], 'mapped', [42799]],
        [[42799, 42801], 'valid'],
        [[42802, 42802], 'mapped', [42803]],
        [[42803, 42803], 'valid'],
        [[42804, 42804], 'mapped', [42805]],
        [[42805, 42805], 'valid'],
        [[42806, 42806], 'mapped', [42807]],
        [[42807, 42807], 'valid'],
        [[42808, 42808], 'mapped', [42809]],
        [[42809, 42809], 'valid'],
        [[42810, 42810], 'mapped', [42811]],
        [[42811, 42811], 'valid'],
        [[42812, 42812], 'mapped', [42813]],
        [[42813, 42813], 'valid'],
        [[42814, 42814], 'mapped', [42815]],
        [[42815, 42815], 'valid'],
        [[42816, 42816], 'mapped', [42817]],
        [[42817, 42817], 'valid'],
        [[42818, 42818], 'mapped', [42819]],
        [[42819, 42819], 'valid'],
        [[42820, 42820], 'mapped', [42821]],
        [[42821, 42821], 'valid'],
        [[42822, 42822], 'mapped', [42823]],
        [[42823, 42823], 'valid'],
        [[42824, 42824], 'mapped', [42825]],
        [[42825, 42825], 'valid'],
        [[42826, 42826], 'mapped', [42827]],
        [[42827, 42827], 'valid'],
        [[42828, 42828], 'mapped', [42829]],
        [[42829, 42829], 'valid'],
        [[42830, 42830], 'mapped', [42831]],
        [[42831, 42831], 'valid'],
        [[42832, 42832], 'mapped', [42833]],
        [[42833, 42833], 'valid'],
        [[42834, 42834], 'mapped', [42835]],
        [[42835, 42835], 'valid'],
        [[42836, 42836], 'mapped', [42837]],
        [[42837, 42837], 'valid'],
        [[42838, 42838], 'mapped', [42839]],
        [[42839, 42839], 'valid'],
        [[42840, 42840], 'mapped', [42841]],
        [[42841, 42841], 'valid'],
        [[42842, 42842], 'mapped', [42843]],
        [[42843, 42843], 'valid'],
        [[42844, 42844], 'mapped', [42845]],
        [[42845, 42845], 'valid'],
        [[42846, 42846], 'mapped', [42847]],
        [[42847, 42847], 'valid'],
        [[42848, 42848], 'mapped', [42849]],
        [[42849, 42849], 'valid'],
        [[42850, 42850], 'mapped', [42851]],
        [[42851, 42851], 'valid'],
        [[42852, 42852], 'mapped', [42853]],
        [[42853, 42853], 'valid'],
        [[42854, 42854], 'mapped', [42855]],
        [[42855, 42855], 'valid'],
        [[42856, 42856], 'mapped', [42857]],
        [[42857, 42857], 'valid'],
        [[42858, 42858], 'mapped', [42859]],
        [[42859, 42859], 'valid'],
        [[42860, 42860], 'mapped', [42861]],
        [[42861, 42861], 'valid'],
        [[42862, 42862], 'mapped', [42863]],
        [[42863, 42863], 'valid'],
        [[42864, 42864], 'mapped', [42863]],
        [[42865, 42872], 'valid'],
        [[42873, 42873], 'mapped', [42874]],
        [[42874, 42874], 'valid'],
        [[42875, 42875], 'mapped', [42876]],
        [[42876, 42876], 'valid'],
        [[42877, 42877], 'mapped', [7545]],
        [[42878, 42878], 'mapped', [42879]],
        [[42879, 42879], 'valid'],
        [[42880, 42880], 'mapped', [42881]],
        [[42881, 42881], 'valid'],
        [[42882, 42882], 'mapped', [42883]],
        [[42883, 42883], 'valid'],
        [[42884, 42884], 'mapped', [42885]],
        [[42885, 42885], 'valid'],
        [[42886, 42886], 'mapped', [42887]],
        [[42887, 42888], 'valid'],
        [[42889, 42890], 'valid', [], 'NV8'],
        [[42891, 42891], 'mapped', [42892]],
        [[42892, 42892], 'valid'],
        [[42893, 42893], 'mapped', [613]],
        [[42894, 42894], 'valid'],
        [[42895, 42895], 'valid'],
        [[42896, 42896], 'mapped', [42897]],
        [[42897, 42897], 'valid'],
        [[42898, 42898], 'mapped', [42899]],
        [[42899, 42899], 'valid'],
        [[42900, 42901], 'valid'],
        [[42902, 42902], 'mapped', [42903]],
        [[42903, 42903], 'valid'],
        [[42904, 42904], 'mapped', [42905]],
        [[42905, 42905], 'valid'],
        [[42906, 42906], 'mapped', [42907]],
        [[42907, 42907], 'valid'],
        [[42908, 42908], 'mapped', [42909]],
        [[42909, 42909], 'valid'],
        [[42910, 42910], 'mapped', [42911]],
        [[42911, 42911], 'valid'],
        [[42912, 42912], 'mapped', [42913]],
        [[42913, 42913], 'valid'],
        [[42914, 42914], 'mapped', [42915]],
        [[42915, 42915], 'valid'],
        [[42916, 42916], 'mapped', [42917]],
        [[42917, 42917], 'valid'],
        [[42918, 42918], 'mapped', [42919]],
        [[42919, 42919], 'valid'],
        [[42920, 42920], 'mapped', [42921]],
        [[42921, 42921], 'valid'],
        [[42922, 42922], 'mapped', [614]],
        [[42923, 42923], 'mapped', [604]],
        [[42924, 42924], 'mapped', [609]],
        [[42925, 42925], 'mapped', [620]],
        [[42926, 42927], 'disallowed'],
        [[42928, 42928], 'mapped', [670]],
        [[42929, 42929], 'mapped', [647]],
        [[42930, 42930], 'mapped', [669]],
        [[42931, 42931], 'mapped', [43859]],
        [[42932, 42932], 'mapped', [42933]],
        [[42933, 42933], 'valid'],
        [[42934, 42934], 'mapped', [42935]],
        [[42935, 42935], 'valid'],
        [[42936, 42998], 'disallowed'],
        [[42999, 42999], 'valid'],
        [[43e3, 43e3], 'mapped', [295]],
        [[43001, 43001], 'mapped', [339]],
        [[43002, 43002], 'valid'],
        [[43003, 43007], 'valid'],
        [[43008, 43047], 'valid'],
        [[43048, 43051], 'valid', [], 'NV8'],
        [[43052, 43055], 'disallowed'],
        [[43056, 43065], 'valid', [], 'NV8'],
        [[43066, 43071], 'disallowed'],
        [[43072, 43123], 'valid'],
        [[43124, 43127], 'valid', [], 'NV8'],
        [[43128, 43135], 'disallowed'],
        [[43136, 43204], 'valid'],
        [[43205, 43213], 'disallowed'],
        [[43214, 43215], 'valid', [], 'NV8'],
        [[43216, 43225], 'valid'],
        [[43226, 43231], 'disallowed'],
        [[43232, 43255], 'valid'],
        [[43256, 43258], 'valid', [], 'NV8'],
        [[43259, 43259], 'valid'],
        [[43260, 43260], 'valid', [], 'NV8'],
        [[43261, 43261], 'valid'],
        [[43262, 43263], 'disallowed'],
        [[43264, 43309], 'valid'],
        [[43310, 43311], 'valid', [], 'NV8'],
        [[43312, 43347], 'valid'],
        [[43348, 43358], 'disallowed'],
        [[43359, 43359], 'valid', [], 'NV8'],
        [[43360, 43388], 'valid', [], 'NV8'],
        [[43389, 43391], 'disallowed'],
        [[43392, 43456], 'valid'],
        [[43457, 43469], 'valid', [], 'NV8'],
        [[43470, 43470], 'disallowed'],
        [[43471, 43481], 'valid'],
        [[43482, 43485], 'disallowed'],
        [[43486, 43487], 'valid', [], 'NV8'],
        [[43488, 43518], 'valid'],
        [[43519, 43519], 'disallowed'],
        [[43520, 43574], 'valid'],
        [[43575, 43583], 'disallowed'],
        [[43584, 43597], 'valid'],
        [[43598, 43599], 'disallowed'],
        [[43600, 43609], 'valid'],
        [[43610, 43611], 'disallowed'],
        [[43612, 43615], 'valid', [], 'NV8'],
        [[43616, 43638], 'valid'],
        [[43639, 43641], 'valid', [], 'NV8'],
        [[43642, 43643], 'valid'],
        [[43644, 43647], 'valid'],
        [[43648, 43714], 'valid'],
        [[43715, 43738], 'disallowed'],
        [[43739, 43741], 'valid'],
        [[43742, 43743], 'valid', [], 'NV8'],
        [[43744, 43759], 'valid'],
        [[43760, 43761], 'valid', [], 'NV8'],
        [[43762, 43766], 'valid'],
        [[43767, 43776], 'disallowed'],
        [[43777, 43782], 'valid'],
        [[43783, 43784], 'disallowed'],
        [[43785, 43790], 'valid'],
        [[43791, 43792], 'disallowed'],
        [[43793, 43798], 'valid'],
        [[43799, 43807], 'disallowed'],
        [[43808, 43814], 'valid'],
        [[43815, 43815], 'disallowed'],
        [[43816, 43822], 'valid'],
        [[43823, 43823], 'disallowed'],
        [[43824, 43866], 'valid'],
        [[43867, 43867], 'valid', [], 'NV8'],
        [[43868, 43868], 'mapped', [42791]],
        [[43869, 43869], 'mapped', [43831]],
        [[43870, 43870], 'mapped', [619]],
        [[43871, 43871], 'mapped', [43858]],
        [[43872, 43875], 'valid'],
        [[43876, 43877], 'valid'],
        [[43878, 43887], 'disallowed'],
        [[43888, 43888], 'mapped', [5024]],
        [[43889, 43889], 'mapped', [5025]],
        [[43890, 43890], 'mapped', [5026]],
        [[43891, 43891], 'mapped', [5027]],
        [[43892, 43892], 'mapped', [5028]],
        [[43893, 43893], 'mapped', [5029]],
        [[43894, 43894], 'mapped', [5030]],
        [[43895, 43895], 'mapped', [5031]],
        [[43896, 43896], 'mapped', [5032]],
        [[43897, 43897], 'mapped', [5033]],
        [[43898, 43898], 'mapped', [5034]],
        [[43899, 43899], 'mapped', [5035]],
        [[43900, 43900], 'mapped', [5036]],
        [[43901, 43901], 'mapped', [5037]],
        [[43902, 43902], 'mapped', [5038]],
        [[43903, 43903], 'mapped', [5039]],
        [[43904, 43904], 'mapped', [5040]],
        [[43905, 43905], 'mapped', [5041]],
        [[43906, 43906], 'mapped', [5042]],
        [[43907, 43907], 'mapped', [5043]],
        [[43908, 43908], 'mapped', [5044]],
        [[43909, 43909], 'mapped', [5045]],
        [[43910, 43910], 'mapped', [5046]],
        [[43911, 43911], 'mapped', [5047]],
        [[43912, 43912], 'mapped', [5048]],
        [[43913, 43913], 'mapped', [5049]],
        [[43914, 43914], 'mapped', [5050]],
        [[43915, 43915], 'mapped', [5051]],
        [[43916, 43916], 'mapped', [5052]],
        [[43917, 43917], 'mapped', [5053]],
        [[43918, 43918], 'mapped', [5054]],
        [[43919, 43919], 'mapped', [5055]],
        [[43920, 43920], 'mapped', [5056]],
        [[43921, 43921], 'mapped', [5057]],
        [[43922, 43922], 'mapped', [5058]],
        [[43923, 43923], 'mapped', [5059]],
        [[43924, 43924], 'mapped', [5060]],
        [[43925, 43925], 'mapped', [5061]],
        [[43926, 43926], 'mapped', [5062]],
        [[43927, 43927], 'mapped', [5063]],
        [[43928, 43928], 'mapped', [5064]],
        [[43929, 43929], 'mapped', [5065]],
        [[43930, 43930], 'mapped', [5066]],
        [[43931, 43931], 'mapped', [5067]],
        [[43932, 43932], 'mapped', [5068]],
        [[43933, 43933], 'mapped', [5069]],
        [[43934, 43934], 'mapped', [5070]],
        [[43935, 43935], 'mapped', [5071]],
        [[43936, 43936], 'mapped', [5072]],
        [[43937, 43937], 'mapped', [5073]],
        [[43938, 43938], 'mapped', [5074]],
        [[43939, 43939], 'mapped', [5075]],
        [[43940, 43940], 'mapped', [5076]],
        [[43941, 43941], 'mapped', [5077]],
        [[43942, 43942], 'mapped', [5078]],
        [[43943, 43943], 'mapped', [5079]],
        [[43944, 43944], 'mapped', [5080]],
        [[43945, 43945], 'mapped', [5081]],
        [[43946, 43946], 'mapped', [5082]],
        [[43947, 43947], 'mapped', [5083]],
        [[43948, 43948], 'mapped', [5084]],
        [[43949, 43949], 'mapped', [5085]],
        [[43950, 43950], 'mapped', [5086]],
        [[43951, 43951], 'mapped', [5087]],
        [[43952, 43952], 'mapped', [5088]],
        [[43953, 43953], 'mapped', [5089]],
        [[43954, 43954], 'mapped', [5090]],
        [[43955, 43955], 'mapped', [5091]],
        [[43956, 43956], 'mapped', [5092]],
        [[43957, 43957], 'mapped', [5093]],
        [[43958, 43958], 'mapped', [5094]],
        [[43959, 43959], 'mapped', [5095]],
        [[43960, 43960], 'mapped', [5096]],
        [[43961, 43961], 'mapped', [5097]],
        [[43962, 43962], 'mapped', [5098]],
        [[43963, 43963], 'mapped', [5099]],
        [[43964, 43964], 'mapped', [5100]],
        [[43965, 43965], 'mapped', [5101]],
        [[43966, 43966], 'mapped', [5102]],
        [[43967, 43967], 'mapped', [5103]],
        [[43968, 44010], 'valid'],
        [[44011, 44011], 'valid', [], 'NV8'],
        [[44012, 44013], 'valid'],
        [[44014, 44015], 'disallowed'],
        [[44016, 44025], 'valid'],
        [[44026, 44031], 'disallowed'],
        [[44032, 55203], 'valid'],
        [[55204, 55215], 'disallowed'],
        [[55216, 55238], 'valid', [], 'NV8'],
        [[55239, 55242], 'disallowed'],
        [[55243, 55291], 'valid', [], 'NV8'],
        [[55292, 55295], 'disallowed'],
        [[55296, 57343], 'disallowed'],
        [[57344, 63743], 'disallowed'],
        [[63744, 63744], 'mapped', [35912]],
        [[63745, 63745], 'mapped', [26356]],
        [[63746, 63746], 'mapped', [36554]],
        [[63747, 63747], 'mapped', [36040]],
        [[63748, 63748], 'mapped', [28369]],
        [[63749, 63749], 'mapped', [20018]],
        [[63750, 63750], 'mapped', [21477]],
        [[63751, 63752], 'mapped', [40860]],
        [[63753, 63753], 'mapped', [22865]],
        [[63754, 63754], 'mapped', [37329]],
        [[63755, 63755], 'mapped', [21895]],
        [[63756, 63756], 'mapped', [22856]],
        [[63757, 63757], 'mapped', [25078]],
        [[63758, 63758], 'mapped', [30313]],
        [[63759, 63759], 'mapped', [32645]],
        [[63760, 63760], 'mapped', [34367]],
        [[63761, 63761], 'mapped', [34746]],
        [[63762, 63762], 'mapped', [35064]],
        [[63763, 63763], 'mapped', [37007]],
        [[63764, 63764], 'mapped', [27138]],
        [[63765, 63765], 'mapped', [27931]],
        [[63766, 63766], 'mapped', [28889]],
        [[63767, 63767], 'mapped', [29662]],
        [[63768, 63768], 'mapped', [33853]],
        [[63769, 63769], 'mapped', [37226]],
        [[63770, 63770], 'mapped', [39409]],
        [[63771, 63771], 'mapped', [20098]],
        [[63772, 63772], 'mapped', [21365]],
        [[63773, 63773], 'mapped', [27396]],
        [[63774, 63774], 'mapped', [29211]],
        [[63775, 63775], 'mapped', [34349]],
        [[63776, 63776], 'mapped', [40478]],
        [[63777, 63777], 'mapped', [23888]],
        [[63778, 63778], 'mapped', [28651]],
        [[63779, 63779], 'mapped', [34253]],
        [[63780, 63780], 'mapped', [35172]],
        [[63781, 63781], 'mapped', [25289]],
        [[63782, 63782], 'mapped', [33240]],
        [[63783, 63783], 'mapped', [34847]],
        [[63784, 63784], 'mapped', [24266]],
        [[63785, 63785], 'mapped', [26391]],
        [[63786, 63786], 'mapped', [28010]],
        [[63787, 63787], 'mapped', [29436]],
        [[63788, 63788], 'mapped', [37070]],
        [[63789, 63789], 'mapped', [20358]],
        [[63790, 63790], 'mapped', [20919]],
        [[63791, 63791], 'mapped', [21214]],
        [[63792, 63792], 'mapped', [25796]],
        [[63793, 63793], 'mapped', [27347]],
        [[63794, 63794], 'mapped', [29200]],
        [[63795, 63795], 'mapped', [30439]],
        [[63796, 63796], 'mapped', [32769]],
        [[63797, 63797], 'mapped', [34310]],
        [[63798, 63798], 'mapped', [34396]],
        [[63799, 63799], 'mapped', [36335]],
        [[63800, 63800], 'mapped', [38706]],
        [[63801, 63801], 'mapped', [39791]],
        [[63802, 63802], 'mapped', [40442]],
        [[63803, 63803], 'mapped', [30860]],
        [[63804, 63804], 'mapped', [31103]],
        [[63805, 63805], 'mapped', [32160]],
        [[63806, 63806], 'mapped', [33737]],
        [[63807, 63807], 'mapped', [37636]],
        [[63808, 63808], 'mapped', [40575]],
        [[63809, 63809], 'mapped', [35542]],
        [[63810, 63810], 'mapped', [22751]],
        [[63811, 63811], 'mapped', [24324]],
        [[63812, 63812], 'mapped', [31840]],
        [[63813, 63813], 'mapped', [32894]],
        [[63814, 63814], 'mapped', [29282]],
        [[63815, 63815], 'mapped', [30922]],
        [[63816, 63816], 'mapped', [36034]],
        [[63817, 63817], 'mapped', [38647]],
        [[63818, 63818], 'mapped', [22744]],
        [[63819, 63819], 'mapped', [23650]],
        [[63820, 63820], 'mapped', [27155]],
        [[63821, 63821], 'mapped', [28122]],
        [[63822, 63822], 'mapped', [28431]],
        [[63823, 63823], 'mapped', [32047]],
        [[63824, 63824], 'mapped', [32311]],
        [[63825, 63825], 'mapped', [38475]],
        [[63826, 63826], 'mapped', [21202]],
        [[63827, 63827], 'mapped', [32907]],
        [[63828, 63828], 'mapped', [20956]],
        [[63829, 63829], 'mapped', [20940]],
        [[63830, 63830], 'mapped', [31260]],
        [[63831, 63831], 'mapped', [32190]],
        [[63832, 63832], 'mapped', [33777]],
        [[63833, 63833], 'mapped', [38517]],
        [[63834, 63834], 'mapped', [35712]],
        [[63835, 63835], 'mapped', [25295]],
        [[63836, 63836], 'mapped', [27138]],
        [[63837, 63837], 'mapped', [35582]],
        [[63838, 63838], 'mapped', [20025]],
        [[63839, 63839], 'mapped', [23527]],
        [[63840, 63840], 'mapped', [24594]],
        [[63841, 63841], 'mapped', [29575]],
        [[63842, 63842], 'mapped', [30064]],
        [[63843, 63843], 'mapped', [21271]],
        [[63844, 63844], 'mapped', [30971]],
        [[63845, 63845], 'mapped', [20415]],
        [[63846, 63846], 'mapped', [24489]],
        [[63847, 63847], 'mapped', [19981]],
        [[63848, 63848], 'mapped', [27852]],
        [[63849, 63849], 'mapped', [25976]],
        [[63850, 63850], 'mapped', [32034]],
        [[63851, 63851], 'mapped', [21443]],
        [[63852, 63852], 'mapped', [22622]],
        [[63853, 63853], 'mapped', [30465]],
        [[63854, 63854], 'mapped', [33865]],
        [[63855, 63855], 'mapped', [35498]],
        [[63856, 63856], 'mapped', [27578]],
        [[63857, 63857], 'mapped', [36784]],
        [[63858, 63858], 'mapped', [27784]],
        [[63859, 63859], 'mapped', [25342]],
        [[63860, 63860], 'mapped', [33509]],
        [[63861, 63861], 'mapped', [25504]],
        [[63862, 63862], 'mapped', [30053]],
        [[63863, 63863], 'mapped', [20142]],
        [[63864, 63864], 'mapped', [20841]],
        [[63865, 63865], 'mapped', [20937]],
        [[63866, 63866], 'mapped', [26753]],
        [[63867, 63867], 'mapped', [31975]],
        [[63868, 63868], 'mapped', [33391]],
        [[63869, 63869], 'mapped', [35538]],
        [[63870, 63870], 'mapped', [37327]],
        [[63871, 63871], 'mapped', [21237]],
        [[63872, 63872], 'mapped', [21570]],
        [[63873, 63873], 'mapped', [22899]],
        [[63874, 63874], 'mapped', [24300]],
        [[63875, 63875], 'mapped', [26053]],
        [[63876, 63876], 'mapped', [28670]],
        [[63877, 63877], 'mapped', [31018]],
        [[63878, 63878], 'mapped', [38317]],
        [[63879, 63879], 'mapped', [39530]],
        [[63880, 63880], 'mapped', [40599]],
        [[63881, 63881], 'mapped', [40654]],
        [[63882, 63882], 'mapped', [21147]],
        [[63883, 63883], 'mapped', [26310]],
        [[63884, 63884], 'mapped', [27511]],
        [[63885, 63885], 'mapped', [36706]],
        [[63886, 63886], 'mapped', [24180]],
        [[63887, 63887], 'mapped', [24976]],
        [[63888, 63888], 'mapped', [25088]],
        [[63889, 63889], 'mapped', [25754]],
        [[63890, 63890], 'mapped', [28451]],
        [[63891, 63891], 'mapped', [29001]],
        [[63892, 63892], 'mapped', [29833]],
        [[63893, 63893], 'mapped', [31178]],
        [[63894, 63894], 'mapped', [32244]],
        [[63895, 63895], 'mapped', [32879]],
        [[63896, 63896], 'mapped', [36646]],
        [[63897, 63897], 'mapped', [34030]],
        [[63898, 63898], 'mapped', [36899]],
        [[63899, 63899], 'mapped', [37706]],
        [[63900, 63900], 'mapped', [21015]],
        [[63901, 63901], 'mapped', [21155]],
        [[63902, 63902], 'mapped', [21693]],
        [[63903, 63903], 'mapped', [28872]],
        [[63904, 63904], 'mapped', [35010]],
        [[63905, 63905], 'mapped', [35498]],
        [[63906, 63906], 'mapped', [24265]],
        [[63907, 63907], 'mapped', [24565]],
        [[63908, 63908], 'mapped', [25467]],
        [[63909, 63909], 'mapped', [27566]],
        [[63910, 63910], 'mapped', [31806]],
        [[63911, 63911], 'mapped', [29557]],
        [[63912, 63912], 'mapped', [20196]],
        [[63913, 63913], 'mapped', [22265]],
        [[63914, 63914], 'mapped', [23527]],
        [[63915, 63915], 'mapped', [23994]],
        [[63916, 63916], 'mapped', [24604]],
        [[63917, 63917], 'mapped', [29618]],
        [[63918, 63918], 'mapped', [29801]],
        [[63919, 63919], 'mapped', [32666]],
        [[63920, 63920], 'mapped', [32838]],
        [[63921, 63921], 'mapped', [37428]],
        [[63922, 63922], 'mapped', [38646]],
        [[63923, 63923], 'mapped', [38728]],
        [[63924, 63924], 'mapped', [38936]],
        [[63925, 63925], 'mapped', [20363]],
        [[63926, 63926], 'mapped', [31150]],
        [[63927, 63927], 'mapped', [37300]],
        [[63928, 63928], 'mapped', [38584]],
        [[63929, 63929], 'mapped', [24801]],
        [[63930, 63930], 'mapped', [20102]],
        [[63931, 63931], 'mapped', [20698]],
        [[63932, 63932], 'mapped', [23534]],
        [[63933, 63933], 'mapped', [23615]],
        [[63934, 63934], 'mapped', [26009]],
        [[63935, 63935], 'mapped', [27138]],
        [[63936, 63936], 'mapped', [29134]],
        [[63937, 63937], 'mapped', [30274]],
        [[63938, 63938], 'mapped', [34044]],
        [[63939, 63939], 'mapped', [36988]],
        [[63940, 63940], 'mapped', [40845]],
        [[63941, 63941], 'mapped', [26248]],
        [[63942, 63942], 'mapped', [38446]],
        [[63943, 63943], 'mapped', [21129]],
        [[63944, 63944], 'mapped', [26491]],
        [[63945, 63945], 'mapped', [26611]],
        [[63946, 63946], 'mapped', [27969]],
        [[63947, 63947], 'mapped', [28316]],
        [[63948, 63948], 'mapped', [29705]],
        [[63949, 63949], 'mapped', [30041]],
        [[63950, 63950], 'mapped', [30827]],
        [[63951, 63951], 'mapped', [32016]],
        [[63952, 63952], 'mapped', [39006]],
        [[63953, 63953], 'mapped', [20845]],
        [[63954, 63954], 'mapped', [25134]],
        [[63955, 63955], 'mapped', [38520]],
        [[63956, 63956], 'mapped', [20523]],
        [[63957, 63957], 'mapped', [23833]],
        [[63958, 63958], 'mapped', [28138]],
        [[63959, 63959], 'mapped', [36650]],
        [[63960, 63960], 'mapped', [24459]],
        [[63961, 63961], 'mapped', [24900]],
        [[63962, 63962], 'mapped', [26647]],
        [[63963, 63963], 'mapped', [29575]],
        [[63964, 63964], 'mapped', [38534]],
        [[63965, 63965], 'mapped', [21033]],
        [[63966, 63966], 'mapped', [21519]],
        [[63967, 63967], 'mapped', [23653]],
        [[63968, 63968], 'mapped', [26131]],
        [[63969, 63969], 'mapped', [26446]],
        [[63970, 63970], 'mapped', [26792]],
        [[63971, 63971], 'mapped', [27877]],
        [[63972, 63972], 'mapped', [29702]],
        [[63973, 63973], 'mapped', [30178]],
        [[63974, 63974], 'mapped', [32633]],
        [[63975, 63975], 'mapped', [35023]],
        [[63976, 63976], 'mapped', [35041]],
        [[63977, 63977], 'mapped', [37324]],
        [[63978, 63978], 'mapped', [38626]],
        [[63979, 63979], 'mapped', [21311]],
        [[63980, 63980], 'mapped', [28346]],
        [[63981, 63981], 'mapped', [21533]],
        [[63982, 63982], 'mapped', [29136]],
        [[63983, 63983], 'mapped', [29848]],
        [[63984, 63984], 'mapped', [34298]],
        [[63985, 63985], 'mapped', [38563]],
        [[63986, 63986], 'mapped', [40023]],
        [[63987, 63987], 'mapped', [40607]],
        [[63988, 63988], 'mapped', [26519]],
        [[63989, 63989], 'mapped', [28107]],
        [[63990, 63990], 'mapped', [33256]],
        [[63991, 63991], 'mapped', [31435]],
        [[63992, 63992], 'mapped', [31520]],
        [[63993, 63993], 'mapped', [31890]],
        [[63994, 63994], 'mapped', [29376]],
        [[63995, 63995], 'mapped', [28825]],
        [[63996, 63996], 'mapped', [35672]],
        [[63997, 63997], 'mapped', [20160]],
        [[63998, 63998], 'mapped', [33590]],
        [[63999, 63999], 'mapped', [21050]],
        [[64e3, 64e3], 'mapped', [20999]],
        [[64001, 64001], 'mapped', [24230]],
        [[64002, 64002], 'mapped', [25299]],
        [[64003, 64003], 'mapped', [31958]],
        [[64004, 64004], 'mapped', [23429]],
        [[64005, 64005], 'mapped', [27934]],
        [[64006, 64006], 'mapped', [26292]],
        [[64007, 64007], 'mapped', [36667]],
        [[64008, 64008], 'mapped', [34892]],
        [[64009, 64009], 'mapped', [38477]],
        [[64010, 64010], 'mapped', [35211]],
        [[64011, 64011], 'mapped', [24275]],
        [[64012, 64012], 'mapped', [20800]],
        [[64013, 64013], 'mapped', [21952]],
        [[64014, 64015], 'valid'],
        [[64016, 64016], 'mapped', [22618]],
        [[64017, 64017], 'valid'],
        [[64018, 64018], 'mapped', [26228]],
        [[64019, 64020], 'valid'],
        [[64021, 64021], 'mapped', [20958]],
        [[64022, 64022], 'mapped', [29482]],
        [[64023, 64023], 'mapped', [30410]],
        [[64024, 64024], 'mapped', [31036]],
        [[64025, 64025], 'mapped', [31070]],
        [[64026, 64026], 'mapped', [31077]],
        [[64027, 64027], 'mapped', [31119]],
        [[64028, 64028], 'mapped', [38742]],
        [[64029, 64029], 'mapped', [31934]],
        [[64030, 64030], 'mapped', [32701]],
        [[64031, 64031], 'valid'],
        [[64032, 64032], 'mapped', [34322]],
        [[64033, 64033], 'valid'],
        [[64034, 64034], 'mapped', [35576]],
        [[64035, 64036], 'valid'],
        [[64037, 64037], 'mapped', [36920]],
        [[64038, 64038], 'mapped', [37117]],
        [[64039, 64041], 'valid'],
        [[64042, 64042], 'mapped', [39151]],
        [[64043, 64043], 'mapped', [39164]],
        [[64044, 64044], 'mapped', [39208]],
        [[64045, 64045], 'mapped', [40372]],
        [[64046, 64046], 'mapped', [37086]],
        [[64047, 64047], 'mapped', [38583]],
        [[64048, 64048], 'mapped', [20398]],
        [[64049, 64049], 'mapped', [20711]],
        [[64050, 64050], 'mapped', [20813]],
        [[64051, 64051], 'mapped', [21193]],
        [[64052, 64052], 'mapped', [21220]],
        [[64053, 64053], 'mapped', [21329]],
        [[64054, 64054], 'mapped', [21917]],
        [[64055, 64055], 'mapped', [22022]],
        [[64056, 64056], 'mapped', [22120]],
        [[64057, 64057], 'mapped', [22592]],
        [[64058, 64058], 'mapped', [22696]],
        [[64059, 64059], 'mapped', [23652]],
        [[64060, 64060], 'mapped', [23662]],
        [[64061, 64061], 'mapped', [24724]],
        [[64062, 64062], 'mapped', [24936]],
        [[64063, 64063], 'mapped', [24974]],
        [[64064, 64064], 'mapped', [25074]],
        [[64065, 64065], 'mapped', [25935]],
        [[64066, 64066], 'mapped', [26082]],
        [[64067, 64067], 'mapped', [26257]],
        [[64068, 64068], 'mapped', [26757]],
        [[64069, 64069], 'mapped', [28023]],
        [[64070, 64070], 'mapped', [28186]],
        [[64071, 64071], 'mapped', [28450]],
        [[64072, 64072], 'mapped', [29038]],
        [[64073, 64073], 'mapped', [29227]],
        [[64074, 64074], 'mapped', [29730]],
        [[64075, 64075], 'mapped', [30865]],
        [[64076, 64076], 'mapped', [31038]],
        [[64077, 64077], 'mapped', [31049]],
        [[64078, 64078], 'mapped', [31048]],
        [[64079, 64079], 'mapped', [31056]],
        [[64080, 64080], 'mapped', [31062]],
        [[64081, 64081], 'mapped', [31069]],
        [[64082, 64082], 'mapped', [31117]],
        [[64083, 64083], 'mapped', [31118]],
        [[64084, 64084], 'mapped', [31296]],
        [[64085, 64085], 'mapped', [31361]],
        [[64086, 64086], 'mapped', [31680]],
        [[64087, 64087], 'mapped', [32244]],
        [[64088, 64088], 'mapped', [32265]],
        [[64089, 64089], 'mapped', [32321]],
        [[64090, 64090], 'mapped', [32626]],
        [[64091, 64091], 'mapped', [32773]],
        [[64092, 64092], 'mapped', [33261]],
        [[64093, 64094], 'mapped', [33401]],
        [[64095, 64095], 'mapped', [33879]],
        [[64096, 64096], 'mapped', [35088]],
        [[64097, 64097], 'mapped', [35222]],
        [[64098, 64098], 'mapped', [35585]],
        [[64099, 64099], 'mapped', [35641]],
        [[64100, 64100], 'mapped', [36051]],
        [[64101, 64101], 'mapped', [36104]],
        [[64102, 64102], 'mapped', [36790]],
        [[64103, 64103], 'mapped', [36920]],
        [[64104, 64104], 'mapped', [38627]],
        [[64105, 64105], 'mapped', [38911]],
        [[64106, 64106], 'mapped', [38971]],
        [[64107, 64107], 'mapped', [24693]],
        [[64108, 64108], 'mapped', [148206]],
        [[64109, 64109], 'mapped', [33304]],
        [[64110, 64111], 'disallowed'],
        [[64112, 64112], 'mapped', [20006]],
        [[64113, 64113], 'mapped', [20917]],
        [[64114, 64114], 'mapped', [20840]],
        [[64115, 64115], 'mapped', [20352]],
        [[64116, 64116], 'mapped', [20805]],
        [[64117, 64117], 'mapped', [20864]],
        [[64118, 64118], 'mapped', [21191]],
        [[64119, 64119], 'mapped', [21242]],
        [[64120, 64120], 'mapped', [21917]],
        [[64121, 64121], 'mapped', [21845]],
        [[64122, 64122], 'mapped', [21913]],
        [[64123, 64123], 'mapped', [21986]],
        [[64124, 64124], 'mapped', [22618]],
        [[64125, 64125], 'mapped', [22707]],
        [[64126, 64126], 'mapped', [22852]],
        [[64127, 64127], 'mapped', [22868]],
        [[64128, 64128], 'mapped', [23138]],
        [[64129, 64129], 'mapped', [23336]],
        [[64130, 64130], 'mapped', [24274]],
        [[64131, 64131], 'mapped', [24281]],
        [[64132, 64132], 'mapped', [24425]],
        [[64133, 64133], 'mapped', [24493]],
        [[64134, 64134], 'mapped', [24792]],
        [[64135, 64135], 'mapped', [24910]],
        [[64136, 64136], 'mapped', [24840]],
        [[64137, 64137], 'mapped', [24974]],
        [[64138, 64138], 'mapped', [24928]],
        [[64139, 64139], 'mapped', [25074]],
        [[64140, 64140], 'mapped', [25140]],
        [[64141, 64141], 'mapped', [25540]],
        [[64142, 64142], 'mapped', [25628]],
        [[64143, 64143], 'mapped', [25682]],
        [[64144, 64144], 'mapped', [25942]],
        [[64145, 64145], 'mapped', [26228]],
        [[64146, 64146], 'mapped', [26391]],
        [[64147, 64147], 'mapped', [26395]],
        [[64148, 64148], 'mapped', [26454]],
        [[64149, 64149], 'mapped', [27513]],
        [[64150, 64150], 'mapped', [27578]],
        [[64151, 64151], 'mapped', [27969]],
        [[64152, 64152], 'mapped', [28379]],
        [[64153, 64153], 'mapped', [28363]],
        [[64154, 64154], 'mapped', [28450]],
        [[64155, 64155], 'mapped', [28702]],
        [[64156, 64156], 'mapped', [29038]],
        [[64157, 64157], 'mapped', [30631]],
        [[64158, 64158], 'mapped', [29237]],
        [[64159, 64159], 'mapped', [29359]],
        [[64160, 64160], 'mapped', [29482]],
        [[64161, 64161], 'mapped', [29809]],
        [[64162, 64162], 'mapped', [29958]],
        [[64163, 64163], 'mapped', [30011]],
        [[64164, 64164], 'mapped', [30237]],
        [[64165, 64165], 'mapped', [30239]],
        [[64166, 64166], 'mapped', [30410]],
        [[64167, 64167], 'mapped', [30427]],
        [[64168, 64168], 'mapped', [30452]],
        [[64169, 64169], 'mapped', [30538]],
        [[64170, 64170], 'mapped', [30528]],
        [[64171, 64171], 'mapped', [30924]],
        [[64172, 64172], 'mapped', [31409]],
        [[64173, 64173], 'mapped', [31680]],
        [[64174, 64174], 'mapped', [31867]],
        [[64175, 64175], 'mapped', [32091]],
        [[64176, 64176], 'mapped', [32244]],
        [[64177, 64177], 'mapped', [32574]],
        [[64178, 64178], 'mapped', [32773]],
        [[64179, 64179], 'mapped', [33618]],
        [[64180, 64180], 'mapped', [33775]],
        [[64181, 64181], 'mapped', [34681]],
        [[64182, 64182], 'mapped', [35137]],
        [[64183, 64183], 'mapped', [35206]],
        [[64184, 64184], 'mapped', [35222]],
        [[64185, 64185], 'mapped', [35519]],
        [[64186, 64186], 'mapped', [35576]],
        [[64187, 64187], 'mapped', [35531]],
        [[64188, 64188], 'mapped', [35585]],
        [[64189, 64189], 'mapped', [35582]],
        [[64190, 64190], 'mapped', [35565]],
        [[64191, 64191], 'mapped', [35641]],
        [[64192, 64192], 'mapped', [35722]],
        [[64193, 64193], 'mapped', [36104]],
        [[64194, 64194], 'mapped', [36664]],
        [[64195, 64195], 'mapped', [36978]],
        [[64196, 64196], 'mapped', [37273]],
        [[64197, 64197], 'mapped', [37494]],
        [[64198, 64198], 'mapped', [38524]],
        [[64199, 64199], 'mapped', [38627]],
        [[64200, 64200], 'mapped', [38742]],
        [[64201, 64201], 'mapped', [38875]],
        [[64202, 64202], 'mapped', [38911]],
        [[64203, 64203], 'mapped', [38923]],
        [[64204, 64204], 'mapped', [38971]],
        [[64205, 64205], 'mapped', [39698]],
        [[64206, 64206], 'mapped', [40860]],
        [[64207, 64207], 'mapped', [141386]],
        [[64208, 64208], 'mapped', [141380]],
        [[64209, 64209], 'mapped', [144341]],
        [[64210, 64210], 'mapped', [15261]],
        [[64211, 64211], 'mapped', [16408]],
        [[64212, 64212], 'mapped', [16441]],
        [[64213, 64213], 'mapped', [152137]],
        [[64214, 64214], 'mapped', [154832]],
        [[64215, 64215], 'mapped', [163539]],
        [[64216, 64216], 'mapped', [40771]],
        [[64217, 64217], 'mapped', [40846]],
        [[64218, 64255], 'disallowed'],
        [[64256, 64256], 'mapped', [102, 102]],
        [[64257, 64257], 'mapped', [102, 105]],
        [[64258, 64258], 'mapped', [102, 108]],
        [[64259, 64259], 'mapped', [102, 102, 105]],
        [[64260, 64260], 'mapped', [102, 102, 108]],
        [[64261, 64262], 'mapped', [115, 116]],
        [[64263, 64274], 'disallowed'],
        [[64275, 64275], 'mapped', [1396, 1398]],
        [[64276, 64276], 'mapped', [1396, 1381]],
        [[64277, 64277], 'mapped', [1396, 1387]],
        [[64278, 64278], 'mapped', [1406, 1398]],
        [[64279, 64279], 'mapped', [1396, 1389]],
        [[64280, 64284], 'disallowed'],
        [[64285, 64285], 'mapped', [1497, 1460]],
        [[64286, 64286], 'valid'],
        [[64287, 64287], 'mapped', [1522, 1463]],
        [[64288, 64288], 'mapped', [1506]],
        [[64289, 64289], 'mapped', [1488]],
        [[64290, 64290], 'mapped', [1491]],
        [[64291, 64291], 'mapped', [1492]],
        [[64292, 64292], 'mapped', [1499]],
        [[64293, 64293], 'mapped', [1500]],
        [[64294, 64294], 'mapped', [1501]],
        [[64295, 64295], 'mapped', [1512]],
        [[64296, 64296], 'mapped', [1514]],
        [[64297, 64297], 'disallowed_STD3_mapped', [43]],
        [[64298, 64298], 'mapped', [1513, 1473]],
        [[64299, 64299], 'mapped', [1513, 1474]],
        [[64300, 64300], 'mapped', [1513, 1468, 1473]],
        [[64301, 64301], 'mapped', [1513, 1468, 1474]],
        [[64302, 64302], 'mapped', [1488, 1463]],
        [[64303, 64303], 'mapped', [1488, 1464]],
        [[64304, 64304], 'mapped', [1488, 1468]],
        [[64305, 64305], 'mapped', [1489, 1468]],
        [[64306, 64306], 'mapped', [1490, 1468]],
        [[64307, 64307], 'mapped', [1491, 1468]],
        [[64308, 64308], 'mapped', [1492, 1468]],
        [[64309, 64309], 'mapped', [1493, 1468]],
        [[64310, 64310], 'mapped', [1494, 1468]],
        [[64311, 64311], 'disallowed'],
        [[64312, 64312], 'mapped', [1496, 1468]],
        [[64313, 64313], 'mapped', [1497, 1468]],
        [[64314, 64314], 'mapped', [1498, 1468]],
        [[64315, 64315], 'mapped', [1499, 1468]],
        [[64316, 64316], 'mapped', [1500, 1468]],
        [[64317, 64317], 'disallowed'],
        [[64318, 64318], 'mapped', [1502, 1468]],
        [[64319, 64319], 'disallowed'],
        [[64320, 64320], 'mapped', [1504, 1468]],
        [[64321, 64321], 'mapped', [1505, 1468]],
        [[64322, 64322], 'disallowed'],
        [[64323, 64323], 'mapped', [1507, 1468]],
        [[64324, 64324], 'mapped', [1508, 1468]],
        [[64325, 64325], 'disallowed'],
        [[64326, 64326], 'mapped', [1510, 1468]],
        [[64327, 64327], 'mapped', [1511, 1468]],
        [[64328, 64328], 'mapped', [1512, 1468]],
        [[64329, 64329], 'mapped', [1513, 1468]],
        [[64330, 64330], 'mapped', [1514, 1468]],
        [[64331, 64331], 'mapped', [1493, 1465]],
        [[64332, 64332], 'mapped', [1489, 1471]],
        [[64333, 64333], 'mapped', [1499, 1471]],
        [[64334, 64334], 'mapped', [1508, 1471]],
        [[64335, 64335], 'mapped', [1488, 1500]],
        [[64336, 64337], 'mapped', [1649]],
        [[64338, 64341], 'mapped', [1659]],
        [[64342, 64345], 'mapped', [1662]],
        [[64346, 64349], 'mapped', [1664]],
        [[64350, 64353], 'mapped', [1658]],
        [[64354, 64357], 'mapped', [1663]],
        [[64358, 64361], 'mapped', [1657]],
        [[64362, 64365], 'mapped', [1700]],
        [[64366, 64369], 'mapped', [1702]],
        [[64370, 64373], 'mapped', [1668]],
        [[64374, 64377], 'mapped', [1667]],
        [[64378, 64381], 'mapped', [1670]],
        [[64382, 64385], 'mapped', [1671]],
        [[64386, 64387], 'mapped', [1677]],
        [[64388, 64389], 'mapped', [1676]],
        [[64390, 64391], 'mapped', [1678]],
        [[64392, 64393], 'mapped', [1672]],
        [[64394, 64395], 'mapped', [1688]],
        [[64396, 64397], 'mapped', [1681]],
        [[64398, 64401], 'mapped', [1705]],
        [[64402, 64405], 'mapped', [1711]],
        [[64406, 64409], 'mapped', [1715]],
        [[64410, 64413], 'mapped', [1713]],
        [[64414, 64415], 'mapped', [1722]],
        [[64416, 64419], 'mapped', [1723]],
        [[64420, 64421], 'mapped', [1728]],
        [[64422, 64425], 'mapped', [1729]],
        [[64426, 64429], 'mapped', [1726]],
        [[64430, 64431], 'mapped', [1746]],
        [[64432, 64433], 'mapped', [1747]],
        [[64434, 64449], 'valid', [], 'NV8'],
        [[64450, 64466], 'disallowed'],
        [[64467, 64470], 'mapped', [1709]],
        [[64471, 64472], 'mapped', [1735]],
        [[64473, 64474], 'mapped', [1734]],
        [[64475, 64476], 'mapped', [1736]],
        [[64477, 64477], 'mapped', [1735, 1652]],
        [[64478, 64479], 'mapped', [1739]],
        [[64480, 64481], 'mapped', [1733]],
        [[64482, 64483], 'mapped', [1737]],
        [[64484, 64487], 'mapped', [1744]],
        [[64488, 64489], 'mapped', [1609]],
        [[64490, 64491], 'mapped', [1574, 1575]],
        [[64492, 64493], 'mapped', [1574, 1749]],
        [[64494, 64495], 'mapped', [1574, 1608]],
        [[64496, 64497], 'mapped', [1574, 1735]],
        [[64498, 64499], 'mapped', [1574, 1734]],
        [[64500, 64501], 'mapped', [1574, 1736]],
        [[64502, 64504], 'mapped', [1574, 1744]],
        [[64505, 64507], 'mapped', [1574, 1609]],
        [[64508, 64511], 'mapped', [1740]],
        [[64512, 64512], 'mapped', [1574, 1580]],
        [[64513, 64513], 'mapped', [1574, 1581]],
        [[64514, 64514], 'mapped', [1574, 1605]],
        [[64515, 64515], 'mapped', [1574, 1609]],
        [[64516, 64516], 'mapped', [1574, 1610]],
        [[64517, 64517], 'mapped', [1576, 1580]],
        [[64518, 64518], 'mapped', [1576, 1581]],
        [[64519, 64519], 'mapped', [1576, 1582]],
        [[64520, 64520], 'mapped', [1576, 1605]],
        [[64521, 64521], 'mapped', [1576, 1609]],
        [[64522, 64522], 'mapped', [1576, 1610]],
        [[64523, 64523], 'mapped', [1578, 1580]],
        [[64524, 64524], 'mapped', [1578, 1581]],
        [[64525, 64525], 'mapped', [1578, 1582]],
        [[64526, 64526], 'mapped', [1578, 1605]],
        [[64527, 64527], 'mapped', [1578, 1609]],
        [[64528, 64528], 'mapped', [1578, 1610]],
        [[64529, 64529], 'mapped', [1579, 1580]],
        [[64530, 64530], 'mapped', [1579, 1605]],
        [[64531, 64531], 'mapped', [1579, 1609]],
        [[64532, 64532], 'mapped', [1579, 1610]],
        [[64533, 64533], 'mapped', [1580, 1581]],
        [[64534, 64534], 'mapped', [1580, 1605]],
        [[64535, 64535], 'mapped', [1581, 1580]],
        [[64536, 64536], 'mapped', [1581, 1605]],
        [[64537, 64537], 'mapped', [1582, 1580]],
        [[64538, 64538], 'mapped', [1582, 1581]],
        [[64539, 64539], 'mapped', [1582, 1605]],
        [[64540, 64540], 'mapped', [1587, 1580]],
        [[64541, 64541], 'mapped', [1587, 1581]],
        [[64542, 64542], 'mapped', [1587, 1582]],
        [[64543, 64543], 'mapped', [1587, 1605]],
        [[64544, 64544], 'mapped', [1589, 1581]],
        [[64545, 64545], 'mapped', [1589, 1605]],
        [[64546, 64546], 'mapped', [1590, 1580]],
        [[64547, 64547], 'mapped', [1590, 1581]],
        [[64548, 64548], 'mapped', [1590, 1582]],
        [[64549, 64549], 'mapped', [1590, 1605]],
        [[64550, 64550], 'mapped', [1591, 1581]],
        [[64551, 64551], 'mapped', [1591, 1605]],
        [[64552, 64552], 'mapped', [1592, 1605]],
        [[64553, 64553], 'mapped', [1593, 1580]],
        [[64554, 64554], 'mapped', [1593, 1605]],
        [[64555, 64555], 'mapped', [1594, 1580]],
        [[64556, 64556], 'mapped', [1594, 1605]],
        [[64557, 64557], 'mapped', [1601, 1580]],
        [[64558, 64558], 'mapped', [1601, 1581]],
        [[64559, 64559], 'mapped', [1601, 1582]],
        [[64560, 64560], 'mapped', [1601, 1605]],
        [[64561, 64561], 'mapped', [1601, 1609]],
        [[64562, 64562], 'mapped', [1601, 1610]],
        [[64563, 64563], 'mapped', [1602, 1581]],
        [[64564, 64564], 'mapped', [1602, 1605]],
        [[64565, 64565], 'mapped', [1602, 1609]],
        [[64566, 64566], 'mapped', [1602, 1610]],
        [[64567, 64567], 'mapped', [1603, 1575]],
        [[64568, 64568], 'mapped', [1603, 1580]],
        [[64569, 64569], 'mapped', [1603, 1581]],
        [[64570, 64570], 'mapped', [1603, 1582]],
        [[64571, 64571], 'mapped', [1603, 1604]],
        [[64572, 64572], 'mapped', [1603, 1605]],
        [[64573, 64573], 'mapped', [1603, 1609]],
        [[64574, 64574], 'mapped', [1603, 1610]],
        [[64575, 64575], 'mapped', [1604, 1580]],
        [[64576, 64576], 'mapped', [1604, 1581]],
        [[64577, 64577], 'mapped', [1604, 1582]],
        [[64578, 64578], 'mapped', [1604, 1605]],
        [[64579, 64579], 'mapped', [1604, 1609]],
        [[64580, 64580], 'mapped', [1604, 1610]],
        [[64581, 64581], 'mapped', [1605, 1580]],
        [[64582, 64582], 'mapped', [1605, 1581]],
        [[64583, 64583], 'mapped', [1605, 1582]],
        [[64584, 64584], 'mapped', [1605, 1605]],
        [[64585, 64585], 'mapped', [1605, 1609]],
        [[64586, 64586], 'mapped', [1605, 1610]],
        [[64587, 64587], 'mapped', [1606, 1580]],
        [[64588, 64588], 'mapped', [1606, 1581]],
        [[64589, 64589], 'mapped', [1606, 1582]],
        [[64590, 64590], 'mapped', [1606, 1605]],
        [[64591, 64591], 'mapped', [1606, 1609]],
        [[64592, 64592], 'mapped', [1606, 1610]],
        [[64593, 64593], 'mapped', [1607, 1580]],
        [[64594, 64594], 'mapped', [1607, 1605]],
        [[64595, 64595], 'mapped', [1607, 1609]],
        [[64596, 64596], 'mapped', [1607, 1610]],
        [[64597, 64597], 'mapped', [1610, 1580]],
        [[64598, 64598], 'mapped', [1610, 1581]],
        [[64599, 64599], 'mapped', [1610, 1582]],
        [[64600, 64600], 'mapped', [1610, 1605]],
        [[64601, 64601], 'mapped', [1610, 1609]],
        [[64602, 64602], 'mapped', [1610, 1610]],
        [[64603, 64603], 'mapped', [1584, 1648]],
        [[64604, 64604], 'mapped', [1585, 1648]],
        [[64605, 64605], 'mapped', [1609, 1648]],
        [[64606, 64606], 'disallowed_STD3_mapped', [32, 1612, 1617]],
        [[64607, 64607], 'disallowed_STD3_mapped', [32, 1613, 1617]],
        [[64608, 64608], 'disallowed_STD3_mapped', [32, 1614, 1617]],
        [[64609, 64609], 'disallowed_STD3_mapped', [32, 1615, 1617]],
        [[64610, 64610], 'disallowed_STD3_mapped', [32, 1616, 1617]],
        [[64611, 64611], 'disallowed_STD3_mapped', [32, 1617, 1648]],
        [[64612, 64612], 'mapped', [1574, 1585]],
        [[64613, 64613], 'mapped', [1574, 1586]],
        [[64614, 64614], 'mapped', [1574, 1605]],
        [[64615, 64615], 'mapped', [1574, 1606]],
        [[64616, 64616], 'mapped', [1574, 1609]],
        [[64617, 64617], 'mapped', [1574, 1610]],
        [[64618, 64618], 'mapped', [1576, 1585]],
        [[64619, 64619], 'mapped', [1576, 1586]],
        [[64620, 64620], 'mapped', [1576, 1605]],
        [[64621, 64621], 'mapped', [1576, 1606]],
        [[64622, 64622], 'mapped', [1576, 1609]],
        [[64623, 64623], 'mapped', [1576, 1610]],
        [[64624, 64624], 'mapped', [1578, 1585]],
        [[64625, 64625], 'mapped', [1578, 1586]],
        [[64626, 64626], 'mapped', [1578, 1605]],
        [[64627, 64627], 'mapped', [1578, 1606]],
        [[64628, 64628], 'mapped', [1578, 1609]],
        [[64629, 64629], 'mapped', [1578, 1610]],
        [[64630, 64630], 'mapped', [1579, 1585]],
        [[64631, 64631], 'mapped', [1579, 1586]],
        [[64632, 64632], 'mapped', [1579, 1605]],
        [[64633, 64633], 'mapped', [1579, 1606]],
        [[64634, 64634], 'mapped', [1579, 1609]],
        [[64635, 64635], 'mapped', [1579, 1610]],
        [[64636, 64636], 'mapped', [1601, 1609]],
        [[64637, 64637], 'mapped', [1601, 1610]],
        [[64638, 64638], 'mapped', [1602, 1609]],
        [[64639, 64639], 'mapped', [1602, 1610]],
        [[64640, 64640], 'mapped', [1603, 1575]],
        [[64641, 64641], 'mapped', [1603, 1604]],
        [[64642, 64642], 'mapped', [1603, 1605]],
        [[64643, 64643], 'mapped', [1603, 1609]],
        [[64644, 64644], 'mapped', [1603, 1610]],
        [[64645, 64645], 'mapped', [1604, 1605]],
        [[64646, 64646], 'mapped', [1604, 1609]],
        [[64647, 64647], 'mapped', [1604, 1610]],
        [[64648, 64648], 'mapped', [1605, 1575]],
        [[64649, 64649], 'mapped', [1605, 1605]],
        [[64650, 64650], 'mapped', [1606, 1585]],
        [[64651, 64651], 'mapped', [1606, 1586]],
        [[64652, 64652], 'mapped', [1606, 1605]],
        [[64653, 64653], 'mapped', [1606, 1606]],
        [[64654, 64654], 'mapped', [1606, 1609]],
        [[64655, 64655], 'mapped', [1606, 1610]],
        [[64656, 64656], 'mapped', [1609, 1648]],
        [[64657, 64657], 'mapped', [1610, 1585]],
        [[64658, 64658], 'mapped', [1610, 1586]],
        [[64659, 64659], 'mapped', [1610, 1605]],
        [[64660, 64660], 'mapped', [1610, 1606]],
        [[64661, 64661], 'mapped', [1610, 1609]],
        [[64662, 64662], 'mapped', [1610, 1610]],
        [[64663, 64663], 'mapped', [1574, 1580]],
        [[64664, 64664], 'mapped', [1574, 1581]],
        [[64665, 64665], 'mapped', [1574, 1582]],
        [[64666, 64666], 'mapped', [1574, 1605]],
        [[64667, 64667], 'mapped', [1574, 1607]],
        [[64668, 64668], 'mapped', [1576, 1580]],
        [[64669, 64669], 'mapped', [1576, 1581]],
        [[64670, 64670], 'mapped', [1576, 1582]],
        [[64671, 64671], 'mapped', [1576, 1605]],
        [[64672, 64672], 'mapped', [1576, 1607]],
        [[64673, 64673], 'mapped', [1578, 1580]],
        [[64674, 64674], 'mapped', [1578, 1581]],
        [[64675, 64675], 'mapped', [1578, 1582]],
        [[64676, 64676], 'mapped', [1578, 1605]],
        [[64677, 64677], 'mapped', [1578, 1607]],
        [[64678, 64678], 'mapped', [1579, 1605]],
        [[64679, 64679], 'mapped', [1580, 1581]],
        [[64680, 64680], 'mapped', [1580, 1605]],
        [[64681, 64681], 'mapped', [1581, 1580]],
        [[64682, 64682], 'mapped', [1581, 1605]],
        [[64683, 64683], 'mapped', [1582, 1580]],
        [[64684, 64684], 'mapped', [1582, 1605]],
        [[64685, 64685], 'mapped', [1587, 1580]],
        [[64686, 64686], 'mapped', [1587, 1581]],
        [[64687, 64687], 'mapped', [1587, 1582]],
        [[64688, 64688], 'mapped', [1587, 1605]],
        [[64689, 64689], 'mapped', [1589, 1581]],
        [[64690, 64690], 'mapped', [1589, 1582]],
        [[64691, 64691], 'mapped', [1589, 1605]],
        [[64692, 64692], 'mapped', [1590, 1580]],
        [[64693, 64693], 'mapped', [1590, 1581]],
        [[64694, 64694], 'mapped', [1590, 1582]],
        [[64695, 64695], 'mapped', [1590, 1605]],
        [[64696, 64696], 'mapped', [1591, 1581]],
        [[64697, 64697], 'mapped', [1592, 1605]],
        [[64698, 64698], 'mapped', [1593, 1580]],
        [[64699, 64699], 'mapped', [1593, 1605]],
        [[64700, 64700], 'mapped', [1594, 1580]],
        [[64701, 64701], 'mapped', [1594, 1605]],
        [[64702, 64702], 'mapped', [1601, 1580]],
        [[64703, 64703], 'mapped', [1601, 1581]],
        [[64704, 64704], 'mapped', [1601, 1582]],
        [[64705, 64705], 'mapped', [1601, 1605]],
        [[64706, 64706], 'mapped', [1602, 1581]],
        [[64707, 64707], 'mapped', [1602, 1605]],
        [[64708, 64708], 'mapped', [1603, 1580]],
        [[64709, 64709], 'mapped', [1603, 1581]],
        [[64710, 64710], 'mapped', [1603, 1582]],
        [[64711, 64711], 'mapped', [1603, 1604]],
        [[64712, 64712], 'mapped', [1603, 1605]],
        [[64713, 64713], 'mapped', [1604, 1580]],
        [[64714, 64714], 'mapped', [1604, 1581]],
        [[64715, 64715], 'mapped', [1604, 1582]],
        [[64716, 64716], 'mapped', [1604, 1605]],
        [[64717, 64717], 'mapped', [1604, 1607]],
        [[64718, 64718], 'mapped', [1605, 1580]],
        [[64719, 64719], 'mapped', [1605, 1581]],
        [[64720, 64720], 'mapped', [1605, 1582]],
        [[64721, 64721], 'mapped', [1605, 1605]],
        [[64722, 64722], 'mapped', [1606, 1580]],
        [[64723, 64723], 'mapped', [1606, 1581]],
        [[64724, 64724], 'mapped', [1606, 1582]],
        [[64725, 64725], 'mapped', [1606, 1605]],
        [[64726, 64726], 'mapped', [1606, 1607]],
        [[64727, 64727], 'mapped', [1607, 1580]],
        [[64728, 64728], 'mapped', [1607, 1605]],
        [[64729, 64729], 'mapped', [1607, 1648]],
        [[64730, 64730], 'mapped', [1610, 1580]],
        [[64731, 64731], 'mapped', [1610, 1581]],
        [[64732, 64732], 'mapped', [1610, 1582]],
        [[64733, 64733], 'mapped', [1610, 1605]],
        [[64734, 64734], 'mapped', [1610, 1607]],
        [[64735, 64735], 'mapped', [1574, 1605]],
        [[64736, 64736], 'mapped', [1574, 1607]],
        [[64737, 64737], 'mapped', [1576, 1605]],
        [[64738, 64738], 'mapped', [1576, 1607]],
        [[64739, 64739], 'mapped', [1578, 1605]],
        [[64740, 64740], 'mapped', [1578, 1607]],
        [[64741, 64741], 'mapped', [1579, 1605]],
        [[64742, 64742], 'mapped', [1579, 1607]],
        [[64743, 64743], 'mapped', [1587, 1605]],
        [[64744, 64744], 'mapped', [1587, 1607]],
        [[64745, 64745], 'mapped', [1588, 1605]],
        [[64746, 64746], 'mapped', [1588, 1607]],
        [[64747, 64747], 'mapped', [1603, 1604]],
        [[64748, 64748], 'mapped', [1603, 1605]],
        [[64749, 64749], 'mapped', [1604, 1605]],
        [[64750, 64750], 'mapped', [1606, 1605]],
        [[64751, 64751], 'mapped', [1606, 1607]],
        [[64752, 64752], 'mapped', [1610, 1605]],
        [[64753, 64753], 'mapped', [1610, 1607]],
        [[64754, 64754], 'mapped', [1600, 1614, 1617]],
        [[64755, 64755], 'mapped', [1600, 1615, 1617]],
        [[64756, 64756], 'mapped', [1600, 1616, 1617]],
        [[64757, 64757], 'mapped', [1591, 1609]],
        [[64758, 64758], 'mapped', [1591, 1610]],
        [[64759, 64759], 'mapped', [1593, 1609]],
        [[64760, 64760], 'mapped', [1593, 1610]],
        [[64761, 64761], 'mapped', [1594, 1609]],
        [[64762, 64762], 'mapped', [1594, 1610]],
        [[64763, 64763], 'mapped', [1587, 1609]],
        [[64764, 64764], 'mapped', [1587, 1610]],
        [[64765, 64765], 'mapped', [1588, 1609]],
        [[64766, 64766], 'mapped', [1588, 1610]],
        [[64767, 64767], 'mapped', [1581, 1609]],
        [[64768, 64768], 'mapped', [1581, 1610]],
        [[64769, 64769], 'mapped', [1580, 1609]],
        [[64770, 64770], 'mapped', [1580, 1610]],
        [[64771, 64771], 'mapped', [1582, 1609]],
        [[64772, 64772], 'mapped', [1582, 1610]],
        [[64773, 64773], 'mapped', [1589, 1609]],
        [[64774, 64774], 'mapped', [1589, 1610]],
        [[64775, 64775], 'mapped', [1590, 1609]],
        [[64776, 64776], 'mapped', [1590, 1610]],
        [[64777, 64777], 'mapped', [1588, 1580]],
        [[64778, 64778], 'mapped', [1588, 1581]],
        [[64779, 64779], 'mapped', [1588, 1582]],
        [[64780, 64780], 'mapped', [1588, 1605]],
        [[64781, 64781], 'mapped', [1588, 1585]],
        [[64782, 64782], 'mapped', [1587, 1585]],
        [[64783, 64783], 'mapped', [1589, 1585]],
        [[64784, 64784], 'mapped', [1590, 1585]],
        [[64785, 64785], 'mapped', [1591, 1609]],
        [[64786, 64786], 'mapped', [1591, 1610]],
        [[64787, 64787], 'mapped', [1593, 1609]],
        [[64788, 64788], 'mapped', [1593, 1610]],
        [[64789, 64789], 'mapped', [1594, 1609]],
        [[64790, 64790], 'mapped', [1594, 1610]],
        [[64791, 64791], 'mapped', [1587, 1609]],
        [[64792, 64792], 'mapped', [1587, 1610]],
        [[64793, 64793], 'mapped', [1588, 1609]],
        [[64794, 64794], 'mapped', [1588, 1610]],
        [[64795, 64795], 'mapped', [1581, 1609]],
        [[64796, 64796], 'mapped', [1581, 1610]],
        [[64797, 64797], 'mapped', [1580, 1609]],
        [[64798, 64798], 'mapped', [1580, 1610]],
        [[64799, 64799], 'mapped', [1582, 1609]],
        [[64800, 64800], 'mapped', [1582, 1610]],
        [[64801, 64801], 'mapped', [1589, 1609]],
        [[64802, 64802], 'mapped', [1589, 1610]],
        [[64803, 64803], 'mapped', [1590, 1609]],
        [[64804, 64804], 'mapped', [1590, 1610]],
        [[64805, 64805], 'mapped', [1588, 1580]],
        [[64806, 64806], 'mapped', [1588, 1581]],
        [[64807, 64807], 'mapped', [1588, 1582]],
        [[64808, 64808], 'mapped', [1588, 1605]],
        [[64809, 64809], 'mapped', [1588, 1585]],
        [[64810, 64810], 'mapped', [1587, 1585]],
        [[64811, 64811], 'mapped', [1589, 1585]],
        [[64812, 64812], 'mapped', [1590, 1585]],
        [[64813, 64813], 'mapped', [1588, 1580]],
        [[64814, 64814], 'mapped', [1588, 1581]],
        [[64815, 64815], 'mapped', [1588, 1582]],
        [[64816, 64816], 'mapped', [1588, 1605]],
        [[64817, 64817], 'mapped', [1587, 1607]],
        [[64818, 64818], 'mapped', [1588, 1607]],
        [[64819, 64819], 'mapped', [1591, 1605]],
        [[64820, 64820], 'mapped', [1587, 1580]],
        [[64821, 64821], 'mapped', [1587, 1581]],
        [[64822, 64822], 'mapped', [1587, 1582]],
        [[64823, 64823], 'mapped', [1588, 1580]],
        [[64824, 64824], 'mapped', [1588, 1581]],
        [[64825, 64825], 'mapped', [1588, 1582]],
        [[64826, 64826], 'mapped', [1591, 1605]],
        [[64827, 64827], 'mapped', [1592, 1605]],
        [[64828, 64829], 'mapped', [1575, 1611]],
        [[64830, 64831], 'valid', [], 'NV8'],
        [[64832, 64847], 'disallowed'],
        [[64848, 64848], 'mapped', [1578, 1580, 1605]],
        [[64849, 64850], 'mapped', [1578, 1581, 1580]],
        [[64851, 64851], 'mapped', [1578, 1581, 1605]],
        [[64852, 64852], 'mapped', [1578, 1582, 1605]],
        [[64853, 64853], 'mapped', [1578, 1605, 1580]],
        [[64854, 64854], 'mapped', [1578, 1605, 1581]],
        [[64855, 64855], 'mapped', [1578, 1605, 1582]],
        [[64856, 64857], 'mapped', [1580, 1605, 1581]],
        [[64858, 64858], 'mapped', [1581, 1605, 1610]],
        [[64859, 64859], 'mapped', [1581, 1605, 1609]],
        [[64860, 64860], 'mapped', [1587, 1581, 1580]],
        [[64861, 64861], 'mapped', [1587, 1580, 1581]],
        [[64862, 64862], 'mapped', [1587, 1580, 1609]],
        [[64863, 64864], 'mapped', [1587, 1605, 1581]],
        [[64865, 64865], 'mapped', [1587, 1605, 1580]],
        [[64866, 64867], 'mapped', [1587, 1605, 1605]],
        [[64868, 64869], 'mapped', [1589, 1581, 1581]],
        [[64870, 64870], 'mapped', [1589, 1605, 1605]],
        [[64871, 64872], 'mapped', [1588, 1581, 1605]],
        [[64873, 64873], 'mapped', [1588, 1580, 1610]],
        [[64874, 64875], 'mapped', [1588, 1605, 1582]],
        [[64876, 64877], 'mapped', [1588, 1605, 1605]],
        [[64878, 64878], 'mapped', [1590, 1581, 1609]],
        [[64879, 64880], 'mapped', [1590, 1582, 1605]],
        [[64881, 64882], 'mapped', [1591, 1605, 1581]],
        [[64883, 64883], 'mapped', [1591, 1605, 1605]],
        [[64884, 64884], 'mapped', [1591, 1605, 1610]],
        [[64885, 64885], 'mapped', [1593, 1580, 1605]],
        [[64886, 64887], 'mapped', [1593, 1605, 1605]],
        [[64888, 64888], 'mapped', [1593, 1605, 1609]],
        [[64889, 64889], 'mapped', [1594, 1605, 1605]],
        [[64890, 64890], 'mapped', [1594, 1605, 1610]],
        [[64891, 64891], 'mapped', [1594, 1605, 1609]],
        [[64892, 64893], 'mapped', [1601, 1582, 1605]],
        [[64894, 64894], 'mapped', [1602, 1605, 1581]],
        [[64895, 64895], 'mapped', [1602, 1605, 1605]],
        [[64896, 64896], 'mapped', [1604, 1581, 1605]],
        [[64897, 64897], 'mapped', [1604, 1581, 1610]],
        [[64898, 64898], 'mapped', [1604, 1581, 1609]],
        [[64899, 64900], 'mapped', [1604, 1580, 1580]],
        [[64901, 64902], 'mapped', [1604, 1582, 1605]],
        [[64903, 64904], 'mapped', [1604, 1605, 1581]],
        [[64905, 64905], 'mapped', [1605, 1581, 1580]],
        [[64906, 64906], 'mapped', [1605, 1581, 1605]],
        [[64907, 64907], 'mapped', [1605, 1581, 1610]],
        [[64908, 64908], 'mapped', [1605, 1580, 1581]],
        [[64909, 64909], 'mapped', [1605, 1580, 1605]],
        [[64910, 64910], 'mapped', [1605, 1582, 1580]],
        [[64911, 64911], 'mapped', [1605, 1582, 1605]],
        [[64912, 64913], 'disallowed'],
        [[64914, 64914], 'mapped', [1605, 1580, 1582]],
        [[64915, 64915], 'mapped', [1607, 1605, 1580]],
        [[64916, 64916], 'mapped', [1607, 1605, 1605]],
        [[64917, 64917], 'mapped', [1606, 1581, 1605]],
        [[64918, 64918], 'mapped', [1606, 1581, 1609]],
        [[64919, 64920], 'mapped', [1606, 1580, 1605]],
        [[64921, 64921], 'mapped', [1606, 1580, 1609]],
        [[64922, 64922], 'mapped', [1606, 1605, 1610]],
        [[64923, 64923], 'mapped', [1606, 1605, 1609]],
        [[64924, 64925], 'mapped', [1610, 1605, 1605]],
        [[64926, 64926], 'mapped', [1576, 1582, 1610]],
        [[64927, 64927], 'mapped', [1578, 1580, 1610]],
        [[64928, 64928], 'mapped', [1578, 1580, 1609]],
        [[64929, 64929], 'mapped', [1578, 1582, 1610]],
        [[64930, 64930], 'mapped', [1578, 1582, 1609]],
        [[64931, 64931], 'mapped', [1578, 1605, 1610]],
        [[64932, 64932], 'mapped', [1578, 1605, 1609]],
        [[64933, 64933], 'mapped', [1580, 1605, 1610]],
        [[64934, 64934], 'mapped', [1580, 1581, 1609]],
        [[64935, 64935], 'mapped', [1580, 1605, 1609]],
        [[64936, 64936], 'mapped', [1587, 1582, 1609]],
        [[64937, 64937], 'mapped', [1589, 1581, 1610]],
        [[64938, 64938], 'mapped', [1588, 1581, 1610]],
        [[64939, 64939], 'mapped', [1590, 1581, 1610]],
        [[64940, 64940], 'mapped', [1604, 1580, 1610]],
        [[64941, 64941], 'mapped', [1604, 1605, 1610]],
        [[64942, 64942], 'mapped', [1610, 1581, 1610]],
        [[64943, 64943], 'mapped', [1610, 1580, 1610]],
        [[64944, 64944], 'mapped', [1610, 1605, 1610]],
        [[64945, 64945], 'mapped', [1605, 1605, 1610]],
        [[64946, 64946], 'mapped', [1602, 1605, 1610]],
        [[64947, 64947], 'mapped', [1606, 1581, 1610]],
        [[64948, 64948], 'mapped', [1602, 1605, 1581]],
        [[64949, 64949], 'mapped', [1604, 1581, 1605]],
        [[64950, 64950], 'mapped', [1593, 1605, 1610]],
        [[64951, 64951], 'mapped', [1603, 1605, 1610]],
        [[64952, 64952], 'mapped', [1606, 1580, 1581]],
        [[64953, 64953], 'mapped', [1605, 1582, 1610]],
        [[64954, 64954], 'mapped', [1604, 1580, 1605]],
        [[64955, 64955], 'mapped', [1603, 1605, 1605]],
        [[64956, 64956], 'mapped', [1604, 1580, 1605]],
        [[64957, 64957], 'mapped', [1606, 1580, 1581]],
        [[64958, 64958], 'mapped', [1580, 1581, 1610]],
        [[64959, 64959], 'mapped', [1581, 1580, 1610]],
        [[64960, 64960], 'mapped', [1605, 1580, 1610]],
        [[64961, 64961], 'mapped', [1601, 1605, 1610]],
        [[64962, 64962], 'mapped', [1576, 1581, 1610]],
        [[64963, 64963], 'mapped', [1603, 1605, 1605]],
        [[64964, 64964], 'mapped', [1593, 1580, 1605]],
        [[64965, 64965], 'mapped', [1589, 1605, 1605]],
        [[64966, 64966], 'mapped', [1587, 1582, 1610]],
        [[64967, 64967], 'mapped', [1606, 1580, 1610]],
        [[64968, 64975], 'disallowed'],
        [[64976, 65007], 'disallowed'],
        [[65008, 65008], 'mapped', [1589, 1604, 1746]],
        [[65009, 65009], 'mapped', [1602, 1604, 1746]],
        [[65010, 65010], 'mapped', [1575, 1604, 1604, 1607]],
        [[65011, 65011], 'mapped', [1575, 1603, 1576, 1585]],
        [[65012, 65012], 'mapped', [1605, 1581, 1605, 1583]],
        [[65013, 65013], 'mapped', [1589, 1604, 1593, 1605]],
        [[65014, 65014], 'mapped', [1585, 1587, 1608, 1604]],
        [[65015, 65015], 'mapped', [1593, 1604, 1610, 1607]],
        [[65016, 65016], 'mapped', [1608, 1587, 1604, 1605]],
        [[65017, 65017], 'mapped', [1589, 1604, 1609]],
        [
            [65018, 65018],
            'disallowed_STD3_mapped',
            [
                1589, 1604, 1609, 32, 1575, 1604, 1604, 1607, 32, 1593, 1604,
                1610, 1607, 32, 1608, 1587, 1604, 1605
            ]
        ],
        [
            [65019, 65019],
            'disallowed_STD3_mapped',
            [1580, 1604, 32, 1580, 1604, 1575, 1604, 1607]
        ],
        [[65020, 65020], 'mapped', [1585, 1740, 1575, 1604]],
        [[65021, 65021], 'valid', [], 'NV8'],
        [[65022, 65023], 'disallowed'],
        [[65024, 65039], 'ignored'],
        [[65040, 65040], 'disallowed_STD3_mapped', [44]],
        [[65041, 65041], 'mapped', [12289]],
        [[65042, 65042], 'disallowed'],
        [[65043, 65043], 'disallowed_STD3_mapped', [58]],
        [[65044, 65044], 'disallowed_STD3_mapped', [59]],
        [[65045, 65045], 'disallowed_STD3_mapped', [33]],
        [[65046, 65046], 'disallowed_STD3_mapped', [63]],
        [[65047, 65047], 'mapped', [12310]],
        [[65048, 65048], 'mapped', [12311]],
        [[65049, 65049], 'disallowed'],
        [[65050, 65055], 'disallowed'],
        [[65056, 65059], 'valid'],
        [[65060, 65062], 'valid'],
        [[65063, 65069], 'valid'],
        [[65070, 65071], 'valid'],
        [[65072, 65072], 'disallowed'],
        [[65073, 65073], 'mapped', [8212]],
        [[65074, 65074], 'mapped', [8211]],
        [[65075, 65076], 'disallowed_STD3_mapped', [95]],
        [[65077, 65077], 'disallowed_STD3_mapped', [40]],
        [[65078, 65078], 'disallowed_STD3_mapped', [41]],
        [[65079, 65079], 'disallowed_STD3_mapped', [123]],
        [[65080, 65080], 'disallowed_STD3_mapped', [125]],
        [[65081, 65081], 'mapped', [12308]],
        [[65082, 65082], 'mapped', [12309]],
        [[65083, 65083], 'mapped', [12304]],
        [[65084, 65084], 'mapped', [12305]],
        [[65085, 65085], 'mapped', [12298]],
        [[65086, 65086], 'mapped', [12299]],
        [[65087, 65087], 'mapped', [12296]],
        [[65088, 65088], 'mapped', [12297]],
        [[65089, 65089], 'mapped', [12300]],
        [[65090, 65090], 'mapped', [12301]],
        [[65091, 65091], 'mapped', [12302]],
        [[65092, 65092], 'mapped', [12303]],
        [[65093, 65094], 'valid', [], 'NV8'],
        [[65095, 65095], 'disallowed_STD3_mapped', [91]],
        [[65096, 65096], 'disallowed_STD3_mapped', [93]],
        [[65097, 65100], 'disallowed_STD3_mapped', [32, 773]],
        [[65101, 65103], 'disallowed_STD3_mapped', [95]],
        [[65104, 65104], 'disallowed_STD3_mapped', [44]],
        [[65105, 65105], 'mapped', [12289]],
        [[65106, 65106], 'disallowed'],
        [[65107, 65107], 'disallowed'],
        [[65108, 65108], 'disallowed_STD3_mapped', [59]],
        [[65109, 65109], 'disallowed_STD3_mapped', [58]],
        [[65110, 65110], 'disallowed_STD3_mapped', [63]],
        [[65111, 65111], 'disallowed_STD3_mapped', [33]],
        [[65112, 65112], 'mapped', [8212]],
        [[65113, 65113], 'disallowed_STD3_mapped', [40]],
        [[65114, 65114], 'disallowed_STD3_mapped', [41]],
        [[65115, 65115], 'disallowed_STD3_mapped', [123]],
        [[65116, 65116], 'disallowed_STD3_mapped', [125]],
        [[65117, 65117], 'mapped', [12308]],
        [[65118, 65118], 'mapped', [12309]],
        [[65119, 65119], 'disallowed_STD3_mapped', [35]],
        [[65120, 65120], 'disallowed_STD3_mapped', [38]],
        [[65121, 65121], 'disallowed_STD3_mapped', [42]],
        [[65122, 65122], 'disallowed_STD3_mapped', [43]],
        [[65123, 65123], 'mapped', [45]],
        [[65124, 65124], 'disallowed_STD3_mapped', [60]],
        [[65125, 65125], 'disallowed_STD3_mapped', [62]],
        [[65126, 65126], 'disallowed_STD3_mapped', [61]],
        [[65127, 65127], 'disallowed'],
        [[65128, 65128], 'disallowed_STD3_mapped', [92]],
        [[65129, 65129], 'disallowed_STD3_mapped', [36]],
        [[65130, 65130], 'disallowed_STD3_mapped', [37]],
        [[65131, 65131], 'disallowed_STD3_mapped', [64]],
        [[65132, 65135], 'disallowed'],
        [[65136, 65136], 'disallowed_STD3_mapped', [32, 1611]],
        [[65137, 65137], 'mapped', [1600, 1611]],
        [[65138, 65138], 'disallowed_STD3_mapped', [32, 1612]],
        [[65139, 65139], 'valid'],
        [[65140, 65140], 'disallowed_STD3_mapped', [32, 1613]],
        [[65141, 65141], 'disallowed'],
        [[65142, 65142], 'disallowed_STD3_mapped', [32, 1614]],
        [[65143, 65143], 'mapped', [1600, 1614]],
        [[65144, 65144], 'disallowed_STD3_mapped', [32, 1615]],
        [[65145, 65145], 'mapped', [1600, 1615]],
        [[65146, 65146], 'disallowed_STD3_mapped', [32, 1616]],
        [[65147, 65147], 'mapped', [1600, 1616]],
        [[65148, 65148], 'disallowed_STD3_mapped', [32, 1617]],
        [[65149, 65149], 'mapped', [1600, 1617]],
        [[65150, 65150], 'disallowed_STD3_mapped', [32, 1618]],
        [[65151, 65151], 'mapped', [1600, 1618]],
        [[65152, 65152], 'mapped', [1569]],
        [[65153, 65154], 'mapped', [1570]],
        [[65155, 65156], 'mapped', [1571]],
        [[65157, 65158], 'mapped', [1572]],
        [[65159, 65160], 'mapped', [1573]],
        [[65161, 65164], 'mapped', [1574]],
        [[65165, 65166], 'mapped', [1575]],
        [[65167, 65170], 'mapped', [1576]],
        [[65171, 65172], 'mapped', [1577]],
        [[65173, 65176], 'mapped', [1578]],
        [[65177, 65180], 'mapped', [1579]],
        [[65181, 65184], 'mapped', [1580]],
        [[65185, 65188], 'mapped', [1581]],
        [[65189, 65192], 'mapped', [1582]],
        [[65193, 65194], 'mapped', [1583]],
        [[65195, 65196], 'mapped', [1584]],
        [[65197, 65198], 'mapped', [1585]],
        [[65199, 65200], 'mapped', [1586]],
        [[65201, 65204], 'mapped', [1587]],
        [[65205, 65208], 'mapped', [1588]],
        [[65209, 65212], 'mapped', [1589]],
        [[65213, 65216], 'mapped', [1590]],
        [[65217, 65220], 'mapped', [1591]],
        [[65221, 65224], 'mapped', [1592]],
        [[65225, 65228], 'mapped', [1593]],
        [[65229, 65232], 'mapped', [1594]],
        [[65233, 65236], 'mapped', [1601]],
        [[65237, 65240], 'mapped', [1602]],
        [[65241, 65244], 'mapped', [1603]],
        [[65245, 65248], 'mapped', [1604]],
        [[65249, 65252], 'mapped', [1605]],
        [[65253, 65256], 'mapped', [1606]],
        [[65257, 65260], 'mapped', [1607]],
        [[65261, 65262], 'mapped', [1608]],
        [[65263, 65264], 'mapped', [1609]],
        [[65265, 65268], 'mapped', [1610]],
        [[65269, 65270], 'mapped', [1604, 1570]],
        [[65271, 65272], 'mapped', [1604, 1571]],
        [[65273, 65274], 'mapped', [1604, 1573]],
        [[65275, 65276], 'mapped', [1604, 1575]],
        [[65277, 65278], 'disallowed'],
        [[65279, 65279], 'ignored'],
        [[65280, 65280], 'disallowed'],
        [[65281, 65281], 'disallowed_STD3_mapped', [33]],
        [[65282, 65282], 'disallowed_STD3_mapped', [34]],
        [[65283, 65283], 'disallowed_STD3_mapped', [35]],
        [[65284, 65284], 'disallowed_STD3_mapped', [36]],
        [[65285, 65285], 'disallowed_STD3_mapped', [37]],
        [[65286, 65286], 'disallowed_STD3_mapped', [38]],
        [[65287, 65287], 'disallowed_STD3_mapped', [39]],
        [[65288, 65288], 'disallowed_STD3_mapped', [40]],
        [[65289, 65289], 'disallowed_STD3_mapped', [41]],
        [[65290, 65290], 'disallowed_STD3_mapped', [42]],
        [[65291, 65291], 'disallowed_STD3_mapped', [43]],
        [[65292, 65292], 'disallowed_STD3_mapped', [44]],
        [[65293, 65293], 'mapped', [45]],
        [[65294, 65294], 'mapped', [46]],
        [[65295, 65295], 'disallowed_STD3_mapped', [47]],
        [[65296, 65296], 'mapped', [48]],
        [[65297, 65297], 'mapped', [49]],
        [[65298, 65298], 'mapped', [50]],
        [[65299, 65299], 'mapped', [51]],
        [[65300, 65300], 'mapped', [52]],
        [[65301, 65301], 'mapped', [53]],
        [[65302, 65302], 'mapped', [54]],
        [[65303, 65303], 'mapped', [55]],
        [[65304, 65304], 'mapped', [56]],
        [[65305, 65305], 'mapped', [57]],
        [[65306, 65306], 'disallowed_STD3_mapped', [58]],
        [[65307, 65307], 'disallowed_STD3_mapped', [59]],
        [[65308, 65308], 'disallowed_STD3_mapped', [60]],
        [[65309, 65309], 'disallowed_STD3_mapped', [61]],
        [[65310, 65310], 'disallowed_STD3_mapped', [62]],
        [[65311, 65311], 'disallowed_STD3_mapped', [63]],
        [[65312, 65312], 'disallowed_STD3_mapped', [64]],
        [[65313, 65313], 'mapped', [97]],
        [[65314, 65314], 'mapped', [98]],
        [[65315, 65315], 'mapped', [99]],
        [[65316, 65316], 'mapped', [100]],
        [[65317, 65317], 'mapped', [101]],
        [[65318, 65318], 'mapped', [102]],
        [[65319, 65319], 'mapped', [103]],
        [[65320, 65320], 'mapped', [104]],
        [[65321, 65321], 'mapped', [105]],
        [[65322, 65322], 'mapped', [106]],
        [[65323, 65323], 'mapped', [107]],
        [[65324, 65324], 'mapped', [108]],
        [[65325, 65325], 'mapped', [109]],
        [[65326, 65326], 'mapped', [110]],
        [[65327, 65327], 'mapped', [111]],
        [[65328, 65328], 'mapped', [112]],
        [[65329, 65329], 'mapped', [113]],
        [[65330, 65330], 'mapped', [114]],
        [[65331, 65331], 'mapped', [115]],
        [[65332, 65332], 'mapped', [116]],
        [[65333, 65333], 'mapped', [117]],
        [[65334, 65334], 'mapped', [118]],
        [[65335, 65335], 'mapped', [119]],
        [[65336, 65336], 'mapped', [120]],
        [[65337, 65337], 'mapped', [121]],
        [[65338, 65338], 'mapped', [122]],
        [[65339, 65339], 'disallowed_STD3_mapped', [91]],
        [[65340, 65340], 'disallowed_STD3_mapped', [92]],
        [[65341, 65341], 'disallowed_STD3_mapped', [93]],
        [[65342, 65342], 'disallowed_STD3_mapped', [94]],
        [[65343, 65343], 'disallowed_STD3_mapped', [95]],
        [[65344, 65344], 'disallowed_STD3_mapped', [96]],
        [[65345, 65345], 'mapped', [97]],
        [[65346, 65346], 'mapped', [98]],
        [[65347, 65347], 'mapped', [99]],
        [[65348, 65348], 'mapped', [100]],
        [[65349, 65349], 'mapped', [101]],
        [[65350, 65350], 'mapped', [102]],
        [[65351, 65351], 'mapped', [103]],
        [[65352, 65352], 'mapped', [104]],
        [[65353, 65353], 'mapped', [105]],
        [[65354, 65354], 'mapped', [106]],
        [[65355, 65355], 'mapped', [107]],
        [[65356, 65356], 'mapped', [108]],
        [[65357, 65357], 'mapped', [109]],
        [[65358, 65358], 'mapped', [110]],
        [[65359, 65359], 'mapped', [111]],
        [[65360, 65360], 'mapped', [112]],
        [[65361, 65361], 'mapped', [113]],
        [[65362, 65362], 'mapped', [114]],
        [[65363, 65363], 'mapped', [115]],
        [[65364, 65364], 'mapped', [116]],
        [[65365, 65365], 'mapped', [117]],
        [[65366, 65366], 'mapped', [118]],
        [[65367, 65367], 'mapped', [119]],
        [[65368, 65368], 'mapped', [120]],
        [[65369, 65369], 'mapped', [121]],
        [[65370, 65370], 'mapped', [122]],
        [[65371, 65371], 'disallowed_STD3_mapped', [123]],
        [[65372, 65372], 'disallowed_STD3_mapped', [124]],
        [[65373, 65373], 'disallowed_STD3_mapped', [125]],
        [[65374, 65374], 'disallowed_STD3_mapped', [126]],
        [[65375, 65375], 'mapped', [10629]],
        [[65376, 65376], 'mapped', [10630]],
        [[65377, 65377], 'mapped', [46]],
        [[65378, 65378], 'mapped', [12300]],
        [[65379, 65379], 'mapped', [12301]],
        [[65380, 65380], 'mapped', [12289]],
        [[65381, 65381], 'mapped', [12539]],
        [[65382, 65382], 'mapped', [12530]],
        [[65383, 65383], 'mapped', [12449]],
        [[65384, 65384], 'mapped', [12451]],
        [[65385, 65385], 'mapped', [12453]],
        [[65386, 65386], 'mapped', [12455]],
        [[65387, 65387], 'mapped', [12457]],
        [[65388, 65388], 'mapped', [12515]],
        [[65389, 65389], 'mapped', [12517]],
        [[65390, 65390], 'mapped', [12519]],
        [[65391, 65391], 'mapped', [12483]],
        [[65392, 65392], 'mapped', [12540]],
        [[65393, 65393], 'mapped', [12450]],
        [[65394, 65394], 'mapped', [12452]],
        [[65395, 65395], 'mapped', [12454]],
        [[65396, 65396], 'mapped', [12456]],
        [[65397, 65397], 'mapped', [12458]],
        [[65398, 65398], 'mapped', [12459]],
        [[65399, 65399], 'mapped', [12461]],
        [[65400, 65400], 'mapped', [12463]],
        [[65401, 65401], 'mapped', [12465]],
        [[65402, 65402], 'mapped', [12467]],
        [[65403, 65403], 'mapped', [12469]],
        [[65404, 65404], 'mapped', [12471]],
        [[65405, 65405], 'mapped', [12473]],
        [[65406, 65406], 'mapped', [12475]],
        [[65407, 65407], 'mapped', [12477]],
        [[65408, 65408], 'mapped', [12479]],
        [[65409, 65409], 'mapped', [12481]],
        [[65410, 65410], 'mapped', [12484]],
        [[65411, 65411], 'mapped', [12486]],
        [[65412, 65412], 'mapped', [12488]],
        [[65413, 65413], 'mapped', [12490]],
        [[65414, 65414], 'mapped', [12491]],
        [[65415, 65415], 'mapped', [12492]],
        [[65416, 65416], 'mapped', [12493]],
        [[65417, 65417], 'mapped', [12494]],
        [[65418, 65418], 'mapped', [12495]],
        [[65419, 65419], 'mapped', [12498]],
        [[65420, 65420], 'mapped', [12501]],
        [[65421, 65421], 'mapped', [12504]],
        [[65422, 65422], 'mapped', [12507]],
        [[65423, 65423], 'mapped', [12510]],
        [[65424, 65424], 'mapped', [12511]],
        [[65425, 65425], 'mapped', [12512]],
        [[65426, 65426], 'mapped', [12513]],
        [[65427, 65427], 'mapped', [12514]],
        [[65428, 65428], 'mapped', [12516]],
        [[65429, 65429], 'mapped', [12518]],
        [[65430, 65430], 'mapped', [12520]],
        [[65431, 65431], 'mapped', [12521]],
        [[65432, 65432], 'mapped', [12522]],
        [[65433, 65433], 'mapped', [12523]],
        [[65434, 65434], 'mapped', [12524]],
        [[65435, 65435], 'mapped', [12525]],
        [[65436, 65436], 'mapped', [12527]],
        [[65437, 65437], 'mapped', [12531]],
        [[65438, 65438], 'mapped', [12441]],
        [[65439, 65439], 'mapped', [12442]],
        [[65440, 65440], 'disallowed'],
        [[65441, 65441], 'mapped', [4352]],
        [[65442, 65442], 'mapped', [4353]],
        [[65443, 65443], 'mapped', [4522]],
        [[65444, 65444], 'mapped', [4354]],
        [[65445, 65445], 'mapped', [4524]],
        [[65446, 65446], 'mapped', [4525]],
        [[65447, 65447], 'mapped', [4355]],
        [[65448, 65448], 'mapped', [4356]],
        [[65449, 65449], 'mapped', [4357]],
        [[65450, 65450], 'mapped', [4528]],
        [[65451, 65451], 'mapped', [4529]],
        [[65452, 65452], 'mapped', [4530]],
        [[65453, 65453], 'mapped', [4531]],
        [[65454, 65454], 'mapped', [4532]],
        [[65455, 65455], 'mapped', [4533]],
        [[65456, 65456], 'mapped', [4378]],
        [[65457, 65457], 'mapped', [4358]],
        [[65458, 65458], 'mapped', [4359]],
        [[65459, 65459], 'mapped', [4360]],
        [[65460, 65460], 'mapped', [4385]],
        [[65461, 65461], 'mapped', [4361]],
        [[65462, 65462], 'mapped', [4362]],
        [[65463, 65463], 'mapped', [4363]],
        [[65464, 65464], 'mapped', [4364]],
        [[65465, 65465], 'mapped', [4365]],
        [[65466, 65466], 'mapped', [4366]],
        [[65467, 65467], 'mapped', [4367]],
        [[65468, 65468], 'mapped', [4368]],
        [[65469, 65469], 'mapped', [4369]],
        [[65470, 65470], 'mapped', [4370]],
        [[65471, 65473], 'disallowed'],
        [[65474, 65474], 'mapped', [4449]],
        [[65475, 65475], 'mapped', [4450]],
        [[65476, 65476], 'mapped', [4451]],
        [[65477, 65477], 'mapped', [4452]],
        [[65478, 65478], 'mapped', [4453]],
        [[65479, 65479], 'mapped', [4454]],
        [[65480, 65481], 'disallowed'],
        [[65482, 65482], 'mapped', [4455]],
        [[65483, 65483], 'mapped', [4456]],
        [[65484, 65484], 'mapped', [4457]],
        [[65485, 65485], 'mapped', [4458]],
        [[65486, 65486], 'mapped', [4459]],
        [[65487, 65487], 'mapped', [4460]],
        [[65488, 65489], 'disallowed'],
        [[65490, 65490], 'mapped', [4461]],
        [[65491, 65491], 'mapped', [4462]],
        [[65492, 65492], 'mapped', [4463]],
        [[65493, 65493], 'mapped', [4464]],
        [[65494, 65494], 'mapped', [4465]],
        [[65495, 65495], 'mapped', [4466]],
        [[65496, 65497], 'disallowed'],
        [[65498, 65498], 'mapped', [4467]],
        [[65499, 65499], 'mapped', [4468]],
        [[65500, 65500], 'mapped', [4469]],
        [[65501, 65503], 'disallowed'],
        [[65504, 65504], 'mapped', [162]],
        [[65505, 65505], 'mapped', [163]],
        [[65506, 65506], 'mapped', [172]],
        [[65507, 65507], 'disallowed_STD3_mapped', [32, 772]],
        [[65508, 65508], 'mapped', [166]],
        [[65509, 65509], 'mapped', [165]],
        [[65510, 65510], 'mapped', [8361]],
        [[65511, 65511], 'disallowed'],
        [[65512, 65512], 'mapped', [9474]],
        [[65513, 65513], 'mapped', [8592]],
        [[65514, 65514], 'mapped', [8593]],
        [[65515, 65515], 'mapped', [8594]],
        [[65516, 65516], 'mapped', [8595]],
        [[65517, 65517], 'mapped', [9632]],
        [[65518, 65518], 'mapped', [9675]],
        [[65519, 65528], 'disallowed'],
        [[65529, 65531], 'disallowed'],
        [[65532, 65532], 'disallowed'],
        [[65533, 65533], 'disallowed'],
        [[65534, 65535], 'disallowed'],
        [[65536, 65547], 'valid'],
        [[65548, 65548], 'disallowed'],
        [[65549, 65574], 'valid'],
        [[65575, 65575], 'disallowed'],
        [[65576, 65594], 'valid'],
        [[65595, 65595], 'disallowed'],
        [[65596, 65597], 'valid'],
        [[65598, 65598], 'disallowed'],
        [[65599, 65613], 'valid'],
        [[65614, 65615], 'disallowed'],
        [[65616, 65629], 'valid'],
        [[65630, 65663], 'disallowed'],
        [[65664, 65786], 'valid'],
        [[65787, 65791], 'disallowed'],
        [[65792, 65794], 'valid', [], 'NV8'],
        [[65795, 65798], 'disallowed'],
        [[65799, 65843], 'valid', [], 'NV8'],
        [[65844, 65846], 'disallowed'],
        [[65847, 65855], 'valid', [], 'NV8'],
        [[65856, 65930], 'valid', [], 'NV8'],
        [[65931, 65932], 'valid', [], 'NV8'],
        [[65933, 65935], 'disallowed'],
        [[65936, 65947], 'valid', [], 'NV8'],
        [[65948, 65951], 'disallowed'],
        [[65952, 65952], 'valid', [], 'NV8'],
        [[65953, 65999], 'disallowed'],
        [[66e3, 66044], 'valid', [], 'NV8'],
        [[66045, 66045], 'valid'],
        [[66046, 66175], 'disallowed'],
        [[66176, 66204], 'valid'],
        [[66205, 66207], 'disallowed'],
        [[66208, 66256], 'valid'],
        [[66257, 66271], 'disallowed'],
        [[66272, 66272], 'valid'],
        [[66273, 66299], 'valid', [], 'NV8'],
        [[66300, 66303], 'disallowed'],
        [[66304, 66334], 'valid'],
        [[66335, 66335], 'valid'],
        [[66336, 66339], 'valid', [], 'NV8'],
        [[66340, 66351], 'disallowed'],
        [[66352, 66368], 'valid'],
        [[66369, 66369], 'valid', [], 'NV8'],
        [[66370, 66377], 'valid'],
        [[66378, 66378], 'valid', [], 'NV8'],
        [[66379, 66383], 'disallowed'],
        [[66384, 66426], 'valid'],
        [[66427, 66431], 'disallowed'],
        [[66432, 66461], 'valid'],
        [[66462, 66462], 'disallowed'],
        [[66463, 66463], 'valid', [], 'NV8'],
        [[66464, 66499], 'valid'],
        [[66500, 66503], 'disallowed'],
        [[66504, 66511], 'valid'],
        [[66512, 66517], 'valid', [], 'NV8'],
        [[66518, 66559], 'disallowed'],
        [[66560, 66560], 'mapped', [66600]],
        [[66561, 66561], 'mapped', [66601]],
        [[66562, 66562], 'mapped', [66602]],
        [[66563, 66563], 'mapped', [66603]],
        [[66564, 66564], 'mapped', [66604]],
        [[66565, 66565], 'mapped', [66605]],
        [[66566, 66566], 'mapped', [66606]],
        [[66567, 66567], 'mapped', [66607]],
        [[66568, 66568], 'mapped', [66608]],
        [[66569, 66569], 'mapped', [66609]],
        [[66570, 66570], 'mapped', [66610]],
        [[66571, 66571], 'mapped', [66611]],
        [[66572, 66572], 'mapped', [66612]],
        [[66573, 66573], 'mapped', [66613]],
        [[66574, 66574], 'mapped', [66614]],
        [[66575, 66575], 'mapped', [66615]],
        [[66576, 66576], 'mapped', [66616]],
        [[66577, 66577], 'mapped', [66617]],
        [[66578, 66578], 'mapped', [66618]],
        [[66579, 66579], 'mapped', [66619]],
        [[66580, 66580], 'mapped', [66620]],
        [[66581, 66581], 'mapped', [66621]],
        [[66582, 66582], 'mapped', [66622]],
        [[66583, 66583], 'mapped', [66623]],
        [[66584, 66584], 'mapped', [66624]],
        [[66585, 66585], 'mapped', [66625]],
        [[66586, 66586], 'mapped', [66626]],
        [[66587, 66587], 'mapped', [66627]],
        [[66588, 66588], 'mapped', [66628]],
        [[66589, 66589], 'mapped', [66629]],
        [[66590, 66590], 'mapped', [66630]],
        [[66591, 66591], 'mapped', [66631]],
        [[66592, 66592], 'mapped', [66632]],
        [[66593, 66593], 'mapped', [66633]],
        [[66594, 66594], 'mapped', [66634]],
        [[66595, 66595], 'mapped', [66635]],
        [[66596, 66596], 'mapped', [66636]],
        [[66597, 66597], 'mapped', [66637]],
        [[66598, 66598], 'mapped', [66638]],
        [[66599, 66599], 'mapped', [66639]],
        [[66600, 66637], 'valid'],
        [[66638, 66717], 'valid'],
        [[66718, 66719], 'disallowed'],
        [[66720, 66729], 'valid'],
        [[66730, 66815], 'disallowed'],
        [[66816, 66855], 'valid'],
        [[66856, 66863], 'disallowed'],
        [[66864, 66915], 'valid'],
        [[66916, 66926], 'disallowed'],
        [[66927, 66927], 'valid', [], 'NV8'],
        [[66928, 67071], 'disallowed'],
        [[67072, 67382], 'valid'],
        [[67383, 67391], 'disallowed'],
        [[67392, 67413], 'valid'],
        [[67414, 67423], 'disallowed'],
        [[67424, 67431], 'valid'],
        [[67432, 67583], 'disallowed'],
        [[67584, 67589], 'valid'],
        [[67590, 67591], 'disallowed'],
        [[67592, 67592], 'valid'],
        [[67593, 67593], 'disallowed'],
        [[67594, 67637], 'valid'],
        [[67638, 67638], 'disallowed'],
        [[67639, 67640], 'valid'],
        [[67641, 67643], 'disallowed'],
        [[67644, 67644], 'valid'],
        [[67645, 67646], 'disallowed'],
        [[67647, 67647], 'valid'],
        [[67648, 67669], 'valid'],
        [[67670, 67670], 'disallowed'],
        [[67671, 67679], 'valid', [], 'NV8'],
        [[67680, 67702], 'valid'],
        [[67703, 67711], 'valid', [], 'NV8'],
        [[67712, 67742], 'valid'],
        [[67743, 67750], 'disallowed'],
        [[67751, 67759], 'valid', [], 'NV8'],
        [[67760, 67807], 'disallowed'],
        [[67808, 67826], 'valid'],
        [[67827, 67827], 'disallowed'],
        [[67828, 67829], 'valid'],
        [[67830, 67834], 'disallowed'],
        [[67835, 67839], 'valid', [], 'NV8'],
        [[67840, 67861], 'valid'],
        [[67862, 67865], 'valid', [], 'NV8'],
        [[67866, 67867], 'valid', [], 'NV8'],
        [[67868, 67870], 'disallowed'],
        [[67871, 67871], 'valid', [], 'NV8'],
        [[67872, 67897], 'valid'],
        [[67898, 67902], 'disallowed'],
        [[67903, 67903], 'valid', [], 'NV8'],
        [[67904, 67967], 'disallowed'],
        [[67968, 68023], 'valid'],
        [[68024, 68027], 'disallowed'],
        [[68028, 68029], 'valid', [], 'NV8'],
        [[68030, 68031], 'valid'],
        [[68032, 68047], 'valid', [], 'NV8'],
        [[68048, 68049], 'disallowed'],
        [[68050, 68095], 'valid', [], 'NV8'],
        [[68096, 68099], 'valid'],
        [[68100, 68100], 'disallowed'],
        [[68101, 68102], 'valid'],
        [[68103, 68107], 'disallowed'],
        [[68108, 68115], 'valid'],
        [[68116, 68116], 'disallowed'],
        [[68117, 68119], 'valid'],
        [[68120, 68120], 'disallowed'],
        [[68121, 68147], 'valid'],
        [[68148, 68151], 'disallowed'],
        [[68152, 68154], 'valid'],
        [[68155, 68158], 'disallowed'],
        [[68159, 68159], 'valid'],
        [[68160, 68167], 'valid', [], 'NV8'],
        [[68168, 68175], 'disallowed'],
        [[68176, 68184], 'valid', [], 'NV8'],
        [[68185, 68191], 'disallowed'],
        [[68192, 68220], 'valid'],
        [[68221, 68223], 'valid', [], 'NV8'],
        [[68224, 68252], 'valid'],
        [[68253, 68255], 'valid', [], 'NV8'],
        [[68256, 68287], 'disallowed'],
        [[68288, 68295], 'valid'],
        [[68296, 68296], 'valid', [], 'NV8'],
        [[68297, 68326], 'valid'],
        [[68327, 68330], 'disallowed'],
        [[68331, 68342], 'valid', [], 'NV8'],
        [[68343, 68351], 'disallowed'],
        [[68352, 68405], 'valid'],
        [[68406, 68408], 'disallowed'],
        [[68409, 68415], 'valid', [], 'NV8'],
        [[68416, 68437], 'valid'],
        [[68438, 68439], 'disallowed'],
        [[68440, 68447], 'valid', [], 'NV8'],
        [[68448, 68466], 'valid'],
        [[68467, 68471], 'disallowed'],
        [[68472, 68479], 'valid', [], 'NV8'],
        [[68480, 68497], 'valid'],
        [[68498, 68504], 'disallowed'],
        [[68505, 68508], 'valid', [], 'NV8'],
        [[68509, 68520], 'disallowed'],
        [[68521, 68527], 'valid', [], 'NV8'],
        [[68528, 68607], 'disallowed'],
        [[68608, 68680], 'valid'],
        [[68681, 68735], 'disallowed'],
        [[68736, 68736], 'mapped', [68800]],
        [[68737, 68737], 'mapped', [68801]],
        [[68738, 68738], 'mapped', [68802]],
        [[68739, 68739], 'mapped', [68803]],
        [[68740, 68740], 'mapped', [68804]],
        [[68741, 68741], 'mapped', [68805]],
        [[68742, 68742], 'mapped', [68806]],
        [[68743, 68743], 'mapped', [68807]],
        [[68744, 68744], 'mapped', [68808]],
        [[68745, 68745], 'mapped', [68809]],
        [[68746, 68746], 'mapped', [68810]],
        [[68747, 68747], 'mapped', [68811]],
        [[68748, 68748], 'mapped', [68812]],
        [[68749, 68749], 'mapped', [68813]],
        [[68750, 68750], 'mapped', [68814]],
        [[68751, 68751], 'mapped', [68815]],
        [[68752, 68752], 'mapped', [68816]],
        [[68753, 68753], 'mapped', [68817]],
        [[68754, 68754], 'mapped', [68818]],
        [[68755, 68755], 'mapped', [68819]],
        [[68756, 68756], 'mapped', [68820]],
        [[68757, 68757], 'mapped', [68821]],
        [[68758, 68758], 'mapped', [68822]],
        [[68759, 68759], 'mapped', [68823]],
        [[68760, 68760], 'mapped', [68824]],
        [[68761, 68761], 'mapped', [68825]],
        [[68762, 68762], 'mapped', [68826]],
        [[68763, 68763], 'mapped', [68827]],
        [[68764, 68764], 'mapped', [68828]],
        [[68765, 68765], 'mapped', [68829]],
        [[68766, 68766], 'mapped', [68830]],
        [[68767, 68767], 'mapped', [68831]],
        [[68768, 68768], 'mapped', [68832]],
        [[68769, 68769], 'mapped', [68833]],
        [[68770, 68770], 'mapped', [68834]],
        [[68771, 68771], 'mapped', [68835]],
        [[68772, 68772], 'mapped', [68836]],
        [[68773, 68773], 'mapped', [68837]],
        [[68774, 68774], 'mapped', [68838]],
        [[68775, 68775], 'mapped', [68839]],
        [[68776, 68776], 'mapped', [68840]],
        [[68777, 68777], 'mapped', [68841]],
        [[68778, 68778], 'mapped', [68842]],
        [[68779, 68779], 'mapped', [68843]],
        [[68780, 68780], 'mapped', [68844]],
        [[68781, 68781], 'mapped', [68845]],
        [[68782, 68782], 'mapped', [68846]],
        [[68783, 68783], 'mapped', [68847]],
        [[68784, 68784], 'mapped', [68848]],
        [[68785, 68785], 'mapped', [68849]],
        [[68786, 68786], 'mapped', [68850]],
        [[68787, 68799], 'disallowed'],
        [[68800, 68850], 'valid'],
        [[68851, 68857], 'disallowed'],
        [[68858, 68863], 'valid', [], 'NV8'],
        [[68864, 69215], 'disallowed'],
        [[69216, 69246], 'valid', [], 'NV8'],
        [[69247, 69631], 'disallowed'],
        [[69632, 69702], 'valid'],
        [[69703, 69709], 'valid', [], 'NV8'],
        [[69710, 69713], 'disallowed'],
        [[69714, 69733], 'valid', [], 'NV8'],
        [[69734, 69743], 'valid'],
        [[69744, 69758], 'disallowed'],
        [[69759, 69759], 'valid'],
        [[69760, 69818], 'valid'],
        [[69819, 69820], 'valid', [], 'NV8'],
        [[69821, 69821], 'disallowed'],
        [[69822, 69825], 'valid', [], 'NV8'],
        [[69826, 69839], 'disallowed'],
        [[69840, 69864], 'valid'],
        [[69865, 69871], 'disallowed'],
        [[69872, 69881], 'valid'],
        [[69882, 69887], 'disallowed'],
        [[69888, 69940], 'valid'],
        [[69941, 69941], 'disallowed'],
        [[69942, 69951], 'valid'],
        [[69952, 69955], 'valid', [], 'NV8'],
        [[69956, 69967], 'disallowed'],
        [[69968, 70003], 'valid'],
        [[70004, 70005], 'valid', [], 'NV8'],
        [[70006, 70006], 'valid'],
        [[70007, 70015], 'disallowed'],
        [[70016, 70084], 'valid'],
        [[70085, 70088], 'valid', [], 'NV8'],
        [[70089, 70089], 'valid', [], 'NV8'],
        [[70090, 70092], 'valid'],
        [[70093, 70093], 'valid', [], 'NV8'],
        [[70094, 70095], 'disallowed'],
        [[70096, 70105], 'valid'],
        [[70106, 70106], 'valid'],
        [[70107, 70107], 'valid', [], 'NV8'],
        [[70108, 70108], 'valid'],
        [[70109, 70111], 'valid', [], 'NV8'],
        [[70112, 70112], 'disallowed'],
        [[70113, 70132], 'valid', [], 'NV8'],
        [[70133, 70143], 'disallowed'],
        [[70144, 70161], 'valid'],
        [[70162, 70162], 'disallowed'],
        [[70163, 70199], 'valid'],
        [[70200, 70205], 'valid', [], 'NV8'],
        [[70206, 70271], 'disallowed'],
        [[70272, 70278], 'valid'],
        [[70279, 70279], 'disallowed'],
        [[70280, 70280], 'valid'],
        [[70281, 70281], 'disallowed'],
        [[70282, 70285], 'valid'],
        [[70286, 70286], 'disallowed'],
        [[70287, 70301], 'valid'],
        [[70302, 70302], 'disallowed'],
        [[70303, 70312], 'valid'],
        [[70313, 70313], 'valid', [], 'NV8'],
        [[70314, 70319], 'disallowed'],
        [[70320, 70378], 'valid'],
        [[70379, 70383], 'disallowed'],
        [[70384, 70393], 'valid'],
        [[70394, 70399], 'disallowed'],
        [[70400, 70400], 'valid'],
        [[70401, 70403], 'valid'],
        [[70404, 70404], 'disallowed'],
        [[70405, 70412], 'valid'],
        [[70413, 70414], 'disallowed'],
        [[70415, 70416], 'valid'],
        [[70417, 70418], 'disallowed'],
        [[70419, 70440], 'valid'],
        [[70441, 70441], 'disallowed'],
        [[70442, 70448], 'valid'],
        [[70449, 70449], 'disallowed'],
        [[70450, 70451], 'valid'],
        [[70452, 70452], 'disallowed'],
        [[70453, 70457], 'valid'],
        [[70458, 70459], 'disallowed'],
        [[70460, 70468], 'valid'],
        [[70469, 70470], 'disallowed'],
        [[70471, 70472], 'valid'],
        [[70473, 70474], 'disallowed'],
        [[70475, 70477], 'valid'],
        [[70478, 70479], 'disallowed'],
        [[70480, 70480], 'valid'],
        [[70481, 70486], 'disallowed'],
        [[70487, 70487], 'valid'],
        [[70488, 70492], 'disallowed'],
        [[70493, 70499], 'valid'],
        [[70500, 70501], 'disallowed'],
        [[70502, 70508], 'valid'],
        [[70509, 70511], 'disallowed'],
        [[70512, 70516], 'valid'],
        [[70517, 70783], 'disallowed'],
        [[70784, 70853], 'valid'],
        [[70854, 70854], 'valid', [], 'NV8'],
        [[70855, 70855], 'valid'],
        [[70856, 70863], 'disallowed'],
        [[70864, 70873], 'valid'],
        [[70874, 71039], 'disallowed'],
        [[71040, 71093], 'valid'],
        [[71094, 71095], 'disallowed'],
        [[71096, 71104], 'valid'],
        [[71105, 71113], 'valid', [], 'NV8'],
        [[71114, 71127], 'valid', [], 'NV8'],
        [[71128, 71133], 'valid'],
        [[71134, 71167], 'disallowed'],
        [[71168, 71232], 'valid'],
        [[71233, 71235], 'valid', [], 'NV8'],
        [[71236, 71236], 'valid'],
        [[71237, 71247], 'disallowed'],
        [[71248, 71257], 'valid'],
        [[71258, 71295], 'disallowed'],
        [[71296, 71351], 'valid'],
        [[71352, 71359], 'disallowed'],
        [[71360, 71369], 'valid'],
        [[71370, 71423], 'disallowed'],
        [[71424, 71449], 'valid'],
        [[71450, 71452], 'disallowed'],
        [[71453, 71467], 'valid'],
        [[71468, 71471], 'disallowed'],
        [[71472, 71481], 'valid'],
        [[71482, 71487], 'valid', [], 'NV8'],
        [[71488, 71839], 'disallowed'],
        [[71840, 71840], 'mapped', [71872]],
        [[71841, 71841], 'mapped', [71873]],
        [[71842, 71842], 'mapped', [71874]],
        [[71843, 71843], 'mapped', [71875]],
        [[71844, 71844], 'mapped', [71876]],
        [[71845, 71845], 'mapped', [71877]],
        [[71846, 71846], 'mapped', [71878]],
        [[71847, 71847], 'mapped', [71879]],
        [[71848, 71848], 'mapped', [71880]],
        [[71849, 71849], 'mapped', [71881]],
        [[71850, 71850], 'mapped', [71882]],
        [[71851, 71851], 'mapped', [71883]],
        [[71852, 71852], 'mapped', [71884]],
        [[71853, 71853], 'mapped', [71885]],
        [[71854, 71854], 'mapped', [71886]],
        [[71855, 71855], 'mapped', [71887]],
        [[71856, 71856], 'mapped', [71888]],
        [[71857, 71857], 'mapped', [71889]],
        [[71858, 71858], 'mapped', [71890]],
        [[71859, 71859], 'mapped', [71891]],
        [[71860, 71860], 'mapped', [71892]],
        [[71861, 71861], 'mapped', [71893]],
        [[71862, 71862], 'mapped', [71894]],
        [[71863, 71863], 'mapped', [71895]],
        [[71864, 71864], 'mapped', [71896]],
        [[71865, 71865], 'mapped', [71897]],
        [[71866, 71866], 'mapped', [71898]],
        [[71867, 71867], 'mapped', [71899]],
        [[71868, 71868], 'mapped', [71900]],
        [[71869, 71869], 'mapped', [71901]],
        [[71870, 71870], 'mapped', [71902]],
        [[71871, 71871], 'mapped', [71903]],
        [[71872, 71913], 'valid'],
        [[71914, 71922], 'valid', [], 'NV8'],
        [[71923, 71934], 'disallowed'],
        [[71935, 71935], 'valid'],
        [[71936, 72383], 'disallowed'],
        [[72384, 72440], 'valid'],
        [[72441, 73727], 'disallowed'],
        [[73728, 74606], 'valid'],
        [[74607, 74648], 'valid'],
        [[74649, 74649], 'valid'],
        [[74650, 74751], 'disallowed'],
        [[74752, 74850], 'valid', [], 'NV8'],
        [[74851, 74862], 'valid', [], 'NV8'],
        [[74863, 74863], 'disallowed'],
        [[74864, 74867], 'valid', [], 'NV8'],
        [[74868, 74868], 'valid', [], 'NV8'],
        [[74869, 74879], 'disallowed'],
        [[74880, 75075], 'valid'],
        [[75076, 77823], 'disallowed'],
        [[77824, 78894], 'valid'],
        [[78895, 82943], 'disallowed'],
        [[82944, 83526], 'valid'],
        [[83527, 92159], 'disallowed'],
        [[92160, 92728], 'valid'],
        [[92729, 92735], 'disallowed'],
        [[92736, 92766], 'valid'],
        [[92767, 92767], 'disallowed'],
        [[92768, 92777], 'valid'],
        [[92778, 92781], 'disallowed'],
        [[92782, 92783], 'valid', [], 'NV8'],
        [[92784, 92879], 'disallowed'],
        [[92880, 92909], 'valid'],
        [[92910, 92911], 'disallowed'],
        [[92912, 92916], 'valid'],
        [[92917, 92917], 'valid', [], 'NV8'],
        [[92918, 92927], 'disallowed'],
        [[92928, 92982], 'valid'],
        [[92983, 92991], 'valid', [], 'NV8'],
        [[92992, 92995], 'valid'],
        [[92996, 92997], 'valid', [], 'NV8'],
        [[92998, 93007], 'disallowed'],
        [[93008, 93017], 'valid'],
        [[93018, 93018], 'disallowed'],
        [[93019, 93025], 'valid', [], 'NV8'],
        [[93026, 93026], 'disallowed'],
        [[93027, 93047], 'valid'],
        [[93048, 93052], 'disallowed'],
        [[93053, 93071], 'valid'],
        [[93072, 93951], 'disallowed'],
        [[93952, 94020], 'valid'],
        [[94021, 94031], 'disallowed'],
        [[94032, 94078], 'valid'],
        [[94079, 94094], 'disallowed'],
        [[94095, 94111], 'valid'],
        [[94112, 110591], 'disallowed'],
        [[110592, 110593], 'valid'],
        [[110594, 113663], 'disallowed'],
        [[113664, 113770], 'valid'],
        [[113771, 113775], 'disallowed'],
        [[113776, 113788], 'valid'],
        [[113789, 113791], 'disallowed'],
        [[113792, 113800], 'valid'],
        [[113801, 113807], 'disallowed'],
        [[113808, 113817], 'valid'],
        [[113818, 113819], 'disallowed'],
        [[113820, 113820], 'valid', [], 'NV8'],
        [[113821, 113822], 'valid'],
        [[113823, 113823], 'valid', [], 'NV8'],
        [[113824, 113827], 'ignored'],
        [[113828, 118783], 'disallowed'],
        [[118784, 119029], 'valid', [], 'NV8'],
        [[119030, 119039], 'disallowed'],
        [[119040, 119078], 'valid', [], 'NV8'],
        [[119079, 119080], 'disallowed'],
        [[119081, 119081], 'valid', [], 'NV8'],
        [[119082, 119133], 'valid', [], 'NV8'],
        [[119134, 119134], 'mapped', [119127, 119141]],
        [[119135, 119135], 'mapped', [119128, 119141]],
        [[119136, 119136], 'mapped', [119128, 119141, 119150]],
        [[119137, 119137], 'mapped', [119128, 119141, 119151]],
        [[119138, 119138], 'mapped', [119128, 119141, 119152]],
        [[119139, 119139], 'mapped', [119128, 119141, 119153]],
        [[119140, 119140], 'mapped', [119128, 119141, 119154]],
        [[119141, 119154], 'valid', [], 'NV8'],
        [[119155, 119162], 'disallowed'],
        [[119163, 119226], 'valid', [], 'NV8'],
        [[119227, 119227], 'mapped', [119225, 119141]],
        [[119228, 119228], 'mapped', [119226, 119141]],
        [[119229, 119229], 'mapped', [119225, 119141, 119150]],
        [[119230, 119230], 'mapped', [119226, 119141, 119150]],
        [[119231, 119231], 'mapped', [119225, 119141, 119151]],
        [[119232, 119232], 'mapped', [119226, 119141, 119151]],
        [[119233, 119261], 'valid', [], 'NV8'],
        [[119262, 119272], 'valid', [], 'NV8'],
        [[119273, 119295], 'disallowed'],
        [[119296, 119365], 'valid', [], 'NV8'],
        [[119366, 119551], 'disallowed'],
        [[119552, 119638], 'valid', [], 'NV8'],
        [[119639, 119647], 'disallowed'],
        [[119648, 119665], 'valid', [], 'NV8'],
        [[119666, 119807], 'disallowed'],
        [[119808, 119808], 'mapped', [97]],
        [[119809, 119809], 'mapped', [98]],
        [[119810, 119810], 'mapped', [99]],
        [[119811, 119811], 'mapped', [100]],
        [[119812, 119812], 'mapped', [101]],
        [[119813, 119813], 'mapped', [102]],
        [[119814, 119814], 'mapped', [103]],
        [[119815, 119815], 'mapped', [104]],
        [[119816, 119816], 'mapped', [105]],
        [[119817, 119817], 'mapped', [106]],
        [[119818, 119818], 'mapped', [107]],
        [[119819, 119819], 'mapped', [108]],
        [[119820, 119820], 'mapped', [109]],
        [[119821, 119821], 'mapped', [110]],
        [[119822, 119822], 'mapped', [111]],
        [[119823, 119823], 'mapped', [112]],
        [[119824, 119824], 'mapped', [113]],
        [[119825, 119825], 'mapped', [114]],
        [[119826, 119826], 'mapped', [115]],
        [[119827, 119827], 'mapped', [116]],
        [[119828, 119828], 'mapped', [117]],
        [[119829, 119829], 'mapped', [118]],
        [[119830, 119830], 'mapped', [119]],
        [[119831, 119831], 'mapped', [120]],
        [[119832, 119832], 'mapped', [121]],
        [[119833, 119833], 'mapped', [122]],
        [[119834, 119834], 'mapped', [97]],
        [[119835, 119835], 'mapped', [98]],
        [[119836, 119836], 'mapped', [99]],
        [[119837, 119837], 'mapped', [100]],
        [[119838, 119838], 'mapped', [101]],
        [[119839, 119839], 'mapped', [102]],
        [[119840, 119840], 'mapped', [103]],
        [[119841, 119841], 'mapped', [104]],
        [[119842, 119842], 'mapped', [105]],
        [[119843, 119843], 'mapped', [106]],
        [[119844, 119844], 'mapped', [107]],
        [[119845, 119845], 'mapped', [108]],
        [[119846, 119846], 'mapped', [109]],
        [[119847, 119847], 'mapped', [110]],
        [[119848, 119848], 'mapped', [111]],
        [[119849, 119849], 'mapped', [112]],
        [[119850, 119850], 'mapped', [113]],
        [[119851, 119851], 'mapped', [114]],
        [[119852, 119852], 'mapped', [115]],
        [[119853, 119853], 'mapped', [116]],
        [[119854, 119854], 'mapped', [117]],
        [[119855, 119855], 'mapped', [118]],
        [[119856, 119856], 'mapped', [119]],
        [[119857, 119857], 'mapped', [120]],
        [[119858, 119858], 'mapped', [121]],
        [[119859, 119859], 'mapped', [122]],
        [[119860, 119860], 'mapped', [97]],
        [[119861, 119861], 'mapped', [98]],
        [[119862, 119862], 'mapped', [99]],
        [[119863, 119863], 'mapped', [100]],
        [[119864, 119864], 'mapped', [101]],
        [[119865, 119865], 'mapped', [102]],
        [[119866, 119866], 'mapped', [103]],
        [[119867, 119867], 'mapped', [104]],
        [[119868, 119868], 'mapped', [105]],
        [[119869, 119869], 'mapped', [106]],
        [[119870, 119870], 'mapped', [107]],
        [[119871, 119871], 'mapped', [108]],
        [[119872, 119872], 'mapped', [109]],
        [[119873, 119873], 'mapped', [110]],
        [[119874, 119874], 'mapped', [111]],
        [[119875, 119875], 'mapped', [112]],
        [[119876, 119876], 'mapped', [113]],
        [[119877, 119877], 'mapped', [114]],
        [[119878, 119878], 'mapped', [115]],
        [[119879, 119879], 'mapped', [116]],
        [[119880, 119880], 'mapped', [117]],
        [[119881, 119881], 'mapped', [118]],
        [[119882, 119882], 'mapped', [119]],
        [[119883, 119883], 'mapped', [120]],
        [[119884, 119884], 'mapped', [121]],
        [[119885, 119885], 'mapped', [122]],
        [[119886, 119886], 'mapped', [97]],
        [[119887, 119887], 'mapped', [98]],
        [[119888, 119888], 'mapped', [99]],
        [[119889, 119889], 'mapped', [100]],
        [[119890, 119890], 'mapped', [101]],
        [[119891, 119891], 'mapped', [102]],
        [[119892, 119892], 'mapped', [103]],
        [[119893, 119893], 'disallowed'],
        [[119894, 119894], 'mapped', [105]],
        [[119895, 119895], 'mapped', [106]],
        [[119896, 119896], 'mapped', [107]],
        [[119897, 119897], 'mapped', [108]],
        [[119898, 119898], 'mapped', [109]],
        [[119899, 119899], 'mapped', [110]],
        [[119900, 119900], 'mapped', [111]],
        [[119901, 119901], 'mapped', [112]],
        [[119902, 119902], 'mapped', [113]],
        [[119903, 119903], 'mapped', [114]],
        [[119904, 119904], 'mapped', [115]],
        [[119905, 119905], 'mapped', [116]],
        [[119906, 119906], 'mapped', [117]],
        [[119907, 119907], 'mapped', [118]],
        [[119908, 119908], 'mapped', [119]],
        [[119909, 119909], 'mapped', [120]],
        [[119910, 119910], 'mapped', [121]],
        [[119911, 119911], 'mapped', [122]],
        [[119912, 119912], 'mapped', [97]],
        [[119913, 119913], 'mapped', [98]],
        [[119914, 119914], 'mapped', [99]],
        [[119915, 119915], 'mapped', [100]],
        [[119916, 119916], 'mapped', [101]],
        [[119917, 119917], 'mapped', [102]],
        [[119918, 119918], 'mapped', [103]],
        [[119919, 119919], 'mapped', [104]],
        [[119920, 119920], 'mapped', [105]],
        [[119921, 119921], 'mapped', [106]],
        [[119922, 119922], 'mapped', [107]],
        [[119923, 119923], 'mapped', [108]],
        [[119924, 119924], 'mapped', [109]],
        [[119925, 119925], 'mapped', [110]],
        [[119926, 119926], 'mapped', [111]],
        [[119927, 119927], 'mapped', [112]],
        [[119928, 119928], 'mapped', [113]],
        [[119929, 119929], 'mapped', [114]],
        [[119930, 119930], 'mapped', [115]],
        [[119931, 119931], 'mapped', [116]],
        [[119932, 119932], 'mapped', [117]],
        [[119933, 119933], 'mapped', [118]],
        [[119934, 119934], 'mapped', [119]],
        [[119935, 119935], 'mapped', [120]],
        [[119936, 119936], 'mapped', [121]],
        [[119937, 119937], 'mapped', [122]],
        [[119938, 119938], 'mapped', [97]],
        [[119939, 119939], 'mapped', [98]],
        [[119940, 119940], 'mapped', [99]],
        [[119941, 119941], 'mapped', [100]],
        [[119942, 119942], 'mapped', [101]],
        [[119943, 119943], 'mapped', [102]],
        [[119944, 119944], 'mapped', [103]],
        [[119945, 119945], 'mapped', [104]],
        [[119946, 119946], 'mapped', [105]],
        [[119947, 119947], 'mapped', [106]],
        [[119948, 119948], 'mapped', [107]],
        [[119949, 119949], 'mapped', [108]],
        [[119950, 119950], 'mapped', [109]],
        [[119951, 119951], 'mapped', [110]],
        [[119952, 119952], 'mapped', [111]],
        [[119953, 119953], 'mapped', [112]],
        [[119954, 119954], 'mapped', [113]],
        [[119955, 119955], 'mapped', [114]],
        [[119956, 119956], 'mapped', [115]],
        [[119957, 119957], 'mapped', [116]],
        [[119958, 119958], 'mapped', [117]],
        [[119959, 119959], 'mapped', [118]],
        [[119960, 119960], 'mapped', [119]],
        [[119961, 119961], 'mapped', [120]],
        [[119962, 119962], 'mapped', [121]],
        [[119963, 119963], 'mapped', [122]],
        [[119964, 119964], 'mapped', [97]],
        [[119965, 119965], 'disallowed'],
        [[119966, 119966], 'mapped', [99]],
        [[119967, 119967], 'mapped', [100]],
        [[119968, 119969], 'disallowed'],
        [[119970, 119970], 'mapped', [103]],
        [[119971, 119972], 'disallowed'],
        [[119973, 119973], 'mapped', [106]],
        [[119974, 119974], 'mapped', [107]],
        [[119975, 119976], 'disallowed'],
        [[119977, 119977], 'mapped', [110]],
        [[119978, 119978], 'mapped', [111]],
        [[119979, 119979], 'mapped', [112]],
        [[119980, 119980], 'mapped', [113]],
        [[119981, 119981], 'disallowed'],
        [[119982, 119982], 'mapped', [115]],
        [[119983, 119983], 'mapped', [116]],
        [[119984, 119984], 'mapped', [117]],
        [[119985, 119985], 'mapped', [118]],
        [[119986, 119986], 'mapped', [119]],
        [[119987, 119987], 'mapped', [120]],
        [[119988, 119988], 'mapped', [121]],
        [[119989, 119989], 'mapped', [122]],
        [[119990, 119990], 'mapped', [97]],
        [[119991, 119991], 'mapped', [98]],
        [[119992, 119992], 'mapped', [99]],
        [[119993, 119993], 'mapped', [100]],
        [[119994, 119994], 'disallowed'],
        [[119995, 119995], 'mapped', [102]],
        [[119996, 119996], 'disallowed'],
        [[119997, 119997], 'mapped', [104]],
        [[119998, 119998], 'mapped', [105]],
        [[119999, 119999], 'mapped', [106]],
        [[12e4, 12e4], 'mapped', [107]],
        [[120001, 120001], 'mapped', [108]],
        [[120002, 120002], 'mapped', [109]],
        [[120003, 120003], 'mapped', [110]],
        [[120004, 120004], 'disallowed'],
        [[120005, 120005], 'mapped', [112]],
        [[120006, 120006], 'mapped', [113]],
        [[120007, 120007], 'mapped', [114]],
        [[120008, 120008], 'mapped', [115]],
        [[120009, 120009], 'mapped', [116]],
        [[120010, 120010], 'mapped', [117]],
        [[120011, 120011], 'mapped', [118]],
        [[120012, 120012], 'mapped', [119]],
        [[120013, 120013], 'mapped', [120]],
        [[120014, 120014], 'mapped', [121]],
        [[120015, 120015], 'mapped', [122]],
        [[120016, 120016], 'mapped', [97]],
        [[120017, 120017], 'mapped', [98]],
        [[120018, 120018], 'mapped', [99]],
        [[120019, 120019], 'mapped', [100]],
        [[120020, 120020], 'mapped', [101]],
        [[120021, 120021], 'mapped', [102]],
        [[120022, 120022], 'mapped', [103]],
        [[120023, 120023], 'mapped', [104]],
        [[120024, 120024], 'mapped', [105]],
        [[120025, 120025], 'mapped', [106]],
        [[120026, 120026], 'mapped', [107]],
        [[120027, 120027], 'mapped', [108]],
        [[120028, 120028], 'mapped', [109]],
        [[120029, 120029], 'mapped', [110]],
        [[120030, 120030], 'mapped', [111]],
        [[120031, 120031], 'mapped', [112]],
        [[120032, 120032], 'mapped', [113]],
        [[120033, 120033], 'mapped', [114]],
        [[120034, 120034], 'mapped', [115]],
        [[120035, 120035], 'mapped', [116]],
        [[120036, 120036], 'mapped', [117]],
        [[120037, 120037], 'mapped', [118]],
        [[120038, 120038], 'mapped', [119]],
        [[120039, 120039], 'mapped', [120]],
        [[120040, 120040], 'mapped', [121]],
        [[120041, 120041], 'mapped', [122]],
        [[120042, 120042], 'mapped', [97]],
        [[120043, 120043], 'mapped', [98]],
        [[120044, 120044], 'mapped', [99]],
        [[120045, 120045], 'mapped', [100]],
        [[120046, 120046], 'mapped', [101]],
        [[120047, 120047], 'mapped', [102]],
        [[120048, 120048], 'mapped', [103]],
        [[120049, 120049], 'mapped', [104]],
        [[120050, 120050], 'mapped', [105]],
        [[120051, 120051], 'mapped', [106]],
        [[120052, 120052], 'mapped', [107]],
        [[120053, 120053], 'mapped', [108]],
        [[120054, 120054], 'mapped', [109]],
        [[120055, 120055], 'mapped', [110]],
        [[120056, 120056], 'mapped', [111]],
        [[120057, 120057], 'mapped', [112]],
        [[120058, 120058], 'mapped', [113]],
        [[120059, 120059], 'mapped', [114]],
        [[120060, 120060], 'mapped', [115]],
        [[120061, 120061], 'mapped', [116]],
        [[120062, 120062], 'mapped', [117]],
        [[120063, 120063], 'mapped', [118]],
        [[120064, 120064], 'mapped', [119]],
        [[120065, 120065], 'mapped', [120]],
        [[120066, 120066], 'mapped', [121]],
        [[120067, 120067], 'mapped', [122]],
        [[120068, 120068], 'mapped', [97]],
        [[120069, 120069], 'mapped', [98]],
        [[120070, 120070], 'disallowed'],
        [[120071, 120071], 'mapped', [100]],
        [[120072, 120072], 'mapped', [101]],
        [[120073, 120073], 'mapped', [102]],
        [[120074, 120074], 'mapped', [103]],
        [[120075, 120076], 'disallowed'],
        [[120077, 120077], 'mapped', [106]],
        [[120078, 120078], 'mapped', [107]],
        [[120079, 120079], 'mapped', [108]],
        [[120080, 120080], 'mapped', [109]],
        [[120081, 120081], 'mapped', [110]],
        [[120082, 120082], 'mapped', [111]],
        [[120083, 120083], 'mapped', [112]],
        [[120084, 120084], 'mapped', [113]],
        [[120085, 120085], 'disallowed'],
        [[120086, 120086], 'mapped', [115]],
        [[120087, 120087], 'mapped', [116]],
        [[120088, 120088], 'mapped', [117]],
        [[120089, 120089], 'mapped', [118]],
        [[120090, 120090], 'mapped', [119]],
        [[120091, 120091], 'mapped', [120]],
        [[120092, 120092], 'mapped', [121]],
        [[120093, 120093], 'disallowed'],
        [[120094, 120094], 'mapped', [97]],
        [[120095, 120095], 'mapped', [98]],
        [[120096, 120096], 'mapped', [99]],
        [[120097, 120097], 'mapped', [100]],
        [[120098, 120098], 'mapped', [101]],
        [[120099, 120099], 'mapped', [102]],
        [[120100, 120100], 'mapped', [103]],
        [[120101, 120101], 'mapped', [104]],
        [[120102, 120102], 'mapped', [105]],
        [[120103, 120103], 'mapped', [106]],
        [[120104, 120104], 'mapped', [107]],
        [[120105, 120105], 'mapped', [108]],
        [[120106, 120106], 'mapped', [109]],
        [[120107, 120107], 'mapped', [110]],
        [[120108, 120108], 'mapped', [111]],
        [[120109, 120109], 'mapped', [112]],
        [[120110, 120110], 'mapped', [113]],
        [[120111, 120111], 'mapped', [114]],
        [[120112, 120112], 'mapped', [115]],
        [[120113, 120113], 'mapped', [116]],
        [[120114, 120114], 'mapped', [117]],
        [[120115, 120115], 'mapped', [118]],
        [[120116, 120116], 'mapped', [119]],
        [[120117, 120117], 'mapped', [120]],
        [[120118, 120118], 'mapped', [121]],
        [[120119, 120119], 'mapped', [122]],
        [[120120, 120120], 'mapped', [97]],
        [[120121, 120121], 'mapped', [98]],
        [[120122, 120122], 'disallowed'],
        [[120123, 120123], 'mapped', [100]],
        [[120124, 120124], 'mapped', [101]],
        [[120125, 120125], 'mapped', [102]],
        [[120126, 120126], 'mapped', [103]],
        [[120127, 120127], 'disallowed'],
        [[120128, 120128], 'mapped', [105]],
        [[120129, 120129], 'mapped', [106]],
        [[120130, 120130], 'mapped', [107]],
        [[120131, 120131], 'mapped', [108]],
        [[120132, 120132], 'mapped', [109]],
        [[120133, 120133], 'disallowed'],
        [[120134, 120134], 'mapped', [111]],
        [[120135, 120137], 'disallowed'],
        [[120138, 120138], 'mapped', [115]],
        [[120139, 120139], 'mapped', [116]],
        [[120140, 120140], 'mapped', [117]],
        [[120141, 120141], 'mapped', [118]],
        [[120142, 120142], 'mapped', [119]],
        [[120143, 120143], 'mapped', [120]],
        [[120144, 120144], 'mapped', [121]],
        [[120145, 120145], 'disallowed'],
        [[120146, 120146], 'mapped', [97]],
        [[120147, 120147], 'mapped', [98]],
        [[120148, 120148], 'mapped', [99]],
        [[120149, 120149], 'mapped', [100]],
        [[120150, 120150], 'mapped', [101]],
        [[120151, 120151], 'mapped', [102]],
        [[120152, 120152], 'mapped', [103]],
        [[120153, 120153], 'mapped', [104]],
        [[120154, 120154], 'mapped', [105]],
        [[120155, 120155], 'mapped', [106]],
        [[120156, 120156], 'mapped', [107]],
        [[120157, 120157], 'mapped', [108]],
        [[120158, 120158], 'mapped', [109]],
        [[120159, 120159], 'mapped', [110]],
        [[120160, 120160], 'mapped', [111]],
        [[120161, 120161], 'mapped', [112]],
        [[120162, 120162], 'mapped', [113]],
        [[120163, 120163], 'mapped', [114]],
        [[120164, 120164], 'mapped', [115]],
        [[120165, 120165], 'mapped', [116]],
        [[120166, 120166], 'mapped', [117]],
        [[120167, 120167], 'mapped', [118]],
        [[120168, 120168], 'mapped', [119]],
        [[120169, 120169], 'mapped', [120]],
        [[120170, 120170], 'mapped', [121]],
        [[120171, 120171], 'mapped', [122]],
        [[120172, 120172], 'mapped', [97]],
        [[120173, 120173], 'mapped', [98]],
        [[120174, 120174], 'mapped', [99]],
        [[120175, 120175], 'mapped', [100]],
        [[120176, 120176], 'mapped', [101]],
        [[120177, 120177], 'mapped', [102]],
        [[120178, 120178], 'mapped', [103]],
        [[120179, 120179], 'mapped', [104]],
        [[120180, 120180], 'mapped', [105]],
        [[120181, 120181], 'mapped', [106]],
        [[120182, 120182], 'mapped', [107]],
        [[120183, 120183], 'mapped', [108]],
        [[120184, 120184], 'mapped', [109]],
        [[120185, 120185], 'mapped', [110]],
        [[120186, 120186], 'mapped', [111]],
        [[120187, 120187], 'mapped', [112]],
        [[120188, 120188], 'mapped', [113]],
        [[120189, 120189], 'mapped', [114]],
        [[120190, 120190], 'mapped', [115]],
        [[120191, 120191], 'mapped', [116]],
        [[120192, 120192], 'mapped', [117]],
        [[120193, 120193], 'mapped', [118]],
        [[120194, 120194], 'mapped', [119]],
        [[120195, 120195], 'mapped', [120]],
        [[120196, 120196], 'mapped', [121]],
        [[120197, 120197], 'mapped', [122]],
        [[120198, 120198], 'mapped', [97]],
        [[120199, 120199], 'mapped', [98]],
        [[120200, 120200], 'mapped', [99]],
        [[120201, 120201], 'mapped', [100]],
        [[120202, 120202], 'mapped', [101]],
        [[120203, 120203], 'mapped', [102]],
        [[120204, 120204], 'mapped', [103]],
        [[120205, 120205], 'mapped', [104]],
        [[120206, 120206], 'mapped', [105]],
        [[120207, 120207], 'mapped', [106]],
        [[120208, 120208], 'mapped', [107]],
        [[120209, 120209], 'mapped', [108]],
        [[120210, 120210], 'mapped', [109]],
        [[120211, 120211], 'mapped', [110]],
        [[120212, 120212], 'mapped', [111]],
        [[120213, 120213], 'mapped', [112]],
        [[120214, 120214], 'mapped', [113]],
        [[120215, 120215], 'mapped', [114]],
        [[120216, 120216], 'mapped', [115]],
        [[120217, 120217], 'mapped', [116]],
        [[120218, 120218], 'mapped', [117]],
        [[120219, 120219], 'mapped', [118]],
        [[120220, 120220], 'mapped', [119]],
        [[120221, 120221], 'mapped', [120]],
        [[120222, 120222], 'mapped', [121]],
        [[120223, 120223], 'mapped', [122]],
        [[120224, 120224], 'mapped', [97]],
        [[120225, 120225], 'mapped', [98]],
        [[120226, 120226], 'mapped', [99]],
        [[120227, 120227], 'mapped', [100]],
        [[120228, 120228], 'mapped', [101]],
        [[120229, 120229], 'mapped', [102]],
        [[120230, 120230], 'mapped', [103]],
        [[120231, 120231], 'mapped', [104]],
        [[120232, 120232], 'mapped', [105]],
        [[120233, 120233], 'mapped', [106]],
        [[120234, 120234], 'mapped', [107]],
        [[120235, 120235], 'mapped', [108]],
        [[120236, 120236], 'mapped', [109]],
        [[120237, 120237], 'mapped', [110]],
        [[120238, 120238], 'mapped', [111]],
        [[120239, 120239], 'mapped', [112]],
        [[120240, 120240], 'mapped', [113]],
        [[120241, 120241], 'mapped', [114]],
        [[120242, 120242], 'mapped', [115]],
        [[120243, 120243], 'mapped', [116]],
        [[120244, 120244], 'mapped', [117]],
        [[120245, 120245], 'mapped', [118]],
        [[120246, 120246], 'mapped', [119]],
        [[120247, 120247], 'mapped', [120]],
        [[120248, 120248], 'mapped', [121]],
        [[120249, 120249], 'mapped', [122]],
        [[120250, 120250], 'mapped', [97]],
        [[120251, 120251], 'mapped', [98]],
        [[120252, 120252], 'mapped', [99]],
        [[120253, 120253], 'mapped', [100]],
        [[120254, 120254], 'mapped', [101]],
        [[120255, 120255], 'mapped', [102]],
        [[120256, 120256], 'mapped', [103]],
        [[120257, 120257], 'mapped', [104]],
        [[120258, 120258], 'mapped', [105]],
        [[120259, 120259], 'mapped', [106]],
        [[120260, 120260], 'mapped', [107]],
        [[120261, 120261], 'mapped', [108]],
        [[120262, 120262], 'mapped', [109]],
        [[120263, 120263], 'mapped', [110]],
        [[120264, 120264], 'mapped', [111]],
        [[120265, 120265], 'mapped', [112]],
        [[120266, 120266], 'mapped', [113]],
        [[120267, 120267], 'mapped', [114]],
        [[120268, 120268], 'mapped', [115]],
        [[120269, 120269], 'mapped', [116]],
        [[120270, 120270], 'mapped', [117]],
        [[120271, 120271], 'mapped', [118]],
        [[120272, 120272], 'mapped', [119]],
        [[120273, 120273], 'mapped', [120]],
        [[120274, 120274], 'mapped', [121]],
        [[120275, 120275], 'mapped', [122]],
        [[120276, 120276], 'mapped', [97]],
        [[120277, 120277], 'mapped', [98]],
        [[120278, 120278], 'mapped', [99]],
        [[120279, 120279], 'mapped', [100]],
        [[120280, 120280], 'mapped', [101]],
        [[120281, 120281], 'mapped', [102]],
        [[120282, 120282], 'mapped', [103]],
        [[120283, 120283], 'mapped', [104]],
        [[120284, 120284], 'mapped', [105]],
        [[120285, 120285], 'mapped', [106]],
        [[120286, 120286], 'mapped', [107]],
        [[120287, 120287], 'mapped', [108]],
        [[120288, 120288], 'mapped', [109]],
        [[120289, 120289], 'mapped', [110]],
        [[120290, 120290], 'mapped', [111]],
        [[120291, 120291], 'mapped', [112]],
        [[120292, 120292], 'mapped', [113]],
        [[120293, 120293], 'mapped', [114]],
        [[120294, 120294], 'mapped', [115]],
        [[120295, 120295], 'mapped', [116]],
        [[120296, 120296], 'mapped', [117]],
        [[120297, 120297], 'mapped', [118]],
        [[120298, 120298], 'mapped', [119]],
        [[120299, 120299], 'mapped', [120]],
        [[120300, 120300], 'mapped', [121]],
        [[120301, 120301], 'mapped', [122]],
        [[120302, 120302], 'mapped', [97]],
        [[120303, 120303], 'mapped', [98]],
        [[120304, 120304], 'mapped', [99]],
        [[120305, 120305], 'mapped', [100]],
        [[120306, 120306], 'mapped', [101]],
        [[120307, 120307], 'mapped', [102]],
        [[120308, 120308], 'mapped', [103]],
        [[120309, 120309], 'mapped', [104]],
        [[120310, 120310], 'mapped', [105]],
        [[120311, 120311], 'mapped', [106]],
        [[120312, 120312], 'mapped', [107]],
        [[120313, 120313], 'mapped', [108]],
        [[120314, 120314], 'mapped', [109]],
        [[120315, 120315], 'mapped', [110]],
        [[120316, 120316], 'mapped', [111]],
        [[120317, 120317], 'mapped', [112]],
        [[120318, 120318], 'mapped', [113]],
        [[120319, 120319], 'mapped', [114]],
        [[120320, 120320], 'mapped', [115]],
        [[120321, 120321], 'mapped', [116]],
        [[120322, 120322], 'mapped', [117]],
        [[120323, 120323], 'mapped', [118]],
        [[120324, 120324], 'mapped', [119]],
        [[120325, 120325], 'mapped', [120]],
        [[120326, 120326], 'mapped', [121]],
        [[120327, 120327], 'mapped', [122]],
        [[120328, 120328], 'mapped', [97]],
        [[120329, 120329], 'mapped', [98]],
        [[120330, 120330], 'mapped', [99]],
        [[120331, 120331], 'mapped', [100]],
        [[120332, 120332], 'mapped', [101]],
        [[120333, 120333], 'mapped', [102]],
        [[120334, 120334], 'mapped', [103]],
        [[120335, 120335], 'mapped', [104]],
        [[120336, 120336], 'mapped', [105]],
        [[120337, 120337], 'mapped', [106]],
        [[120338, 120338], 'mapped', [107]],
        [[120339, 120339], 'mapped', [108]],
        [[120340, 120340], 'mapped', [109]],
        [[120341, 120341], 'mapped', [110]],
        [[120342, 120342], 'mapped', [111]],
        [[120343, 120343], 'mapped', [112]],
        [[120344, 120344], 'mapped', [113]],
        [[120345, 120345], 'mapped', [114]],
        [[120346, 120346], 'mapped', [115]],
        [[120347, 120347], 'mapped', [116]],
        [[120348, 120348], 'mapped', [117]],
        [[120349, 120349], 'mapped', [118]],
        [[120350, 120350], 'mapped', [119]],
        [[120351, 120351], 'mapped', [120]],
        [[120352, 120352], 'mapped', [121]],
        [[120353, 120353], 'mapped', [122]],
        [[120354, 120354], 'mapped', [97]],
        [[120355, 120355], 'mapped', [98]],
        [[120356, 120356], 'mapped', [99]],
        [[120357, 120357], 'mapped', [100]],
        [[120358, 120358], 'mapped', [101]],
        [[120359, 120359], 'mapped', [102]],
        [[120360, 120360], 'mapped', [103]],
        [[120361, 120361], 'mapped', [104]],
        [[120362, 120362], 'mapped', [105]],
        [[120363, 120363], 'mapped', [106]],
        [[120364, 120364], 'mapped', [107]],
        [[120365, 120365], 'mapped', [108]],
        [[120366, 120366], 'mapped', [109]],
        [[120367, 120367], 'mapped', [110]],
        [[120368, 120368], 'mapped', [111]],
        [[120369, 120369], 'mapped', [112]],
        [[120370, 120370], 'mapped', [113]],
        [[120371, 120371], 'mapped', [114]],
        [[120372, 120372], 'mapped', [115]],
        [[120373, 120373], 'mapped', [116]],
        [[120374, 120374], 'mapped', [117]],
        [[120375, 120375], 'mapped', [118]],
        [[120376, 120376], 'mapped', [119]],
        [[120377, 120377], 'mapped', [120]],
        [[120378, 120378], 'mapped', [121]],
        [[120379, 120379], 'mapped', [122]],
        [[120380, 120380], 'mapped', [97]],
        [[120381, 120381], 'mapped', [98]],
        [[120382, 120382], 'mapped', [99]],
        [[120383, 120383], 'mapped', [100]],
        [[120384, 120384], 'mapped', [101]],
        [[120385, 120385], 'mapped', [102]],
        [[120386, 120386], 'mapped', [103]],
        [[120387, 120387], 'mapped', [104]],
        [[120388, 120388], 'mapped', [105]],
        [[120389, 120389], 'mapped', [106]],
        [[120390, 120390], 'mapped', [107]],
        [[120391, 120391], 'mapped', [108]],
        [[120392, 120392], 'mapped', [109]],
        [[120393, 120393], 'mapped', [110]],
        [[120394, 120394], 'mapped', [111]],
        [[120395, 120395], 'mapped', [112]],
        [[120396, 120396], 'mapped', [113]],
        [[120397, 120397], 'mapped', [114]],
        [[120398, 120398], 'mapped', [115]],
        [[120399, 120399], 'mapped', [116]],
        [[120400, 120400], 'mapped', [117]],
        [[120401, 120401], 'mapped', [118]],
        [[120402, 120402], 'mapped', [119]],
        [[120403, 120403], 'mapped', [120]],
        [[120404, 120404], 'mapped', [121]],
        [[120405, 120405], 'mapped', [122]],
        [[120406, 120406], 'mapped', [97]],
        [[120407, 120407], 'mapped', [98]],
        [[120408, 120408], 'mapped', [99]],
        [[120409, 120409], 'mapped', [100]],
        [[120410, 120410], 'mapped', [101]],
        [[120411, 120411], 'mapped', [102]],
        [[120412, 120412], 'mapped', [103]],
        [[120413, 120413], 'mapped', [104]],
        [[120414, 120414], 'mapped', [105]],
        [[120415, 120415], 'mapped', [106]],
        [[120416, 120416], 'mapped', [107]],
        [[120417, 120417], 'mapped', [108]],
        [[120418, 120418], 'mapped', [109]],
        [[120419, 120419], 'mapped', [110]],
        [[120420, 120420], 'mapped', [111]],
        [[120421, 120421], 'mapped', [112]],
        [[120422, 120422], 'mapped', [113]],
        [[120423, 120423], 'mapped', [114]],
        [[120424, 120424], 'mapped', [115]],
        [[120425, 120425], 'mapped', [116]],
        [[120426, 120426], 'mapped', [117]],
        [[120427, 120427], 'mapped', [118]],
        [[120428, 120428], 'mapped', [119]],
        [[120429, 120429], 'mapped', [120]],
        [[120430, 120430], 'mapped', [121]],
        [[120431, 120431], 'mapped', [122]],
        [[120432, 120432], 'mapped', [97]],
        [[120433, 120433], 'mapped', [98]],
        [[120434, 120434], 'mapped', [99]],
        [[120435, 120435], 'mapped', [100]],
        [[120436, 120436], 'mapped', [101]],
        [[120437, 120437], 'mapped', [102]],
        [[120438, 120438], 'mapped', [103]],
        [[120439, 120439], 'mapped', [104]],
        [[120440, 120440], 'mapped', [105]],
        [[120441, 120441], 'mapped', [106]],
        [[120442, 120442], 'mapped', [107]],
        [[120443, 120443], 'mapped', [108]],
        [[120444, 120444], 'mapped', [109]],
        [[120445, 120445], 'mapped', [110]],
        [[120446, 120446], 'mapped', [111]],
        [[120447, 120447], 'mapped', [112]],
        [[120448, 120448], 'mapped', [113]],
        [[120449, 120449], 'mapped', [114]],
        [[120450, 120450], 'mapped', [115]],
        [[120451, 120451], 'mapped', [116]],
        [[120452, 120452], 'mapped', [117]],
        [[120453, 120453], 'mapped', [118]],
        [[120454, 120454], 'mapped', [119]],
        [[120455, 120455], 'mapped', [120]],
        [[120456, 120456], 'mapped', [121]],
        [[120457, 120457], 'mapped', [122]],
        [[120458, 120458], 'mapped', [97]],
        [[120459, 120459], 'mapped', [98]],
        [[120460, 120460], 'mapped', [99]],
        [[120461, 120461], 'mapped', [100]],
        [[120462, 120462], 'mapped', [101]],
        [[120463, 120463], 'mapped', [102]],
        [[120464, 120464], 'mapped', [103]],
        [[120465, 120465], 'mapped', [104]],
        [[120466, 120466], 'mapped', [105]],
        [[120467, 120467], 'mapped', [106]],
        [[120468, 120468], 'mapped', [107]],
        [[120469, 120469], 'mapped', [108]],
        [[120470, 120470], 'mapped', [109]],
        [[120471, 120471], 'mapped', [110]],
        [[120472, 120472], 'mapped', [111]],
        [[120473, 120473], 'mapped', [112]],
        [[120474, 120474], 'mapped', [113]],
        [[120475, 120475], 'mapped', [114]],
        [[120476, 120476], 'mapped', [115]],
        [[120477, 120477], 'mapped', [116]],
        [[120478, 120478], 'mapped', [117]],
        [[120479, 120479], 'mapped', [118]],
        [[120480, 120480], 'mapped', [119]],
        [[120481, 120481], 'mapped', [120]],
        [[120482, 120482], 'mapped', [121]],
        [[120483, 120483], 'mapped', [122]],
        [[120484, 120484], 'mapped', [305]],
        [[120485, 120485], 'mapped', [567]],
        [[120486, 120487], 'disallowed'],
        [[120488, 120488], 'mapped', [945]],
        [[120489, 120489], 'mapped', [946]],
        [[120490, 120490], 'mapped', [947]],
        [[120491, 120491], 'mapped', [948]],
        [[120492, 120492], 'mapped', [949]],
        [[120493, 120493], 'mapped', [950]],
        [[120494, 120494], 'mapped', [951]],
        [[120495, 120495], 'mapped', [952]],
        [[120496, 120496], 'mapped', [953]],
        [[120497, 120497], 'mapped', [954]],
        [[120498, 120498], 'mapped', [955]],
        [[120499, 120499], 'mapped', [956]],
        [[120500, 120500], 'mapped', [957]],
        [[120501, 120501], 'mapped', [958]],
        [[120502, 120502], 'mapped', [959]],
        [[120503, 120503], 'mapped', [960]],
        [[120504, 120504], 'mapped', [961]],
        [[120505, 120505], 'mapped', [952]],
        [[120506, 120506], 'mapped', [963]],
        [[120507, 120507], 'mapped', [964]],
        [[120508, 120508], 'mapped', [965]],
        [[120509, 120509], 'mapped', [966]],
        [[120510, 120510], 'mapped', [967]],
        [[120511, 120511], 'mapped', [968]],
        [[120512, 120512], 'mapped', [969]],
        [[120513, 120513], 'mapped', [8711]],
        [[120514, 120514], 'mapped', [945]],
        [[120515, 120515], 'mapped', [946]],
        [[120516, 120516], 'mapped', [947]],
        [[120517, 120517], 'mapped', [948]],
        [[120518, 120518], 'mapped', [949]],
        [[120519, 120519], 'mapped', [950]],
        [[120520, 120520], 'mapped', [951]],
        [[120521, 120521], 'mapped', [952]],
        [[120522, 120522], 'mapped', [953]],
        [[120523, 120523], 'mapped', [954]],
        [[120524, 120524], 'mapped', [955]],
        [[120525, 120525], 'mapped', [956]],
        [[120526, 120526], 'mapped', [957]],
        [[120527, 120527], 'mapped', [958]],
        [[120528, 120528], 'mapped', [959]],
        [[120529, 120529], 'mapped', [960]],
        [[120530, 120530], 'mapped', [961]],
        [[120531, 120532], 'mapped', [963]],
        [[120533, 120533], 'mapped', [964]],
        [[120534, 120534], 'mapped', [965]],
        [[120535, 120535], 'mapped', [966]],
        [[120536, 120536], 'mapped', [967]],
        [[120537, 120537], 'mapped', [968]],
        [[120538, 120538], 'mapped', [969]],
        [[120539, 120539], 'mapped', [8706]],
        [[120540, 120540], 'mapped', [949]],
        [[120541, 120541], 'mapped', [952]],
        [[120542, 120542], 'mapped', [954]],
        [[120543, 120543], 'mapped', [966]],
        [[120544, 120544], 'mapped', [961]],
        [[120545, 120545], 'mapped', [960]],
        [[120546, 120546], 'mapped', [945]],
        [[120547, 120547], 'mapped', [946]],
        [[120548, 120548], 'mapped', [947]],
        [[120549, 120549], 'mapped', [948]],
        [[120550, 120550], 'mapped', [949]],
        [[120551, 120551], 'mapped', [950]],
        [[120552, 120552], 'mapped', [951]],
        [[120553, 120553], 'mapped', [952]],
        [[120554, 120554], 'mapped', [953]],
        [[120555, 120555], 'mapped', [954]],
        [[120556, 120556], 'mapped', [955]],
        [[120557, 120557], 'mapped', [956]],
        [[120558, 120558], 'mapped', [957]],
        [[120559, 120559], 'mapped', [958]],
        [[120560, 120560], 'mapped', [959]],
        [[120561, 120561], 'mapped', [960]],
        [[120562, 120562], 'mapped', [961]],
        [[120563, 120563], 'mapped', [952]],
        [[120564, 120564], 'mapped', [963]],
        [[120565, 120565], 'mapped', [964]],
        [[120566, 120566], 'mapped', [965]],
        [[120567, 120567], 'mapped', [966]],
        [[120568, 120568], 'mapped', [967]],
        [[120569, 120569], 'mapped', [968]],
        [[120570, 120570], 'mapped', [969]],
        [[120571, 120571], 'mapped', [8711]],
        [[120572, 120572], 'mapped', [945]],
        [[120573, 120573], 'mapped', [946]],
        [[120574, 120574], 'mapped', [947]],
        [[120575, 120575], 'mapped', [948]],
        [[120576, 120576], 'mapped', [949]],
        [[120577, 120577], 'mapped', [950]],
        [[120578, 120578], 'mapped', [951]],
        [[120579, 120579], 'mapped', [952]],
        [[120580, 120580], 'mapped', [953]],
        [[120581, 120581], 'mapped', [954]],
        [[120582, 120582], 'mapped', [955]],
        [[120583, 120583], 'mapped', [956]],
        [[120584, 120584], 'mapped', [957]],
        [[120585, 120585], 'mapped', [958]],
        [[120586, 120586], 'mapped', [959]],
        [[120587, 120587], 'mapped', [960]],
        [[120588, 120588], 'mapped', [961]],
        [[120589, 120590], 'mapped', [963]],
        [[120591, 120591], 'mapped', [964]],
        [[120592, 120592], 'mapped', [965]],
        [[120593, 120593], 'mapped', [966]],
        [[120594, 120594], 'mapped', [967]],
        [[120595, 120595], 'mapped', [968]],
        [[120596, 120596], 'mapped', [969]],
        [[120597, 120597], 'mapped', [8706]],
        [[120598, 120598], 'mapped', [949]],
        [[120599, 120599], 'mapped', [952]],
        [[120600, 120600], 'mapped', [954]],
        [[120601, 120601], 'mapped', [966]],
        [[120602, 120602], 'mapped', [961]],
        [[120603, 120603], 'mapped', [960]],
        [[120604, 120604], 'mapped', [945]],
        [[120605, 120605], 'mapped', [946]],
        [[120606, 120606], 'mapped', [947]],
        [[120607, 120607], 'mapped', [948]],
        [[120608, 120608], 'mapped', [949]],
        [[120609, 120609], 'mapped', [950]],
        [[120610, 120610], 'mapped', [951]],
        [[120611, 120611], 'mapped', [952]],
        [[120612, 120612], 'mapped', [953]],
        [[120613, 120613], 'mapped', [954]],
        [[120614, 120614], 'mapped', [955]],
        [[120615, 120615], 'mapped', [956]],
        [[120616, 120616], 'mapped', [957]],
        [[120617, 120617], 'mapped', [958]],
        [[120618, 120618], 'mapped', [959]],
        [[120619, 120619], 'mapped', [960]],
        [[120620, 120620], 'mapped', [961]],
        [[120621, 120621], 'mapped', [952]],
        [[120622, 120622], 'mapped', [963]],
        [[120623, 120623], 'mapped', [964]],
        [[120624, 120624], 'mapped', [965]],
        [[120625, 120625], 'mapped', [966]],
        [[120626, 120626], 'mapped', [967]],
        [[120627, 120627], 'mapped', [968]],
        [[120628, 120628], 'mapped', [969]],
        [[120629, 120629], 'mapped', [8711]],
        [[120630, 120630], 'mapped', [945]],
        [[120631, 120631], 'mapped', [946]],
        [[120632, 120632], 'mapped', [947]],
        [[120633, 120633], 'mapped', [948]],
        [[120634, 120634], 'mapped', [949]],
        [[120635, 120635], 'mapped', [950]],
        [[120636, 120636], 'mapped', [951]],
        [[120637, 120637], 'mapped', [952]],
        [[120638, 120638], 'mapped', [953]],
        [[120639, 120639], 'mapped', [954]],
        [[120640, 120640], 'mapped', [955]],
        [[120641, 120641], 'mapped', [956]],
        [[120642, 120642], 'mapped', [957]],
        [[120643, 120643], 'mapped', [958]],
        [[120644, 120644], 'mapped', [959]],
        [[120645, 120645], 'mapped', [960]],
        [[120646, 120646], 'mapped', [961]],
        [[120647, 120648], 'mapped', [963]],
        [[120649, 120649], 'mapped', [964]],
        [[120650, 120650], 'mapped', [965]],
        [[120651, 120651], 'mapped', [966]],
        [[120652, 120652], 'mapped', [967]],
        [[120653, 120653], 'mapped', [968]],
        [[120654, 120654], 'mapped', [969]],
        [[120655, 120655], 'mapped', [8706]],
        [[120656, 120656], 'mapped', [949]],
        [[120657, 120657], 'mapped', [952]],
        [[120658, 120658], 'mapped', [954]],
        [[120659, 120659], 'mapped', [966]],
        [[120660, 120660], 'mapped', [961]],
        [[120661, 120661], 'mapped', [960]],
        [[120662, 120662], 'mapped', [945]],
        [[120663, 120663], 'mapped', [946]],
        [[120664, 120664], 'mapped', [947]],
        [[120665, 120665], 'mapped', [948]],
        [[120666, 120666], 'mapped', [949]],
        [[120667, 120667], 'mapped', [950]],
        [[120668, 120668], 'mapped', [951]],
        [[120669, 120669], 'mapped', [952]],
        [[120670, 120670], 'mapped', [953]],
        [[120671, 120671], 'mapped', [954]],
        [[120672, 120672], 'mapped', [955]],
        [[120673, 120673], 'mapped', [956]],
        [[120674, 120674], 'mapped', [957]],
        [[120675, 120675], 'mapped', [958]],
        [[120676, 120676], 'mapped', [959]],
        [[120677, 120677], 'mapped', [960]],
        [[120678, 120678], 'mapped', [961]],
        [[120679, 120679], 'mapped', [952]],
        [[120680, 120680], 'mapped', [963]],
        [[120681, 120681], 'mapped', [964]],
        [[120682, 120682], 'mapped', [965]],
        [[120683, 120683], 'mapped', [966]],
        [[120684, 120684], 'mapped', [967]],
        [[120685, 120685], 'mapped', [968]],
        [[120686, 120686], 'mapped', [969]],
        [[120687, 120687], 'mapped', [8711]],
        [[120688, 120688], 'mapped', [945]],
        [[120689, 120689], 'mapped', [946]],
        [[120690, 120690], 'mapped', [947]],
        [[120691, 120691], 'mapped', [948]],
        [[120692, 120692], 'mapped', [949]],
        [[120693, 120693], 'mapped', [950]],
        [[120694, 120694], 'mapped', [951]],
        [[120695, 120695], 'mapped', [952]],
        [[120696, 120696], 'mapped', [953]],
        [[120697, 120697], 'mapped', [954]],
        [[120698, 120698], 'mapped', [955]],
        [[120699, 120699], 'mapped', [956]],
        [[120700, 120700], 'mapped', [957]],
        [[120701, 120701], 'mapped', [958]],
        [[120702, 120702], 'mapped', [959]],
        [[120703, 120703], 'mapped', [960]],
        [[120704, 120704], 'mapped', [961]],
        [[120705, 120706], 'mapped', [963]],
        [[120707, 120707], 'mapped', [964]],
        [[120708, 120708], 'mapped', [965]],
        [[120709, 120709], 'mapped', [966]],
        [[120710, 120710], 'mapped', [967]],
        [[120711, 120711], 'mapped', [968]],
        [[120712, 120712], 'mapped', [969]],
        [[120713, 120713], 'mapped', [8706]],
        [[120714, 120714], 'mapped', [949]],
        [[120715, 120715], 'mapped', [952]],
        [[120716, 120716], 'mapped', [954]],
        [[120717, 120717], 'mapped', [966]],
        [[120718, 120718], 'mapped', [961]],
        [[120719, 120719], 'mapped', [960]],
        [[120720, 120720], 'mapped', [945]],
        [[120721, 120721], 'mapped', [946]],
        [[120722, 120722], 'mapped', [947]],
        [[120723, 120723], 'mapped', [948]],
        [[120724, 120724], 'mapped', [949]],
        [[120725, 120725], 'mapped', [950]],
        [[120726, 120726], 'mapped', [951]],
        [[120727, 120727], 'mapped', [952]],
        [[120728, 120728], 'mapped', [953]],
        [[120729, 120729], 'mapped', [954]],
        [[120730, 120730], 'mapped', [955]],
        [[120731, 120731], 'mapped', [956]],
        [[120732, 120732], 'mapped', [957]],
        [[120733, 120733], 'mapped', [958]],
        [[120734, 120734], 'mapped', [959]],
        [[120735, 120735], 'mapped', [960]],
        [[120736, 120736], 'mapped', [961]],
        [[120737, 120737], 'mapped', [952]],
        [[120738, 120738], 'mapped', [963]],
        [[120739, 120739], 'mapped', [964]],
        [[120740, 120740], 'mapped', [965]],
        [[120741, 120741], 'mapped', [966]],
        [[120742, 120742], 'mapped', [967]],
        [[120743, 120743], 'mapped', [968]],
        [[120744, 120744], 'mapped', [969]],
        [[120745, 120745], 'mapped', [8711]],
        [[120746, 120746], 'mapped', [945]],
        [[120747, 120747], 'mapped', [946]],
        [[120748, 120748], 'mapped', [947]],
        [[120749, 120749], 'mapped', [948]],
        [[120750, 120750], 'mapped', [949]],
        [[120751, 120751], 'mapped', [950]],
        [[120752, 120752], 'mapped', [951]],
        [[120753, 120753], 'mapped', [952]],
        [[120754, 120754], 'mapped', [953]],
        [[120755, 120755], 'mapped', [954]],
        [[120756, 120756], 'mapped', [955]],
        [[120757, 120757], 'mapped', [956]],
        [[120758, 120758], 'mapped', [957]],
        [[120759, 120759], 'mapped', [958]],
        [[120760, 120760], 'mapped', [959]],
        [[120761, 120761], 'mapped', [960]],
        [[120762, 120762], 'mapped', [961]],
        [[120763, 120764], 'mapped', [963]],
        [[120765, 120765], 'mapped', [964]],
        [[120766, 120766], 'mapped', [965]],
        [[120767, 120767], 'mapped', [966]],
        [[120768, 120768], 'mapped', [967]],
        [[120769, 120769], 'mapped', [968]],
        [[120770, 120770], 'mapped', [969]],
        [[120771, 120771], 'mapped', [8706]],
        [[120772, 120772], 'mapped', [949]],
        [[120773, 120773], 'mapped', [952]],
        [[120774, 120774], 'mapped', [954]],
        [[120775, 120775], 'mapped', [966]],
        [[120776, 120776], 'mapped', [961]],
        [[120777, 120777], 'mapped', [960]],
        [[120778, 120779], 'mapped', [989]],
        [[120780, 120781], 'disallowed'],
        [[120782, 120782], 'mapped', [48]],
        [[120783, 120783], 'mapped', [49]],
        [[120784, 120784], 'mapped', [50]],
        [[120785, 120785], 'mapped', [51]],
        [[120786, 120786], 'mapped', [52]],
        [[120787, 120787], 'mapped', [53]],
        [[120788, 120788], 'mapped', [54]],
        [[120789, 120789], 'mapped', [55]],
        [[120790, 120790], 'mapped', [56]],
        [[120791, 120791], 'mapped', [57]],
        [[120792, 120792], 'mapped', [48]],
        [[120793, 120793], 'mapped', [49]],
        [[120794, 120794], 'mapped', [50]],
        [[120795, 120795], 'mapped', [51]],
        [[120796, 120796], 'mapped', [52]],
        [[120797, 120797], 'mapped', [53]],
        [[120798, 120798], 'mapped', [54]],
        [[120799, 120799], 'mapped', [55]],
        [[120800, 120800], 'mapped', [56]],
        [[120801, 120801], 'mapped', [57]],
        [[120802, 120802], 'mapped', [48]],
        [[120803, 120803], 'mapped', [49]],
        [[120804, 120804], 'mapped', [50]],
        [[120805, 120805], 'mapped', [51]],
        [[120806, 120806], 'mapped', [52]],
        [[120807, 120807], 'mapped', [53]],
        [[120808, 120808], 'mapped', [54]],
        [[120809, 120809], 'mapped', [55]],
        [[120810, 120810], 'mapped', [56]],
        [[120811, 120811], 'mapped', [57]],
        [[120812, 120812], 'mapped', [48]],
        [[120813, 120813], 'mapped', [49]],
        [[120814, 120814], 'mapped', [50]],
        [[120815, 120815], 'mapped', [51]],
        [[120816, 120816], 'mapped', [52]],
        [[120817, 120817], 'mapped', [53]],
        [[120818, 120818], 'mapped', [54]],
        [[120819, 120819], 'mapped', [55]],
        [[120820, 120820], 'mapped', [56]],
        [[120821, 120821], 'mapped', [57]],
        [[120822, 120822], 'mapped', [48]],
        [[120823, 120823], 'mapped', [49]],
        [[120824, 120824], 'mapped', [50]],
        [[120825, 120825], 'mapped', [51]],
        [[120826, 120826], 'mapped', [52]],
        [[120827, 120827], 'mapped', [53]],
        [[120828, 120828], 'mapped', [54]],
        [[120829, 120829], 'mapped', [55]],
        [[120830, 120830], 'mapped', [56]],
        [[120831, 120831], 'mapped', [57]],
        [[120832, 121343], 'valid', [], 'NV8'],
        [[121344, 121398], 'valid'],
        [[121399, 121402], 'valid', [], 'NV8'],
        [[121403, 121452], 'valid'],
        [[121453, 121460], 'valid', [], 'NV8'],
        [[121461, 121461], 'valid'],
        [[121462, 121475], 'valid', [], 'NV8'],
        [[121476, 121476], 'valid'],
        [[121477, 121483], 'valid', [], 'NV8'],
        [[121484, 121498], 'disallowed'],
        [[121499, 121503], 'valid'],
        [[121504, 121504], 'disallowed'],
        [[121505, 121519], 'valid'],
        [[121520, 124927], 'disallowed'],
        [[124928, 125124], 'valid'],
        [[125125, 125126], 'disallowed'],
        [[125127, 125135], 'valid', [], 'NV8'],
        [[125136, 125142], 'valid'],
        [[125143, 126463], 'disallowed'],
        [[126464, 126464], 'mapped', [1575]],
        [[126465, 126465], 'mapped', [1576]],
        [[126466, 126466], 'mapped', [1580]],
        [[126467, 126467], 'mapped', [1583]],
        [[126468, 126468], 'disallowed'],
        [[126469, 126469], 'mapped', [1608]],
        [[126470, 126470], 'mapped', [1586]],
        [[126471, 126471], 'mapped', [1581]],
        [[126472, 126472], 'mapped', [1591]],
        [[126473, 126473], 'mapped', [1610]],
        [[126474, 126474], 'mapped', [1603]],
        [[126475, 126475], 'mapped', [1604]],
        [[126476, 126476], 'mapped', [1605]],
        [[126477, 126477], 'mapped', [1606]],
        [[126478, 126478], 'mapped', [1587]],
        [[126479, 126479], 'mapped', [1593]],
        [[126480, 126480], 'mapped', [1601]],
        [[126481, 126481], 'mapped', [1589]],
        [[126482, 126482], 'mapped', [1602]],
        [[126483, 126483], 'mapped', [1585]],
        [[126484, 126484], 'mapped', [1588]],
        [[126485, 126485], 'mapped', [1578]],
        [[126486, 126486], 'mapped', [1579]],
        [[126487, 126487], 'mapped', [1582]],
        [[126488, 126488], 'mapped', [1584]],
        [[126489, 126489], 'mapped', [1590]],
        [[126490, 126490], 'mapped', [1592]],
        [[126491, 126491], 'mapped', [1594]],
        [[126492, 126492], 'mapped', [1646]],
        [[126493, 126493], 'mapped', [1722]],
        [[126494, 126494], 'mapped', [1697]],
        [[126495, 126495], 'mapped', [1647]],
        [[126496, 126496], 'disallowed'],
        [[126497, 126497], 'mapped', [1576]],
        [[126498, 126498], 'mapped', [1580]],
        [[126499, 126499], 'disallowed'],
        [[126500, 126500], 'mapped', [1607]],
        [[126501, 126502], 'disallowed'],
        [[126503, 126503], 'mapped', [1581]],
        [[126504, 126504], 'disallowed'],
        [[126505, 126505], 'mapped', [1610]],
        [[126506, 126506], 'mapped', [1603]],
        [[126507, 126507], 'mapped', [1604]],
        [[126508, 126508], 'mapped', [1605]],
        [[126509, 126509], 'mapped', [1606]],
        [[126510, 126510], 'mapped', [1587]],
        [[126511, 126511], 'mapped', [1593]],
        [[126512, 126512], 'mapped', [1601]],
        [[126513, 126513], 'mapped', [1589]],
        [[126514, 126514], 'mapped', [1602]],
        [[126515, 126515], 'disallowed'],
        [[126516, 126516], 'mapped', [1588]],
        [[126517, 126517], 'mapped', [1578]],
        [[126518, 126518], 'mapped', [1579]],
        [[126519, 126519], 'mapped', [1582]],
        [[126520, 126520], 'disallowed'],
        [[126521, 126521], 'mapped', [1590]],
        [[126522, 126522], 'disallowed'],
        [[126523, 126523], 'mapped', [1594]],
        [[126524, 126529], 'disallowed'],
        [[126530, 126530], 'mapped', [1580]],
        [[126531, 126534], 'disallowed'],
        [[126535, 126535], 'mapped', [1581]],
        [[126536, 126536], 'disallowed'],
        [[126537, 126537], 'mapped', [1610]],
        [[126538, 126538], 'disallowed'],
        [[126539, 126539], 'mapped', [1604]],
        [[126540, 126540], 'disallowed'],
        [[126541, 126541], 'mapped', [1606]],
        [[126542, 126542], 'mapped', [1587]],
        [[126543, 126543], 'mapped', [1593]],
        [[126544, 126544], 'disallowed'],
        [[126545, 126545], 'mapped', [1589]],
        [[126546, 126546], 'mapped', [1602]],
        [[126547, 126547], 'disallowed'],
        [[126548, 126548], 'mapped', [1588]],
        [[126549, 126550], 'disallowed'],
        [[126551, 126551], 'mapped', [1582]],
        [[126552, 126552], 'disallowed'],
        [[126553, 126553], 'mapped', [1590]],
        [[126554, 126554], 'disallowed'],
        [[126555, 126555], 'mapped', [1594]],
        [[126556, 126556], 'disallowed'],
        [[126557, 126557], 'mapped', [1722]],
        [[126558, 126558], 'disallowed'],
        [[126559, 126559], 'mapped', [1647]],
        [[126560, 126560], 'disallowed'],
        [[126561, 126561], 'mapped', [1576]],
        [[126562, 126562], 'mapped', [1580]],
        [[126563, 126563], 'disallowed'],
        [[126564, 126564], 'mapped', [1607]],
        [[126565, 126566], 'disallowed'],
        [[126567, 126567], 'mapped', [1581]],
        [[126568, 126568], 'mapped', [1591]],
        [[126569, 126569], 'mapped', [1610]],
        [[126570, 126570], 'mapped', [1603]],
        [[126571, 126571], 'disallowed'],
        [[126572, 126572], 'mapped', [1605]],
        [[126573, 126573], 'mapped', [1606]],
        [[126574, 126574], 'mapped', [1587]],
        [[126575, 126575], 'mapped', [1593]],
        [[126576, 126576], 'mapped', [1601]],
        [[126577, 126577], 'mapped', [1589]],
        [[126578, 126578], 'mapped', [1602]],
        [[126579, 126579], 'disallowed'],
        [[126580, 126580], 'mapped', [1588]],
        [[126581, 126581], 'mapped', [1578]],
        [[126582, 126582], 'mapped', [1579]],
        [[126583, 126583], 'mapped', [1582]],
        [[126584, 126584], 'disallowed'],
        [[126585, 126585], 'mapped', [1590]],
        [[126586, 126586], 'mapped', [1592]],
        [[126587, 126587], 'mapped', [1594]],
        [[126588, 126588], 'mapped', [1646]],
        [[126589, 126589], 'disallowed'],
        [[126590, 126590], 'mapped', [1697]],
        [[126591, 126591], 'disallowed'],
        [[126592, 126592], 'mapped', [1575]],
        [[126593, 126593], 'mapped', [1576]],
        [[126594, 126594], 'mapped', [1580]],
        [[126595, 126595], 'mapped', [1583]],
        [[126596, 126596], 'mapped', [1607]],
        [[126597, 126597], 'mapped', [1608]],
        [[126598, 126598], 'mapped', [1586]],
        [[126599, 126599], 'mapped', [1581]],
        [[126600, 126600], 'mapped', [1591]],
        [[126601, 126601], 'mapped', [1610]],
        [[126602, 126602], 'disallowed'],
        [[126603, 126603], 'mapped', [1604]],
        [[126604, 126604], 'mapped', [1605]],
        [[126605, 126605], 'mapped', [1606]],
        [[126606, 126606], 'mapped', [1587]],
        [[126607, 126607], 'mapped', [1593]],
        [[126608, 126608], 'mapped', [1601]],
        [[126609, 126609], 'mapped', [1589]],
        [[126610, 126610], 'mapped', [1602]],
        [[126611, 126611], 'mapped', [1585]],
        [[126612, 126612], 'mapped', [1588]],
        [[126613, 126613], 'mapped', [1578]],
        [[126614, 126614], 'mapped', [1579]],
        [[126615, 126615], 'mapped', [1582]],
        [[126616, 126616], 'mapped', [1584]],
        [[126617, 126617], 'mapped', [1590]],
        [[126618, 126618], 'mapped', [1592]],
        [[126619, 126619], 'mapped', [1594]],
        [[126620, 126624], 'disallowed'],
        [[126625, 126625], 'mapped', [1576]],
        [[126626, 126626], 'mapped', [1580]],
        [[126627, 126627], 'mapped', [1583]],
        [[126628, 126628], 'disallowed'],
        [[126629, 126629], 'mapped', [1608]],
        [[126630, 126630], 'mapped', [1586]],
        [[126631, 126631], 'mapped', [1581]],
        [[126632, 126632], 'mapped', [1591]],
        [[126633, 126633], 'mapped', [1610]],
        [[126634, 126634], 'disallowed'],
        [[126635, 126635], 'mapped', [1604]],
        [[126636, 126636], 'mapped', [1605]],
        [[126637, 126637], 'mapped', [1606]],
        [[126638, 126638], 'mapped', [1587]],
        [[126639, 126639], 'mapped', [1593]],
        [[126640, 126640], 'mapped', [1601]],
        [[126641, 126641], 'mapped', [1589]],
        [[126642, 126642], 'mapped', [1602]],
        [[126643, 126643], 'mapped', [1585]],
        [[126644, 126644], 'mapped', [1588]],
        [[126645, 126645], 'mapped', [1578]],
        [[126646, 126646], 'mapped', [1579]],
        [[126647, 126647], 'mapped', [1582]],
        [[126648, 126648], 'mapped', [1584]],
        [[126649, 126649], 'mapped', [1590]],
        [[126650, 126650], 'mapped', [1592]],
        [[126651, 126651], 'mapped', [1594]],
        [[126652, 126703], 'disallowed'],
        [[126704, 126705], 'valid', [], 'NV8'],
        [[126706, 126975], 'disallowed'],
        [[126976, 127019], 'valid', [], 'NV8'],
        [[127020, 127023], 'disallowed'],
        [[127024, 127123], 'valid', [], 'NV8'],
        [[127124, 127135], 'disallowed'],
        [[127136, 127150], 'valid', [], 'NV8'],
        [[127151, 127152], 'disallowed'],
        [[127153, 127166], 'valid', [], 'NV8'],
        [[127167, 127167], 'valid', [], 'NV8'],
        [[127168, 127168], 'disallowed'],
        [[127169, 127183], 'valid', [], 'NV8'],
        [[127184, 127184], 'disallowed'],
        [[127185, 127199], 'valid', [], 'NV8'],
        [[127200, 127221], 'valid', [], 'NV8'],
        [[127222, 127231], 'disallowed'],
        [[127232, 127232], 'disallowed'],
        [[127233, 127233], 'disallowed_STD3_mapped', [48, 44]],
        [[127234, 127234], 'disallowed_STD3_mapped', [49, 44]],
        [[127235, 127235], 'disallowed_STD3_mapped', [50, 44]],
        [[127236, 127236], 'disallowed_STD3_mapped', [51, 44]],
        [[127237, 127237], 'disallowed_STD3_mapped', [52, 44]],
        [[127238, 127238], 'disallowed_STD3_mapped', [53, 44]],
        [[127239, 127239], 'disallowed_STD3_mapped', [54, 44]],
        [[127240, 127240], 'disallowed_STD3_mapped', [55, 44]],
        [[127241, 127241], 'disallowed_STD3_mapped', [56, 44]],
        [[127242, 127242], 'disallowed_STD3_mapped', [57, 44]],
        [[127243, 127244], 'valid', [], 'NV8'],
        [[127245, 127247], 'disallowed'],
        [[127248, 127248], 'disallowed_STD3_mapped', [40, 97, 41]],
        [[127249, 127249], 'disallowed_STD3_mapped', [40, 98, 41]],
        [[127250, 127250], 'disallowed_STD3_mapped', [40, 99, 41]],
        [[127251, 127251], 'disallowed_STD3_mapped', [40, 100, 41]],
        [[127252, 127252], 'disallowed_STD3_mapped', [40, 101, 41]],
        [[127253, 127253], 'disallowed_STD3_mapped', [40, 102, 41]],
        [[127254, 127254], 'disallowed_STD3_mapped', [40, 103, 41]],
        [[127255, 127255], 'disallowed_STD3_mapped', [40, 104, 41]],
        [[127256, 127256], 'disallowed_STD3_mapped', [40, 105, 41]],
        [[127257, 127257], 'disallowed_STD3_mapped', [40, 106, 41]],
        [[127258, 127258], 'disallowed_STD3_mapped', [40, 107, 41]],
        [[127259, 127259], 'disallowed_STD3_mapped', [40, 108, 41]],
        [[127260, 127260], 'disallowed_STD3_mapped', [40, 109, 41]],
        [[127261, 127261], 'disallowed_STD3_mapped', [40, 110, 41]],
        [[127262, 127262], 'disallowed_STD3_mapped', [40, 111, 41]],
        [[127263, 127263], 'disallowed_STD3_mapped', [40, 112, 41]],
        [[127264, 127264], 'disallowed_STD3_mapped', [40, 113, 41]],
        [[127265, 127265], 'disallowed_STD3_mapped', [40, 114, 41]],
        [[127266, 127266], 'disallowed_STD3_mapped', [40, 115, 41]],
        [[127267, 127267], 'disallowed_STD3_mapped', [40, 116, 41]],
        [[127268, 127268], 'disallowed_STD3_mapped', [40, 117, 41]],
        [[127269, 127269], 'disallowed_STD3_mapped', [40, 118, 41]],
        [[127270, 127270], 'disallowed_STD3_mapped', [40, 119, 41]],
        [[127271, 127271], 'disallowed_STD3_mapped', [40, 120, 41]],
        [[127272, 127272], 'disallowed_STD3_mapped', [40, 121, 41]],
        [[127273, 127273], 'disallowed_STD3_mapped', [40, 122, 41]],
        [[127274, 127274], 'mapped', [12308, 115, 12309]],
        [[127275, 127275], 'mapped', [99]],
        [[127276, 127276], 'mapped', [114]],
        [[127277, 127277], 'mapped', [99, 100]],
        [[127278, 127278], 'mapped', [119, 122]],
        [[127279, 127279], 'disallowed'],
        [[127280, 127280], 'mapped', [97]],
        [[127281, 127281], 'mapped', [98]],
        [[127282, 127282], 'mapped', [99]],
        [[127283, 127283], 'mapped', [100]],
        [[127284, 127284], 'mapped', [101]],
        [[127285, 127285], 'mapped', [102]],
        [[127286, 127286], 'mapped', [103]],
        [[127287, 127287], 'mapped', [104]],
        [[127288, 127288], 'mapped', [105]],
        [[127289, 127289], 'mapped', [106]],
        [[127290, 127290], 'mapped', [107]],
        [[127291, 127291], 'mapped', [108]],
        [[127292, 127292], 'mapped', [109]],
        [[127293, 127293], 'mapped', [110]],
        [[127294, 127294], 'mapped', [111]],
        [[127295, 127295], 'mapped', [112]],
        [[127296, 127296], 'mapped', [113]],
        [[127297, 127297], 'mapped', [114]],
        [[127298, 127298], 'mapped', [115]],
        [[127299, 127299], 'mapped', [116]],
        [[127300, 127300], 'mapped', [117]],
        [[127301, 127301], 'mapped', [118]],
        [[127302, 127302], 'mapped', [119]],
        [[127303, 127303], 'mapped', [120]],
        [[127304, 127304], 'mapped', [121]],
        [[127305, 127305], 'mapped', [122]],
        [[127306, 127306], 'mapped', [104, 118]],
        [[127307, 127307], 'mapped', [109, 118]],
        [[127308, 127308], 'mapped', [115, 100]],
        [[127309, 127309], 'mapped', [115, 115]],
        [[127310, 127310], 'mapped', [112, 112, 118]],
        [[127311, 127311], 'mapped', [119, 99]],
        [[127312, 127318], 'valid', [], 'NV8'],
        [[127319, 127319], 'valid', [], 'NV8'],
        [[127320, 127326], 'valid', [], 'NV8'],
        [[127327, 127327], 'valid', [], 'NV8'],
        [[127328, 127337], 'valid', [], 'NV8'],
        [[127338, 127338], 'mapped', [109, 99]],
        [[127339, 127339], 'mapped', [109, 100]],
        [[127340, 127343], 'disallowed'],
        [[127344, 127352], 'valid', [], 'NV8'],
        [[127353, 127353], 'valid', [], 'NV8'],
        [[127354, 127354], 'valid', [], 'NV8'],
        [[127355, 127356], 'valid', [], 'NV8'],
        [[127357, 127358], 'valid', [], 'NV8'],
        [[127359, 127359], 'valid', [], 'NV8'],
        [[127360, 127369], 'valid', [], 'NV8'],
        [[127370, 127373], 'valid', [], 'NV8'],
        [[127374, 127375], 'valid', [], 'NV8'],
        [[127376, 127376], 'mapped', [100, 106]],
        [[127377, 127386], 'valid', [], 'NV8'],
        [[127387, 127461], 'disallowed'],
        [[127462, 127487], 'valid', [], 'NV8'],
        [[127488, 127488], 'mapped', [12411, 12363]],
        [[127489, 127489], 'mapped', [12467, 12467]],
        [[127490, 127490], 'mapped', [12469]],
        [[127491, 127503], 'disallowed'],
        [[127504, 127504], 'mapped', [25163]],
        [[127505, 127505], 'mapped', [23383]],
        [[127506, 127506], 'mapped', [21452]],
        [[127507, 127507], 'mapped', [12487]],
        [[127508, 127508], 'mapped', [20108]],
        [[127509, 127509], 'mapped', [22810]],
        [[127510, 127510], 'mapped', [35299]],
        [[127511, 127511], 'mapped', [22825]],
        [[127512, 127512], 'mapped', [20132]],
        [[127513, 127513], 'mapped', [26144]],
        [[127514, 127514], 'mapped', [28961]],
        [[127515, 127515], 'mapped', [26009]],
        [[127516, 127516], 'mapped', [21069]],
        [[127517, 127517], 'mapped', [24460]],
        [[127518, 127518], 'mapped', [20877]],
        [[127519, 127519], 'mapped', [26032]],
        [[127520, 127520], 'mapped', [21021]],
        [[127521, 127521], 'mapped', [32066]],
        [[127522, 127522], 'mapped', [29983]],
        [[127523, 127523], 'mapped', [36009]],
        [[127524, 127524], 'mapped', [22768]],
        [[127525, 127525], 'mapped', [21561]],
        [[127526, 127526], 'mapped', [28436]],
        [[127527, 127527], 'mapped', [25237]],
        [[127528, 127528], 'mapped', [25429]],
        [[127529, 127529], 'mapped', [19968]],
        [[127530, 127530], 'mapped', [19977]],
        [[127531, 127531], 'mapped', [36938]],
        [[127532, 127532], 'mapped', [24038]],
        [[127533, 127533], 'mapped', [20013]],
        [[127534, 127534], 'mapped', [21491]],
        [[127535, 127535], 'mapped', [25351]],
        [[127536, 127536], 'mapped', [36208]],
        [[127537, 127537], 'mapped', [25171]],
        [[127538, 127538], 'mapped', [31105]],
        [[127539, 127539], 'mapped', [31354]],
        [[127540, 127540], 'mapped', [21512]],
        [[127541, 127541], 'mapped', [28288]],
        [[127542, 127542], 'mapped', [26377]],
        [[127543, 127543], 'mapped', [26376]],
        [[127544, 127544], 'mapped', [30003]],
        [[127545, 127545], 'mapped', [21106]],
        [[127546, 127546], 'mapped', [21942]],
        [[127547, 127551], 'disallowed'],
        [[127552, 127552], 'mapped', [12308, 26412, 12309]],
        [[127553, 127553], 'mapped', [12308, 19977, 12309]],
        [[127554, 127554], 'mapped', [12308, 20108, 12309]],
        [[127555, 127555], 'mapped', [12308, 23433, 12309]],
        [[127556, 127556], 'mapped', [12308, 28857, 12309]],
        [[127557, 127557], 'mapped', [12308, 25171, 12309]],
        [[127558, 127558], 'mapped', [12308, 30423, 12309]],
        [[127559, 127559], 'mapped', [12308, 21213, 12309]],
        [[127560, 127560], 'mapped', [12308, 25943, 12309]],
        [[127561, 127567], 'disallowed'],
        [[127568, 127568], 'mapped', [24471]],
        [[127569, 127569], 'mapped', [21487]],
        [[127570, 127743], 'disallowed'],
        [[127744, 127776], 'valid', [], 'NV8'],
        [[127777, 127788], 'valid', [], 'NV8'],
        [[127789, 127791], 'valid', [], 'NV8'],
        [[127792, 127797], 'valid', [], 'NV8'],
        [[127798, 127798], 'valid', [], 'NV8'],
        [[127799, 127868], 'valid', [], 'NV8'],
        [[127869, 127869], 'valid', [], 'NV8'],
        [[127870, 127871], 'valid', [], 'NV8'],
        [[127872, 127891], 'valid', [], 'NV8'],
        [[127892, 127903], 'valid', [], 'NV8'],
        [[127904, 127940], 'valid', [], 'NV8'],
        [[127941, 127941], 'valid', [], 'NV8'],
        [[127942, 127946], 'valid', [], 'NV8'],
        [[127947, 127950], 'valid', [], 'NV8'],
        [[127951, 127955], 'valid', [], 'NV8'],
        [[127956, 127967], 'valid', [], 'NV8'],
        [[127968, 127984], 'valid', [], 'NV8'],
        [[127985, 127991], 'valid', [], 'NV8'],
        [[127992, 127999], 'valid', [], 'NV8'],
        [[128e3, 128062], 'valid', [], 'NV8'],
        [[128063, 128063], 'valid', [], 'NV8'],
        [[128064, 128064], 'valid', [], 'NV8'],
        [[128065, 128065], 'valid', [], 'NV8'],
        [[128066, 128247], 'valid', [], 'NV8'],
        [[128248, 128248], 'valid', [], 'NV8'],
        [[128249, 128252], 'valid', [], 'NV8'],
        [[128253, 128254], 'valid', [], 'NV8'],
        [[128255, 128255], 'valid', [], 'NV8'],
        [[128256, 128317], 'valid', [], 'NV8'],
        [[128318, 128319], 'valid', [], 'NV8'],
        [[128320, 128323], 'valid', [], 'NV8'],
        [[128324, 128330], 'valid', [], 'NV8'],
        [[128331, 128335], 'valid', [], 'NV8'],
        [[128336, 128359], 'valid', [], 'NV8'],
        [[128360, 128377], 'valid', [], 'NV8'],
        [[128378, 128378], 'disallowed'],
        [[128379, 128419], 'valid', [], 'NV8'],
        [[128420, 128420], 'disallowed'],
        [[128421, 128506], 'valid', [], 'NV8'],
        [[128507, 128511], 'valid', [], 'NV8'],
        [[128512, 128512], 'valid', [], 'NV8'],
        [[128513, 128528], 'valid', [], 'NV8'],
        [[128529, 128529], 'valid', [], 'NV8'],
        [[128530, 128532], 'valid', [], 'NV8'],
        [[128533, 128533], 'valid', [], 'NV8'],
        [[128534, 128534], 'valid', [], 'NV8'],
        [[128535, 128535], 'valid', [], 'NV8'],
        [[128536, 128536], 'valid', [], 'NV8'],
        [[128537, 128537], 'valid', [], 'NV8'],
        [[128538, 128538], 'valid', [], 'NV8'],
        [[128539, 128539], 'valid', [], 'NV8'],
        [[128540, 128542], 'valid', [], 'NV8'],
        [[128543, 128543], 'valid', [], 'NV8'],
        [[128544, 128549], 'valid', [], 'NV8'],
        [[128550, 128551], 'valid', [], 'NV8'],
        [[128552, 128555], 'valid', [], 'NV8'],
        [[128556, 128556], 'valid', [], 'NV8'],
        [[128557, 128557], 'valid', [], 'NV8'],
        [[128558, 128559], 'valid', [], 'NV8'],
        [[128560, 128563], 'valid', [], 'NV8'],
        [[128564, 128564], 'valid', [], 'NV8'],
        [[128565, 128576], 'valid', [], 'NV8'],
        [[128577, 128578], 'valid', [], 'NV8'],
        [[128579, 128580], 'valid', [], 'NV8'],
        [[128581, 128591], 'valid', [], 'NV8'],
        [[128592, 128639], 'valid', [], 'NV8'],
        [[128640, 128709], 'valid', [], 'NV8'],
        [[128710, 128719], 'valid', [], 'NV8'],
        [[128720, 128720], 'valid', [], 'NV8'],
        [[128721, 128735], 'disallowed'],
        [[128736, 128748], 'valid', [], 'NV8'],
        [[128749, 128751], 'disallowed'],
        [[128752, 128755], 'valid', [], 'NV8'],
        [[128756, 128767], 'disallowed'],
        [[128768, 128883], 'valid', [], 'NV8'],
        [[128884, 128895], 'disallowed'],
        [[128896, 128980], 'valid', [], 'NV8'],
        [[128981, 129023], 'disallowed'],
        [[129024, 129035], 'valid', [], 'NV8'],
        [[129036, 129039], 'disallowed'],
        [[129040, 129095], 'valid', [], 'NV8'],
        [[129096, 129103], 'disallowed'],
        [[129104, 129113], 'valid', [], 'NV8'],
        [[129114, 129119], 'disallowed'],
        [[129120, 129159], 'valid', [], 'NV8'],
        [[129160, 129167], 'disallowed'],
        [[129168, 129197], 'valid', [], 'NV8'],
        [[129198, 129295], 'disallowed'],
        [[129296, 129304], 'valid', [], 'NV8'],
        [[129305, 129407], 'disallowed'],
        [[129408, 129412], 'valid', [], 'NV8'],
        [[129413, 129471], 'disallowed'],
        [[129472, 129472], 'valid', [], 'NV8'],
        [[129473, 131069], 'disallowed'],
        [[131070, 131071], 'disallowed'],
        [[131072, 173782], 'valid'],
        [[173783, 173823], 'disallowed'],
        [[173824, 177972], 'valid'],
        [[177973, 177983], 'disallowed'],
        [[177984, 178205], 'valid'],
        [[178206, 178207], 'disallowed'],
        [[178208, 183969], 'valid'],
        [[183970, 194559], 'disallowed'],
        [[194560, 194560], 'mapped', [20029]],
        [[194561, 194561], 'mapped', [20024]],
        [[194562, 194562], 'mapped', [20033]],
        [[194563, 194563], 'mapped', [131362]],
        [[194564, 194564], 'mapped', [20320]],
        [[194565, 194565], 'mapped', [20398]],
        [[194566, 194566], 'mapped', [20411]],
        [[194567, 194567], 'mapped', [20482]],
        [[194568, 194568], 'mapped', [20602]],
        [[194569, 194569], 'mapped', [20633]],
        [[194570, 194570], 'mapped', [20711]],
        [[194571, 194571], 'mapped', [20687]],
        [[194572, 194572], 'mapped', [13470]],
        [[194573, 194573], 'mapped', [132666]],
        [[194574, 194574], 'mapped', [20813]],
        [[194575, 194575], 'mapped', [20820]],
        [[194576, 194576], 'mapped', [20836]],
        [[194577, 194577], 'mapped', [20855]],
        [[194578, 194578], 'mapped', [132380]],
        [[194579, 194579], 'mapped', [13497]],
        [[194580, 194580], 'mapped', [20839]],
        [[194581, 194581], 'mapped', [20877]],
        [[194582, 194582], 'mapped', [132427]],
        [[194583, 194583], 'mapped', [20887]],
        [[194584, 194584], 'mapped', [20900]],
        [[194585, 194585], 'mapped', [20172]],
        [[194586, 194586], 'mapped', [20908]],
        [[194587, 194587], 'mapped', [20917]],
        [[194588, 194588], 'mapped', [168415]],
        [[194589, 194589], 'mapped', [20981]],
        [[194590, 194590], 'mapped', [20995]],
        [[194591, 194591], 'mapped', [13535]],
        [[194592, 194592], 'mapped', [21051]],
        [[194593, 194593], 'mapped', [21062]],
        [[194594, 194594], 'mapped', [21106]],
        [[194595, 194595], 'mapped', [21111]],
        [[194596, 194596], 'mapped', [13589]],
        [[194597, 194597], 'mapped', [21191]],
        [[194598, 194598], 'mapped', [21193]],
        [[194599, 194599], 'mapped', [21220]],
        [[194600, 194600], 'mapped', [21242]],
        [[194601, 194601], 'mapped', [21253]],
        [[194602, 194602], 'mapped', [21254]],
        [[194603, 194603], 'mapped', [21271]],
        [[194604, 194604], 'mapped', [21321]],
        [[194605, 194605], 'mapped', [21329]],
        [[194606, 194606], 'mapped', [21338]],
        [[194607, 194607], 'mapped', [21363]],
        [[194608, 194608], 'mapped', [21373]],
        [[194609, 194611], 'mapped', [21375]],
        [[194612, 194612], 'mapped', [133676]],
        [[194613, 194613], 'mapped', [28784]],
        [[194614, 194614], 'mapped', [21450]],
        [[194615, 194615], 'mapped', [21471]],
        [[194616, 194616], 'mapped', [133987]],
        [[194617, 194617], 'mapped', [21483]],
        [[194618, 194618], 'mapped', [21489]],
        [[194619, 194619], 'mapped', [21510]],
        [[194620, 194620], 'mapped', [21662]],
        [[194621, 194621], 'mapped', [21560]],
        [[194622, 194622], 'mapped', [21576]],
        [[194623, 194623], 'mapped', [21608]],
        [[194624, 194624], 'mapped', [21666]],
        [[194625, 194625], 'mapped', [21750]],
        [[194626, 194626], 'mapped', [21776]],
        [[194627, 194627], 'mapped', [21843]],
        [[194628, 194628], 'mapped', [21859]],
        [[194629, 194630], 'mapped', [21892]],
        [[194631, 194631], 'mapped', [21913]],
        [[194632, 194632], 'mapped', [21931]],
        [[194633, 194633], 'mapped', [21939]],
        [[194634, 194634], 'mapped', [21954]],
        [[194635, 194635], 'mapped', [22294]],
        [[194636, 194636], 'mapped', [22022]],
        [[194637, 194637], 'mapped', [22295]],
        [[194638, 194638], 'mapped', [22097]],
        [[194639, 194639], 'mapped', [22132]],
        [[194640, 194640], 'mapped', [20999]],
        [[194641, 194641], 'mapped', [22766]],
        [[194642, 194642], 'mapped', [22478]],
        [[194643, 194643], 'mapped', [22516]],
        [[194644, 194644], 'mapped', [22541]],
        [[194645, 194645], 'mapped', [22411]],
        [[194646, 194646], 'mapped', [22578]],
        [[194647, 194647], 'mapped', [22577]],
        [[194648, 194648], 'mapped', [22700]],
        [[194649, 194649], 'mapped', [136420]],
        [[194650, 194650], 'mapped', [22770]],
        [[194651, 194651], 'mapped', [22775]],
        [[194652, 194652], 'mapped', [22790]],
        [[194653, 194653], 'mapped', [22810]],
        [[194654, 194654], 'mapped', [22818]],
        [[194655, 194655], 'mapped', [22882]],
        [[194656, 194656], 'mapped', [136872]],
        [[194657, 194657], 'mapped', [136938]],
        [[194658, 194658], 'mapped', [23020]],
        [[194659, 194659], 'mapped', [23067]],
        [[194660, 194660], 'mapped', [23079]],
        [[194661, 194661], 'mapped', [23e3]],
        [[194662, 194662], 'mapped', [23142]],
        [[194663, 194663], 'mapped', [14062]],
        [[194664, 194664], 'disallowed'],
        [[194665, 194665], 'mapped', [23304]],
        [[194666, 194667], 'mapped', [23358]],
        [[194668, 194668], 'mapped', [137672]],
        [[194669, 194669], 'mapped', [23491]],
        [[194670, 194670], 'mapped', [23512]],
        [[194671, 194671], 'mapped', [23527]],
        [[194672, 194672], 'mapped', [23539]],
        [[194673, 194673], 'mapped', [138008]],
        [[194674, 194674], 'mapped', [23551]],
        [[194675, 194675], 'mapped', [23558]],
        [[194676, 194676], 'disallowed'],
        [[194677, 194677], 'mapped', [23586]],
        [[194678, 194678], 'mapped', [14209]],
        [[194679, 194679], 'mapped', [23648]],
        [[194680, 194680], 'mapped', [23662]],
        [[194681, 194681], 'mapped', [23744]],
        [[194682, 194682], 'mapped', [23693]],
        [[194683, 194683], 'mapped', [138724]],
        [[194684, 194684], 'mapped', [23875]],
        [[194685, 194685], 'mapped', [138726]],
        [[194686, 194686], 'mapped', [23918]],
        [[194687, 194687], 'mapped', [23915]],
        [[194688, 194688], 'mapped', [23932]],
        [[194689, 194689], 'mapped', [24033]],
        [[194690, 194690], 'mapped', [24034]],
        [[194691, 194691], 'mapped', [14383]],
        [[194692, 194692], 'mapped', [24061]],
        [[194693, 194693], 'mapped', [24104]],
        [[194694, 194694], 'mapped', [24125]],
        [[194695, 194695], 'mapped', [24169]],
        [[194696, 194696], 'mapped', [14434]],
        [[194697, 194697], 'mapped', [139651]],
        [[194698, 194698], 'mapped', [14460]],
        [[194699, 194699], 'mapped', [24240]],
        [[194700, 194700], 'mapped', [24243]],
        [[194701, 194701], 'mapped', [24246]],
        [[194702, 194702], 'mapped', [24266]],
        [[194703, 194703], 'mapped', [172946]],
        [[194704, 194704], 'mapped', [24318]],
        [[194705, 194706], 'mapped', [140081]],
        [[194707, 194707], 'mapped', [33281]],
        [[194708, 194709], 'mapped', [24354]],
        [[194710, 194710], 'mapped', [14535]],
        [[194711, 194711], 'mapped', [144056]],
        [[194712, 194712], 'mapped', [156122]],
        [[194713, 194713], 'mapped', [24418]],
        [[194714, 194714], 'mapped', [24427]],
        [[194715, 194715], 'mapped', [14563]],
        [[194716, 194716], 'mapped', [24474]],
        [[194717, 194717], 'mapped', [24525]],
        [[194718, 194718], 'mapped', [24535]],
        [[194719, 194719], 'mapped', [24569]],
        [[194720, 194720], 'mapped', [24705]],
        [[194721, 194721], 'mapped', [14650]],
        [[194722, 194722], 'mapped', [14620]],
        [[194723, 194723], 'mapped', [24724]],
        [[194724, 194724], 'mapped', [141012]],
        [[194725, 194725], 'mapped', [24775]],
        [[194726, 194726], 'mapped', [24904]],
        [[194727, 194727], 'mapped', [24908]],
        [[194728, 194728], 'mapped', [24910]],
        [[194729, 194729], 'mapped', [24908]],
        [[194730, 194730], 'mapped', [24954]],
        [[194731, 194731], 'mapped', [24974]],
        [[194732, 194732], 'mapped', [25010]],
        [[194733, 194733], 'mapped', [24996]],
        [[194734, 194734], 'mapped', [25007]],
        [[194735, 194735], 'mapped', [25054]],
        [[194736, 194736], 'mapped', [25074]],
        [[194737, 194737], 'mapped', [25078]],
        [[194738, 194738], 'mapped', [25104]],
        [[194739, 194739], 'mapped', [25115]],
        [[194740, 194740], 'mapped', [25181]],
        [[194741, 194741], 'mapped', [25265]],
        [[194742, 194742], 'mapped', [25300]],
        [[194743, 194743], 'mapped', [25424]],
        [[194744, 194744], 'mapped', [142092]],
        [[194745, 194745], 'mapped', [25405]],
        [[194746, 194746], 'mapped', [25340]],
        [[194747, 194747], 'mapped', [25448]],
        [[194748, 194748], 'mapped', [25475]],
        [[194749, 194749], 'mapped', [25572]],
        [[194750, 194750], 'mapped', [142321]],
        [[194751, 194751], 'mapped', [25634]],
        [[194752, 194752], 'mapped', [25541]],
        [[194753, 194753], 'mapped', [25513]],
        [[194754, 194754], 'mapped', [14894]],
        [[194755, 194755], 'mapped', [25705]],
        [[194756, 194756], 'mapped', [25726]],
        [[194757, 194757], 'mapped', [25757]],
        [[194758, 194758], 'mapped', [25719]],
        [[194759, 194759], 'mapped', [14956]],
        [[194760, 194760], 'mapped', [25935]],
        [[194761, 194761], 'mapped', [25964]],
        [[194762, 194762], 'mapped', [143370]],
        [[194763, 194763], 'mapped', [26083]],
        [[194764, 194764], 'mapped', [26360]],
        [[194765, 194765], 'mapped', [26185]],
        [[194766, 194766], 'mapped', [15129]],
        [[194767, 194767], 'mapped', [26257]],
        [[194768, 194768], 'mapped', [15112]],
        [[194769, 194769], 'mapped', [15076]],
        [[194770, 194770], 'mapped', [20882]],
        [[194771, 194771], 'mapped', [20885]],
        [[194772, 194772], 'mapped', [26368]],
        [[194773, 194773], 'mapped', [26268]],
        [[194774, 194774], 'mapped', [32941]],
        [[194775, 194775], 'mapped', [17369]],
        [[194776, 194776], 'mapped', [26391]],
        [[194777, 194777], 'mapped', [26395]],
        [[194778, 194778], 'mapped', [26401]],
        [[194779, 194779], 'mapped', [26462]],
        [[194780, 194780], 'mapped', [26451]],
        [[194781, 194781], 'mapped', [144323]],
        [[194782, 194782], 'mapped', [15177]],
        [[194783, 194783], 'mapped', [26618]],
        [[194784, 194784], 'mapped', [26501]],
        [[194785, 194785], 'mapped', [26706]],
        [[194786, 194786], 'mapped', [26757]],
        [[194787, 194787], 'mapped', [144493]],
        [[194788, 194788], 'mapped', [26766]],
        [[194789, 194789], 'mapped', [26655]],
        [[194790, 194790], 'mapped', [26900]],
        [[194791, 194791], 'mapped', [15261]],
        [[194792, 194792], 'mapped', [26946]],
        [[194793, 194793], 'mapped', [27043]],
        [[194794, 194794], 'mapped', [27114]],
        [[194795, 194795], 'mapped', [27304]],
        [[194796, 194796], 'mapped', [145059]],
        [[194797, 194797], 'mapped', [27355]],
        [[194798, 194798], 'mapped', [15384]],
        [[194799, 194799], 'mapped', [27425]],
        [[194800, 194800], 'mapped', [145575]],
        [[194801, 194801], 'mapped', [27476]],
        [[194802, 194802], 'mapped', [15438]],
        [[194803, 194803], 'mapped', [27506]],
        [[194804, 194804], 'mapped', [27551]],
        [[194805, 194805], 'mapped', [27578]],
        [[194806, 194806], 'mapped', [27579]],
        [[194807, 194807], 'mapped', [146061]],
        [[194808, 194808], 'mapped', [138507]],
        [[194809, 194809], 'mapped', [146170]],
        [[194810, 194810], 'mapped', [27726]],
        [[194811, 194811], 'mapped', [146620]],
        [[194812, 194812], 'mapped', [27839]],
        [[194813, 194813], 'mapped', [27853]],
        [[194814, 194814], 'mapped', [27751]],
        [[194815, 194815], 'mapped', [27926]],
        [[194816, 194816], 'mapped', [27966]],
        [[194817, 194817], 'mapped', [28023]],
        [[194818, 194818], 'mapped', [27969]],
        [[194819, 194819], 'mapped', [28009]],
        [[194820, 194820], 'mapped', [28024]],
        [[194821, 194821], 'mapped', [28037]],
        [[194822, 194822], 'mapped', [146718]],
        [[194823, 194823], 'mapped', [27956]],
        [[194824, 194824], 'mapped', [28207]],
        [[194825, 194825], 'mapped', [28270]],
        [[194826, 194826], 'mapped', [15667]],
        [[194827, 194827], 'mapped', [28363]],
        [[194828, 194828], 'mapped', [28359]],
        [[194829, 194829], 'mapped', [147153]],
        [[194830, 194830], 'mapped', [28153]],
        [[194831, 194831], 'mapped', [28526]],
        [[194832, 194832], 'mapped', [147294]],
        [[194833, 194833], 'mapped', [147342]],
        [[194834, 194834], 'mapped', [28614]],
        [[194835, 194835], 'mapped', [28729]],
        [[194836, 194836], 'mapped', [28702]],
        [[194837, 194837], 'mapped', [28699]],
        [[194838, 194838], 'mapped', [15766]],
        [[194839, 194839], 'mapped', [28746]],
        [[194840, 194840], 'mapped', [28797]],
        [[194841, 194841], 'mapped', [28791]],
        [[194842, 194842], 'mapped', [28845]],
        [[194843, 194843], 'mapped', [132389]],
        [[194844, 194844], 'mapped', [28997]],
        [[194845, 194845], 'mapped', [148067]],
        [[194846, 194846], 'mapped', [29084]],
        [[194847, 194847], 'disallowed'],
        [[194848, 194848], 'mapped', [29224]],
        [[194849, 194849], 'mapped', [29237]],
        [[194850, 194850], 'mapped', [29264]],
        [[194851, 194851], 'mapped', [149e3]],
        [[194852, 194852], 'mapped', [29312]],
        [[194853, 194853], 'mapped', [29333]],
        [[194854, 194854], 'mapped', [149301]],
        [[194855, 194855], 'mapped', [149524]],
        [[194856, 194856], 'mapped', [29562]],
        [[194857, 194857], 'mapped', [29579]],
        [[194858, 194858], 'mapped', [16044]],
        [[194859, 194859], 'mapped', [29605]],
        [[194860, 194861], 'mapped', [16056]],
        [[194862, 194862], 'mapped', [29767]],
        [[194863, 194863], 'mapped', [29788]],
        [[194864, 194864], 'mapped', [29809]],
        [[194865, 194865], 'mapped', [29829]],
        [[194866, 194866], 'mapped', [29898]],
        [[194867, 194867], 'mapped', [16155]],
        [[194868, 194868], 'mapped', [29988]],
        [[194869, 194869], 'mapped', [150582]],
        [[194870, 194870], 'mapped', [30014]],
        [[194871, 194871], 'mapped', [150674]],
        [[194872, 194872], 'mapped', [30064]],
        [[194873, 194873], 'mapped', [139679]],
        [[194874, 194874], 'mapped', [30224]],
        [[194875, 194875], 'mapped', [151457]],
        [[194876, 194876], 'mapped', [151480]],
        [[194877, 194877], 'mapped', [151620]],
        [[194878, 194878], 'mapped', [16380]],
        [[194879, 194879], 'mapped', [16392]],
        [[194880, 194880], 'mapped', [30452]],
        [[194881, 194881], 'mapped', [151795]],
        [[194882, 194882], 'mapped', [151794]],
        [[194883, 194883], 'mapped', [151833]],
        [[194884, 194884], 'mapped', [151859]],
        [[194885, 194885], 'mapped', [30494]],
        [[194886, 194887], 'mapped', [30495]],
        [[194888, 194888], 'mapped', [30538]],
        [[194889, 194889], 'mapped', [16441]],
        [[194890, 194890], 'mapped', [30603]],
        [[194891, 194891], 'mapped', [16454]],
        [[194892, 194892], 'mapped', [16534]],
        [[194893, 194893], 'mapped', [152605]],
        [[194894, 194894], 'mapped', [30798]],
        [[194895, 194895], 'mapped', [30860]],
        [[194896, 194896], 'mapped', [30924]],
        [[194897, 194897], 'mapped', [16611]],
        [[194898, 194898], 'mapped', [153126]],
        [[194899, 194899], 'mapped', [31062]],
        [[194900, 194900], 'mapped', [153242]],
        [[194901, 194901], 'mapped', [153285]],
        [[194902, 194902], 'mapped', [31119]],
        [[194903, 194903], 'mapped', [31211]],
        [[194904, 194904], 'mapped', [16687]],
        [[194905, 194905], 'mapped', [31296]],
        [[194906, 194906], 'mapped', [31306]],
        [[194907, 194907], 'mapped', [31311]],
        [[194908, 194908], 'mapped', [153980]],
        [[194909, 194910], 'mapped', [154279]],
        [[194911, 194911], 'disallowed'],
        [[194912, 194912], 'mapped', [16898]],
        [[194913, 194913], 'mapped', [154539]],
        [[194914, 194914], 'mapped', [31686]],
        [[194915, 194915], 'mapped', [31689]],
        [[194916, 194916], 'mapped', [16935]],
        [[194917, 194917], 'mapped', [154752]],
        [[194918, 194918], 'mapped', [31954]],
        [[194919, 194919], 'mapped', [17056]],
        [[194920, 194920], 'mapped', [31976]],
        [[194921, 194921], 'mapped', [31971]],
        [[194922, 194922], 'mapped', [32e3]],
        [[194923, 194923], 'mapped', [155526]],
        [[194924, 194924], 'mapped', [32099]],
        [[194925, 194925], 'mapped', [17153]],
        [[194926, 194926], 'mapped', [32199]],
        [[194927, 194927], 'mapped', [32258]],
        [[194928, 194928], 'mapped', [32325]],
        [[194929, 194929], 'mapped', [17204]],
        [[194930, 194930], 'mapped', [156200]],
        [[194931, 194931], 'mapped', [156231]],
        [[194932, 194932], 'mapped', [17241]],
        [[194933, 194933], 'mapped', [156377]],
        [[194934, 194934], 'mapped', [32634]],
        [[194935, 194935], 'mapped', [156478]],
        [[194936, 194936], 'mapped', [32661]],
        [[194937, 194937], 'mapped', [32762]],
        [[194938, 194938], 'mapped', [32773]],
        [[194939, 194939], 'mapped', [156890]],
        [[194940, 194940], 'mapped', [156963]],
        [[194941, 194941], 'mapped', [32864]],
        [[194942, 194942], 'mapped', [157096]],
        [[194943, 194943], 'mapped', [32880]],
        [[194944, 194944], 'mapped', [144223]],
        [[194945, 194945], 'mapped', [17365]],
        [[194946, 194946], 'mapped', [32946]],
        [[194947, 194947], 'mapped', [33027]],
        [[194948, 194948], 'mapped', [17419]],
        [[194949, 194949], 'mapped', [33086]],
        [[194950, 194950], 'mapped', [23221]],
        [[194951, 194951], 'mapped', [157607]],
        [[194952, 194952], 'mapped', [157621]],
        [[194953, 194953], 'mapped', [144275]],
        [[194954, 194954], 'mapped', [144284]],
        [[194955, 194955], 'mapped', [33281]],
        [[194956, 194956], 'mapped', [33284]],
        [[194957, 194957], 'mapped', [36766]],
        [[194958, 194958], 'mapped', [17515]],
        [[194959, 194959], 'mapped', [33425]],
        [[194960, 194960], 'mapped', [33419]],
        [[194961, 194961], 'mapped', [33437]],
        [[194962, 194962], 'mapped', [21171]],
        [[194963, 194963], 'mapped', [33457]],
        [[194964, 194964], 'mapped', [33459]],
        [[194965, 194965], 'mapped', [33469]],
        [[194966, 194966], 'mapped', [33510]],
        [[194967, 194967], 'mapped', [158524]],
        [[194968, 194968], 'mapped', [33509]],
        [[194969, 194969], 'mapped', [33565]],
        [[194970, 194970], 'mapped', [33635]],
        [[194971, 194971], 'mapped', [33709]],
        [[194972, 194972], 'mapped', [33571]],
        [[194973, 194973], 'mapped', [33725]],
        [[194974, 194974], 'mapped', [33767]],
        [[194975, 194975], 'mapped', [33879]],
        [[194976, 194976], 'mapped', [33619]],
        [[194977, 194977], 'mapped', [33738]],
        [[194978, 194978], 'mapped', [33740]],
        [[194979, 194979], 'mapped', [33756]],
        [[194980, 194980], 'mapped', [158774]],
        [[194981, 194981], 'mapped', [159083]],
        [[194982, 194982], 'mapped', [158933]],
        [[194983, 194983], 'mapped', [17707]],
        [[194984, 194984], 'mapped', [34033]],
        [[194985, 194985], 'mapped', [34035]],
        [[194986, 194986], 'mapped', [34070]],
        [[194987, 194987], 'mapped', [160714]],
        [[194988, 194988], 'mapped', [34148]],
        [[194989, 194989], 'mapped', [159532]],
        [[194990, 194990], 'mapped', [17757]],
        [[194991, 194991], 'mapped', [17761]],
        [[194992, 194992], 'mapped', [159665]],
        [[194993, 194993], 'mapped', [159954]],
        [[194994, 194994], 'mapped', [17771]],
        [[194995, 194995], 'mapped', [34384]],
        [[194996, 194996], 'mapped', [34396]],
        [[194997, 194997], 'mapped', [34407]],
        [[194998, 194998], 'mapped', [34409]],
        [[194999, 194999], 'mapped', [34473]],
        [[195e3, 195e3], 'mapped', [34440]],
        [[195001, 195001], 'mapped', [34574]],
        [[195002, 195002], 'mapped', [34530]],
        [[195003, 195003], 'mapped', [34681]],
        [[195004, 195004], 'mapped', [34600]],
        [[195005, 195005], 'mapped', [34667]],
        [[195006, 195006], 'mapped', [34694]],
        [[195007, 195007], 'disallowed'],
        [[195008, 195008], 'mapped', [34785]],
        [[195009, 195009], 'mapped', [34817]],
        [[195010, 195010], 'mapped', [17913]],
        [[195011, 195011], 'mapped', [34912]],
        [[195012, 195012], 'mapped', [34915]],
        [[195013, 195013], 'mapped', [161383]],
        [[195014, 195014], 'mapped', [35031]],
        [[195015, 195015], 'mapped', [35038]],
        [[195016, 195016], 'mapped', [17973]],
        [[195017, 195017], 'mapped', [35066]],
        [[195018, 195018], 'mapped', [13499]],
        [[195019, 195019], 'mapped', [161966]],
        [[195020, 195020], 'mapped', [162150]],
        [[195021, 195021], 'mapped', [18110]],
        [[195022, 195022], 'mapped', [18119]],
        [[195023, 195023], 'mapped', [35488]],
        [[195024, 195024], 'mapped', [35565]],
        [[195025, 195025], 'mapped', [35722]],
        [[195026, 195026], 'mapped', [35925]],
        [[195027, 195027], 'mapped', [162984]],
        [[195028, 195028], 'mapped', [36011]],
        [[195029, 195029], 'mapped', [36033]],
        [[195030, 195030], 'mapped', [36123]],
        [[195031, 195031], 'mapped', [36215]],
        [[195032, 195032], 'mapped', [163631]],
        [[195033, 195033], 'mapped', [133124]],
        [[195034, 195034], 'mapped', [36299]],
        [[195035, 195035], 'mapped', [36284]],
        [[195036, 195036], 'mapped', [36336]],
        [[195037, 195037], 'mapped', [133342]],
        [[195038, 195038], 'mapped', [36564]],
        [[195039, 195039], 'mapped', [36664]],
        [[195040, 195040], 'mapped', [165330]],
        [[195041, 195041], 'mapped', [165357]],
        [[195042, 195042], 'mapped', [37012]],
        [[195043, 195043], 'mapped', [37105]],
        [[195044, 195044], 'mapped', [37137]],
        [[195045, 195045], 'mapped', [165678]],
        [[195046, 195046], 'mapped', [37147]],
        [[195047, 195047], 'mapped', [37432]],
        [[195048, 195048], 'mapped', [37591]],
        [[195049, 195049], 'mapped', [37592]],
        [[195050, 195050], 'mapped', [37500]],
        [[195051, 195051], 'mapped', [37881]],
        [[195052, 195052], 'mapped', [37909]],
        [[195053, 195053], 'mapped', [166906]],
        [[195054, 195054], 'mapped', [38283]],
        [[195055, 195055], 'mapped', [18837]],
        [[195056, 195056], 'mapped', [38327]],
        [[195057, 195057], 'mapped', [167287]],
        [[195058, 195058], 'mapped', [18918]],
        [[195059, 195059], 'mapped', [38595]],
        [[195060, 195060], 'mapped', [23986]],
        [[195061, 195061], 'mapped', [38691]],
        [[195062, 195062], 'mapped', [168261]],
        [[195063, 195063], 'mapped', [168474]],
        [[195064, 195064], 'mapped', [19054]],
        [[195065, 195065], 'mapped', [19062]],
        [[195066, 195066], 'mapped', [38880]],
        [[195067, 195067], 'mapped', [168970]],
        [[195068, 195068], 'mapped', [19122]],
        [[195069, 195069], 'mapped', [169110]],
        [[195070, 195071], 'mapped', [38923]],
        [[195072, 195072], 'mapped', [38953]],
        [[195073, 195073], 'mapped', [169398]],
        [[195074, 195074], 'mapped', [39138]],
        [[195075, 195075], 'mapped', [19251]],
        [[195076, 195076], 'mapped', [39209]],
        [[195077, 195077], 'mapped', [39335]],
        [[195078, 195078], 'mapped', [39362]],
        [[195079, 195079], 'mapped', [39422]],
        [[195080, 195080], 'mapped', [19406]],
        [[195081, 195081], 'mapped', [170800]],
        [[195082, 195082], 'mapped', [39698]],
        [[195083, 195083], 'mapped', [4e4]],
        [[195084, 195084], 'mapped', [40189]],
        [[195085, 195085], 'mapped', [19662]],
        [[195086, 195086], 'mapped', [19693]],
        [[195087, 195087], 'mapped', [40295]],
        [[195088, 195088], 'mapped', [172238]],
        [[195089, 195089], 'mapped', [19704]],
        [[195090, 195090], 'mapped', [172293]],
        [[195091, 195091], 'mapped', [172558]],
        [[195092, 195092], 'mapped', [172689]],
        [[195093, 195093], 'mapped', [40635]],
        [[195094, 195094], 'mapped', [19798]],
        [[195095, 195095], 'mapped', [40697]],
        [[195096, 195096], 'mapped', [40702]],
        [[195097, 195097], 'mapped', [40709]],
        [[195098, 195098], 'mapped', [40719]],
        [[195099, 195099], 'mapped', [40726]],
        [[195100, 195100], 'mapped', [40763]],
        [[195101, 195101], 'mapped', [173568]],
        [[195102, 196605], 'disallowed'],
        [[196606, 196607], 'disallowed'],
        [[196608, 262141], 'disallowed'],
        [[262142, 262143], 'disallowed'],
        [[262144, 327677], 'disallowed'],
        [[327678, 327679], 'disallowed'],
        [[327680, 393213], 'disallowed'],
        [[393214, 393215], 'disallowed'],
        [[393216, 458749], 'disallowed'],
        [[458750, 458751], 'disallowed'],
        [[458752, 524285], 'disallowed'],
        [[524286, 524287], 'disallowed'],
        [[524288, 589821], 'disallowed'],
        [[589822, 589823], 'disallowed'],
        [[589824, 655357], 'disallowed'],
        [[655358, 655359], 'disallowed'],
        [[655360, 720893], 'disallowed'],
        [[720894, 720895], 'disallowed'],
        [[720896, 786429], 'disallowed'],
        [[786430, 786431], 'disallowed'],
        [[786432, 851965], 'disallowed'],
        [[851966, 851967], 'disallowed'],
        [[851968, 917501], 'disallowed'],
        [[917502, 917503], 'disallowed'],
        [[917504, 917504], 'disallowed'],
        [[917505, 917505], 'disallowed'],
        [[917506, 917535], 'disallowed'],
        [[917536, 917631], 'disallowed'],
        [[917632, 917759], 'disallowed'],
        [[917760, 917999], 'ignored'],
        [[918e3, 983037], 'disallowed'],
        [[983038, 983039], 'disallowed'],
        [[983040, 1048573], 'disallowed'],
        [[1048574, 1048575], 'disallowed'],
        [[1048576, 1114109], 'disallowed'],
        [[1114110, 1114111], 'disallowed']
    ]
})
var y0 = se((c3, jp) => {
    'use strict'
    var v0 = Ae('punycode'),
        h0 = f0(),
        Wa = { TRANSITIONAL: 0, NONTRANSITIONAL: 1 }
    function b0(e) {
        return e
            .split('\0')
            .map(function (a) {
                return a.normalize('NFC')
            })
            .join('\0')
    }
    function w0(e) {
        for (var a = 0, t = h0.length - 1; a <= t; ) {
            var r = Math.floor((a + t) / 2),
                i = h0[r]
            if (i[0][0] <= e && i[0][1] >= e) return i
            i[0][0] > e ? (t = r - 1) : (a = r + 1)
        }
        return null
    }
    var Dl = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g
    function _0(e) {
        return e.replace(Dl, '_').length
    }
    function Al(e, a, t) {
        for (var r = !1, i = '', o = _0(e), m = 0; m < o; ++m) {
            var u = e.codePointAt(m),
                s = w0(u)
            switch (s[1]) {
                case 'disallowed':
                    ;(r = !0), (i += String.fromCodePoint(u))
                    break
                case 'ignored':
                    break
                case 'mapped':
                    i += String.fromCodePoint.apply(String, s[2])
                    break
                case 'deviation':
                    t === Wa.TRANSITIONAL
                        ? (i += String.fromCodePoint.apply(String, s[2]))
                        : (i += String.fromCodePoint(u))
                    break
                case 'valid':
                    i += String.fromCodePoint(u)
                    break
                case 'disallowed_STD3_mapped':
                    a
                        ? ((r = !0), (i += String.fromCodePoint(u)))
                        : (i += String.fromCodePoint.apply(String, s[2]))
                    break
                case 'disallowed_STD3_valid':
                    a && (r = !0), (i += String.fromCodePoint(u))
                    break
            }
        }
        return { string: i, error: r }
    }
    var Vl =
        /[\u0300-\u036F\u0483-\u0489\u0591-\u05BD\u05BF\u05C1\u05C2\u05C4\u05C5\u05C7\u0610-\u061A\u064B-\u065F\u0670\u06D6-\u06DC\u06DF-\u06E4\u06E7\u06E8\u06EA-\u06ED\u0711\u0730-\u074A\u07A6-\u07B0\u07EB-\u07F3\u0816-\u0819\u081B-\u0823\u0825-\u0827\u0829-\u082D\u0859-\u085B\u08E4-\u0903\u093A-\u093C\u093E-\u094F\u0951-\u0957\u0962\u0963\u0981-\u0983\u09BC\u09BE-\u09C4\u09C7\u09C8\u09CB-\u09CD\u09D7\u09E2\u09E3\u0A01-\u0A03\u0A3C\u0A3E-\u0A42\u0A47\u0A48\u0A4B-\u0A4D\u0A51\u0A70\u0A71\u0A75\u0A81-\u0A83\u0ABC\u0ABE-\u0AC5\u0AC7-\u0AC9\u0ACB-\u0ACD\u0AE2\u0AE3\u0B01-\u0B03\u0B3C\u0B3E-\u0B44\u0B47\u0B48\u0B4B-\u0B4D\u0B56\u0B57\u0B62\u0B63\u0B82\u0BBE-\u0BC2\u0BC6-\u0BC8\u0BCA-\u0BCD\u0BD7\u0C00-\u0C03\u0C3E-\u0C44\u0C46-\u0C48\u0C4A-\u0C4D\u0C55\u0C56\u0C62\u0C63\u0C81-\u0C83\u0CBC\u0CBE-\u0CC4\u0CC6-\u0CC8\u0CCA-\u0CCD\u0CD5\u0CD6\u0CE2\u0CE3\u0D01-\u0D03\u0D3E-\u0D44\u0D46-\u0D48\u0D4A-\u0D4D\u0D57\u0D62\u0D63\u0D82\u0D83\u0DCA\u0DCF-\u0DD4\u0DD6\u0DD8-\u0DDF\u0DF2\u0DF3\u0E31\u0E34-\u0E3A\u0E47-\u0E4E\u0EB1\u0EB4-\u0EB9\u0EBB\u0EBC\u0EC8-\u0ECD\u0F18\u0F19\u0F35\u0F37\u0F39\u0F3E\u0F3F\u0F71-\u0F84\u0F86\u0F87\u0F8D-\u0F97\u0F99-\u0FBC\u0FC6\u102B-\u103E\u1056-\u1059\u105E-\u1060\u1062-\u1064\u1067-\u106D\u1071-\u1074\u1082-\u108D\u108F\u109A-\u109D\u135D-\u135F\u1712-\u1714\u1732-\u1734\u1752\u1753\u1772\u1773\u17B4-\u17D3\u17DD\u180B-\u180D\u18A9\u1920-\u192B\u1930-\u193B\u19B0-\u19C0\u19C8\u19C9\u1A17-\u1A1B\u1A55-\u1A5E\u1A60-\u1A7C\u1A7F\u1AB0-\u1ABE\u1B00-\u1B04\u1B34-\u1B44\u1B6B-\u1B73\u1B80-\u1B82\u1BA1-\u1BAD\u1BE6-\u1BF3\u1C24-\u1C37\u1CD0-\u1CD2\u1CD4-\u1CE8\u1CED\u1CF2-\u1CF4\u1CF8\u1CF9\u1DC0-\u1DF5\u1DFC-\u1DFF\u20D0-\u20F0\u2CEF-\u2CF1\u2D7F\u2DE0-\u2DFF\u302A-\u302F\u3099\u309A\uA66F-\uA672\uA674-\uA67D\uA69F\uA6F0\uA6F1\uA802\uA806\uA80B\uA823-\uA827\uA880\uA881\uA8B4-\uA8C4\uA8E0-\uA8F1\uA926-\uA92D\uA947-\uA953\uA980-\uA983\uA9B3-\uA9C0\uA9E5\uAA29-\uAA36\uAA43\uAA4C\uAA4D\uAA7B-\uAA7D\uAAB0\uAAB2-\uAAB4\uAAB7\uAAB8\uAABE\uAABF\uAAC1\uAAEB-\uAAEF\uAAF5\uAAF6\uABE3-\uABEA\uABEC\uABED\uFB1E\uFE00-\uFE0F\uFE20-\uFE2D]|\uD800[\uDDFD\uDEE0\uDF76-\uDF7A]|\uD802[\uDE01-\uDE03\uDE05\uDE06\uDE0C-\uDE0F\uDE38-\uDE3A\uDE3F\uDEE5\uDEE6]|\uD804[\uDC00-\uDC02\uDC38-\uDC46\uDC7F-\uDC82\uDCB0-\uDCBA\uDD00-\uDD02\uDD27-\uDD34\uDD73\uDD80-\uDD82\uDDB3-\uDDC0\uDE2C-\uDE37\uDEDF-\uDEEA\uDF01-\uDF03\uDF3C\uDF3E-\uDF44\uDF47\uDF48\uDF4B-\uDF4D\uDF57\uDF62\uDF63\uDF66-\uDF6C\uDF70-\uDF74]|\uD805[\uDCB0-\uDCC3\uDDAF-\uDDB5\uDDB8-\uDDC0\uDE30-\uDE40\uDEAB-\uDEB7]|\uD81A[\uDEF0-\uDEF4\uDF30-\uDF36]|\uD81B[\uDF51-\uDF7E\uDF8F-\uDF92]|\uD82F[\uDC9D\uDC9E]|\uD834[\uDD65-\uDD69\uDD6D-\uDD72\uDD7B-\uDD82\uDD85-\uDD8B\uDDAA-\uDDAD\uDE42-\uDE44]|\uD83A[\uDCD0-\uDCD6]|\uDB40[\uDD00-\uDDEF]/
    function ql(e, a) {
        e.substr(0, 4) === 'xn--' &&
            ((e = v0.toUnicode(e)), (a = Wa.NONTRANSITIONAL))
        var t = !1
        ;(b0(e) !== e ||
            (e[3] === '-' && e[4] === '-') ||
            e[0] === '-' ||
            e[e.length - 1] === '-' ||
            e.indexOf('.') !== -1 ||
            e.search(Vl) === 0) &&
            (t = !0)
        for (var r = _0(e), i = 0; i < r; ++i) {
            var o = w0(e.codePointAt(i))
            if (
                (Ip === Wa.TRANSITIONAL && o[1] !== 'valid') ||
                (Ip === Wa.NONTRANSITIONAL &&
                    o[1] !== 'valid' &&
                    o[1] !== 'deviation')
            ) {
                t = !0
                break
            }
        }
        return { label: e, error: t }
    }
    function Ip(e, a, t) {
        var r = Al(e, a, t)
        r.string = b0(r.string)
        for (var i = r.string.split('.'), o = 0; o < i.length; ++o)
            try {
                var m = ql(i[o])
                ;(i[o] = m.label), (r.error = r.error || m.error)
            } catch {
                r.error = !0
            }
        return { string: i.join('.'), error: r.error }
    }
    jp.exports.toASCII = function (e, a, t, r) {
        var i = Ip(e, a, t),
            o = i.string.split('.')
        if (
            ((o = o.map(function (s) {
                try {
                    return v0.toASCII(s)
                } catch {
                    return (i.error = !0), s
                }
            })),
            r)
        ) {
            var m = o.slice(0, o.length - 1).join('.').length
            ;(m.length > 253 || m.length === 0) && (i.error = !0)
            for (var u = 0; u < o.length; ++u)
                if (o.length > 63 || o.length === 0) {
                    i.error = !0
                    break
                }
        }
        return i.error ? null : o.join('.')
    }
    jp.exports.toUnicode = function (e, a) {
        var t = Ip(e, a, Wa.NONTRANSITIONAL)
        return { domain: t.string, error: t.error }
    }
    jp.exports.PROCESSING_OPTIONS = Wa
})
var Xe = se((f3, Ve) => {
    'use strict'
    var za = Ae('punycode'),
        g0 = y0(),
        C0 = {
            ftp: 21,
            file: null,
            gopher: 70,
            http: 80,
            https: 443,
            ws: 80,
            wss: 443
        },
        j = Symbol('failure')
    function S0(e) {
        return za.ucs2.decode(e).length
    }
    function T0(e, a) {
        let t = e[a]
        return isNaN(t) ? void 0 : String.fromCodePoint(t)
    }
    function Pt(e) {
        return e >= 48 && e <= 57
    }
    function Nt(e) {
        return (e >= 65 && e <= 90) || (e >= 97 && e <= 122)
    }
    function Bl(e) {
        return Nt(e) || Pt(e)
    }
    function ze(e) {
        return Pt(e) || (e >= 65 && e <= 70) || (e >= 97 && e <= 102)
    }
    function R0(e) {
        return e === '.' || e.toLowerCase() === '%2e'
    }
    function kl(e) {
        return (
            (e = e.toLowerCase()),
            e === '..' || e === '%2e.' || e === '.%2e' || e === '%2e%2e'
        )
    }
    function Ol(e, a) {
        return Nt(e) && (a === 58 || a === 124)
    }
    function E0(e) {
        return (
            e.length === 2 &&
            Nt(e.codePointAt(0)) &&
            (e[1] === ':' || e[1] === '|')
        )
    }
    function Fl(e) {
        return e.length === 2 && Nt(e.codePointAt(0)) && e[1] === ':'
    }
    function xl(e) {
        return (
            e.search(
                /\u0000|\u0009|\u000A|\u000D|\u0020|#|%|\/|:|\?|@|\[|\\|\]/
            ) !== -1
        )
    }
    function Ll(e) {
        return (
            e.search(
                /\u0000|\u0009|\u000A|\u000D|\u0020|#|\/|:|\?|@|\[|\\|\]/
            ) !== -1
        )
    }
    function Vd(e) {
        return C0[e] !== void 0
    }
    function me(e) {
        return Vd(e.scheme)
    }
    function Il(e) {
        return C0[e]
    }
    function P0(e) {
        let a = e.toString(16).toUpperCase()
        return a.length === 1 && (a = '0' + a), '%' + a
    }
    function jl(e) {
        let a = new Buffer(e),
            t = ''
        for (let r = 0; r < a.length; ++r) t += P0(a[r])
        return t
    }
    function Wl(e) {
        let a = new Buffer(e),
            t = []
        for (let r = 0; r < a.length; ++r)
            a[r] !== 37
                ? t.push(a[r])
                : a[r] === 37 && ze(a[r + 1]) && ze(a[r + 2])
                ? (t.push(parseInt(a.slice(r + 1, r + 3).toString(), 16)),
                  (r += 2))
                : t.push(a[r])
        return new Buffer(t).toString()
    }
    function Wp(e) {
        return e <= 31 || e > 126
    }
    var zl = new Set([32, 34, 35, 60, 62, 63, 96, 123, 125])
    function N0(e) {
        return Wp(e) || zl.has(e)
    }
    var Ml = new Set([47, 58, 59, 61, 64, 91, 92, 93, 94, 124])
    function Bd(e) {
        return N0(e) || Ml.has(e)
    }
    function ma(e, a) {
        let t = String.fromCodePoint(e)
        return a(e) ? jl(t) : t
    }
    function $l(e) {
        let a = 10
        return (
            e.length >= 2 &&
            e.charAt(0) === '0' &&
            e.charAt(1).toLowerCase() === 'x'
                ? ((e = e.substring(2)), (a = 16))
                : e.length >= 2 &&
                  e.charAt(0) === '0' &&
                  ((e = e.substring(1)), (a = 8)),
            e === ''
                ? 0
                : (a === 10
                      ? /[^0-9]/
                      : a === 16
                      ? /[^0-9A-Fa-f]/
                      : /[^0-7]/
                  ).test(e)
                ? j
                : parseInt(e, a)
        )
    }
    function Ul(e) {
        let a = e.split('.')
        if ((a[a.length - 1] === '' && a.length > 1 && a.pop(), a.length > 4))
            return e
        let t = []
        for (let o of a) {
            if (o === '') return e
            let m = $l(o)
            if (m === j) return e
            t.push(m)
        }
        for (let o = 0; o < t.length - 1; ++o) if (t[o] > 255) return j
        if (t[t.length - 1] >= Math.pow(256, 5 - t.length)) return j
        let r = t.pop(),
            i = 0
        for (let o of t) (r += o * Math.pow(256, 3 - i)), ++i
        return r
    }
    function Hl(e) {
        let a = '',
            t = e
        for (let r = 1; r <= 4; ++r)
            (a = String(t % 256) + a),
                r !== 4 && (a = '.' + a),
                (t = Math.floor(t / 256))
        return a
    }
    function Ql(e) {
        let a = [0, 0, 0, 0, 0, 0, 0, 0],
            t = 0,
            r = null,
            i = 0
        if (((e = za.ucs2.decode(e)), e[i] === 58)) {
            if (e[i + 1] !== 58) return j
            ;(i += 2), ++t, (r = t)
        }
        for (; i < e.length; ) {
            if (t === 8) return j
            if (e[i] === 58) {
                if (r !== null) return j
                ++i, ++t, (r = t)
                continue
            }
            let o = 0,
                m = 0
            for (; m < 4 && ze(e[i]); )
                (o = o * 16 + parseInt(T0(e, i), 16)), ++i, ++m
            if (e[i] === 46) {
                if (m === 0 || ((i -= m), t > 6)) return j
                let u = 0
                for (; e[i] !== void 0; ) {
                    let s = null
                    if (u > 0)
                        if (e[i] === 46 && u < 4) ++i
                        else return j
                    if (!Pt(e[i])) return j
                    for (; Pt(e[i]); ) {
                        let f = parseInt(T0(e, i))
                        if (s === null) s = f
                        else {
                            if (s === 0) return j
                            s = s * 10 + f
                        }
                        if (s > 255) return j
                        ++i
                    }
                    ;(a[t] = a[t] * 256 + s), ++u, (u === 2 || u === 4) && ++t
                }
                if (u !== 4) return j
                break
            } else if (e[i] === 58) {
                if ((++i, e[i] === void 0)) return j
            } else if (e[i] !== void 0) return j
            ;(a[t] = o), ++t
        }
        if (r !== null) {
            let o = t - r
            for (t = 7; t !== 0 && o > 0; ) {
                let m = a[r + o - 1]
                ;(a[r + o - 1] = a[t]), (a[t] = m), --t, --o
            }
        } else if (r === null && t !== 8) return j
        return a
    }
    function Gl(e) {
        let a = '',
            r = Kl(e).idx,
            i = !1
        for (let o = 0; o <= 7; ++o)
            if (!(i && e[o] === 0)) {
                if ((i && (i = !1), r === o)) {
                    ;(a += o === 0 ? '::' : ':'), (i = !0)
                    continue
                }
                ;(a += e[o].toString(16)), o !== 7 && (a += ':')
            }
        return a
    }
    function qd(e, a) {
        if (e[0] === '[')
            return e[e.length - 1] !== ']'
                ? j
                : Ql(e.substring(1, e.length - 1))
        if (!a) return Yl(e)
        let t = Wl(e),
            r = g0.toASCII(t, !1, g0.PROCESSING_OPTIONS.NONTRANSITIONAL, !1)
        if (r === null || xl(r)) return j
        let i = Ul(r)
        return typeof i == 'number' || i === j ? i : r
    }
    function Yl(e) {
        if (Ll(e)) return j
        let a = '',
            t = za.ucs2.decode(e)
        for (let r = 0; r < t.length; ++r) a += ma(t[r], Wp)
        return a
    }
    function Kl(e) {
        let a = null,
            t = 1,
            r = null,
            i = 0
        for (let o = 0; o < e.length; ++o)
            e[o] !== 0
                ? (i > t && ((a = r), (t = i)), (r = null), (i = 0))
                : (r === null && (r = o), ++i)
        return i > t && ((a = r), (t = i)), { idx: a, len: t }
    }
    function kd(e) {
        return typeof e == 'number'
            ? Hl(e)
            : e instanceof Array
            ? '[' + Gl(e) + ']'
            : e
    }
    function Xl(e) {
        return e.replace(/^[\u0000-\u001F\u0020]+|[\u0000-\u001F\u0020]+$/g, '')
    }
    function Jl(e) {
        return e.replace(/\u0009|\u000A|\u000D/g, '')
    }
    function D0(e) {
        let a = e.path
        a.length !== 0 &&
            ((e.scheme === 'file' && a.length === 1 && es(a[0])) || a.pop())
    }
    function A0(e) {
        return e.username !== '' || e.password !== ''
    }
    function Zl(e) {
        return (
            e.host === null ||
            e.host === '' ||
            e.cannotBeABaseURL ||
            e.scheme === 'file'
        )
    }
    function es(e) {
        return /^[A-Za-z]:$/.test(e)
    }
    function te(e, a, t, r, i) {
        if (
            ((this.pointer = 0),
            (this.input = e),
            (this.base = a || null),
            (this.encodingOverride = t || 'utf-8'),
            (this.stateOverride = i),
            (this.url = r),
            (this.failure = !1),
            (this.parseError = !1),
            !this.url)
        ) {
            this.url = {
                scheme: '',
                username: '',
                password: '',
                host: null,
                port: null,
                path: [],
                query: null,
                fragment: null,
                cannotBeABaseURL: !1
            }
            let m = Xl(this.input)
            m !== this.input && (this.parseError = !0), (this.input = m)
        }
        let o = Jl(this.input)
        for (
            o !== this.input && (this.parseError = !0),
                this.input = o,
                this.state = i || 'scheme start',
                this.buffer = '',
                this.atFlag = !1,
                this.arrFlag = !1,
                this.passwordTokenSeenFlag = !1,
                this.input = za.ucs2.decode(this.input);
            this.pointer <= this.input.length;
            ++this.pointer
        ) {
            let m = this.input[this.pointer],
                u = isNaN(m) ? void 0 : String.fromCodePoint(m),
                s = this['parse ' + this.state](m, u)
            if (s) {
                if (s === j) {
                    this.failure = !0
                    break
                }
            } else break
        }
    }
    te.prototype['parse scheme start'] = function (a, t) {
        if (Nt(a)) (this.buffer += t.toLowerCase()), (this.state = 'scheme')
        else if (!this.stateOverride) (this.state = 'no scheme'), --this.pointer
        else return (this.parseError = !0), j
        return !0
    }
    te.prototype['parse scheme'] = function (a, t) {
        if (Bl(a) || a === 43 || a === 45 || a === 46)
            this.buffer += t.toLowerCase()
        else if (a === 58) {
            if (
                (this.stateOverride &&
                    ((me(this.url) && !Vd(this.buffer)) ||
                        (!me(this.url) && Vd(this.buffer)) ||
                        ((A0(this.url) || this.url.port !== null) &&
                            this.buffer === 'file') ||
                        (this.url.scheme === 'file' &&
                            (this.url.host === '' ||
                                this.url.host === null)))) ||
                ((this.url.scheme = this.buffer),
                (this.buffer = ''),
                this.stateOverride)
            )
                return !1
            this.url.scheme === 'file'
                ? ((this.input[this.pointer + 1] !== 47 ||
                      this.input[this.pointer + 2] !== 47) &&
                      (this.parseError = !0),
                  (this.state = 'file'))
                : me(this.url) &&
                  this.base !== null &&
                  this.base.scheme === this.url.scheme
                ? (this.state = 'special relative or authority')
                : me(this.url)
                ? (this.state = 'special authority slashes')
                : this.input[this.pointer + 1] === 47
                ? ((this.state = 'path or authority'), ++this.pointer)
                : ((this.url.cannotBeABaseURL = !0),
                  this.url.path.push(''),
                  (this.state = 'cannot-be-a-base-URL path'))
        } else if (!this.stateOverride)
            (this.buffer = ''), (this.state = 'no scheme'), (this.pointer = -1)
        else return (this.parseError = !0), j
        return !0
    }
    te.prototype['parse no scheme'] = function (a) {
        return this.base === null || (this.base.cannotBeABaseURL && a !== 35)
            ? j
            : (this.base.cannotBeABaseURL && a === 35
                  ? ((this.url.scheme = this.base.scheme),
                    (this.url.path = this.base.path.slice()),
                    (this.url.query = this.base.query),
                    (this.url.fragment = ''),
                    (this.url.cannotBeABaseURL = !0),
                    (this.state = 'fragment'))
                  : this.base.scheme === 'file'
                  ? ((this.state = 'file'), --this.pointer)
                  : ((this.state = 'relative'), --this.pointer),
              !0)
    }
    te.prototype['parse special relative or authority'] = function (a) {
        return (
            a === 47 && this.input[this.pointer + 1] === 47
                ? ((this.state = 'special authority ignore slashes'),
                  ++this.pointer)
                : ((this.parseError = !0),
                  (this.state = 'relative'),
                  --this.pointer),
            !0
        )
    }
    te.prototype['parse path or authority'] = function (a) {
        return (
            a === 47
                ? (this.state = 'authority')
                : ((this.state = 'path'), --this.pointer),
            !0
        )
    }
    te.prototype['parse relative'] = function (a) {
        return (
            (this.url.scheme = this.base.scheme),
            isNaN(a)
                ? ((this.url.username = this.base.username),
                  (this.url.password = this.base.password),
                  (this.url.host = this.base.host),
                  (this.url.port = this.base.port),
                  (this.url.path = this.base.path.slice()),
                  (this.url.query = this.base.query))
                : a === 47
                ? (this.state = 'relative slash')
                : a === 63
                ? ((this.url.username = this.base.username),
                  (this.url.password = this.base.password),
                  (this.url.host = this.base.host),
                  (this.url.port = this.base.port),
                  (this.url.path = this.base.path.slice()),
                  (this.url.query = ''),
                  (this.state = 'query'))
                : a === 35
                ? ((this.url.username = this.base.username),
                  (this.url.password = this.base.password),
                  (this.url.host = this.base.host),
                  (this.url.port = this.base.port),
                  (this.url.path = this.base.path.slice()),
                  (this.url.query = this.base.query),
                  (this.url.fragment = ''),
                  (this.state = 'fragment'))
                : me(this.url) && a === 92
                ? ((this.parseError = !0), (this.state = 'relative slash'))
                : ((this.url.username = this.base.username),
                  (this.url.password = this.base.password),
                  (this.url.host = this.base.host),
                  (this.url.port = this.base.port),
                  (this.url.path = this.base.path.slice(
                      0,
                      this.base.path.length - 1
                  )),
                  (this.state = 'path'),
                  --this.pointer),
            !0
        )
    }
    te.prototype['parse relative slash'] = function (a) {
        return (
            me(this.url) && (a === 47 || a === 92)
                ? (a === 92 && (this.parseError = !0),
                  (this.state = 'special authority ignore slashes'))
                : a === 47
                ? (this.state = 'authority')
                : ((this.url.username = this.base.username),
                  (this.url.password = this.base.password),
                  (this.url.host = this.base.host),
                  (this.url.port = this.base.port),
                  (this.state = 'path'),
                  --this.pointer),
            !0
        )
    }
    te.prototype['parse special authority slashes'] = function (a) {
        return (
            a === 47 && this.input[this.pointer + 1] === 47
                ? ((this.state = 'special authority ignore slashes'),
                  ++this.pointer)
                : ((this.parseError = !0),
                  (this.state = 'special authority ignore slashes'),
                  --this.pointer),
            !0
        )
    }
    te.prototype['parse special authority ignore slashes'] = function (a) {
        return (
            a !== 47 && a !== 92
                ? ((this.state = 'authority'), --this.pointer)
                : (this.parseError = !0),
            !0
        )
    }
    te.prototype['parse authority'] = function (a, t) {
        if (a === 64) {
            ;(this.parseError = !0),
                this.atFlag && (this.buffer = '%40' + this.buffer),
                (this.atFlag = !0)
            let r = S0(this.buffer)
            for (let i = 0; i < r; ++i) {
                let o = this.buffer.codePointAt(i)
                if (o === 58 && !this.passwordTokenSeenFlag) {
                    this.passwordTokenSeenFlag = !0
                    continue
                }
                let m = ma(o, Bd)
                this.passwordTokenSeenFlag
                    ? (this.url.password += m)
                    : (this.url.username += m)
            }
            this.buffer = ''
        } else if (
            isNaN(a) ||
            a === 47 ||
            a === 63 ||
            a === 35 ||
            (me(this.url) && a === 92)
        ) {
            if (this.atFlag && this.buffer === '')
                return (this.parseError = !0), j
            ;(this.pointer -= S0(this.buffer) + 1),
                (this.buffer = ''),
                (this.state = 'host')
        } else this.buffer += t
        return !0
    }
    te.prototype['parse hostname'] = te.prototype['parse host'] = function (
        a,
        t
    ) {
        if (this.stateOverride && this.url.scheme === 'file')
            --this.pointer, (this.state = 'file host')
        else if (a === 58 && !this.arrFlag) {
            if (this.buffer === '') return (this.parseError = !0), j
            let r = qd(this.buffer, me(this.url))
            if (r === j) return j
            if (
                ((this.url.host = r),
                (this.buffer = ''),
                (this.state = 'port'),
                this.stateOverride === 'hostname')
            )
                return !1
        } else if (
            isNaN(a) ||
            a === 47 ||
            a === 63 ||
            a === 35 ||
            (me(this.url) && a === 92)
        ) {
            if ((--this.pointer, me(this.url) && this.buffer === ''))
                return (this.parseError = !0), j
            if (
                this.stateOverride &&
                this.buffer === '' &&
                (A0(this.url) || this.url.port !== null)
            )
                return (this.parseError = !0), !1
            let r = qd(this.buffer, me(this.url))
            if (r === j) return j
            if (
                ((this.url.host = r),
                (this.buffer = ''),
                (this.state = 'path start'),
                this.stateOverride)
            )
                return !1
        } else
            a === 91 ? (this.arrFlag = !0) : a === 93 && (this.arrFlag = !1),
                (this.buffer += t)
        return !0
    }
    te.prototype['parse port'] = function (a, t) {
        if (Pt(a)) this.buffer += t
        else if (
            isNaN(a) ||
            a === 47 ||
            a === 63 ||
            a === 35 ||
            (me(this.url) && a === 92) ||
            this.stateOverride
        ) {
            if (this.buffer !== '') {
                let r = parseInt(this.buffer)
                if (r > Math.pow(2, 16) - 1) return (this.parseError = !0), j
                ;(this.url.port = r === Il(this.url.scheme) ? null : r),
                    (this.buffer = '')
            }
            if (this.stateOverride) return !1
            ;(this.state = 'path start'), --this.pointer
        } else return (this.parseError = !0), j
        return !0
    }
    var as = new Set([47, 92, 63, 35])
    te.prototype['parse file'] = function (a) {
        return (
            (this.url.scheme = 'file'),
            a === 47 || a === 92
                ? (a === 92 && (this.parseError = !0),
                  (this.state = 'file slash'))
                : this.base !== null && this.base.scheme === 'file'
                ? isNaN(a)
                    ? ((this.url.host = this.base.host),
                      (this.url.path = this.base.path.slice()),
                      (this.url.query = this.base.query))
                    : a === 63
                    ? ((this.url.host = this.base.host),
                      (this.url.path = this.base.path.slice()),
                      (this.url.query = ''),
                      (this.state = 'query'))
                    : a === 35
                    ? ((this.url.host = this.base.host),
                      (this.url.path = this.base.path.slice()),
                      (this.url.query = this.base.query),
                      (this.url.fragment = ''),
                      (this.state = 'fragment'))
                    : (this.input.length - this.pointer - 1 === 0 ||
                      !Ol(a, this.input[this.pointer + 1]) ||
                      (this.input.length - this.pointer - 1 >= 2 &&
                          !as.has(this.input[this.pointer + 2]))
                          ? ((this.url.host = this.base.host),
                            (this.url.path = this.base.path.slice()),
                            D0(this.url))
                          : (this.parseError = !0),
                      (this.state = 'path'),
                      --this.pointer)
                : ((this.state = 'path'), --this.pointer),
            !0
        )
    }
    te.prototype['parse file slash'] = function (a) {
        return (
            a === 47 || a === 92
                ? (a === 92 && (this.parseError = !0),
                  (this.state = 'file host'))
                : (this.base !== null &&
                      this.base.scheme === 'file' &&
                      (Fl(this.base.path[0])
                          ? this.url.path.push(this.base.path[0])
                          : (this.url.host = this.base.host)),
                  (this.state = 'path'),
                  --this.pointer),
            !0
        )
    }
    te.prototype['parse file host'] = function (a, t) {
        if (isNaN(a) || a === 47 || a === 92 || a === 63 || a === 35)
            if ((--this.pointer, !this.stateOverride && E0(this.buffer)))
                (this.parseError = !0), (this.state = 'path')
            else if (this.buffer === '') {
                if (((this.url.host = ''), this.stateOverride)) return !1
                this.state = 'path start'
            } else {
                let r = qd(this.buffer, me(this.url))
                if (r === j) return j
                if (
                    (r === 'localhost' && (r = ''),
                    (this.url.host = r),
                    this.stateOverride)
                )
                    return !1
                ;(this.buffer = ''), (this.state = 'path start')
            }
        else this.buffer += t
        return !0
    }
    te.prototype['parse path start'] = function (a) {
        return (
            me(this.url)
                ? (a === 92 && (this.parseError = !0),
                  (this.state = 'path'),
                  a !== 47 && a !== 92 && --this.pointer)
                : !this.stateOverride && a === 63
                ? ((this.url.query = ''), (this.state = 'query'))
                : !this.stateOverride && a === 35
                ? ((this.url.fragment = ''), (this.state = 'fragment'))
                : a !== void 0 &&
                  ((this.state = 'path'), a !== 47 && --this.pointer),
            !0
        )
    }
    te.prototype['parse path'] = function (a) {
        if (
            isNaN(a) ||
            a === 47 ||
            (me(this.url) && a === 92) ||
            (!this.stateOverride && (a === 63 || a === 35))
        ) {
            if (
                (me(this.url) && a === 92 && (this.parseError = !0),
                kl(this.buffer)
                    ? (D0(this.url),
                      a !== 47 &&
                          !(me(this.url) && a === 92) &&
                          this.url.path.push(''))
                    : R0(this.buffer) && a !== 47 && !(me(this.url) && a === 92)
                    ? this.url.path.push('')
                    : R0(this.buffer) ||
                      (this.url.scheme === 'file' &&
                          this.url.path.length === 0 &&
                          E0(this.buffer) &&
                          (this.url.host !== '' &&
                              this.url.host !== null &&
                              ((this.parseError = !0), (this.url.host = '')),
                          (this.buffer = this.buffer[0] + ':')),
                      this.url.path.push(this.buffer)),
                (this.buffer = ''),
                this.url.scheme === 'file' &&
                    (a === void 0 || a === 63 || a === 35))
            )
                for (; this.url.path.length > 1 && this.url.path[0] === ''; )
                    (this.parseError = !0), this.url.path.shift()
            a === 63 && ((this.url.query = ''), (this.state = 'query')),
                a === 35 &&
                    ((this.url.fragment = ''), (this.state = 'fragment'))
        } else
            a === 37 &&
                (!ze(this.input[this.pointer + 1]) ||
                    !ze(this.input[this.pointer + 2])) &&
                (this.parseError = !0),
                (this.buffer += ma(a, N0))
        return !0
    }
    te.prototype['parse cannot-be-a-base-URL path'] = function (a) {
        return (
            a === 63
                ? ((this.url.query = ''), (this.state = 'query'))
                : a === 35
                ? ((this.url.fragment = ''), (this.state = 'fragment'))
                : (!isNaN(a) && a !== 37 && (this.parseError = !0),
                  a === 37 &&
                      (!ze(this.input[this.pointer + 1]) ||
                          !ze(this.input[this.pointer + 2])) &&
                      (this.parseError = !0),
                  isNaN(a) ||
                      (this.url.path[0] = this.url.path[0] + ma(a, Wp))),
            !0
        )
    }
    te.prototype['parse query'] = function (a, t) {
        if (isNaN(a) || (!this.stateOverride && a === 35)) {
            ;(!me(this.url) ||
                this.url.scheme === 'ws' ||
                this.url.scheme === 'wss') &&
                (this.encodingOverride = 'utf-8')
            let r = new Buffer(this.buffer)
            for (let i = 0; i < r.length; ++i)
                r[i] < 33 ||
                r[i] > 126 ||
                r[i] === 34 ||
                r[i] === 35 ||
                r[i] === 60 ||
                r[i] === 62
                    ? (this.url.query += P0(r[i]))
                    : (this.url.query += String.fromCodePoint(r[i]))
            ;(this.buffer = ''),
                a === 35 &&
                    ((this.url.fragment = ''), (this.state = 'fragment'))
        } else
            a === 37 &&
                (!ze(this.input[this.pointer + 1]) ||
                    !ze(this.input[this.pointer + 2])) &&
                (this.parseError = !0),
                (this.buffer += t)
        return !0
    }
    te.prototype['parse fragment'] = function (a) {
        return (
            isNaN(a) ||
                (a === 0
                    ? (this.parseError = !0)
                    : (a === 37 &&
                          (!ze(this.input[this.pointer + 1]) ||
                              !ze(this.input[this.pointer + 2])) &&
                          (this.parseError = !0),
                      (this.url.fragment += ma(a, Wp)))),
            !0
        )
    }
    function ts(e, a) {
        let t = e.scheme + ':'
        if (
            (e.host !== null
                ? ((t += '//'),
                  (e.username !== '' || e.password !== '') &&
                      ((t += e.username),
                      e.password !== '' && (t += ':' + e.password),
                      (t += '@')),
                  (t += kd(e.host)),
                  e.port !== null && (t += ':' + e.port))
                : e.host === null && e.scheme === 'file' && (t += '//'),
            e.cannotBeABaseURL)
        )
            t += e.path[0]
        else for (let r of e.path) t += '/' + r
        return (
            e.query !== null && (t += '?' + e.query),
            !a && e.fragment !== null && (t += '#' + e.fragment),
            t
        )
    }
    function ps(e) {
        let a = e.scheme + '://'
        return (a += kd(e.host)), e.port !== null && (a += ':' + e.port), a
    }
    Ve.exports.serializeURL = ts
    Ve.exports.serializeURLOrigin = function (e) {
        switch (e.scheme) {
            case 'blob':
                try {
                    return Ve.exports.serializeURLOrigin(
                        Ve.exports.parseURL(e.path[0])
                    )
                } catch {
                    return 'null'
                }
            case 'ftp':
            case 'gopher':
            case 'http':
            case 'https':
            case 'ws':
            case 'wss':
                return ps({ scheme: e.scheme, host: e.host, port: e.port })
            case 'file':
                return 'file://'
            default:
                return 'null'
        }
    }
    Ve.exports.basicURLParse = function (e, a) {
        a === void 0 && (a = {})
        let t = new te(e, a.baseURL, a.encodingOverride, a.url, a.stateOverride)
        return t.failure ? 'failure' : t.url
    }
    Ve.exports.setTheUsername = function (e, a) {
        e.username = ''
        let t = za.ucs2.decode(a)
        for (let r = 0; r < t.length; ++r) e.username += ma(t[r], Bd)
    }
    Ve.exports.setThePassword = function (e, a) {
        e.password = ''
        let t = za.ucs2.decode(a)
        for (let r = 0; r < t.length; ++r) e.password += ma(t[r], Bd)
    }
    Ve.exports.serializeHost = kd
    Ve.exports.cannotHaveAUsernamePasswordPort = Zl
    Ve.exports.serializeInteger = function (e) {
        return String(e)
    }
    Ve.exports.parseURL = function (e, a) {
        return (
            a === void 0 && (a = {}),
            Ve.exports.basicURLParse(e, {
                baseURL: a.baseURL,
                encodingOverride: a.encodingOverride
            })
        )
    }
})
var q0 = se((V0) => {
    'use strict'
    var pe = Xe()
    V0.implementation = class {
        constructor(a) {
            let t = a[0],
                r = a[1],
                i = null
            if (r !== void 0 && ((i = pe.basicURLParse(r)), i === 'failure'))
                throw new TypeError('Invalid base URL')
            let o = pe.basicURLParse(t, { baseURL: i })
            if (o === 'failure') throw new TypeError('Invalid URL')
            this._url = o
        }
        get href() {
            return pe.serializeURL(this._url)
        }
        set href(a) {
            let t = pe.basicURLParse(a)
            if (t === 'failure') throw new TypeError('Invalid URL')
            this._url = t
        }
        get origin() {
            return pe.serializeURLOrigin(this._url)
        }
        get protocol() {
            return this._url.scheme + ':'
        }
        set protocol(a) {
            pe.basicURLParse(a + ':', {
                url: this._url,
                stateOverride: 'scheme start'
            })
        }
        get username() {
            return this._url.username
        }
        set username(a) {
            pe.cannotHaveAUsernamePasswordPort(this._url) ||
                pe.setTheUsername(this._url, a)
        }
        get password() {
            return this._url.password
        }
        set password(a) {
            pe.cannotHaveAUsernamePasswordPort(this._url) ||
                pe.setThePassword(this._url, a)
        }
        get host() {
            let a = this._url
            return a.host === null
                ? ''
                : a.port === null
                ? pe.serializeHost(a.host)
                : pe.serializeHost(a.host) + ':' + pe.serializeInteger(a.port)
        }
        set host(a) {
            this._url.cannotBeABaseURL ||
                pe.basicURLParse(a, { url: this._url, stateOverride: 'host' })
        }
        get hostname() {
            return this._url.host === null
                ? ''
                : pe.serializeHost(this._url.host)
        }
        set hostname(a) {
            this._url.cannotBeABaseURL ||
                pe.basicURLParse(a, {
                    url: this._url,
                    stateOverride: 'hostname'
                })
        }
        get port() {
            return this._url.port === null
                ? ''
                : pe.serializeInteger(this._url.port)
        }
        set port(a) {
            pe.cannotHaveAUsernamePasswordPort(this._url) ||
                (a === ''
                    ? (this._url.port = null)
                    : pe.basicURLParse(a, {
                          url: this._url,
                          stateOverride: 'port'
                      }))
        }
        get pathname() {
            return this._url.cannotBeABaseURL
                ? this._url.path[0]
                : this._url.path.length === 0
                ? ''
                : '/' + this._url.path.join('/')
        }
        set pathname(a) {
            this._url.cannotBeABaseURL ||
                ((this._url.path = []),
                pe.basicURLParse(a, {
                    url: this._url,
                    stateOverride: 'path start'
                }))
        }
        get search() {
            return this._url.query === null || this._url.query === ''
                ? ''
                : '?' + this._url.query
        }
        set search(a) {
            let t = this._url
            if (a === '') {
                t.query = null
                return
            }
            let r = a[0] === '?' ? a.substring(1) : a
            ;(t.query = ''),
                pe.basicURLParse(r, { url: t, stateOverride: 'query' })
        }
        get hash() {
            return this._url.fragment === null || this._url.fragment === ''
                ? ''
                : '#' + this._url.fragment
        }
        set hash(a) {
            if (a === '') {
                this._url.fragment = null
                return
            }
            let t = a[0] === '#' ? a.substring(1) : a
            ;(this._url.fragment = ''),
                pe.basicURLParse(t, {
                    url: this._url,
                    stateOverride: 'fragment'
                })
        }
        toJSON() {
            return this.href
        }
    }
})
var O0 = se((b3, Dt) => {
    'use strict'
    var Oe = u0(),
        k0 = c0(),
        B0 = q0(),
        Y = k0.implSymbol
    function ce(e) {
        if (!this || this[Y] || !(this instanceof ce))
            throw new TypeError(
                "Failed to construct 'URL': Please use the 'new' operator, this DOM object constructor cannot be called as a function."
            )
        if (arguments.length < 1)
            throw new TypeError(
                "Failed to construct 'URL': 1 argument required, but only " +
                    arguments.length +
                    ' present.'
            )
        let a = []
        for (let t = 0; t < arguments.length && t < 2; ++t) a[t] = arguments[t]
        ;(a[0] = Oe.USVString(a[0])),
            a[1] !== void 0 && (a[1] = Oe.USVString(a[1])),
            Dt.exports.setup(this, a)
    }
    ce.prototype.toJSON = function () {
        if (!this || !Dt.exports.is(this))
            throw new TypeError('Illegal invocation')
        let a = []
        for (let t = 0; t < arguments.length && t < 0; ++t) a[t] = arguments[t]
        return this[Y].toJSON.apply(this[Y], a)
    }
    Object.defineProperty(ce.prototype, 'href', {
        get() {
            return this[Y].href
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].href = e)
        },
        enumerable: !0,
        configurable: !0
    })
    ce.prototype.toString = function () {
        if (!this || !Dt.exports.is(this))
            throw new TypeError('Illegal invocation')
        return this.href
    }
    Object.defineProperty(ce.prototype, 'origin', {
        get() {
            return this[Y].origin
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'protocol', {
        get() {
            return this[Y].protocol
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].protocol = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'username', {
        get() {
            return this[Y].username
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].username = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'password', {
        get() {
            return this[Y].password
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].password = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'host', {
        get() {
            return this[Y].host
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].host = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'hostname', {
        get() {
            return this[Y].hostname
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].hostname = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'port', {
        get() {
            return this[Y].port
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].port = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'pathname', {
        get() {
            return this[Y].pathname
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].pathname = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'search', {
        get() {
            return this[Y].search
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].search = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Object.defineProperty(ce.prototype, 'hash', {
        get() {
            return this[Y].hash
        },
        set(e) {
            ;(e = Oe.USVString(e)), (this[Y].hash = e)
        },
        enumerable: !0,
        configurable: !0
    })
    Dt.exports = {
        is(e) {
            return !!e && e[Y] instanceof B0.implementation
        },
        create(e, a) {
            let t = Object.create(ce.prototype)
            return this.setup(t, e, a), t
        },
        setup(e, a, t) {
            t || (t = {}),
                (t.wrapper = e),
                (e[Y] = new B0.implementation(a, t)),
                (e[Y][k0.wrapperSymbol] = e)
        },
        interface: ce,
        expose: { Window: { URL: ce }, Worker: { URL: ce } }
    }
})
var F0 = se((Je) => {
    'use strict'
    Je.URL = O0().interface
    Je.serializeURL = Xe().serializeURL
    Je.serializeURLOrigin = Xe().serializeURLOrigin
    Je.basicURLParse = Xe().basicURLParse
    Je.setTheUsername = Xe().setTheUsername
    Je.setThePassword = Xe().setThePassword
    Je.serializeHost = Xe().serializeHost
    Je.serializeInteger = Xe().serializeInteger
    Je.parseURL = Xe().parseURL
})
var G0 = se(($e, Q0) => {
    'use strict'
    Object.defineProperty($e, '__esModule', { value: !0 })
    function Ha(e) {
        return e && typeof e == 'object' && 'default' in e ? e.default : e
    }
    var Me = Ha(Ae('stream')),
        j0 = Ha(Ae('http')),
        Mp = Ha(Ae('url')),
        W0 = Ha(F0()),
        rs = Ha(Ae('https')),
        ua = Ha(Ae('zlib')),
        ds = Me.Readable,
        u1 = Symbol('buffer'),
        Od = Symbol('type'),
        Vt = class e {
            constructor() {
                this[Od] = ''
                let a = arguments[0],
                    t = arguments[1],
                    r = [],
                    i = 0
                if (a) {
                    let m = a,
                        u = Number(m.length)
                    for (let s = 0; s < u; s++) {
                        let f = m[s],
                            h
                        f instanceof Buffer
                            ? (h = f)
                            : ArrayBuffer.isView(f)
                            ? (h = Buffer.from(
                                  f.buffer,
                                  f.byteOffset,
                                  f.byteLength
                              ))
                            : f instanceof ArrayBuffer
                            ? (h = Buffer.from(f))
                            : f instanceof e
                            ? (h = f[u1])
                            : (h = Buffer.from(
                                  typeof f == 'string' ? f : String(f)
                              )),
                            (i += h.length),
                            r.push(h)
                    }
                }
                this[u1] = Buffer.concat(r)
                let o = t && t.type !== void 0 && String(t.type).toLowerCase()
                o && !/[^\u0020-\u007E]/.test(o) && (this[Od] = o)
            }
            get size() {
                return this[u1].length
            }
            get type() {
                return this[Od]
            }
            text() {
                return Promise.resolve(this[u1].toString())
            }
            arrayBuffer() {
                let a = this[u1],
                    t = a.buffer.slice(
                        a.byteOffset,
                        a.byteOffset + a.byteLength
                    )
                return Promise.resolve(t)
            }
            stream() {
                let a = new ds()
                return (
                    (a._read = function () {}),
                    a.push(this[u1]),
                    a.push(null),
                    a
                )
            }
            toString() {
                return '[object Blob]'
            }
            slice() {
                let a = this.size,
                    t = arguments[0],
                    r = arguments[1],
                    i,
                    o
                t === void 0
                    ? (i = 0)
                    : t < 0
                    ? (i = Math.max(a + t, 0))
                    : (i = Math.min(t, a)),
                    r === void 0
                        ? (o = a)
                        : r < 0
                        ? (o = Math.max(a + r, 0))
                        : (o = Math.min(r, a))
                let m = Math.max(o - i, 0),
                    s = this[u1].slice(i, i + m),
                    f = new e([], { type: arguments[2] })
                return (f[u1] = s), f
            }
        }
    Object.defineProperties(Vt.prototype, {
        size: { enumerable: !0 },
        type: { enumerable: !0 },
        slice: { enumerable: !0 }
    })
    Object.defineProperty(Vt.prototype, Symbol.toStringTag, {
        value: 'Blob',
        writable: !1,
        enumerable: !1,
        configurable: !0
    })
    function be(e, a, t) {
        Error.call(this, e),
            (this.message = e),
            (this.type = a),
            t && (this.code = this.errno = t.code),
            Error.captureStackTrace(this, this.constructor)
    }
    be.prototype = Object.create(Error.prototype)
    be.prototype.constructor = be
    be.prototype.name = 'FetchError'
    var Id
    try {
        Id = Ae('encoding').convert
    } catch {}
    var f1 = Symbol('Body internals'),
        x0 = Me.PassThrough
    function fe(e) {
        var a = this,
            t =
                arguments.length > 1 && arguments[1] !== void 0
                    ? arguments[1]
                    : {},
            r = t.size
        let i = r === void 0 ? 0 : r
        var o = t.timeout
        let m = o === void 0 ? 0 : o
        e == null
            ? (e = null)
            : z0(e)
            ? (e = Buffer.from(e.toString()))
            : Bt(e) ||
              Buffer.isBuffer(e) ||
              (Object.prototype.toString.call(e) === '[object ArrayBuffer]'
                  ? (e = Buffer.from(e))
                  : ArrayBuffer.isView(e)
                  ? (e = Buffer.from(e.buffer, e.byteOffset, e.byteLength))
                  : e instanceof Me || (e = Buffer.from(String(e)))),
            (this[f1] = { body: e, disturbed: !1, error: null }),
            (this.size = i),
            (this.timeout = m),
            e instanceof Me &&
                e.on('error', function (u) {
                    let s =
                        u.name === 'AbortError'
                            ? u
                            : new be(
                                  `Invalid response body while trying to fetch ${a.url}: ${u.message}`,
                                  'system',
                                  u
                              )
                    a[f1].error = s
                })
    }
    fe.prototype = {
        get body() {
            return this[f1].body
        },
        get bodyUsed() {
            return this[f1].disturbed
        },
        arrayBuffer() {
            return Ma.call(this).then(function (e) {
                return e.buffer.slice(e.byteOffset, e.byteOffset + e.byteLength)
            })
        },
        blob() {
            let e = (this.headers && this.headers.get('content-type')) || ''
            return Ma.call(this).then(function (a) {
                return Object.assign(new Vt([], { type: e.toLowerCase() }), {
                    [u1]: a
                })
            })
        },
        json() {
            var e = this
            return Ma.call(this).then(function (a) {
                try {
                    return JSON.parse(a.toString())
                } catch (t) {
                    return fe.Promise.reject(
                        new be(
                            `invalid json response body at ${e.url} reason: ${t.message}`,
                            'invalid-json'
                        )
                    )
                }
            })
        },
        text() {
            return Ma.call(this).then(function (e) {
                return e.toString()
            })
        },
        buffer() {
            return Ma.call(this)
        },
        textConverted() {
            var e = this
            return Ma.call(this).then(function (a) {
                return is(a, e.headers)
            })
        }
    }
    Object.defineProperties(fe.prototype, {
        body: { enumerable: !0 },
        bodyUsed: { enumerable: !0 },
        arrayBuffer: { enumerable: !0 },
        blob: { enumerable: !0 },
        json: { enumerable: !0 },
        text: { enumerable: !0 }
    })
    fe.mixIn = function (e) {
        for (let a of Object.getOwnPropertyNames(fe.prototype))
            if (!(a in e)) {
                let t = Object.getOwnPropertyDescriptor(fe.prototype, a)
                Object.defineProperty(e, a, t)
            }
    }
    function Ma() {
        var e = this
        if (this[f1].disturbed)
            return fe.Promise.reject(
                new TypeError(`body used already for: ${this.url}`)
            )
        if (((this[f1].disturbed = !0), this[f1].error))
            return fe.Promise.reject(this[f1].error)
        let a = this.body
        if (a === null) return fe.Promise.resolve(Buffer.alloc(0))
        if ((Bt(a) && (a = a.stream()), Buffer.isBuffer(a)))
            return fe.Promise.resolve(a)
        if (!(a instanceof Me)) return fe.Promise.resolve(Buffer.alloc(0))
        let t = [],
            r = 0,
            i = !1
        return new fe.Promise(function (o, m) {
            let u
            e.timeout &&
                (u = setTimeout(function () {
                    ;(i = !0),
                        m(
                            new be(
                                `Response timeout while trying to fetch ${e.url} (over ${e.timeout}ms)`,
                                'body-timeout'
                            )
                        )
                }, e.timeout)),
                a.on('error', function (s) {
                    s.name === 'AbortError'
                        ? ((i = !0), m(s))
                        : m(
                              new be(
                                  `Invalid response body while trying to fetch ${e.url}: ${s.message}`,
                                  'system',
                                  s
                              )
                          )
                }),
                a.on('data', function (s) {
                    if (!(i || s === null)) {
                        if (e.size && r + s.length > e.size) {
                            ;(i = !0),
                                m(
                                    new be(
                                        `content size at ${e.url} over limit: ${e.size}`,
                                        'max-size'
                                    )
                                )
                            return
                        }
                        ;(r += s.length), t.push(s)
                    }
                }),
                a.on('end', function () {
                    if (!i) {
                        clearTimeout(u)
                        try {
                            o(Buffer.concat(t, r))
                        } catch (s) {
                            m(
                                new be(
                                    `Could not create Buffer from response body for ${e.url}: ${s.message}`,
                                    'system',
                                    s
                                )
                            )
                        }
                    }
                })
        })
    }
    function is(e, a) {
        if (typeof Id != 'function')
            throw new Error(
                'The package `encoding` must be installed to use the textConverted() function'
            )
        let t = a.get('content-type'),
            r = 'utf-8',
            i,
            o
        return (
            t && (i = /charset=([^;]*)/i.exec(t)),
            (o = e.slice(0, 1024).toString()),
            !i && o && (i = /<meta.+?charset=(['"])(.+?)\1/i.exec(o)),
            !i &&
                o &&
                ((i =
                    /<meta[\s]+?http-equiv=(['"])content-type\1[\s]+?content=(['"])(.+?)\2/i.exec(
                        o
                    )),
                i ||
                    ((i =
                        /<meta[\s]+?content=(['"])(.+?)\1[\s]+?http-equiv=(['"])content-type\3/i.exec(
                            o
                        )),
                    i && i.pop()),
                i && (i = /charset=(.*)/i.exec(i.pop()))),
            !i && o && (i = /<\?xml.+?encoding=(['"])(.+?)\1/i.exec(o)),
            i &&
                ((r = i.pop()),
                (r === 'gb2312' || r === 'gbk') && (r = 'gb18030')),
            Id(e, 'UTF-8', r).toString()
        )
    }
    function z0(e) {
        return typeof e != 'object' ||
            typeof e.append != 'function' ||
            typeof e.delete != 'function' ||
            typeof e.get != 'function' ||
            typeof e.getAll != 'function' ||
            typeof e.has != 'function' ||
            typeof e.set != 'function'
            ? !1
            : e.constructor.name === 'URLSearchParams' ||
                  Object.prototype.toString.call(e) ===
                      '[object URLSearchParams]' ||
                  typeof e.sort == 'function'
    }
    function Bt(e) {
        return (
            typeof e == 'object' &&
            typeof e.arrayBuffer == 'function' &&
            typeof e.type == 'string' &&
            typeof e.stream == 'function' &&
            typeof e.constructor == 'function' &&
            typeof e.constructor.name == 'string' &&
            /^(Blob|File)$/.test(e.constructor.name) &&
            /^(Blob|File)$/.test(e[Symbol.toStringTag])
        )
    }
    function M0(e) {
        let a,
            t,
            r = e.body
        if (e.bodyUsed) throw new Error('cannot clone body after it is used')
        return (
            r instanceof Me &&
                typeof r.getBoundary != 'function' &&
                ((a = new x0()),
                (t = new x0()),
                r.pipe(a),
                r.pipe(t),
                (e[f1].body = a),
                (r = t)),
            r
        )
    }
    function $0(e) {
        return e === null
            ? null
            : typeof e == 'string'
            ? 'text/plain;charset=UTF-8'
            : z0(e)
            ? 'application/x-www-form-urlencoded;charset=UTF-8'
            : Bt(e)
            ? e.type || null
            : Buffer.isBuffer(e) ||
              Object.prototype.toString.call(e) === '[object ArrayBuffer]' ||
              ArrayBuffer.isView(e)
            ? null
            : typeof e.getBoundary == 'function'
            ? `multipart/form-data;boundary=${e.getBoundary()}`
            : e instanceof Me
            ? null
            : 'text/plain;charset=UTF-8'
    }
    function U0(e) {
        let a = e.body
        return a === null
            ? 0
            : Bt(a)
            ? a.size
            : Buffer.isBuffer(a)
            ? a.length
            : a &&
              typeof a.getLengthSync == 'function' &&
              ((a._lengthRetrievers && a._lengthRetrievers.length == 0) ||
                  (a.hasKnownLength && a.hasKnownLength()))
            ? a.getLengthSync()
            : null
    }
    function ns(e, a) {
        let t = a.body
        t === null
            ? e.end()
            : Bt(t)
            ? t.stream().pipe(e)
            : Buffer.isBuffer(t)
            ? (e.write(t), e.end())
            : t.pipe(e)
    }
    fe.Promise = global.Promise
    var H0 = /[^\^_`a-zA-Z\-0-9!#$%&'*+.|~]/,
        jd = /[^\t\x20-\x7e\x80-\xff]/
    function At(e) {
        if (((e = `${e}`), H0.test(e) || e === ''))
            throw new TypeError(`${e} is not a legal HTTP header name`)
    }
    function L0(e) {
        if (((e = `${e}`), jd.test(e)))
            throw new TypeError(`${e} is not a legal HTTP header value`)
    }
    function $a(e, a) {
        a = a.toLowerCase()
        for (let t in e) if (t.toLowerCase() === a) return t
    }
    var re = Symbol('map'),
        xe = class e {
            constructor() {
                let a =
                    arguments.length > 0 && arguments[0] !== void 0
                        ? arguments[0]
                        : void 0
                if (((this[re] = Object.create(null)), a instanceof e)) {
                    let t = a.raw(),
                        r = Object.keys(t)
                    for (let i of r) for (let o of t[i]) this.append(i, o)
                    return
                }
                if (a != null)
                    if (typeof a == 'object') {
                        let t = a[Symbol.iterator]
                        if (t != null) {
                            if (typeof t != 'function')
                                throw new TypeError(
                                    'Header pairs must be iterable'
                                )
                            let r = []
                            for (let i of a) {
                                if (
                                    typeof i != 'object' ||
                                    typeof i[Symbol.iterator] != 'function'
                                )
                                    throw new TypeError(
                                        'Each header pair must be iterable'
                                    )
                                r.push(Array.from(i))
                            }
                            for (let i of r) {
                                if (i.length !== 2)
                                    throw new TypeError(
                                        'Each header pair must be a name/value tuple'
                                    )
                                this.append(i[0], i[1])
                            }
                        } else
                            for (let r of Object.keys(a)) {
                                let i = a[r]
                                this.append(r, i)
                            }
                    } else
                        throw new TypeError(
                            'Provided initializer must be an object'
                        )
            }
            get(a) {
                ;(a = `${a}`), At(a)
                let t = $a(this[re], a)
                return t === void 0 ? null : this[re][t].join(', ')
            }
            forEach(a) {
                let t =
                        arguments.length > 1 && arguments[1] !== void 0
                            ? arguments[1]
                            : void 0,
                    r = Wd(this),
                    i = 0
                for (; i < r.length; ) {
                    var o = r[i]
                    let m = o[0],
                        u = o[1]
                    a.call(t, u, m, this), (r = Wd(this)), i++
                }
            }
            set(a, t) {
                ;(a = `${a}`), (t = `${t}`), At(a), L0(t)
                let r = $a(this[re], a)
                this[re][r !== void 0 ? r : a] = [t]
            }
            append(a, t) {
                ;(a = `${a}`), (t = `${t}`), At(a), L0(t)
                let r = $a(this[re], a)
                r !== void 0 ? this[re][r].push(t) : (this[re][a] = [t])
            }
            has(a) {
                return (a = `${a}`), At(a), $a(this[re], a) !== void 0
            }
            delete(a) {
                ;(a = `${a}`), At(a)
                let t = $a(this[re], a)
                t !== void 0 && delete this[re][t]
            }
            raw() {
                return this[re]
            }
            keys() {
                return Fd(this, 'key')
            }
            values() {
                return Fd(this, 'value')
            }
            [Symbol.iterator]() {
                return Fd(this, 'key+value')
            }
        }
    xe.prototype.entries = xe.prototype[Symbol.iterator]
    Object.defineProperty(xe.prototype, Symbol.toStringTag, {
        value: 'Headers',
        writable: !1,
        enumerable: !1,
        configurable: !0
    })
    Object.defineProperties(xe.prototype, {
        get: { enumerable: !0 },
        forEach: { enumerable: !0 },
        set: { enumerable: !0 },
        append: { enumerable: !0 },
        has: { enumerable: !0 },
        delete: { enumerable: !0 },
        keys: { enumerable: !0 },
        values: { enumerable: !0 },
        entries: { enumerable: !0 }
    })
    function Wd(e) {
        let a =
            arguments.length > 1 && arguments[1] !== void 0
                ? arguments[1]
                : 'key+value'
        return Object.keys(e[re])
            .sort()
            .map(
                a === 'key'
                    ? function (r) {
                          return r.toLowerCase()
                      }
                    : a === 'value'
                    ? function (r) {
                          return e[re][r].join(', ')
                      }
                    : function (r) {
                          return [r.toLowerCase(), e[re][r].join(', ')]
                      }
            )
    }
    var zd = Symbol('internal')
    function Fd(e, a) {
        let t = Object.create(Md)
        return (t[zd] = { target: e, kind: a, index: 0 }), t
    }
    var Md = Object.setPrototypeOf(
        {
            next() {
                if (!this || Object.getPrototypeOf(this) !== Md)
                    throw new TypeError(
                        'Value of `this` is not a HeadersIterator'
                    )
                var e = this[zd]
                let a = e.target,
                    t = e.kind,
                    r = e.index,
                    i = Wd(a, t),
                    o = i.length
                return r >= o
                    ? { value: void 0, done: !0 }
                    : ((this[zd].index = r + 1), { value: i[r], done: !1 })
            }
        },
        Object.getPrototypeOf(Object.getPrototypeOf([][Symbol.iterator]()))
    )
    Object.defineProperty(Md, Symbol.toStringTag, {
        value: 'HeadersIterator',
        writable: !1,
        enumerable: !1,
        configurable: !0
    })
    function os(e) {
        let a = Object.assign({ __proto__: null }, e[re]),
            t = $a(e[re], 'Host')
        return t !== void 0 && (a[t] = a[t][0]), a
    }
    function ls(e) {
        let a = new xe()
        for (let t of Object.keys(e))
            if (!H0.test(t))
                if (Array.isArray(e[t]))
                    for (let r of e[t])
                        jd.test(r) ||
                            (a[re][t] === void 0
                                ? (a[re][t] = [r])
                                : a[re][t].push(r))
                else jd.test(e[t]) || (a[re][t] = [e[t]])
        return a
    }
    var j1 = Symbol('Response internals'),
        ss = j0.STATUS_CODES,
        Fe = class e {
            constructor() {
                let a =
                        arguments.length > 0 && arguments[0] !== void 0
                            ? arguments[0]
                            : null,
                    t =
                        arguments.length > 1 && arguments[1] !== void 0
                            ? arguments[1]
                            : {}
                fe.call(this, a, t)
                let r = t.status || 200,
                    i = new xe(t.headers)
                if (a != null && !i.has('Content-Type')) {
                    let o = $0(a)
                    o && i.append('Content-Type', o)
                }
                this[j1] = {
                    url: t.url,
                    status: r,
                    statusText: t.statusText || ss[r],
                    headers: i,
                    counter: t.counter
                }
            }
            get url() {
                return this[j1].url || ''
            }
            get status() {
                return this[j1].status
            }
            get ok() {
                return this[j1].status >= 200 && this[j1].status < 300
            }
            get redirected() {
                return this[j1].counter > 0
            }
            get statusText() {
                return this[j1].statusText
            }
            get headers() {
                return this[j1].headers
            }
            clone() {
                return new e(M0(this), {
                    url: this.url,
                    status: this.status,
                    statusText: this.statusText,
                    headers: this.headers,
                    ok: this.ok,
                    redirected: this.redirected
                })
            }
        }
    fe.mixIn(Fe.prototype)
    Object.defineProperties(Fe.prototype, {
        url: { enumerable: !0 },
        status: { enumerable: !0 },
        ok: { enumerable: !0 },
        redirected: { enumerable: !0 },
        statusText: { enumerable: !0 },
        headers: { enumerable: !0 },
        clone: { enumerable: !0 }
    })
    Object.defineProperty(Fe.prototype, Symbol.toStringTag, {
        value: 'Response',
        writable: !1,
        enumerable: !1,
        configurable: !0
    })
    var c1 = Symbol('Request internals'),
        ms = Mp.URL || W0.URL,
        us = Mp.parse,
        cs = Mp.format
    function xd(e) {
        return (
            /^[a-zA-Z][a-zA-Z\d+\-.]*:/.exec(e) && (e = new ms(e).toString()),
            us(e)
        )
    }
    var fs = 'destroy' in Me.Readable.prototype
    function zp(e) {
        return typeof e == 'object' && typeof e[c1] == 'object'
    }
    function hs(e) {
        let a = e && typeof e == 'object' && Object.getPrototypeOf(e)
        return !!(a && a.constructor.name === 'AbortSignal')
    }
    var z1 = class e {
        constructor(a) {
            let t =
                    arguments.length > 1 && arguments[1] !== void 0
                        ? arguments[1]
                        : {},
                r
            zp(a)
                ? (r = xd(a.url))
                : (a && a.href ? (r = xd(a.href)) : (r = xd(`${a}`)), (a = {}))
            let i = t.method || a.method || 'GET'
            if (
                ((i = i.toUpperCase()),
                (t.body != null || (zp(a) && a.body !== null)) &&
                    (i === 'GET' || i === 'HEAD'))
            )
                throw new TypeError(
                    'Request with GET/HEAD method cannot have body'
                )
            let o =
                t.body != null
                    ? t.body
                    : zp(a) && a.body !== null
                    ? M0(a)
                    : null
            fe.call(this, o, {
                timeout: t.timeout || a.timeout || 0,
                size: t.size || a.size || 0
            })
            let m = new xe(t.headers || a.headers || {})
            if (o != null && !m.has('Content-Type')) {
                let s = $0(o)
                s && m.append('Content-Type', s)
            }
            let u = zp(a) ? a.signal : null
            if (('signal' in t && (u = t.signal), u != null && !hs(u)))
                throw new TypeError(
                    'Expected signal to be an instanceof AbortSignal'
                )
            ;(this[c1] = {
                method: i,
                redirect: t.redirect || a.redirect || 'follow',
                headers: m,
                parsedURL: r,
                signal: u
            }),
                (this.follow =
                    t.follow !== void 0
                        ? t.follow
                        : a.follow !== void 0
                        ? a.follow
                        : 20),
                (this.compress =
                    t.compress !== void 0
                        ? t.compress
                        : a.compress !== void 0
                        ? a.compress
                        : !0),
                (this.counter = t.counter || a.counter || 0),
                (this.agent = t.agent || a.agent)
        }
        get method() {
            return this[c1].method
        }
        get url() {
            return cs(this[c1].parsedURL)
        }
        get headers() {
            return this[c1].headers
        }
        get redirect() {
            return this[c1].redirect
        }
        get signal() {
            return this[c1].signal
        }
        clone() {
            return new e(this)
        }
    }
    fe.mixIn(z1.prototype)
    Object.defineProperty(z1.prototype, Symbol.toStringTag, {
        value: 'Request',
        writable: !1,
        enumerable: !1,
        configurable: !0
    })
    Object.defineProperties(z1.prototype, {
        method: { enumerable: !0 },
        url: { enumerable: !0 },
        headers: { enumerable: !0 },
        redirect: { enumerable: !0 },
        clone: { enumerable: !0 },
        signal: { enumerable: !0 }
    })
    function vs(e) {
        let a = e[c1].parsedURL,
            t = new xe(e[c1].headers)
        if (
            (t.has('Accept') || t.set('Accept', '*/*'),
            !a.protocol || !a.hostname)
        )
            throw new TypeError('Only absolute URLs are supported')
        if (!/^https?:$/.test(a.protocol))
            throw new TypeError('Only HTTP(S) protocols are supported')
        if (e.signal && e.body instanceof Me.Readable && !fs)
            throw new Error(
                'Cancellation of streamed requests with AbortSignal is not supported in node < 8'
            )
        let r = null
        if (
            (e.body == null && /^(POST|PUT)$/i.test(e.method) && (r = '0'),
            e.body != null)
        ) {
            let o = U0(e)
            typeof o == 'number' && (r = String(o))
        }
        r && t.set('Content-Length', r),
            t.has('User-Agent') ||
                t.set(
                    'User-Agent',
                    'node-fetch/1.0 (+https://github.com/bitinn/node-fetch)'
                ),
            e.compress &&
                !t.has('Accept-Encoding') &&
                t.set('Accept-Encoding', 'gzip,deflate')
        let i = e.agent
        return (
            typeof i == 'function' && (i = i(a)),
            Object.assign({}, a, { method: e.method, headers: os(t), agent: i })
        )
    }
    function Ua(e) {
        Error.call(this, e),
            (this.type = 'aborted'),
            (this.message = e),
            Error.captureStackTrace(this, this.constructor)
    }
    Ua.prototype = Object.create(Error.prototype)
    Ua.prototype.constructor = Ua
    Ua.prototype.name = 'AbortError'
    var qt = Mp.URL || W0.URL,
        I0 = Me.PassThrough,
        bs = function (a, t) {
            let r = new qt(t).hostname,
                i = new qt(a).hostname
            return (
                r === i || (r[r.length - i.length - 1] === '.' && r.endsWith(i))
            )
        },
        ws = function (a, t) {
            let r = new qt(t).protocol,
                i = new qt(a).protocol
            return r === i
        }
    function W1(e, a) {
        if (!W1.Promise)
            throw new Error(
                'native promise missing, set fetch.Promise to your favorite alternative'
            )
        return (
            (fe.Promise = W1.Promise),
            new W1.Promise(function (t, r) {
                let i = new z1(e, a),
                    o = vs(i),
                    m = (o.protocol === 'https:' ? rs : j0).request,
                    u = i.signal,
                    s = null,
                    f = function () {
                        let C = new Ua('The user aborted a request.')
                        r(C),
                            i.body &&
                                i.body instanceof Me.Readable &&
                                Ld(i.body, C),
                            !(!s || !s.body) && s.body.emit('error', C)
                    }
                if (u && u.aborted) {
                    f()
                    return
                }
                let h = function () {
                        f(), g()
                    },
                    _ = m(o),
                    w
                u && u.addEventListener('abort', h)
                function g() {
                    _.abort(),
                        u && u.removeEventListener('abort', h),
                        clearTimeout(w)
                }
                i.timeout &&
                    _.once('socket', function (y) {
                        w = setTimeout(function () {
                            r(
                                new be(
                                    `network timeout at: ${i.url}`,
                                    'request-timeout'
                                )
                            ),
                                g()
                        }, i.timeout)
                    }),
                    _.on('error', function (y) {
                        r(
                            new be(
                                `request to ${i.url} failed, reason: ${y.message}`,
                                'system',
                                y
                            )
                        ),
                            s && s.body && Ld(s.body, y),
                            g()
                    }),
                    _s(_, function (y) {
                        ;(u && u.aborted) || (s && s.body && Ld(s.body, y))
                    }),
                    parseInt(process.version.substring(1)) < 14 &&
                        _.on('socket', function (y) {
                            y.addListener('close', function (C) {
                                let R = y.listenerCount('data') > 0
                                if (s && R && !C && !(u && u.aborted)) {
                                    let T = new Error('Premature close')
                                    ;(T.code = 'ERR_STREAM_PREMATURE_CLOSE'),
                                        s.body.emit('error', T)
                                }
                            })
                        }),
                    _.on('response', function (y) {
                        clearTimeout(w)
                        let C = ls(y.headers)
                        if (W1.isRedirect(y.statusCode)) {
                            let I = C.get('Location'),
                                k = null
                            try {
                                k =
                                    I === null
                                        ? null
                                        : new qt(I, i.url).toString()
                            } catch {
                                if (i.redirect !== 'manual') {
                                    r(
                                        new be(
                                            `uri requested responds with an invalid redirect URL: ${I}`,
                                            'invalid-redirect'
                                        )
                                    ),
                                        g()
                                    return
                                }
                            }
                            switch (i.redirect) {
                                case 'error':
                                    r(
                                        new be(
                                            `uri requested responds with a redirect, redirect mode is set to error: ${i.url}`,
                                            'no-redirect'
                                        )
                                    ),
                                        g()
                                    return
                                case 'manual':
                                    if (k !== null)
                                        try {
                                            C.set('Location', k)
                                        } catch (ve) {
                                            r(ve)
                                        }
                                    break
                                case 'follow':
                                    if (k === null) break
                                    if (i.counter >= i.follow) {
                                        r(
                                            new be(
                                                `maximum redirect reached at: ${i.url}`,
                                                'max-redirect'
                                            )
                                        ),
                                            g()
                                        return
                                    }
                                    let Z = {
                                        headers: new xe(i.headers),
                                        follow: i.follow,
                                        counter: i.counter + 1,
                                        agent: i.agent,
                                        compress: i.compress,
                                        method: i.method,
                                        body: i.body,
                                        signal: i.signal,
                                        timeout: i.timeout,
                                        size: i.size
                                    }
                                    if (!bs(i.url, k) || !ws(i.url, k))
                                        for (let ve of [
                                            'authorization',
                                            'www-authenticate',
                                            'cookie',
                                            'cookie2'
                                        ])
                                            Z.headers.delete(ve)
                                    if (
                                        y.statusCode !== 303 &&
                                        i.body &&
                                        U0(i) === null
                                    ) {
                                        r(
                                            new be(
                                                'Cannot follow redirect with body being a readable stream',
                                                'unsupported-redirect'
                                            )
                                        ),
                                            g()
                                        return
                                    }
                                    ;(y.statusCode === 303 ||
                                        ((y.statusCode === 301 ||
                                            y.statusCode === 302) &&
                                            i.method === 'POST')) &&
                                        ((Z.method = 'GET'),
                                        (Z.body = void 0),
                                        Z.headers.delete('content-length')),
                                        t(W1(new z1(k, Z))),
                                        g()
                                    return
                            }
                        }
                        y.once('end', function () {
                            u && u.removeEventListener('abort', h)
                        })
                        let R = y.pipe(new I0()),
                            T = {
                                url: i.url,
                                status: y.statusCode,
                                statusText: y.statusMessage,
                                headers: C,
                                size: i.size,
                                timeout: i.timeout,
                                counter: i.counter
                            },
                            F = C.get('Content-Encoding')
                        if (
                            !i.compress ||
                            i.method === 'HEAD' ||
                            F === null ||
                            y.statusCode === 204 ||
                            y.statusCode === 304
                        ) {
                            ;(s = new Fe(R, T)), t(s)
                            return
                        }
                        let K = {
                            flush: ua.Z_SYNC_FLUSH,
                            finishFlush: ua.Z_SYNC_FLUSH
                        }
                        if (F == 'gzip' || F == 'x-gzip') {
                            ;(R = R.pipe(ua.createGunzip(K))),
                                (s = new Fe(R, T)),
                                t(s)
                            return
                        }
                        if (F == 'deflate' || F == 'x-deflate') {
                            let I = y.pipe(new I0())
                            I.once('data', function (k) {
                                ;(k[0] & 15) === 8
                                    ? (R = R.pipe(ua.createInflate()))
                                    : (R = R.pipe(ua.createInflateRaw())),
                                    (s = new Fe(R, T)),
                                    t(s)
                            }),
                                I.on('end', function () {
                                    s || ((s = new Fe(R, T)), t(s))
                                })
                            return
                        }
                        if (
                            F == 'br' &&
                            typeof ua.createBrotliDecompress == 'function'
                        ) {
                            ;(R = R.pipe(ua.createBrotliDecompress())),
                                (s = new Fe(R, T)),
                                t(s)
                            return
                        }
                        ;(s = new Fe(R, T)), t(s)
                    }),
                    ns(_, i)
            })
        )
    }
    function _s(e, a) {
        let t
        e.on('socket', function (r) {
            t = r
        }),
            e.on('response', function (r) {
                let i = r.headers
                i['transfer-encoding'] === 'chunked' &&
                    !i['content-length'] &&
                    r.once('close', function (o) {
                        if (t && t.listenerCount('data') > 0 && !o) {
                            let u = new Error('Premature close')
                            ;(u.code = 'ERR_STREAM_PREMATURE_CLOSE'), a(u)
                        }
                    })
            })
    }
    function Ld(e, a) {
        e.destroy ? e.destroy(a) : (e.emit('error', a), e.end())
    }
    W1.isRedirect = function (e) {
        return e === 301 || e === 302 || e === 303 || e === 307 || e === 308
    }
    W1.Promise = global.Promise
    Q0.exports = $e = W1
    Object.defineProperty($e, '__esModule', { value: !0 })
    $e.default = $e
    $e.Headers = xe
    $e.Request = z1
    $e.Response = Fe
    $e.FetchError = be
    $e.AbortError = Ua
})
function yn() {}
function ye(e) {
    return (typeof e == 'object' && e !== null) || typeof e == 'function'
}
function U(e, a) {
    try {
        Object.defineProperty(e, 'name', { value: a, configurable: !0 })
    } catch {}
}
function Te(e) {
    return new t2(e)
}
function H(e) {
    return gs(e)
}
function A(e) {
    return Ss(e)
}
function g1(e, a, t) {
    return ys.call(e, a, t)
}
function _e(e, a, t) {
    g1(g1(e, a, t), void 0, gn)
}
function Y0(e, a) {
    _e(e, a)
}
function K0(e, a) {
    _e(e, void 0, a)
}
function Ue(e, a, t) {
    return g1(e, a, t)
}
function Ka(e) {
    g1(e, void 0, gn)
}
function ur(e, a, t) {
    if (typeof e != 'function')
        throw new TypeError('Argument is not a function')
    return Function.prototype.apply.call(e, a, t)
}
function _a(e, a, t) {
    try {
        return H(ur(e, a, t))
    } catch (r) {
        return A(r)
    }
}
function Rn(e, a) {
    ;(e._ownerReadableStream = a),
        (a._reader = e),
        a._state === 'readable'
            ? Yd(e)
            : a._state === 'closed'
            ? (function (t) {
                  Yd(t), Nn(t)
              })(e)
            : Pn(e, a._storedError)
}
function Cn(e, a) {
    return ro(e._ownerReadableStream, a)
}
function En(e) {
    let a = e._ownerReadableStream
    a._state === 'readable'
        ? i2(
              e,
              new TypeError(
                  "Reader was released and can no longer be used to monitor the stream's closedness"
              )
          )
        : (function (t, r) {
              Pn(t, r)
          })(
              e,
              new TypeError(
                  "Reader was released and can no longer be used to monitor the stream's closedness"
              )
          ),
        a._readableStreamController[d2](),
        (a._reader = void 0),
        (e._ownerReadableStream = void 0)
}
function Xa(e) {
    return new TypeError('Cannot ' + e + ' a stream using a released reader')
}
function Yd(e) {
    e._closedPromise = Te((a, t) => {
        ;(e._closedPromise_resolve = a), (e._closedPromise_reject = t)
    })
}
function Pn(e, a) {
    Yd(e), i2(e, a)
}
function i2(e, a) {
    e._closedPromise_reject !== void 0 &&
        (Ka(e._closedPromise),
        e._closedPromise_reject(a),
        (e._closedPromise_resolve = void 0),
        (e._closedPromise_reject = void 0))
}
function Nn(e) {
    e._closedPromise_resolve !== void 0 &&
        (e._closedPromise_resolve(void 0),
        (e._closedPromise_resolve = void 0),
        (e._closedPromise_reject = void 0))
}
function S1(e, a) {
    if (e !== void 0 && typeof (t = e) != 'object' && typeof t != 'function')
        throw new TypeError(`${a} is not an object.`)
    var t
}
function He(e, a) {
    if (typeof e != 'function') throw new TypeError(`${a} is not a function.`)
}
function Dn(e, a) {
    if (
        !(function (t) {
            return (
                (typeof t == 'object' && t !== null) || typeof t == 'function'
            )
        })(e)
    )
        throw new TypeError(`${a} is not an object.`)
}
function T1(e, a, t) {
    if (e === void 0)
        throw new TypeError(`Parameter ${a} is required in '${t}'.`)
}
function Kd(e, a, t) {
    if (e === void 0) throw new TypeError(`${a} is required in '${t}'.`)
}
function n2(e) {
    return Number(e)
}
function J0(e) {
    return e === 0 ? 0 : e
}
function An(e, a) {
    let t = Number.MAX_SAFE_INTEGER,
        r = Number(e)
    if (((r = J0(r)), !X0(r)))
        throw new TypeError(`${a} is not a finite number`)
    if (
        ((r = (function (i) {
            return J0(Ts(i))
        })(r)),
        r < 0 || r > t)
    )
        throw new TypeError(
            `${a} is outside the accepted range of 0 to ${t}, inclusive`
        )
    return X0(r) && r !== 0 ? r : 0
}
function Ft(e) {
    if (!ye(e) || typeof e.getReader != 'function') return !1
    try {
        return typeof e.locked == 'boolean'
    } catch {
        return !1
    }
}
function Vn(e) {
    if (!ye(e) || typeof e.getWriter != 'function') return !1
    try {
        return typeof e.locked == 'boolean'
    } catch {
        return !1
    }
}
function qn(e, a) {
    if (!ba(e)) throw new TypeError(`${a} is not a ReadableStream.`)
}
function Bn(e, a) {
    e._reader._readRequests.push(a)
}
function o2(e, a, t) {
    let r = e._reader._readRequests.shift()
    t ? r._closeSteps() : r._chunkSteps(a)
}
function cr(e) {
    return e._reader._readRequests.length
}
function kn(e) {
    let a = e._reader
    return a !== void 0 && !!va(a)
}
function va(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(e, '_readRequests') &&
        e instanceof h1
    )
}
function On(e, a) {
    let t = e._readRequests
    ;(e._readRequests = new Ee()),
        t.forEach((r) => {
            r._errorSteps(a)
        })
}
function $p(e) {
    return new TypeError(
        `ReadableStreamDefaultReader.prototype.${e} can only be used on a ReadableStreamDefaultReader`
    )
}
function Z0(e) {
    if (
        !ye(e) ||
        !Object.prototype.hasOwnProperty.call(e, '_asyncIteratorImpl')
    )
        return !1
    try {
        return e._asyncIteratorImpl instanceof pr
    } catch {
        return !1
    }
}
function en(e) {
    return new TypeError(
        `ReadableStreamAsyncIterator.${e} can only be used on a ReadableSteamAsyncIterator`
    )
}
function Ln(e, a, t, r, i) {
    new Uint8Array(e).set(new Uint8Array(t, r, i), a)
}
function an(e) {
    let a = (function (t, r, i) {
        if (t.slice) return t.slice(r, i)
        let o = i - r,
            m = new ArrayBuffer(o)
        return Ln(m, 0, t, r, o), m
    })(e.buffer, e.byteOffset, e.byteOffset + e.byteLength)
    return new Uint8Array(a)
}
function Xd(e) {
    let a = e._queue.shift()
    return (
        (e._queueTotalSize -= a.size),
        e._queueTotalSize < 0 && (e._queueTotalSize = 0),
        a.value
    )
}
function l2(e, a, t) {
    if (typeof (r = t) != 'number' || xn(r) || r < 0 || t === 1 / 0)
        throw new RangeError(
            'Size must be a finite, non-NaN, non-negative number.'
        )
    var r
    e._queue.push({ value: a, size: t }), (e._queueTotalSize += t)
}
function U1(e) {
    ;(e._queue = new Ee()), (e._queueTotalSize = 0)
}
function Qa(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_controlledReadableByteStream'
        ) &&
        e instanceof v1
    )
}
function $d(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_associatedReadableByteStreamController'
        ) &&
        e instanceof M1
    )
}
function wa(e) {
    if (
        (function (t) {
            let r = t._controlledReadableByteStream
            return r._state !== 'readable' || t._closeRequested || !t._started
                ? !1
                : !!((kn(r) && cr(r) > 0) || (m2(r) && Qn(r) > 0) || Hn(t) > 0)
        })(e)
    ) {
        if (e._pulling) return void (e._pullAgain = !0)
        ;(e._pulling = !0),
            _e(
                e._pullAlgorithm(),
                () => (
                    (e._pulling = !1),
                    e._pullAgain && ((e._pullAgain = !1), wa(e)),
                    null
                ),
                (t) => (Ja(e, t), null)
            )
    }
}
function In(e) {
    s2(e), (e._pendingPullIntos = new Ee())
}
function Jd(e, a) {
    let t = !1
    e._state === 'closed' && (t = !0)
    let r = jn(a)
    a.readerType === 'default'
        ? o2(e, r, t)
        : (function (i, o, m) {
              let u = i._reader._readIntoRequests.shift()
              m ? u._closeSteps(o) : u._chunkSteps(o)
          })(e, r, t)
}
function jn(e) {
    let a = e.bytesFilled,
        t = e.elementSize
    return new e.viewConstructor(e.buffer, e.byteOffset, a / t)
}
function Zp(e, a, t, r) {
    e._queue.push({ buffer: a, byteOffset: t, byteLength: r }),
        (e._queueTotalSize += r)
}
function Wn(e, a, t, r) {
    let i
    try {
        i = a.slice(t, t + r)
    } catch (o) {
        throw (Ja(e, o), o)
    }
    Zp(e, i, 0, r)
}
function zn(e, a) {
    a.bytesFilled > 0 && Wn(e, a.buffer, a.byteOffset, a.bytesFilled), Ya(e)
}
function Mn(e, a) {
    let t = a.elementSize,
        r = a.bytesFilled - (a.bytesFilled % t),
        i = Math.min(e._queueTotalSize, a.byteLength - a.bytesFilled),
        o = a.bytesFilled + i,
        m = o - (o % t),
        u = i,
        s = !1
    m > r && ((u = m - a.bytesFilled), (s = !0))
    let f = e._queue
    for (; u > 0; ) {
        let h = f.peek(),
            _ = Math.min(u, h.byteLength),
            w = a.byteOffset + a.bytesFilled
        Ln(a.buffer, w, h.buffer, h.byteOffset, _),
            h.byteLength === _
                ? f.shift()
                : ((h.byteOffset += _), (h.byteLength -= _)),
            (e._queueTotalSize -= _),
            $n(e, _, a),
            (u -= _)
    }
    return s
}
function $n(e, a, t) {
    t.bytesFilled += a
}
function Un(e) {
    e._queueTotalSize === 0 && e._closeRequested
        ? (rr(e), Wt(e._controlledReadableByteStream))
        : wa(e)
}
function s2(e) {
    e._byobRequest !== null &&
        ((e._byobRequest._associatedReadableByteStreamController = void 0),
        (e._byobRequest._view = null),
        (e._byobRequest = null))
}
function Zd(e) {
    for (; e._pendingPullIntos.length > 0; ) {
        if (e._queueTotalSize === 0) return
        let a = e._pendingPullIntos.peek()
        Mn(e, a) && (Ya(e), Jd(e._controlledReadableByteStream, a))
    }
}
function tn(e, a) {
    let t = e._pendingPullIntos.peek()
    s2(e),
        e._controlledReadableByteStream._state === 'closed'
            ? (function (r, i) {
                  i.readerType === 'none' && Ya(r)
                  let o = r._controlledReadableByteStream
                  if (m2(o)) for (; Qn(o) > 0; ) Jd(o, Ya(r))
              })(e, t)
            : (function (r, i, o) {
                  if (($n(0, i, o), o.readerType === 'none'))
                      return zn(r, o), void Zd(r)
                  if (o.bytesFilled < o.elementSize) return
                  Ya(r)
                  let m = o.bytesFilled % o.elementSize
                  if (m > 0) {
                      let u = o.byteOffset + o.bytesFilled
                      Wn(r, o.buffer, u - m, m)
                  }
                  ;(o.bytesFilled -= m),
                      Jd(r._controlledReadableByteStream, o),
                      Zd(r)
              })(e, a, t),
        wa(e)
}
function Ya(e) {
    return e._pendingPullIntos.shift()
}
function rr(e) {
    ;(e._pullAlgorithm = void 0), (e._cancelAlgorithm = void 0)
}
function Ja(e, a) {
    let t = e._controlledReadableByteStream
    t._state === 'readable' && (In(e), U1(e), rr(e), io(t, a))
}
function pn(e, a) {
    let t = e._queue.shift()
    ;(e._queueTotalSize -= t.byteLength), Un(e)
    let r = new Uint8Array(t.buffer, t.byteOffset, t.byteLength)
    a._chunkSteps(r)
}
function Hn(e) {
    let a = e._controlledReadableByteStream._state
    return a === 'errored'
        ? null
        : a === 'closed'
        ? 0
        : e._strategyHWM - e._queueTotalSize
}
function Rs(e, a, t) {
    let r = Object.create(v1.prototype),
        i,
        o,
        m
    ;(i = a.start !== void 0 ? () => a.start(r) : () => {}),
        (o = a.pull !== void 0 ? () => a.pull(r) : () => H(void 0)),
        (m = a.cancel !== void 0 ? (s) => a.cancel(s) : () => H(void 0))
    let u = a.autoAllocateChunkSize
    if (u === 0)
        throw new TypeError('autoAllocateChunkSize must be greater than 0')
    ;(function (s, f, h, _, w, g, y) {
        ;(f._controlledReadableByteStream = s),
            (f._pullAgain = !1),
            (f._pulling = !1),
            (f._byobRequest = null),
            (f._queue = f._queueTotalSize = void 0),
            U1(f),
            (f._closeRequested = !1),
            (f._started = !1),
            (f._strategyHWM = g),
            (f._pullAlgorithm = _),
            (f._cancelAlgorithm = w),
            (f._autoAllocateChunkSize = y),
            (f._pendingPullIntos = new Ee()),
            (s._readableStreamController = f),
            _e(
                H(h()),
                () => ((f._started = !0), wa(f), null),
                (C) => (Ja(f, C), null)
            )
    })(e, r, i, o, m, t, u)
}
function Ud(e) {
    return new TypeError(
        `ReadableStreamBYOBRequest.prototype.${e} can only be used on a ReadableStreamBYOBRequest`
    )
}
function kt(e) {
    return new TypeError(
        `ReadableByteStreamController.prototype.${e} can only be used on a ReadableByteStreamController`
    )
}
function rn(e, a) {
    e._reader._readIntoRequests.push(a)
}
function Qn(e) {
    return e._reader._readIntoRequests.length
}
function m2(e) {
    let a = e._reader
    return a !== void 0 && !!Ga(a)
}
function Ga(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(e, '_readIntoRequests') &&
        e instanceof b1
    )
}
function Gn(e, a) {
    let t = e._readIntoRequests
    ;(e._readIntoRequests = new Ee()),
        t.forEach((r) => {
            r._errorSteps(a)
        })
}
function Up(e) {
    return new TypeError(
        `ReadableStreamBYOBReader.prototype.${e} can only be used on a ReadableStreamBYOBReader`
    )
}
function jt(e, a) {
    let { highWaterMark: t } = e
    if (t === void 0) return a
    if (xn(t) || t < 0) throw new RangeError('Invalid highWaterMark')
    return t
}
function dr(e) {
    let { size: a } = e
    return a || (() => 1)
}
function ir(e, a) {
    S1(e, a)
    let t = e?.highWaterMark,
        r = e?.size
    return {
        highWaterMark: t === void 0 ? void 0 : n2(t),
        size: r === void 0 ? void 0 : Cs(r, `${a} has member 'size' that`)
    }
}
function Cs(e, a) {
    return He(e, a), (t) => n2(e(t))
}
function Es(e, a, t) {
    return He(e, t), (r) => _a(e, a, [r])
}
function Ps(e, a, t) {
    return He(e, t), () => _a(e, a, [])
}
function Ns(e, a, t) {
    return He(e, t), (r) => ur(e, a, [r])
}
function Ds(e, a, t) {
    return He(e, t), (r, i) => _a(e, a, [r, i])
}
function e1(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_writableStreamController'
        ) &&
        e instanceof w1
    )
}
function er(e) {
    return e._writer !== void 0
}
function Yn(e, a) {
    var t
    if (e._state === 'closed' || e._state === 'errored') return H(void 0)
    ;(e._writableStreamController._abortReason = a),
        (t = e._writableStreamController._abortController) === null ||
            t === void 0 ||
            t.abort(a)
    let r = e._state
    if (r === 'closed' || r === 'errored') return H(void 0)
    if (e._pendingAbortRequest !== void 0)
        return e._pendingAbortRequest._promise
    let i = !1
    r === 'erroring' && ((i = !0), (a = void 0))
    let o = Te((m, u) => {
        e._pendingAbortRequest = {
            _promise: void 0,
            _resolve: m,
            _reject: u,
            _reason: a,
            _wasAlreadyErroring: i
        }
    })
    return (e._pendingAbortRequest._promise = o), i || u2(e, a), o
}
function Kn(e) {
    let a = e._state
    if (a === 'closed' || a === 'errored')
        return A(
            new TypeError(
                `The stream (in ${a} state) is not in the writable state and cannot be closed`
            )
        )
    let t = Te((o, m) => {
            let u = { _resolve: o, _reject: m }
            e._closeRequest = u
        }),
        r = e._writer
    var i
    return (
        r !== void 0 && e._backpressure && a === 'writable' && b2(r),
        l2((i = e._writableStreamController), Jn, 0),
        fr(i),
        t
    )
}
function e2(e, a) {
    e._state !== 'writable' ? c2(e) : u2(e, a)
}
function u2(e, a) {
    let t = e._writableStreamController
    ;(e._state = 'erroring'), (e._storedError = a)
    let r = e._writer
    r !== void 0 && Xn(r, a),
        !(function (i) {
            return !(
                i._inFlightWriteRequest === void 0 &&
                i._inFlightCloseRequest === void 0
            )
        })(e) &&
            t._started &&
            c2(e)
}
function c2(e) {
    ;(e._state = 'errored'), e._writableStreamController[Tn]()
    let a = e._storedError
    if (
        (e._writeRequests.forEach((r) => {
            r._reject(a)
        }),
        (e._writeRequests = new Ee()),
        e._pendingAbortRequest === void 0)
    )
        return void Hp(e)
    let t = e._pendingAbortRequest
    if (((e._pendingAbortRequest = void 0), t._wasAlreadyErroring))
        return t._reject(a), void Hp(e)
    _e(
        e._writableStreamController[Sn](t._reason),
        () => (t._resolve(), Hp(e), null),
        (r) => (t._reject(r), Hp(e), null)
    )
}
function $1(e) {
    return e._closeRequest !== void 0 || e._inFlightCloseRequest !== void 0
}
function Hp(e) {
    e._closeRequest !== void 0 &&
        (e._closeRequest._reject(e._storedError), (e._closeRequest = void 0))
    let a = e._writer
    a !== void 0 && v2(a, e._storedError)
}
function f2(e, a) {
    let t = e._writer
    t !== void 0 &&
        a !== e._backpressure &&
        (a
            ? (function (r) {
                  hr(r)
              })(t)
            : b2(t)),
        (e._backpressure = a)
}
function ca(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(e, '_ownerWritableStream') &&
        e instanceof Ze
    )
}
function Xn(e, a) {
    e._readyPromiseState === 'pending'
        ? to(e, a)
        : (function (t, r) {
              a2(t, r)
          })(e, a)
}
function Hd(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_controlledWritableStream'
        ) &&
        e instanceof Za
    )
}
function nr(e) {
    ;(e._writeAlgorithm = void 0),
        (e._closeAlgorithm = void 0),
        (e._abortAlgorithm = void 0),
        (e._strategySizeAlgorithm = void 0)
}
function Zn(e) {
    return e._strategyHWM - e._queueTotalSize
}
function fr(e) {
    let a = e._controlledWritableStream
    if (!e._started || a._inFlightWriteRequest !== void 0) return
    if (a._state === 'erroring') return void c2(a)
    if (e._queue.length === 0) return
    let t = e._queue.peek().value
    t === Jn
        ? (function (r) {
              let i = r._controlledWritableStream
              ;(function (m) {
                  ;(m._inFlightCloseRequest = m._closeRequest),
                      (m._closeRequest = void 0)
              })(i),
                  Xd(r)
              let o = r._closeAlgorithm()
              nr(r),
                  _e(
                      o,
                      () => (
                          (function (m) {
                              m._inFlightCloseRequest._resolve(void 0),
                                  (m._inFlightCloseRequest = void 0),
                                  m._state === 'erroring' &&
                                      ((m._storedError = void 0),
                                      m._pendingAbortRequest !== void 0 &&
                                          (m._pendingAbortRequest._resolve(),
                                          (m._pendingAbortRequest = void 0))),
                                  (m._state = 'closed')
                              let u = m._writer
                              u !== void 0 && ao(u)
                          })(i),
                          null
                      ),
                      (m) => (
                          (function (u, s) {
                              u._inFlightCloseRequest._reject(s),
                                  (u._inFlightCloseRequest = void 0),
                                  u._pendingAbortRequest !== void 0 &&
                                      (u._pendingAbortRequest._reject(s),
                                      (u._pendingAbortRequest = void 0)),
                                  e2(u, s)
                          })(i, m),
                          null
                      )
                  )
          })(e)
        : (function (r, i) {
              let o = r._controlledWritableStream
              ;(function (m) {
                  m._inFlightWriteRequest = m._writeRequests.shift()
              })(o),
                  _e(
                      r._writeAlgorithm(i),
                      () => {
                          ;(function (u) {
                              u._inFlightWriteRequest._resolve(void 0),
                                  (u._inFlightWriteRequest = void 0)
                          })(o)
                          let m = o._state
                          if ((Xd(r), !$1(o) && m === 'writable')) {
                              let u = h2(r)
                              f2(o, u)
                          }
                          return fr(r), null
                      },
                      (m) => (
                          o._state === 'writable' && nr(r),
                          (function (u, s) {
                              u._inFlightWriteRequest._reject(s),
                                  (u._inFlightWriteRequest = void 0),
                                  e2(u, s)
                          })(o, m),
                          null
                      )
                  )
          })(e, t)
}
function dn(e, a) {
    e._controlledWritableStream._state === 'writable' && eo(e, a)
}
function h2(e) {
    return Zn(e) <= 0
}
function eo(e, a) {
    let t = e._controlledWritableStream
    nr(e), u2(t, a)
}
function Qp(e) {
    return new TypeError(
        `WritableStream.prototype.${e} can only be used on a WritableStream`
    )
}
function Qd(e) {
    return new TypeError(
        `WritableStreamDefaultController.prototype.${e} can only be used on a WritableStreamDefaultController`
    )
}
function fa(e) {
    return new TypeError(
        `WritableStreamDefaultWriter.prototype.${e} can only be used on a WritableStreamDefaultWriter`
    )
}
function Ot(e) {
    return new TypeError('Cannot ' + e + ' a stream using a released writer')
}
function ar(e) {
    e._closedPromise = Te((a, t) => {
        ;(e._closedPromise_resolve = a),
            (e._closedPromise_reject = t),
            (e._closedPromiseState = 'pending')
    })
}
function nn(e, a) {
    ar(e), v2(e, a)
}
function v2(e, a) {
    e._closedPromise_reject !== void 0 &&
        (Ka(e._closedPromise),
        e._closedPromise_reject(a),
        (e._closedPromise_resolve = void 0),
        (e._closedPromise_reject = void 0),
        (e._closedPromiseState = 'rejected'))
}
function ao(e) {
    e._closedPromise_resolve !== void 0 &&
        (e._closedPromise_resolve(void 0),
        (e._closedPromise_resolve = void 0),
        (e._closedPromise_reject = void 0),
        (e._closedPromiseState = 'resolved'))
}
function hr(e) {
    ;(e._readyPromise = Te((a, t) => {
        ;(e._readyPromise_resolve = a), (e._readyPromise_reject = t)
    })),
        (e._readyPromiseState = 'pending')
}
function a2(e, a) {
    hr(e), to(e, a)
}
function on(e) {
    hr(e), b2(e)
}
function to(e, a) {
    e._readyPromise_reject !== void 0 &&
        (Ka(e._readyPromise),
        e._readyPromise_reject(a),
        (e._readyPromise_resolve = void 0),
        (e._readyPromise_reject = void 0),
        (e._readyPromiseState = 'rejected'))
}
function b2(e) {
    e._readyPromise_resolve !== void 0 &&
        (e._readyPromise_resolve(void 0),
        (e._readyPromise_resolve = void 0),
        (e._readyPromise_reject = void 0),
        (e._readyPromiseState = 'fulfilled'))
}
function sn(e, a, t, r, i, o) {
    let m = e.getReader(),
        u = a.getWriter()
    ba(e) && (e._disturbed = !0)
    let s,
        f,
        h,
        _ = !1,
        w = !1,
        g = 'readable',
        y = 'writable',
        C = !1,
        R = !1,
        T = Te((K) => {
            h = K
        }),
        F = Promise.resolve(void 0)
    return Te((K, I) => {
        let k
        function Z() {
            if (_) return
            let q = Te((M, O) => {
                ;(function $(ge) {
                    ge
                        ? M()
                        : g1(
                              (function () {
                                  return _
                                      ? H(!0)
                                      : g1(u.ready, () =>
                                            g1(
                                                m.read(),
                                                (le) =>
                                                    !!le.done ||
                                                    ((F = u.write(le.value)),
                                                    Ka(F),
                                                    !1)
                                            )
                                        )
                              })(),
                              $,
                              O
                          )
                })(!1)
            })
            Ka(q)
        }
        function ve() {
            return (
                (g = 'closed'),
                t
                    ? Q()
                    : x(
                          () => (
                              e1(a) && ((C = $1(a)), (y = a._state)),
                              C || y === 'closed'
                                  ? H(void 0)
                                  : y === 'erroring' || y === 'errored'
                                  ? A(f)
                                  : ((C = !0), u.close())
                          ),
                          !1,
                          void 0
                      ),
                null
            )
        }
        function N(q) {
            return (
                _ ||
                    ((g = 'errored'),
                    (s = q),
                    r ? Q(!0, q) : x(() => u.abort(q), !0, q)),
                null
            )
        }
        function D(q) {
            return (
                w ||
                    ((y = 'errored'),
                    (f = q),
                    i ? Q(!0, q) : x(() => m.cancel(q), !0, q)),
                null
            )
        }
        if (
            (o !== void 0 &&
                ((k = () => {
                    let q =
                            o.reason !== void 0
                                ? o.reason
                                : new Vs('Aborted', 'AbortError'),
                        M = []
                    r ||
                        M.push(() =>
                            y === 'writable' ? u.abort(q) : H(void 0)
                        ),
                        i ||
                            M.push(() =>
                                g === 'readable' ? m.cancel(q) : H(void 0)
                            ),
                        x(() => Promise.all(M.map((O) => O())), !0, q)
                }),
                o.aborted ? k() : o.addEventListener('abort', k)),
            ba(e) && ((g = e._state), (s = e._storedError)),
            e1(a) && ((y = a._state), (f = a._storedError), (C = $1(a))),
            ba(e) && e1(a) && ((R = !0), h()),
            g === 'errored')
        )
            N(s)
        else if (y === 'erroring' || y === 'errored') D(f)
        else if (g === 'closed') ve()
        else if (C || y === 'closed') {
            let q = new TypeError(
                'the destination writable stream closed before all data could be piped to it'
            )
            i ? Q(!0, q) : x(() => m.cancel(q), !0, q)
        }
        function x(q, M, O) {
            function $() {
                return (
                    y !== 'writable' || C
                        ? ge()
                        : Y0(
                              (function () {
                                  let le
                                  return H(
                                      (function E1() {
                                          if (le !== F)
                                              return (le = F), Ue(F, E1, E1)
                                      })()
                                  )
                              })(),
                              ge
                          ),
                    null
                )
            }
            function ge() {
                return (
                    q
                        ? _e(
                              q(),
                              () => ie(M, O),
                              (le) => ie(!0, le)
                          )
                        : ie(M, O),
                    null
                )
            }
            _ || ((_ = !0), R ? $() : Y0(T, $))
        }
        function Q(q, M) {
            x(void 0, q, M)
        }
        function ie(q, M) {
            return (
                (w = !0),
                u.releaseLock(),
                m.releaseLock(),
                o !== void 0 && o.removeEventListener('abort', k),
                q ? I(M) : K(void 0),
                null
            )
        }
        _ ||
            (_e(m.closed, ve, N),
            _e(
                u.closed,
                function () {
                    return w || (y = 'closed'), null
                },
                D
            )),
            R
                ? Z()
                : Jp(() => {
                      ;(R = !0), h(), Z()
                  })
    })
}
function qs(e, a) {
    return (function (t) {
        try {
            return t.getReader({ mode: 'byob' }).releaseLock(), !0
        } catch {
            return !1
        }
    })(e)
        ? (function (t) {
              let r,
                  i,
                  o,
                  m,
                  u,
                  s = t.getReader(),
                  f = !1,
                  h = !1,
                  _ = !1,
                  w = !1,
                  g = !1,
                  y = !1,
                  C = Te((D) => {
                      u = D
                  })
              function R(D) {
                  K0(
                      D.closed,
                      (x) => (
                          D !== s ||
                              (o.error(x), m.error(x), (g && y) || u(void 0)),
                          null
                      )
                  )
              }
              function T() {
                  f && (s.releaseLock(), (s = t.getReader()), R(s), (f = !1)),
                      _e(
                          s.read(),
                          (D) => {
                              var x, Q
                              if (((_ = !1), (w = !1), D.done))
                                  return (
                                      g || o.close(),
                                      y || m.close(),
                                      (x = o.byobRequest) === null ||
                                          x === void 0 ||
                                          x.respond(0),
                                      (Q = m.byobRequest) === null ||
                                          Q === void 0 ||
                                          Q.respond(0),
                                      (g && y) || u(void 0),
                                      null
                                  )
                              let ie = D.value,
                                  q = ie,
                                  M = ie
                              if (!g && !y)
                                  try {
                                      M = an(ie)
                                  } catch (O) {
                                      return (
                                          o.error(O),
                                          m.error(O),
                                          u(s.cancel(O)),
                                          null
                                      )
                                  }
                              return (
                                  g || o.enqueue(q),
                                  y || m.enqueue(M),
                                  (h = !1),
                                  _ ? K() : w && I(),
                                  null
                              )
                          },
                          () => ((h = !1), null)
                      )
              }
              function F(D, x) {
                  f ||
                      (s.releaseLock(),
                      (s = t.getReader({ mode: 'byob' })),
                      R(s),
                      (f = !0))
                  let Q = x ? m : o,
                      ie = x ? o : m
                  _e(
                      s.read(D),
                      (q) => {
                          var M
                          ;(_ = !1), (w = !1)
                          let O = x ? y : g,
                              $ = x ? g : y
                          if (q.done) {
                              O || Q.close(), $ || ie.close()
                              let le = q.value
                              return (
                                  le !== void 0 &&
                                      (O ||
                                          Q.byobRequest.respondWithNewView(le),
                                      $ ||
                                          (M = ie.byobRequest) === null ||
                                          M === void 0 ||
                                          M.respond(0)),
                                  (O && $) || u(void 0),
                                  null
                              )
                          }
                          let ge = q.value
                          if ($) O || Q.byobRequest.respondWithNewView(ge)
                          else {
                              let le
                              try {
                                  le = an(ge)
                              } catch (E1) {
                                  return (
                                      Q.error(E1),
                                      ie.error(E1),
                                      u(s.cancel(E1)),
                                      null
                                  )
                              }
                              O || Q.byobRequest.respondWithNewView(ge),
                                  ie.enqueue(le)
                          }
                          return (h = !1), _ ? K() : w && I(), null
                      },
                      () => ((h = !1), null)
                  )
              }
              function K() {
                  if (h) return (_ = !0), H(void 0)
                  h = !0
                  let D = o.byobRequest
                  return D === null ? T() : F(D.view, !1), H(void 0)
              }
              function I() {
                  if (h) return (w = !0), H(void 0)
                  h = !0
                  let D = m.byobRequest
                  return D === null ? T() : F(D.view, !0), H(void 0)
              }
              function k(D) {
                  if (((g = !0), (r = D), y)) {
                      let x = [r, i],
                          Q = s.cancel(x)
                      u(Q)
                  }
                  return C
              }
              function Z(D) {
                  if (((y = !0), (i = D), g)) {
                      let x = [r, i],
                          Q = s.cancel(x)
                      u(Q)
                  }
                  return C
              }
              let ve = new ne({
                      type: 'bytes',
                      start(D) {
                          o = D
                      },
                      pull: K,
                      cancel: k
                  }),
                  N = new ne({
                      type: 'bytes',
                      start(D) {
                          m = D
                      },
                      pull: I,
                      cancel: Z
                  })
              return R(s), [ve, N]
          })(e)
        : (function (t, r) {
              let i = t.getReader(),
                  o,
                  m,
                  u,
                  s,
                  f,
                  h = !1,
                  _ = !1,
                  w = !1,
                  g = !1,
                  y = Te((I) => {
                      f = I
                  })
              function C() {
                  return h
                      ? ((_ = !0), H(void 0))
                      : ((h = !0),
                        _e(
                            i.read(),
                            (I) => {
                                if (((_ = !1), I.done))
                                    return (
                                        w || u.close(),
                                        g || s.close(),
                                        (w && g) || f(void 0),
                                        null
                                    )
                                let k = I.value,
                                    Z = k,
                                    ve = k
                                return (
                                    w || u.enqueue(Z),
                                    g || s.enqueue(ve),
                                    (h = !1),
                                    _ && C(),
                                    null
                                )
                            },
                            () => ((h = !1), null)
                        ),
                        H(void 0))
              }
              function R(I) {
                  if (((w = !0), (o = I), g)) {
                      let k = [o, m],
                          Z = i.cancel(k)
                      f(Z)
                  }
                  return y
              }
              function T(I) {
                  if (((g = !0), (m = I), w)) {
                      let k = [o, m],
                          Z = i.cancel(k)
                      f(Z)
                  }
                  return y
              }
              let F = new ne({
                      start(I) {
                          u = I
                      },
                      pull: C,
                      cancel: R
                  }),
                  K = new ne({
                      start(I) {
                          s = I
                      },
                      pull: C,
                      cancel: T
                  })
              return (
                  K0(
                      i.closed,
                      (I) => (
                          u.error(I), s.error(I), (w && g) || f(void 0), null
                      )
                  ),
                  [F, K]
              )
          })(e)
}
function Gp(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_controlledReadableStream'
        ) &&
        e instanceof _1
    )
}
function Lt(e) {
    if (
        (function (t) {
            let r = t._controlledReadableStream
            return !xt(t) || !t._started
                ? !1
                : !!((et(r) && cr(r) > 0) || po(t) > 0)
        })(e)
    ) {
        if (e._pulling) return void (e._pullAgain = !0)
        ;(e._pulling = !0),
            _e(
                e._pullAlgorithm(),
                () => (
                    (e._pulling = !1),
                    e._pullAgain && ((e._pullAgain = !1), Lt(e)),
                    null
                ),
                (t) => (It(e, t), null)
            )
    }
}
function tr(e) {
    ;(e._pullAlgorithm = void 0),
        (e._cancelAlgorithm = void 0),
        (e._strategySizeAlgorithm = void 0)
}
function It(e, a) {
    let t = e._controlledReadableStream
    t._state === 'readable' && (U1(e), tr(e), io(t, a))
}
function po(e) {
    let a = e._controlledReadableStream._state
    return a === 'errored'
        ? null
        : a === 'closed'
        ? 0
        : e._strategyHWM - e._queueTotalSize
}
function xt(e) {
    return (
        !e._closeRequested && e._controlledReadableStream._state === 'readable'
    )
}
function Bs(e, a, t, r) {
    let i = Object.create(_1.prototype),
        o,
        m,
        u
    ;(o = a.start !== void 0 ? () => a.start(i) : () => {}),
        (m = a.pull !== void 0 ? () => a.pull(i) : () => H(void 0)),
        (u = a.cancel !== void 0 ? (s) => a.cancel(s) : () => H(void 0)),
        (function (s, f, h, _, w, g, y) {
            ;(f._controlledReadableStream = s),
                (f._queue = void 0),
                (f._queueTotalSize = void 0),
                U1(f),
                (f._started = !1),
                (f._closeRequested = !1),
                (f._pullAgain = !1),
                (f._pulling = !1),
                (f._strategySizeAlgorithm = y),
                (f._strategyHWM = g),
                (f._pullAlgorithm = _),
                (f._cancelAlgorithm = w),
                (s._readableStreamController = f),
                _e(
                    H(h()),
                    () => ((f._started = !0), Lt(f), null),
                    (C) => (It(f, C), null)
                )
        })(e, i, o, m, u, t, r)
}
function Yp(e) {
    return new TypeError(
        `ReadableStreamDefaultController.prototype.${e} can only be used on a ReadableStreamDefaultController`
    )
}
function ks(e, a, t) {
    return He(e, t), (r) => _a(e, a, [r])
}
function Os(e, a, t) {
    return He(e, t), (r) => _a(e, a, [r])
}
function Fs(e, a, t) {
    return He(e, t), (r) => ur(e, a, [r])
}
function xs(e, a) {
    if ((e = `${e}`) != 'bytes')
        throw new TypeError(
            `${a} '${e}' is not a valid enumeration value for ReadableStreamType`
        )
    return e
}
function Ls(e, a) {
    if ((e = `${e}`) != 'byob')
        throw new TypeError(
            `${a} '${e}' is not a valid enumeration value for ReadableStreamReaderMode`
        )
    return e
}
function mn(e, a) {
    S1(e, a)
    let t = e?.preventAbort,
        r = e?.preventCancel,
        i = e?.preventClose,
        o = e?.signal
    return (
        o !== void 0 &&
            (function (m, u) {
                if (
                    !(function (s) {
                        if (typeof s != 'object' || s === null) return !1
                        try {
                            return typeof s.aborted == 'boolean'
                        } catch {
                            return !1
                        }
                    })(m)
                )
                    throw new TypeError(`${u} is not an AbortSignal.`)
            })(o, `${a} has member 'signal' that`),
        { preventAbort: !!t, preventCancel: !!r, preventClose: !!i, signal: o }
    )
}
function Is(e, a) {
    S1(e, a)
    let t = e?.readable
    Kd(t, 'readable', 'ReadableWritablePair'),
        (function (i, o) {
            if (!Ft(i)) throw new TypeError(`${o} is not a ReadableStream.`)
        })(t, `${a} has member 'readable' that`)
    let r = e?.writable
    return (
        Kd(r, 'writable', 'ReadableWritablePair'),
        (function (i, o) {
            if (!Vn(i)) throw new TypeError(`${o} is not a WritableStream.`)
        })(r, `${a} has member 'writable' that`),
        { readable: t, writable: r }
    )
}
function ba(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_readableStreamController'
        ) &&
        e instanceof ne
    )
}
function et(e) {
    return e._reader !== void 0
}
function ro(e, a) {
    if (((e._disturbed = !0), e._state === 'closed')) return H(void 0)
    if (e._state === 'errored') return A(e._storedError)
    Wt(e)
    let t = e._reader
    if (t !== void 0 && Ga(t)) {
        let r = t._readIntoRequests
        ;(t._readIntoRequests = new Ee()),
            r.forEach((i) => {
                i._closeSteps(void 0)
            })
    }
    return Ue(e._readableStreamController[p2](a), yn)
}
function Wt(e) {
    e._state = 'closed'
    let a = e._reader
    if (a !== void 0 && (Nn(a), va(a))) {
        let t = a._readRequests
        ;(a._readRequests = new Ee()),
            t.forEach((r) => {
                r._closeSteps()
            })
    }
}
function io(e, a) {
    ;(e._state = 'errored'), (e._storedError = a)
    let t = e._reader
    t !== void 0 && (i2(t, a), va(t) ? On(t, a) : Gn(t, a))
}
function ha(e) {
    return new TypeError(
        `ReadableStream.prototype.${e} can only be used on a ReadableStream`
    )
}
function no(e, a) {
    S1(e, a)
    let t = e?.highWaterMark
    return (
        Kd(t, 'highWaterMark', 'QueuingStrategyInit'), { highWaterMark: n2(t) }
    )
}
function un(e) {
    return new TypeError(
        `ByteLengthQueuingStrategy.prototype.${e} can only be used on a ByteLengthQueuingStrategy`
    )
}
function cn(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_byteLengthQueuingStrategyHighWaterMark'
        ) &&
        e instanceof zt
    )
}
function fn(e) {
    return new TypeError(
        `CountQueuingStrategy.prototype.${e} can only be used on a CountQueuingStrategy`
    )
}
function hn(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_countQueuingStrategyHighWaterMark'
        ) &&
        e instanceof Mt
    )
}
function js(e, a, t) {
    return He(e, t), (r) => _a(e, a, [r])
}
function Ws(e, a, t) {
    return He(e, t), (r) => ur(e, a, [r])
}
function zs(e, a, t) {
    return He(e, t), (r, i) => _a(e, a, [r, i])
}
function vn(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_transformStreamController'
        ) &&
        e instanceof $t
    )
}
function or(e, a) {
    mr(e, a), vr(e, a)
}
function vr(e, a) {
    so(e._transformStreamController),
        (function (t, r) {
            t._writableController.error(r),
                t._writableState === 'writable' && fo(t, r)
        })(e, a),
        e._backpressure && lr(e, !1)
}
function lr(e, a) {
    e._backpressureChangePromise !== void 0 &&
        e._backpressureChangePromise_resolve(),
        (e._backpressureChangePromise = Te((t) => {
            e._backpressureChangePromise_resolve = t
        })),
        (e._backpressure = a)
}
function Kp(e) {
    return (
        !!ye(e) &&
        !!Object.prototype.hasOwnProperty.call(
            e,
            '_controlledTransformStream'
        ) &&
        e instanceof y1
    )
}
function so(e) {
    ;(e._transformAlgorithm = void 0), (e._flushAlgorithm = void 0)
}
function mo(e, a) {
    let t = e._controlledTransformStream
    if (!sr(t))
        throw new TypeError(
            'Readable side is not in a state that permits enqueue'
        )
    try {
        ;(function (i, o) {
            i._readablePulling = !1
            try {
                i._readableController.enqueue(o)
            } catch (m) {
                throw (mr(i, m), m)
            }
        })(t, a)
    } catch (i) {
        throw (vr(t, i), t._readableStoredError)
    }
    ;(function (i) {
        return !(function (o) {
            return sr(o) ? !!(o._readablePulling || co(o) > 0) : !1
        })(i)
    })(t) !== t._backpressure && lr(t, !0)
}
function bn(e, a) {
    return Ue(e._transformAlgorithm(a), void 0, (t) => {
        throw (or(e._controlledTransformStream, t), t)
    })
}
function Xp(e) {
    return new TypeError(
        `TransformStreamDefaultController.prototype.${e} can only be used on a TransformStreamDefaultController`
    )
}
function wn(e) {
    return new TypeError(
        `TransformStream.prototype.${e} can only be used on a TransformStream`
    )
}
function sr(e) {
    return !e._readableCloseRequested && e._readableState === 'readable'
}
function uo(e) {
    ;(e._readableState = 'closed'),
        (e._readableCloseRequested = !0),
        e._readableController.close()
}
function mr(e, a) {
    e._readableState === 'readable' &&
        ((e._readableState = 'errored'), (e._readableStoredError = a)),
        e._readableController.error(a)
}
function co(e) {
    return e._readableController.desiredSize
}
function Gd(e, a) {
    e._writableState !== 'writable' ? w2(e) : fo(e, a)
}
function fo(e, a) {
    ;(e._writableState = 'erroring'),
        (e._writableStoredError = a),
        !(function (t) {
            return t._writableHasInFlightOperation
        })(e) &&
            e._writableStarted &&
            w2(e)
}
function w2(e) {
    e._writableState = 'errored'
}
function _n(e) {
    e._writableState === 'erroring' && w2(e)
}
var z,
    gn,
    t2,
    ys,
    gs,
    Ss,
    Jp,
    Ee,
    Sn,
    Tn,
    p2,
    r2,
    d2,
    X0,
    Ts,
    h1,
    pr,
    Fn,
    xn,
    M1,
    v1,
    b1,
    As,
    w1,
    Ze,
    Jn,
    Za,
    ln,
    Vs,
    _1,
    ne,
    oo,
    zt,
    lo,
    Mt,
    $t,
    y1,
    ho = x1(() => {
        z =
            typeof Symbol == 'function' && typeof Symbol.iterator == 'symbol'
                ? Symbol
                : (e) => `Symbol(${e})`
        gn = yn
        ;(t2 = Promise),
            (ys = Promise.prototype.then),
            (gs = Promise.resolve.bind(t2)),
            (Ss = Promise.reject.bind(t2))
        Jp = (e) => {
            if (typeof queueMicrotask == 'function') Jp = queueMicrotask
            else {
                let a = H(void 0)
                Jp = (t) => g1(a, t)
            }
            return Jp(e)
        }
        ;(Ee = class {
            constructor() {
                ;(this._cursor = 0),
                    (this._size = 0),
                    (this._front = { _elements: [], _next: void 0 }),
                    (this._back = this._front),
                    (this._cursor = 0),
                    (this._size = 0)
            }
            get length() {
                return this._size
            }
            push(a) {
                let t = this._back,
                    r = t
                t._elements.length === 16383 &&
                    (r = { _elements: [], _next: void 0 }),
                    t._elements.push(a),
                    r !== t && ((this._back = r), (t._next = r)),
                    ++this._size
            }
            shift() {
                let a = this._front,
                    t = a,
                    r = this._cursor,
                    i = r + 1,
                    o = a._elements,
                    m = o[r]
                return (
                    i === 16384 && ((t = a._next), (i = 0)),
                    --this._size,
                    (this._cursor = i),
                    a !== t && (this._front = t),
                    (o[r] = void 0),
                    m
                )
            }
            forEach(a) {
                let t = this._cursor,
                    r = this._front,
                    i = r._elements
                for (
                    ;
                    !(
                        (t === i.length && r._next === void 0) ||
                        (t === i.length &&
                            ((r = r._next),
                            (i = r._elements),
                            (t = 0),
                            i.length === 0))
                    );

                )
                    a(i[t]), ++t
            }
            peek() {
                let a = this._front,
                    t = this._cursor
                return a._elements[t]
            }
        }),
            (Sn = z('[[AbortSteps]]')),
            (Tn = z('[[ErrorSteps]]')),
            (p2 = z('[[CancelSteps]]')),
            (r2 = z('[[PullSteps]]')),
            (d2 = z('[[ReleaseSteps]]'))
        ;(X0 =
            Number.isFinite ||
            function (e) {
                return typeof e == 'number' && isFinite(e)
            }),
            (Ts =
                Math.trunc ||
                function (e) {
                    return e < 0 ? Math.ceil(e) : Math.floor(e)
                })
        h1 = class {
            constructor(a) {
                if (
                    (T1(a, 1, 'ReadableStreamDefaultReader'),
                    qn(a, 'First parameter'),
                    et(a))
                )
                    throw new TypeError(
                        'This stream has already been locked for exclusive reading by another reader'
                    )
                Rn(this, a), (this._readRequests = new Ee())
            }
            get closed() {
                return va(this) ? this._closedPromise : A($p('closed'))
            }
            cancel(a) {
                return va(this)
                    ? this._ownerReadableStream === void 0
                        ? A(Xa('cancel'))
                        : Cn(this, a)
                    : A($p('cancel'))
            }
            read() {
                if (!va(this)) return A($p('read'))
                if (this._ownerReadableStream === void 0)
                    return A(Xa('read from'))
                let a,
                    t,
                    r = Te((i, o) => {
                        ;(a = i), (t = o)
                    })
                return (
                    (function (i, o) {
                        let m = i._ownerReadableStream
                        ;(m._disturbed = !0),
                            m._state === 'closed'
                                ? o._closeSteps()
                                : m._state === 'errored'
                                ? o._errorSteps(m._storedError)
                                : m._readableStreamController[r2](o)
                    })(this, {
                        _chunkSteps: (i) => a({ value: i, done: !1 }),
                        _closeSteps: () => a({ value: void 0, done: !0 }),
                        _errorSteps: (i) => t(i)
                    }),
                    r
                )
            }
            releaseLock() {
                if (!va(this)) throw $p('releaseLock')
                this._ownerReadableStream !== void 0 &&
                    (function (a) {
                        En(a)
                        let t = new TypeError('Reader was released')
                        On(a, t)
                    })(this)
            }
        }
        Object.defineProperties(h1.prototype, {
            cancel: { enumerable: !0 },
            read: { enumerable: !0 },
            releaseLock: { enumerable: !0 },
            closed: { enumerable: !0 }
        }),
            U(h1.prototype.cancel, 'cancel'),
            U(h1.prototype.read, 'read'),
            U(h1.prototype.releaseLock, 'releaseLock'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(h1.prototype, z.toStringTag, {
                    value: 'ReadableStreamDefaultReader',
                    configurable: !0
                })
        ;(pr = class {
            constructor(a, t) {
                ;(this._ongoingPromise = void 0),
                    (this._isFinished = !1),
                    (this._reader = a),
                    (this._preventCancel = t)
            }
            next() {
                let a = () => this._nextSteps()
                return (
                    (this._ongoingPromise = this._ongoingPromise
                        ? Ue(this._ongoingPromise, a, a)
                        : a()),
                    this._ongoingPromise
                )
            }
            return(a) {
                let t = () => this._returnSteps(a)
                return this._ongoingPromise
                    ? Ue(this._ongoingPromise, t, t)
                    : t()
            }
            _nextSteps() {
                if (this._isFinished)
                    return Promise.resolve({ value: void 0, done: !0 })
                let a = this._reader
                return a === void 0
                    ? A(Xa('iterate'))
                    : g1(
                          a.read(),
                          (t) => {
                              var r
                              return (
                                  (this._ongoingPromise = void 0),
                                  t.done &&
                                      ((this._isFinished = !0),
                                      (r = this._reader) === null ||
                                          r === void 0 ||
                                          r.releaseLock(),
                                      (this._reader = void 0)),
                                  t
                              )
                          },
                          (t) => {
                              var r
                              throw (
                                  ((this._ongoingPromise = void 0),
                                  (this._isFinished = !0),
                                  (r = this._reader) === null ||
                                      r === void 0 ||
                                      r.releaseLock(),
                                  (this._reader = void 0),
                                  t)
                              )
                          }
                      )
            }
            _returnSteps(a) {
                if (this._isFinished)
                    return Promise.resolve({ value: a, done: !0 })
                this._isFinished = !0
                let t = this._reader
                if (t === void 0) return A(Xa('finish iterating'))
                if (((this._reader = void 0), !this._preventCancel)) {
                    let r = t.cancel(a)
                    return (
                        t.releaseLock(), Ue(r, () => ({ value: a, done: !0 }))
                    )
                }
                return t.releaseLock(), H({ value: a, done: !0 })
            }
        }),
            (Fn = {
                next() {
                    return Z0(this)
                        ? this._asyncIteratorImpl.next()
                        : A(en('next'))
                },
                return(e) {
                    return Z0(this)
                        ? this._asyncIteratorImpl.return(e)
                        : A(en('return'))
                }
            })
        typeof z.asyncIterator == 'symbol' &&
            Object.defineProperty(Fn, z.asyncIterator, {
                value() {
                    return this
                },
                writable: !0,
                configurable: !0
            })
        xn =
            Number.isNaN ||
            function (e) {
                return e != e
            }
        M1 = class {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get view() {
                if (!$d(this)) throw Ud('view')
                return this._view
            }
            respond(a) {
                if (!$d(this)) throw Ud('respond')
                if (
                    (T1(a, 1, 'respond'),
                    (a = An(a, 'First parameter')),
                    this._associatedReadableByteStreamController === void 0)
                )
                    throw new TypeError(
                        'This BYOB request has been invalidated'
                    )
                this._view.buffer,
                    (function (t, r) {
                        let i = t._pendingPullIntos.peek()
                        if (
                            t._controlledReadableByteStream._state === 'closed'
                        ) {
                            if (r !== 0)
                                throw new TypeError(
                                    'bytesWritten must be 0 when calling respond() on a closed stream'
                                )
                        } else {
                            if (r === 0)
                                throw new TypeError(
                                    'bytesWritten must be greater than 0 when calling respond() on a readable stream'
                                )
                            if (i.bytesFilled + r > i.byteLength)
                                throw new RangeError(
                                    'bytesWritten out of range'
                                )
                        }
                        ;(i.buffer = i.buffer), tn(t, r)
                    })(this._associatedReadableByteStreamController, a)
            }
            respondWithNewView(a) {
                if (!$d(this)) throw Ud('respondWithNewView')
                if ((T1(a, 1, 'respondWithNewView'), !ArrayBuffer.isView(a)))
                    throw new TypeError(
                        'You can only respond with array buffer views'
                    )
                if (this._associatedReadableByteStreamController === void 0)
                    throw new TypeError(
                        'This BYOB request has been invalidated'
                    )
                a.buffer,
                    (function (t, r) {
                        let i = t._pendingPullIntos.peek()
                        if (
                            t._controlledReadableByteStream._state === 'closed'
                        ) {
                            if (r.byteLength !== 0)
                                throw new TypeError(
                                    "The view's length must be 0 when calling respondWithNewView() on a closed stream"
                                )
                        } else if (r.byteLength === 0)
                            throw new TypeError(
                                "The view's length must be greater than 0 when calling respondWithNewView() on a readable stream"
                            )
                        if (i.byteOffset + i.bytesFilled !== r.byteOffset)
                            throw new RangeError(
                                'The region specified by view does not match byobRequest'
                            )
                        if (i.bufferByteLength !== r.buffer.byteLength)
                            throw new RangeError(
                                'The buffer of view has different capacity than byobRequest'
                            )
                        if (i.bytesFilled + r.byteLength > i.byteLength)
                            throw new RangeError(
                                'The region specified by view is larger than byobRequest'
                            )
                        let o = r.byteLength
                        ;(i.buffer = r.buffer), tn(t, o)
                    })(this._associatedReadableByteStreamController, a)
            }
        }
        Object.defineProperties(M1.prototype, {
            respond: { enumerable: !0 },
            respondWithNewView: { enumerable: !0 },
            view: { enumerable: !0 }
        }),
            U(M1.prototype.respond, 'respond'),
            U(M1.prototype.respondWithNewView, 'respondWithNewView'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(M1.prototype, z.toStringTag, {
                    value: 'ReadableStreamBYOBRequest',
                    configurable: !0
                })
        v1 = class {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get byobRequest() {
                if (!Qa(this)) throw kt('byobRequest')
                return (function (a) {
                    if (
                        a._byobRequest === null &&
                        a._pendingPullIntos.length > 0
                    ) {
                        let t = a._pendingPullIntos.peek(),
                            r = new Uint8Array(
                                t.buffer,
                                t.byteOffset + t.bytesFilled,
                                t.byteLength - t.bytesFilled
                            ),
                            i = Object.create(M1.prototype)
                        ;(function (o, m, u) {
                            ;(o._associatedReadableByteStreamController = m),
                                (o._view = u)
                        })(i, a, r),
                            (a._byobRequest = i)
                    }
                    return a._byobRequest
                })(this)
            }
            get desiredSize() {
                if (!Qa(this)) throw kt('desiredSize')
                return Hn(this)
            }
            close() {
                if (!Qa(this)) throw kt('close')
                if (this._closeRequested)
                    throw new TypeError(
                        'The stream has already been closed; do not close it again!'
                    )
                let a = this._controlledReadableByteStream._state
                if (a !== 'readable')
                    throw new TypeError(
                        `The stream (in ${a} state) is not in the readable state and cannot be closed`
                    )
                ;(function (t) {
                    let r = t._controlledReadableByteStream
                    if (!(t._closeRequested || r._state !== 'readable')) {
                        if (t._queueTotalSize > 0)
                            return void (t._closeRequested = !0)
                        if (
                            t._pendingPullIntos.length > 0 &&
                            t._pendingPullIntos.peek().bytesFilled > 0
                        ) {
                            let i = new TypeError(
                                'Insufficient bytes to fill elements in the given buffer'
                            )
                            throw (Ja(t, i), i)
                        }
                        rr(t), Wt(r)
                    }
                })(this)
            }
            enqueue(a) {
                if (!Qa(this)) throw kt('enqueue')
                if ((T1(a, 1, 'enqueue'), !ArrayBuffer.isView(a)))
                    throw new TypeError('chunk must be an array buffer view')
                if (a.byteLength === 0)
                    throw new TypeError('chunk must have non-zero byteLength')
                if (a.buffer.byteLength === 0)
                    throw new TypeError(
                        "chunk's buffer must have non-zero byteLength"
                    )
                if (this._closeRequested)
                    throw new TypeError('stream is closed or draining')
                let t = this._controlledReadableByteStream._state
                if (t !== 'readable')
                    throw new TypeError(
                        `The stream (in ${t} state) is not in the readable state and cannot be enqueued to`
                    )
                ;(function (r, i) {
                    let o = r._controlledReadableByteStream
                    if (r._closeRequested || o._state !== 'readable') return
                    let m = i.buffer,
                        u = i.byteOffset,
                        s = i.byteLength,
                        f = m
                    if (r._pendingPullIntos.length > 0) {
                        let h = r._pendingPullIntos.peek()
                        h.buffer,
                            s2(r),
                            (h.buffer = h.buffer),
                            h.readerType === 'none' && zn(r, h)
                    }
                    kn(o)
                        ? ((function (h) {
                              let _ = h._controlledReadableByteStream._reader
                              for (; _._readRequests.length > 0; ) {
                                  if (h._queueTotalSize === 0) return
                                  pn(h, _._readRequests.shift())
                              }
                          })(r),
                          cr(o) === 0
                              ? Zp(r, f, u, s)
                              : (r._pendingPullIntos.length > 0 && Ya(r),
                                o2(o, new Uint8Array(f, u, s), !1)))
                        : m2(o)
                        ? (Zp(r, f, u, s), Zd(r))
                        : Zp(r, f, u, s),
                        wa(r)
                })(this, a)
            }
            error(a) {
                if (!Qa(this)) throw kt('error')
                Ja(this, a)
            }
            [p2](a) {
                In(this), U1(this)
                let t = this._cancelAlgorithm(a)
                return rr(this), t
            }
            [r2](a) {
                let t = this._controlledReadableByteStream
                if (this._queueTotalSize > 0) return void pn(this, a)
                let r = this._autoAllocateChunkSize
                if (r !== void 0) {
                    let i
                    try {
                        i = new ArrayBuffer(r)
                    } catch (m) {
                        return void a._errorSteps(m)
                    }
                    let o = {
                        buffer: i,
                        bufferByteLength: r,
                        byteOffset: 0,
                        byteLength: r,
                        bytesFilled: 0,
                        elementSize: 1,
                        viewConstructor: Uint8Array,
                        readerType: 'default'
                    }
                    this._pendingPullIntos.push(o)
                }
                Bn(t, a), wa(this)
            }
            [d2]() {
                if (this._pendingPullIntos.length > 0) {
                    let a = this._pendingPullIntos.peek()
                    ;(a.readerType = 'none'),
                        (this._pendingPullIntos = new Ee()),
                        this._pendingPullIntos.push(a)
                }
            }
        }
        Object.defineProperties(v1.prototype, {
            close: { enumerable: !0 },
            enqueue: { enumerable: !0 },
            error: { enumerable: !0 },
            byobRequest: { enumerable: !0 },
            desiredSize: { enumerable: !0 }
        }),
            U(v1.prototype.close, 'close'),
            U(v1.prototype.enqueue, 'enqueue'),
            U(v1.prototype.error, 'error'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(v1.prototype, z.toStringTag, {
                    value: 'ReadableByteStreamController',
                    configurable: !0
                })
        b1 = class {
            constructor(a) {
                if (
                    (T1(a, 1, 'ReadableStreamBYOBReader'),
                    qn(a, 'First parameter'),
                    et(a))
                )
                    throw new TypeError(
                        'This stream has already been locked for exclusive reading by another reader'
                    )
                if (!Qa(a._readableStreamController))
                    throw new TypeError(
                        'Cannot construct a ReadableStreamBYOBReader for a stream not constructed with a byte source'
                    )
                Rn(this, a), (this._readIntoRequests = new Ee())
            }
            get closed() {
                return Ga(this) ? this._closedPromise : A(Up('closed'))
            }
            cancel(a) {
                return Ga(this)
                    ? this._ownerReadableStream === void 0
                        ? A(Xa('cancel'))
                        : Cn(this, a)
                    : A(Up('cancel'))
            }
            read(a) {
                if (!Ga(this)) return A(Up('read'))
                if (!ArrayBuffer.isView(a))
                    return A(new TypeError('view must be an array buffer view'))
                if (a.byteLength === 0)
                    return A(
                        new TypeError('view must have non-zero byteLength')
                    )
                if (a.buffer.byteLength === 0)
                    return A(
                        new TypeError(
                            "view's buffer must have non-zero byteLength"
                        )
                    )
                if ((a.buffer, this._ownerReadableStream === void 0))
                    return A(Xa('read from'))
                let t,
                    r,
                    i = Te((o, m) => {
                        ;(t = o), (r = m)
                    })
                return (
                    (function (o, m, u) {
                        let s = o._ownerReadableStream
                        ;(s._disturbed = !0),
                            s._state === 'errored'
                                ? u._errorSteps(s._storedError)
                                : (function (f, h, _) {
                                      let w = f._controlledReadableByteStream,
                                          g = 1
                                      h.constructor !== DataView &&
                                          (g = h.constructor.BYTES_PER_ELEMENT)
                                      let y = h.constructor,
                                          C = h.buffer,
                                          R = {
                                              buffer: C,
                                              bufferByteLength: C.byteLength,
                                              byteOffset: h.byteOffset,
                                              byteLength: h.byteLength,
                                              bytesFilled: 0,
                                              elementSize: g,
                                              viewConstructor: y,
                                              readerType: 'byob'
                                          }
                                      if (f._pendingPullIntos.length > 0)
                                          return (
                                              f._pendingPullIntos.push(R),
                                              void rn(w, _)
                                          )
                                      if (w._state !== 'closed') {
                                          if (f._queueTotalSize > 0) {
                                              if (Mn(f, R)) {
                                                  let T = jn(R)
                                                  return (
                                                      Un(f),
                                                      void _._chunkSteps(T)
                                                  )
                                              }
                                              if (f._closeRequested) {
                                                  let T = new TypeError(
                                                      'Insufficient bytes to fill elements in the given buffer'
                                                  )
                                                  return (
                                                      Ja(f, T),
                                                      void _._errorSteps(T)
                                                  )
                                              }
                                          }
                                          f._pendingPullIntos.push(R),
                                              rn(w, _),
                                              wa(f)
                                      } else {
                                          let T = new y(
                                              R.buffer,
                                              R.byteOffset,
                                              0
                                          )
                                          _._closeSteps(T)
                                      }
                                  })(s._readableStreamController, m, u)
                    })(this, a, {
                        _chunkSteps: (o) => t({ value: o, done: !1 }),
                        _closeSteps: (o) => t({ value: o, done: !0 }),
                        _errorSteps: (o) => r(o)
                    }),
                    i
                )
            }
            releaseLock() {
                if (!Ga(this)) throw Up('releaseLock')
                this._ownerReadableStream !== void 0 &&
                    (function (a) {
                        En(a)
                        let t = new TypeError('Reader was released')
                        Gn(a, t)
                    })(this)
            }
        }
        Object.defineProperties(b1.prototype, {
            cancel: { enumerable: !0 },
            read: { enumerable: !0 },
            releaseLock: { enumerable: !0 },
            closed: { enumerable: !0 }
        }),
            U(b1.prototype.cancel, 'cancel'),
            U(b1.prototype.read, 'read'),
            U(b1.prototype.releaseLock, 'releaseLock'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(b1.prototype, z.toStringTag, {
                    value: 'ReadableStreamBYOBReader',
                    configurable: !0
                })
        ;(As = typeof AbortController == 'function'),
            (w1 = class {
                constructor(a = {}, t = {}) {
                    a === void 0 ? (a = null) : Dn(a, 'First parameter')
                    let r = ir(t, 'Second parameter'),
                        i = (function (u, s) {
                            S1(u, s)
                            let f = u?.abort,
                                h = u?.close,
                                _ = u?.start,
                                w = u?.type,
                                g = u?.write
                            return {
                                abort:
                                    f === void 0
                                        ? void 0
                                        : Es(
                                              f,
                                              u,
                                              `${s} has member 'abort' that`
                                          ),
                                close:
                                    h === void 0
                                        ? void 0
                                        : Ps(
                                              h,
                                              u,
                                              `${s} has member 'close' that`
                                          ),
                                start:
                                    _ === void 0
                                        ? void 0
                                        : Ns(
                                              _,
                                              u,
                                              `${s} has member 'start' that`
                                          ),
                                write:
                                    g === void 0
                                        ? void 0
                                        : Ds(
                                              g,
                                              u,
                                              `${s} has member 'write' that`
                                          ),
                                type: w
                            }
                        })(a, 'First parameter')
                    var o
                    if (
                        (((o = this)._state = 'writable'),
                        (o._storedError = void 0),
                        (o._writer = void 0),
                        (o._writableStreamController = void 0),
                        (o._writeRequests = new Ee()),
                        (o._inFlightWriteRequest = void 0),
                        (o._closeRequest = void 0),
                        (o._inFlightCloseRequest = void 0),
                        (o._pendingAbortRequest = void 0),
                        (o._backpressure = !1),
                        i.type !== void 0)
                    )
                        throw new RangeError('Invalid type is specified')
                    let m = dr(r)
                    ;(function (u, s, f, h) {
                        let _ = Object.create(Za.prototype),
                            w,
                            g,
                            y,
                            C
                        ;(w = s.start !== void 0 ? () => s.start(_) : () => {}),
                            (g =
                                s.write !== void 0
                                    ? (R) => s.write(R, _)
                                    : () => H(void 0)),
                            (y =
                                s.close !== void 0
                                    ? () => s.close()
                                    : () => H(void 0)),
                            (C =
                                s.abort !== void 0
                                    ? (R) => s.abort(R)
                                    : () => H(void 0)),
                            (function (R, T, F, K, I, k, Z, ve) {
                                ;(T._controlledWritableStream = R),
                                    (R._writableStreamController = T),
                                    (T._queue = void 0),
                                    (T._queueTotalSize = void 0),
                                    U1(T),
                                    (T._abortReason = void 0),
                                    (T._abortController = (function () {
                                        if (As) return new AbortController()
                                    })()),
                                    (T._started = !1),
                                    (T._strategySizeAlgorithm = ve),
                                    (T._strategyHWM = Z),
                                    (T._writeAlgorithm = K),
                                    (T._closeAlgorithm = I),
                                    (T._abortAlgorithm = k)
                                let N = h2(T)
                                f2(R, N)
                                let D = F()
                                _e(
                                    H(D),
                                    () => ((T._started = !0), fr(T), null),
                                    (x) => ((T._started = !0), e2(R, x), null)
                                )
                            })(u, _, w, g, y, C, f, h)
                    })(this, i, jt(r, 1), m)
                }
                get locked() {
                    if (!e1(this)) throw Qp('locked')
                    return er(this)
                }
                abort(a) {
                    return e1(this)
                        ? er(this)
                            ? A(
                                  new TypeError(
                                      'Cannot abort a stream that already has a writer'
                                  )
                              )
                            : Yn(this, a)
                        : A(Qp('abort'))
                }
                close() {
                    return e1(this)
                        ? er(this)
                            ? A(
                                  new TypeError(
                                      'Cannot close a stream that already has a writer'
                                  )
                              )
                            : $1(this)
                            ? A(
                                  new TypeError(
                                      'Cannot close an already-closing stream'
                                  )
                              )
                            : Kn(this)
                        : A(Qp('close'))
                }
                getWriter() {
                    if (!e1(this)) throw Qp('getWriter')
                    return new Ze(this)
                }
            })
        Object.defineProperties(w1.prototype, {
            abort: { enumerable: !0 },
            close: { enumerable: !0 },
            getWriter: { enumerable: !0 },
            locked: { enumerable: !0 }
        }),
            U(w1.prototype.abort, 'abort'),
            U(w1.prototype.close, 'close'),
            U(w1.prototype.getWriter, 'getWriter'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(w1.prototype, z.toStringTag, {
                    value: 'WritableStream',
                    configurable: !0
                })
        Ze = class {
            constructor(a) {
                if (
                    (T1(a, 1, 'WritableStreamDefaultWriter'),
                    (function (i, o) {
                        if (!e1(i))
                            throw new TypeError(`${o} is not a WritableStream.`)
                    })(a, 'First parameter'),
                    er(a))
                )
                    throw new TypeError(
                        'This stream has already been locked for exclusive writing by another writer'
                    )
                ;(this._ownerWritableStream = a), (a._writer = this)
                let t = a._state
                if (t === 'writable')
                    !$1(a) && a._backpressure ? hr(this) : on(this), ar(this)
                else if (t === 'erroring') a2(this, a._storedError), ar(this)
                else if (t === 'closed') on(this), ar((r = this)), ao(r)
                else {
                    let i = a._storedError
                    a2(this, i), nn(this, i)
                }
                var r
            }
            get closed() {
                return ca(this) ? this._closedPromise : A(fa('closed'))
            }
            get desiredSize() {
                if (!ca(this)) throw fa('desiredSize')
                if (this._ownerWritableStream === void 0)
                    throw Ot('desiredSize')
                return (function (a) {
                    let t = a._ownerWritableStream,
                        r = t._state
                    return r === 'errored' || r === 'erroring'
                        ? null
                        : r === 'closed'
                        ? 0
                        : Zn(t._writableStreamController)
                })(this)
            }
            get ready() {
                return ca(this) ? this._readyPromise : A(fa('ready'))
            }
            abort(a) {
                return ca(this)
                    ? this._ownerWritableStream === void 0
                        ? A(Ot('abort'))
                        : (function (t, r) {
                              return Yn(t._ownerWritableStream, r)
                          })(this, a)
                    : A(fa('abort'))
            }
            close() {
                if (!ca(this)) return A(fa('close'))
                let a = this._ownerWritableStream
                return a === void 0
                    ? A(Ot('close'))
                    : $1(a)
                    ? A(new TypeError('Cannot close an already-closing stream'))
                    : Kn(this._ownerWritableStream)
            }
            releaseLock() {
                if (!ca(this)) throw fa('releaseLock')
                this._ownerWritableStream !== void 0 &&
                    (function (a) {
                        let t = a._ownerWritableStream,
                            r = new TypeError(
                                "Writer was released and can no longer be used to monitor the stream's closedness"
                            )
                        Xn(a, r),
                            (function (i, o) {
                                i._closedPromiseState === 'pending'
                                    ? v2(i, o)
                                    : (function (m, u) {
                                          nn(m, u)
                                      })(i, o)
                            })(a, r),
                            (t._writer = void 0),
                            (a._ownerWritableStream = void 0)
                    })(this)
            }
            write(a) {
                return ca(this)
                    ? this._ownerWritableStream === void 0
                        ? A(Ot('write to'))
                        : (function (t, r) {
                              let i = t._ownerWritableStream,
                                  o = i._writableStreamController,
                                  m = (function (f, h) {
                                      try {
                                          return f._strategySizeAlgorithm(h)
                                      } catch (_) {
                                          return dn(f, _), 1
                                      }
                                  })(o, r)
                              if (i !== t._ownerWritableStream)
                                  return A(Ot('write to'))
                              let u = i._state
                              if (u === 'errored') return A(i._storedError)
                              if ($1(i) || u === 'closed')
                                  return A(
                                      new TypeError(
                                          'The stream is closing or closed and cannot be written to'
                                      )
                                  )
                              if (u === 'erroring') return A(i._storedError)
                              let s = (function (f) {
                                  return Te((h, _) => {
                                      let w = { _resolve: h, _reject: _ }
                                      f._writeRequests.push(w)
                                  })
                              })(i)
                              return (
                                  (function (f, h, _) {
                                      try {
                                          l2(f, h, _)
                                      } catch (g) {
                                          return void dn(f, g)
                                      }
                                      let w = f._controlledWritableStream
                                      !$1(w) &&
                                          w._state === 'writable' &&
                                          f2(w, h2(f)),
                                          fr(f)
                                  })(o, r, m),
                                  s
                              )
                          })(this, a)
                    : A(fa('write'))
            }
        }
        Object.defineProperties(Ze.prototype, {
            abort: { enumerable: !0 },
            close: { enumerable: !0 },
            releaseLock: { enumerable: !0 },
            write: { enumerable: !0 },
            closed: { enumerable: !0 },
            desiredSize: { enumerable: !0 },
            ready: { enumerable: !0 }
        }),
            U(Ze.prototype.abort, 'abort'),
            U(Ze.prototype.close, 'close'),
            U(Ze.prototype.releaseLock, 'releaseLock'),
            U(Ze.prototype.write, 'write'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(Ze.prototype, z.toStringTag, {
                    value: 'WritableStreamDefaultWriter',
                    configurable: !0
                })
        ;(Jn = {}),
            (Za = class {
                constructor() {
                    throw new TypeError('Illegal constructor')
                }
                get abortReason() {
                    if (!Hd(this)) throw Qd('abortReason')
                    return this._abortReason
                }
                get signal() {
                    if (!Hd(this)) throw Qd('signal')
                    if (this._abortController === void 0)
                        throw new TypeError(
                            'WritableStreamDefaultController.prototype.signal is not supported'
                        )
                    return this._abortController.signal
                }
                error(a) {
                    if (!Hd(this)) throw Qd('error')
                    this._controlledWritableStream._state === 'writable' &&
                        eo(this, a)
                }
                [Sn](a) {
                    let t = this._abortAlgorithm(a)
                    return nr(this), t
                }
                [Tn]() {
                    U1(this)
                }
            })
        Object.defineProperties(Za.prototype, {
            abortReason: { enumerable: !0 },
            signal: { enumerable: !0 },
            error: { enumerable: !0 }
        }),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(Za.prototype, z.toStringTag, {
                    value: 'WritableStreamDefaultController',
                    configurable: !0
                })
        ;(ln = typeof DOMException < 'u' ? DOMException : void 0),
            (Vs = (function (e) {
                if (typeof e != 'function' && typeof e != 'object') return !1
                try {
                    return new e(), !0
                } catch {
                    return !1
                }
            })(ln)
                ? ln
                : (function () {
                      let e = function (a, t) {
                          ;(this.message = a || ''),
                              (this.name = t || 'Error'),
                              Error.captureStackTrace &&
                                  Error.captureStackTrace(
                                      this,
                                      this.constructor
                                  )
                      }
                      return (
                          (e.prototype = Object.create(Error.prototype)),
                          Object.defineProperty(e.prototype, 'constructor', {
                              value: e,
                              writable: !0,
                              configurable: !0
                          }),
                          e
                      )
                  })())
        _1 = class {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get desiredSize() {
                if (!Gp(this)) throw Yp('desiredSize')
                return po(this)
            }
            close() {
                if (!Gp(this)) throw Yp('close')
                if (!xt(this))
                    throw new TypeError(
                        'The stream is not in a state that permits close'
                    )
                ;(function (a) {
                    if (!xt(a)) return
                    let t = a._controlledReadableStream
                    ;(a._closeRequested = !0),
                        a._queue.length === 0 && (tr(a), Wt(t))
                })(this)
            }
            enqueue(a) {
                if (!Gp(this)) throw Yp('enqueue')
                if (!xt(this))
                    throw new TypeError(
                        'The stream is not in a state that permits enqueue'
                    )
                return (function (t, r) {
                    if (!xt(t)) return
                    let i = t._controlledReadableStream
                    if (et(i) && cr(i) > 0) o2(i, r, !1)
                    else {
                        let o
                        try {
                            o = t._strategySizeAlgorithm(r)
                        } catch (m) {
                            throw (It(t, m), m)
                        }
                        try {
                            l2(t, r, o)
                        } catch (m) {
                            throw (It(t, m), m)
                        }
                    }
                    Lt(t)
                })(this, a)
            }
            error(a) {
                if (!Gp(this)) throw Yp('error')
                It(this, a)
            }
            [p2](a) {
                U1(this)
                let t = this._cancelAlgorithm(a)
                return tr(this), t
            }
            [r2](a) {
                let t = this._controlledReadableStream
                if (this._queue.length > 0) {
                    let r = Xd(this)
                    this._closeRequested && this._queue.length === 0
                        ? (tr(this), Wt(t))
                        : Lt(this),
                        a._chunkSteps(r)
                } else Bn(t, a), Lt(this)
            }
            [d2]() {}
        }
        Object.defineProperties(_1.prototype, {
            close: { enumerable: !0 },
            enqueue: { enumerable: !0 },
            error: { enumerable: !0 },
            desiredSize: { enumerable: !0 }
        }),
            U(_1.prototype.close, 'close'),
            U(_1.prototype.enqueue, 'enqueue'),
            U(_1.prototype.error, 'error'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(_1.prototype, z.toStringTag, {
                    value: 'ReadableStreamDefaultController',
                    configurable: !0
                })
        ne = class {
            constructor(a = {}, t = {}) {
                a === void 0 ? (a = null) : Dn(a, 'First parameter')
                let r = ir(t, 'Second parameter'),
                    i = (function (m, u) {
                        S1(m, u)
                        let s = m,
                            f = s?.autoAllocateChunkSize,
                            h = s?.cancel,
                            _ = s?.pull,
                            w = s?.start,
                            g = s?.type
                        return {
                            autoAllocateChunkSize:
                                f === void 0
                                    ? void 0
                                    : An(
                                          f,
                                          `${u} has member 'autoAllocateChunkSize' that`
                                      ),
                            cancel:
                                h === void 0
                                    ? void 0
                                    : ks(h, s, `${u} has member 'cancel' that`),
                            pull:
                                _ === void 0
                                    ? void 0
                                    : Os(_, s, `${u} has member 'pull' that`),
                            start:
                                w === void 0
                                    ? void 0
                                    : Fs(w, s, `${u} has member 'start' that`),
                            type:
                                g === void 0
                                    ? void 0
                                    : xs(g, `${u} has member 'type' that`)
                        }
                    })(a, 'First parameter')
                var o
                if (
                    (((o = this)._state = 'readable'),
                    (o._reader = void 0),
                    (o._storedError = void 0),
                    (o._disturbed = !1),
                    i.type === 'bytes')
                ) {
                    if (r.size !== void 0)
                        throw new RangeError(
                            'The strategy for a byte stream cannot have a size function'
                        )
                    Rs(this, i, jt(r, 0))
                } else {
                    let m = dr(r)
                    Bs(this, i, jt(r, 1), m)
                }
            }
            get locked() {
                if (!ba(this)) throw ha('locked')
                return et(this)
            }
            cancel(a) {
                return ba(this)
                    ? et(this)
                        ? A(
                              new TypeError(
                                  'Cannot cancel a stream that already has a reader'
                              )
                          )
                        : ro(this, a)
                    : A(ha('cancel'))
            }
            getReader(a) {
                if (!ba(this)) throw ha('getReader')
                return (function (t, r) {
                    S1(t, r)
                    let i = t?.mode
                    return {
                        mode:
                            i === void 0
                                ? void 0
                                : Ls(i, `${r} has member 'mode' that`)
                    }
                })(a, 'First parameter').mode === void 0
                    ? new h1(this)
                    : (function (t) {
                          return new b1(t)
                      })(this)
            }
            pipeThrough(a, t = {}) {
                if (!Ft(this)) throw ha('pipeThrough')
                T1(a, 1, 'pipeThrough')
                let r = Is(a, 'First parameter'),
                    i = mn(t, 'Second parameter')
                if (this.locked)
                    throw new TypeError(
                        'ReadableStream.prototype.pipeThrough cannot be used on a locked ReadableStream'
                    )
                if (r.writable.locked)
                    throw new TypeError(
                        'ReadableStream.prototype.pipeThrough cannot be used on a locked WritableStream'
                    )
                return (
                    Ka(
                        sn(
                            this,
                            r.writable,
                            i.preventClose,
                            i.preventAbort,
                            i.preventCancel,
                            i.signal
                        )
                    ),
                    r.readable
                )
            }
            pipeTo(a, t = {}) {
                if (!Ft(this)) return A(ha('pipeTo'))
                if (a === void 0)
                    return A("Parameter 1 is required in 'pipeTo'.")
                if (!Vn(a))
                    return A(
                        new TypeError(
                            "ReadableStream.prototype.pipeTo's first argument must be a WritableStream"
                        )
                    )
                let r
                try {
                    r = mn(t, 'Second parameter')
                } catch (i) {
                    return A(i)
                }
                return this.locked
                    ? A(
                          new TypeError(
                              'ReadableStream.prototype.pipeTo cannot be used on a locked ReadableStream'
                          )
                      )
                    : a.locked
                    ? A(
                          new TypeError(
                              'ReadableStream.prototype.pipeTo cannot be used on a locked WritableStream'
                          )
                      )
                    : sn(
                          this,
                          a,
                          r.preventClose,
                          r.preventAbort,
                          r.preventCancel,
                          r.signal
                      )
            }
            tee() {
                if (!Ft(this)) throw ha('tee')
                if (this.locked)
                    throw new TypeError(
                        'Cannot tee a stream that already has a reader'
                    )
                return qs(this)
            }
            values(a) {
                if (!Ft(this)) throw ha('values')
                return (function (t, r) {
                    let i = t.getReader(),
                        o = new pr(i, r),
                        m = Object.create(Fn)
                    return (m._asyncIteratorImpl = o), m
                })(
                    this,
                    (function (t, r) {
                        return S1(t, r), { preventCancel: !!t?.preventCancel }
                    })(a, 'First parameter').preventCancel
                )
            }
        }
        Object.defineProperties(ne.prototype, {
            cancel: { enumerable: !0 },
            getReader: { enumerable: !0 },
            pipeThrough: { enumerable: !0 },
            pipeTo: { enumerable: !0 },
            tee: { enumerable: !0 },
            values: { enumerable: !0 },
            locked: { enumerable: !0 }
        }),
            U(ne.prototype.cancel, 'cancel'),
            U(ne.prototype.getReader, 'getReader'),
            U(ne.prototype.pipeThrough, 'pipeThrough'),
            U(ne.prototype.pipeTo, 'pipeTo'),
            U(ne.prototype.tee, 'tee'),
            U(ne.prototype.values, 'values'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(ne.prototype, z.toStringTag, {
                    value: 'ReadableStream',
                    configurable: !0
                }),
            typeof z.asyncIterator == 'symbol' &&
                Object.defineProperty(ne.prototype, z.asyncIterator, {
                    value: ne.prototype.values,
                    writable: !0,
                    configurable: !0
                })
        oo = (e) => e.byteLength
        U(oo, 'size')
        zt = class {
            constructor(a) {
                T1(a, 1, 'ByteLengthQueuingStrategy'),
                    (a = no(a, 'First parameter')),
                    (this._byteLengthQueuingStrategyHighWaterMark =
                        a.highWaterMark)
            }
            get highWaterMark() {
                if (!cn(this)) throw un('highWaterMark')
                return this._byteLengthQueuingStrategyHighWaterMark
            }
            get size() {
                if (!cn(this)) throw un('size')
                return oo
            }
        }
        Object.defineProperties(zt.prototype, {
            highWaterMark: { enumerable: !0 },
            size: { enumerable: !0 }
        }),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(zt.prototype, z.toStringTag, {
                    value: 'ByteLengthQueuingStrategy',
                    configurable: !0
                })
        lo = () => 1
        U(lo, 'size')
        Mt = class {
            constructor(a) {
                T1(a, 1, 'CountQueuingStrategy'),
                    (a = no(a, 'First parameter')),
                    (this._countQueuingStrategyHighWaterMark = a.highWaterMark)
            }
            get highWaterMark() {
                if (!hn(this)) throw fn('highWaterMark')
                return this._countQueuingStrategyHighWaterMark
            }
            get size() {
                if (!hn(this)) throw fn('size')
                return lo
            }
        }
        Object.defineProperties(Mt.prototype, {
            highWaterMark: { enumerable: !0 },
            size: { enumerable: !0 }
        }),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(Mt.prototype, z.toStringTag, {
                    value: 'CountQueuingStrategy',
                    configurable: !0
                })
        $t = class {
            constructor(a = {}, t = {}, r = {}) {
                a === void 0 && (a = null)
                let i = ir(t, 'Second parameter'),
                    o = ir(r, 'Third parameter'),
                    m = (function (w, g) {
                        S1(w, g)
                        let y = w?.flush,
                            C = w?.readableType,
                            R = w?.start,
                            T = w?.transform,
                            F = w?.writableType
                        return {
                            flush:
                                y === void 0
                                    ? void 0
                                    : js(y, w, `${g} has member 'flush' that`),
                            readableType: C,
                            start:
                                R === void 0
                                    ? void 0
                                    : Ws(R, w, `${g} has member 'start' that`),
                            transform:
                                T === void 0
                                    ? void 0
                                    : zs(
                                          T,
                                          w,
                                          `${g} has member 'transform' that`
                                      ),
                            writableType: F
                        }
                    })(a, 'First parameter')
                if (m.readableType !== void 0)
                    throw new RangeError('Invalid readableType specified')
                if (m.writableType !== void 0)
                    throw new RangeError('Invalid writableType specified')
                let u = jt(o, 0),
                    s = dr(o),
                    f = jt(i, 1),
                    h = dr(i),
                    _
                ;(function (w, g, y, C, R, T) {
                    function F() {
                        return g
                    }
                    function K(N) {
                        return (function (D, x) {
                            let Q = D._transformStreamController
                            return D._backpressure
                                ? Ue(D._backpressureChangePromise, () => {
                                      if (
                                          (e1(D._writable)
                                              ? D._writable._state
                                              : D._writableState) === 'erroring'
                                      )
                                          throw e1(D._writable)
                                              ? D._writable._storedError
                                              : D._writableStoredError
                                      return bn(Q, x)
                                  })
                                : bn(Q, x)
                        })(w, N)
                    }
                    function I(N) {
                        return (function (D, x) {
                            return or(D, x), H(void 0)
                        })(w, N)
                    }
                    function k() {
                        return (function (N) {
                            let D = N._transformStreamController,
                                x = D._flushAlgorithm()
                            return (
                                so(D),
                                Ue(
                                    x,
                                    () => {
                                        if (N._readableState === 'errored')
                                            throw N._readableStoredError
                                        sr(N) && uo(N)
                                    },
                                    (Q) => {
                                        throw (or(N, Q), N._readableStoredError)
                                    }
                                )
                            )
                        })(w)
                    }
                    function Z() {
                        return (function (N) {
                            return lr(N, !1), N._backpressureChangePromise
                        })(w)
                    }
                    function ve(N) {
                        return vr(w, N), H(void 0)
                    }
                    ;(w._writableState = 'writable'),
                        (w._writableStoredError = void 0),
                        (w._writableHasInFlightOperation = !1),
                        (w._writableStarted = !1),
                        (w._writable = (function (N, D, x, Q, ie, q, M) {
                            return new w1(
                                {
                                    start(O) {
                                        N._writableController = O
                                        try {
                                            let $ = O.signal
                                            $ !== void 0 &&
                                                $.addEventListener(
                                                    'abort',
                                                    () => {
                                                        N._writableState ===
                                                            'writable' &&
                                                            ((N._writableState =
                                                                'erroring'),
                                                            $.reason &&
                                                                (N._writableStoredError =
                                                                    $.reason))
                                                    }
                                                )
                                        } catch {}
                                        return Ue(
                                            D(),
                                            () => (
                                                (N._writableStarted = !0),
                                                _n(N),
                                                null
                                            ),
                                            ($) => {
                                                throw (
                                                    ((N._writableStarted = !0),
                                                    Gd(N, $),
                                                    $)
                                                )
                                            }
                                        )
                                    },
                                    write: (O) => (
                                        (function ($) {
                                            $._writableHasInFlightOperation = !0
                                        })(N),
                                        Ue(
                                            x(O),
                                            () => (
                                                (function ($) {
                                                    $._writableHasInFlightOperation =
                                                        !1
                                                })(N),
                                                _n(N),
                                                null
                                            ),
                                            ($) => {
                                                throw (
                                                    ((function (ge, le) {
                                                        ;(ge._writableHasInFlightOperation =
                                                            !1),
                                                            Gd(ge, le)
                                                    })(N, $),
                                                    $)
                                                )
                                            }
                                        )
                                    ),
                                    close: () => (
                                        (function (O) {
                                            O._writableHasInFlightOperation = !0
                                        })(N),
                                        Ue(
                                            Q(),
                                            () => (
                                                (function (O) {
                                                    ;(O._writableHasInFlightOperation =
                                                        !1),
                                                        O._writableState ===
                                                            'erroring' &&
                                                            (O._writableStoredError =
                                                                void 0),
                                                        (O._writableState =
                                                            'closed')
                                                })(N),
                                                null
                                            ),
                                            (O) => {
                                                throw (
                                                    ((function ($, ge) {
                                                        ;($._writableHasInFlightOperation =
                                                            !1),
                                                            $._writableState,
                                                            Gd($, ge)
                                                    })(N, O),
                                                    O)
                                                )
                                            }
                                        )
                                    ),
                                    abort: (O) => (
                                        (N._writableState = 'errored'),
                                        (N._writableStoredError = O),
                                        ie(O)
                                    )
                                },
                                { highWaterMark: q, size: M }
                            )
                        })(w, F, K, k, I, y, C)),
                        (w._readableState = 'readable'),
                        (w._readableStoredError = void 0),
                        (w._readableCloseRequested = !1),
                        (w._readablePulling = !1),
                        (w._readable = (function (N, D, x, Q, ie, q) {
                            return new ne(
                                {
                                    start: (M) => (
                                        (N._readableController = M),
                                        D().catch((O) => {
                                            mr(N, O)
                                        })
                                    ),
                                    pull: () => (
                                        (N._readablePulling = !0),
                                        x().catch((M) => {
                                            mr(N, M)
                                        })
                                    ),
                                    cancel: (M) => (
                                        (N._readableState = 'closed'), Q(M)
                                    )
                                },
                                { highWaterMark: ie, size: q }
                            )
                        })(w, F, Z, ve, R, T)),
                        (w._backpressure = void 0),
                        (w._backpressureChangePromise = void 0),
                        (w._backpressureChangePromise_resolve = void 0),
                        lr(w, !0),
                        (w._transformStreamController = void 0)
                })(
                    this,
                    Te((w) => {
                        _ = w
                    }),
                    f,
                    h,
                    u,
                    s
                ),
                    (function (w, g) {
                        let y = Object.create(y1.prototype),
                            C,
                            R
                        ;(C =
                            g.transform !== void 0
                                ? (T) => g.transform(T, y)
                                : (T) => {
                                      try {
                                          return mo(y, T), H(void 0)
                                      } catch (F) {
                                          return A(F)
                                      }
                                  }),
                            (R =
                                g.flush !== void 0
                                    ? () => g.flush(y)
                                    : () => H(void 0)),
                            (function (T, F, K, I) {
                                ;(F._controlledTransformStream = T),
                                    (T._transformStreamController = F),
                                    (F._transformAlgorithm = K),
                                    (F._flushAlgorithm = I)
                            })(w, y, C, R)
                    })(this, m),
                    m.start !== void 0
                        ? _(m.start(this._transformStreamController))
                        : _(void 0)
            }
            get readable() {
                if (!vn(this)) throw wn('readable')
                return this._readable
            }
            get writable() {
                if (!vn(this)) throw wn('writable')
                return this._writable
            }
        }
        Object.defineProperties($t.prototype, {
            readable: { enumerable: !0 },
            writable: { enumerable: !0 }
        }),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty($t.prototype, z.toStringTag, {
                    value: 'TransformStream',
                    configurable: !0
                })
        y1 = class {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get desiredSize() {
                if (!Kp(this)) throw Xp('desiredSize')
                return co(this._controlledTransformStream)
            }
            enqueue(a) {
                if (!Kp(this)) throw Xp('enqueue')
                mo(this, a)
            }
            error(a) {
                if (!Kp(this)) throw Xp('error')
                var t
                ;(t = a), or(this._controlledTransformStream, t)
            }
            terminate() {
                if (!Kp(this)) throw Xp('terminate')
                ;(function (a) {
                    let t = a._controlledTransformStream
                    sr(t) && uo(t)
                    let r = new TypeError('TransformStream terminated')
                    vr(t, r)
                })(this)
            }
        }
        Object.defineProperties(y1.prototype, {
            enqueue: { enumerable: !0 },
            error: { enumerable: !0 },
            terminate: { enumerable: !0 },
            desiredSize: { enumerable: !0 }
        }),
            U(y1.prototype.enqueue, 'enqueue'),
            U(y1.prototype.error, 'error'),
            U(y1.prototype.terminate, 'terminate'),
            typeof z.toStringTag == 'symbol' &&
                Object.defineProperty(y1.prototype, z.toStringTag, {
                    value: 'TransformStreamDefaultController',
                    configurable: !0
                })
    })
var de,
    br = x1(() => {
        de = (e) => typeof e == 'function'
    })
async function* Ms(e) {
    let a = e.byteOffset + e.byteLength,
        t = e.byteOffset
    for (; t !== a; ) {
        let r = Math.min(a - t, vo),
            i = e.buffer.slice(t, t + r)
        ;(t += i.byteLength), yield new Uint8Array(i)
    }
}
async function* $s(e) {
    let a = 0
    for (; a !== e.size; ) {
        let r = await e.slice(a, Math.min(e.size, a + vo)).arrayBuffer()
        ;(a += r.byteLength), yield new Uint8Array(r)
    }
}
async function* wr(e, a = !1) {
    for (let t of e)
        ArrayBuffer.isView(t)
            ? a
                ? yield* Ms(t)
                : yield t
            : de(t.stream)
            ? yield* t.stream()
            : yield* $s(t)
}
function* bo(e, a, t = 0, r) {
    r ?? (r = a)
    let i = t < 0 ? Math.max(a + t, 0) : Math.min(t, a),
        o = r < 0 ? Math.max(a + r, 0) : Math.min(r, a),
        m = Math.max(o - i, 0),
        u = 0
    for (let s of e) {
        if (u >= m) break
        let f = ArrayBuffer.isView(s) ? s.byteLength : s.size
        if (i && f <= i) (i -= f), (o -= f)
        else {
            let h
            ArrayBuffer.isView(s)
                ? ((h = s.subarray(i, Math.min(f, o))), (u += h.byteLength))
                : ((h = s.slice(i, Math.min(f, o))), (u += h.size)),
                (o -= f),
                (i = 0),
                yield h
        }
    }
}
var vo,
    wo = x1(() => {
        br()
        vo = 65536
    })
var H1,
    _o,
    ya,
    _r,
    Ut,
    a1,
    yr = x1(() => {
        ho()
        br()
        wo()
        ;(H1 = function (e, a, t, r) {
            if (t === 'a' && !r)
                throw new TypeError(
                    'Private accessor was defined without a getter'
                )
            if (typeof a == 'function' ? e !== a || !r : !a.has(e))
                throw new TypeError(
                    'Cannot read private member from an object whose class did not declare it'
                )
            return t === 'm'
                ? r
                : t === 'a'
                ? r.call(e)
                : r
                ? r.value
                : a.get(e)
        }),
            (_o = function (e, a, t, r, i) {
                if (r === 'm')
                    throw new TypeError('Private method is not writable')
                if (r === 'a' && !i)
                    throw new TypeError(
                        'Private accessor was defined without a setter'
                    )
                if (typeof a == 'function' ? e !== a || !i : !a.has(e))
                    throw new TypeError(
                        'Cannot write private member to an object whose class did not declare it'
                    )
                return (
                    r === 'a' ? i.call(e, t) : i ? (i.value = t) : a.set(e, t),
                    t
                )
            }),
            (a1 = class e {
                constructor(a = [], t = {}) {
                    if (
                        (ya.set(this, []),
                        _r.set(this, ''),
                        Ut.set(this, 0),
                        t ?? (t = {}),
                        typeof a != 'object' || a === null)
                    )
                        throw new TypeError(
                            "Failed to construct 'Blob': The provided value cannot be converted to a sequence."
                        )
                    if (!de(a[Symbol.iterator]))
                        throw new TypeError(
                            "Failed to construct 'Blob': The object must have a callable @@iterator property."
                        )
                    if (typeof t != 'object' && !de(t))
                        throw new TypeError(
                            "Failed to construct 'Blob': parameter 2 cannot convert to dictionary."
                        )
                    let r = new TextEncoder()
                    for (let o of a) {
                        let m
                        ArrayBuffer.isView(o)
                            ? (m = new Uint8Array(
                                  o.buffer.slice(
                                      o.byteOffset,
                                      o.byteOffset + o.byteLength
                                  )
                              ))
                            : o instanceof ArrayBuffer
                            ? (m = new Uint8Array(o.slice(0)))
                            : o instanceof e
                            ? (m = o)
                            : (m = r.encode(String(o))),
                            _o(
                                this,
                                Ut,
                                H1(this, Ut, 'f') +
                                    (ArrayBuffer.isView(m)
                                        ? m.byteLength
                                        : m.size),
                                'f'
                            ),
                            H1(this, ya, 'f').push(m)
                    }
                    let i = t.type === void 0 ? '' : String(t.type)
                    _o(this, _r, /^[\x20-\x7E]*$/.test(i) ? i : '', 'f')
                }
                static [((ya = new WeakMap()),
                (_r = new WeakMap()),
                (Ut = new WeakMap()),
                Symbol.hasInstance)](a) {
                    return !!(
                        a &&
                        typeof a == 'object' &&
                        de(a.constructor) &&
                        (de(a.stream) || de(a.arrayBuffer)) &&
                        /^(Blob|File)$/.test(a[Symbol.toStringTag])
                    )
                }
                get type() {
                    return H1(this, _r, 'f')
                }
                get size() {
                    return H1(this, Ut, 'f')
                }
                slice(a, t, r) {
                    return new e(bo(H1(this, ya, 'f'), this.size, a, t), {
                        type: r
                    })
                }
                async text() {
                    let a = new TextDecoder(),
                        t = ''
                    for await (let r of wr(H1(this, ya, 'f')))
                        t += a.decode(r, { stream: !0 })
                    return (t += a.decode()), t
                }
                async arrayBuffer() {
                    let a = new Uint8Array(this.size),
                        t = 0
                    for await (let r of wr(H1(this, ya, 'f')))
                        a.set(r, t), (t += r.length)
                    return a.buffer
                }
                stream() {
                    let a = wr(H1(this, ya, 'f'), !0)
                    return new ne({
                        async pull(t) {
                            let { value: r, done: i } = await a.next()
                            if (i) return queueMicrotask(() => t.close())
                            t.enqueue(r)
                        },
                        async cancel() {
                            await a.return()
                        }
                    })
                }
                get [Symbol.toStringTag]() {
                    return 'Blob'
                }
            })
        Object.defineProperties(a1.prototype, {
            type: { enumerable: !0 },
            size: { enumerable: !0 },
            slice: { enumerable: !0 },
            stream: { enumerable: !0 },
            text: { enumerable: !0 },
            arrayBuffer: { enumerable: !0 }
        })
    })
var yo,
    go,
    gr,
    Sr,
    Qe,
    Ht = x1(() => {
        yr()
        ;(yo = function (e, a, t, r, i) {
            if (r === 'm') throw new TypeError('Private method is not writable')
            if (r === 'a' && !i)
                throw new TypeError(
                    'Private accessor was defined without a setter'
                )
            if (typeof a == 'function' ? e !== a || !i : !a.has(e))
                throw new TypeError(
                    'Cannot write private member to an object whose class did not declare it'
                )
            return r === 'a' ? i.call(e, t) : i ? (i.value = t) : a.set(e, t), t
        }),
            (go = function (e, a, t, r) {
                if (t === 'a' && !r)
                    throw new TypeError(
                        'Private accessor was defined without a getter'
                    )
                if (typeof a == 'function' ? e !== a || !r : !a.has(e))
                    throw new TypeError(
                        'Cannot read private member from an object whose class did not declare it'
                    )
                return t === 'm'
                    ? r
                    : t === 'a'
                    ? r.call(e)
                    : r
                    ? r.value
                    : a.get(e)
            }),
            (Qe = class extends a1 {
                constructor(a, t, r = {}) {
                    if (
                        (super(a, r),
                        gr.set(this, void 0),
                        Sr.set(this, 0),
                        arguments.length < 2)
                    )
                        throw new TypeError(
                            `Failed to construct 'File': 2 arguments required, but only ${arguments.length} present.`
                        )
                    yo(this, gr, String(t), 'f')
                    let i =
                        r.lastModified === void 0
                            ? Date.now()
                            : Number(r.lastModified)
                    Number.isNaN(i) || yo(this, Sr, i, 'f')
                }
                static [((gr = new WeakMap()),
                (Sr = new WeakMap()),
                Symbol.hasInstance)](a) {
                    return (
                        a instanceof a1 &&
                        a[Symbol.toStringTag] === 'File' &&
                        typeof a.name == 'string'
                    )
                }
                get name() {
                    return go(this, gr, 'f')
                }
                get lastModified() {
                    return go(this, Sr, 'f')
                }
                get webkitRelativePath() {
                    return ''
                }
                get [Symbol.toStringTag]() {
                    return 'File'
                }
            })
    })
var _2,
    y2 = x1(() => {
        Ht()
        _2 = (e) => e instanceof Qe
    })
var Co = se((U3, Ro) => {
    var at = 1e3,
        tt = at * 60,
        pt = tt * 60,
        ga = pt * 24,
        Gs = ga * 7,
        Ys = ga * 365.25
    Ro.exports = function (e, a) {
        a = a || {}
        var t = typeof e
        if (t === 'string' && e.length > 0) return Ks(e)
        if (t === 'number' && isFinite(e)) return a.long ? Js(e) : Xs(e)
        throw new Error(
            'val is not a non-empty string or a valid number. val=' +
                JSON.stringify(e)
        )
    }
    function Ks(e) {
        if (((e = String(e)), !(e.length > 100))) {
            var a =
                /^(-?(?:\d+)?\.?\d+) *(milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|years?|yrs?|y)?$/i.exec(
                    e
                )
            if (a) {
                var t = parseFloat(a[1]),
                    r = (a[2] || 'ms').toLowerCase()
                switch (r) {
                    case 'years':
                    case 'year':
                    case 'yrs':
                    case 'yr':
                    case 'y':
                        return t * Ys
                    case 'weeks':
                    case 'week':
                    case 'w':
                        return t * Gs
                    case 'days':
                    case 'day':
                    case 'd':
                        return t * ga
                    case 'hours':
                    case 'hour':
                    case 'hrs':
                    case 'hr':
                    case 'h':
                        return t * pt
                    case 'minutes':
                    case 'minute':
                    case 'mins':
                    case 'min':
                    case 'm':
                        return t * tt
                    case 'seconds':
                    case 'second':
                    case 'secs':
                    case 'sec':
                    case 's':
                        return t * at
                    case 'milliseconds':
                    case 'millisecond':
                    case 'msecs':
                    case 'msec':
                    case 'ms':
                        return t
                    default:
                        return
                }
            }
        }
    }
    function Xs(e) {
        var a = Math.abs(e)
        return a >= ga
            ? Math.round(e / ga) + 'd'
            : a >= pt
            ? Math.round(e / pt) + 'h'
            : a >= tt
            ? Math.round(e / tt) + 'm'
            : a >= at
            ? Math.round(e / at) + 's'
            : e + 'ms'
    }
    function Js(e) {
        var a = Math.abs(e)
        return a >= ga
            ? Cr(e, a, ga, 'day')
            : a >= pt
            ? Cr(e, a, pt, 'hour')
            : a >= tt
            ? Cr(e, a, tt, 'minute')
            : a >= at
            ? Cr(e, a, at, 'second')
            : e + ' ms'
    }
    function Cr(e, a, t, r) {
        var i = a >= t * 1.5
        return Math.round(e / t) + ' ' + r + (i ? 's' : '')
    }
})
var Po = se((H3, Eo) => {
    'use strict'
    var Zs = Ae('util'),
        e9 = Co()
    Eo.exports = function (e) {
        if (typeof e == 'number') return e
        var a = e9(e)
        if (a === void 0) {
            var t = new Error(Zs.format('humanize-ms(%j) result undefined', e))
            console.warn(t.stack)
        }
        return a
    }
})
var Er = se((Q3, No) => {
    'use strict'
    No.exports = {
        CURRENT_ID: Symbol('agentkeepalive#currentId'),
        CREATE_ID: Symbol('agentkeepalive#createId'),
        INIT_SOCKET: Symbol('agentkeepalive#initSocket'),
        CREATE_HTTPS_CONNECTION: Symbol('agentkeepalive#createHttpsConnection'),
        SOCKET_CREATED_TIME: Symbol('agentkeepalive#socketCreatedTime'),
        SOCKET_NAME: Symbol('agentkeepalive#socketName'),
        SOCKET_REQUEST_COUNT: Symbol('agentkeepalive#socketRequestCount'),
        SOCKET_REQUEST_FINISHED_COUNT: Symbol(
            'agentkeepalive#socketRequestFinishedCount'
        )
    }
})
var P2 = se((G3, qo) => {
    'use strict'
    var a9 = Ae('http').Agent,
        S2 = Po(),
        Re = Ae('util').debuglog('agentkeepalive'),
        {
            INIT_SOCKET: Do,
            CURRENT_ID: Pr,
            CREATE_ID: Ao,
            SOCKET_CREATED_TIME: Vo,
            SOCKET_NAME: Pe,
            SOCKET_REQUEST_COUNT: Le,
            SOCKET_REQUEST_FINISHED_COUNT: r1
        } = Er(),
        C2 = 1,
        T2 = parseInt(process.version.split('.', 1)[0].substring(1))
    T2 >= 11 && T2 <= 12 ? (C2 = 2) : T2 >= 13 && (C2 = 3)
    function Qt(e) {
        console.log('[agentkeepalive:deprecated] %s', e)
    }
    var E2 = class extends a9 {
        constructor(a) {
            ;(a = a || {}),
                (a.keepAlive = a.keepAlive !== !1),
                a.freeSocketTimeout === void 0 && (a.freeSocketTimeout = 4e3),
                a.keepAliveTimeout &&
                    (Qt(
                        'options.keepAliveTimeout is deprecated, please use options.freeSocketTimeout instead'
                    ),
                    (a.freeSocketTimeout = a.keepAliveTimeout),
                    delete a.keepAliveTimeout),
                a.freeSocketKeepAliveTimeout &&
                    (Qt(
                        'options.freeSocketKeepAliveTimeout is deprecated, please use options.freeSocketTimeout instead'
                    ),
                    (a.freeSocketTimeout = a.freeSocketKeepAliveTimeout),
                    delete a.freeSocketKeepAliveTimeout),
                a.timeout === void 0 &&
                    (a.timeout = Math.max(a.freeSocketTimeout * 2, 8e3)),
                (a.timeout = S2(a.timeout)),
                (a.freeSocketTimeout = S2(a.freeSocketTimeout)),
                (a.socketActiveTTL = a.socketActiveTTL
                    ? S2(a.socketActiveTTL)
                    : 0),
                super(a),
                (this[Pr] = 0),
                (this.createSocketCount = 0),
                (this.createSocketCountLastCheck = 0),
                (this.createSocketErrorCount = 0),
                (this.createSocketErrorCountLastCheck = 0),
                (this.closeSocketCount = 0),
                (this.closeSocketCountLastCheck = 0),
                (this.errorSocketCount = 0),
                (this.errorSocketCountLastCheck = 0),
                (this.requestCount = 0),
                (this.requestCountLastCheck = 0),
                (this.timeoutSocketCount = 0),
                (this.timeoutSocketCountLastCheck = 0),
                this.on('free', (t) => {
                    let r = this.calcSocketTimeout(t)
                    r > 0 && t.timeout !== r && t.setTimeout(r)
                })
        }
        get freeSocketKeepAliveTimeout() {
            return (
                Qt(
                    'agent.freeSocketKeepAliveTimeout is deprecated, please use agent.options.freeSocketTimeout instead'
                ),
                this.options.freeSocketTimeout
            )
        }
        get timeout() {
            return (
                Qt(
                    'agent.timeout is deprecated, please use agent.options.timeout instead'
                ),
                this.options.timeout
            )
        }
        get socketActiveTTL() {
            return (
                Qt(
                    'agent.socketActiveTTL is deprecated, please use agent.options.socketActiveTTL instead'
                ),
                this.options.socketActiveTTL
            )
        }
        calcSocketTimeout(a) {
            let t = this.options.freeSocketTimeout,
                r = this.options.socketActiveTTL
            if (r) {
                let i = Date.now() - a[Vo],
                    o = r - i
                if (o <= 0) return o
                t && o < t && (t = o)
            }
            if (t)
                return a.freeSocketTimeout || a.freeSocketKeepAliveTimeout || t
        }
        keepSocketAlive(a) {
            let t = super.keepSocketAlive(a)
            if (!t) return t
            let r = this.calcSocketTimeout(a)
            return typeof r > 'u'
                ? !0
                : r <= 0
                ? (Re(
                      '%s(requests: %s, finished: %s) free but need to destroy by TTL, request count %s, diff is %s',
                      a[Pe],
                      a[Le],
                      a[r1],
                      r
                  ),
                  !1)
                : (a.timeout !== r && a.setTimeout(r), !0)
        }
        reuseSocket(...a) {
            super.reuseSocket(...a)
            let t = a[0],
                r = a[1]
            r.reusedSocket = !0
            let i = this.options.timeout
            Gt(t) !== i &&
                (t.setTimeout(i), Re('%s reset timeout to %sms', t[Pe], i)),
                t[Le]++,
                Re(
                    '%s(requests: %s, finished: %s) reuse on addRequest, timeout %sms',
                    t[Pe],
                    t[Le],
                    t[r1],
                    Gt(t)
                )
        }
        [Ao]() {
            let a = this[Pr]++
            return this[Pr] === Number.MAX_SAFE_INTEGER && (this[Pr] = 0), a
        }
        [Do](a, t) {
            t.timeout && (Gt(a) || a.setTimeout(t.timeout)),
                this.options.keepAlive && a.setNoDelay(!0),
                this.createSocketCount++,
                this.options.socketActiveTTL && (a[Vo] = Date.now()),
                (a[Pe] = `sock[${this[Ao]()}#${t._agentKey}]`.split(
                    '-----BEGIN',
                    1
                )[0]),
                (a[Le] = 1),
                (a[r1] = 0),
                t9(this, a, t)
        }
        createConnection(a, t) {
            let r = !1,
                i = (m, u) => {
                    if (!r) {
                        if (((r = !0), m))
                            return this.createSocketErrorCount++, t(m)
                        this[Do](u, a), t(m, u)
                    }
                },
                o = super.createConnection(a, i)
            return o && i(null, o), o
        }
        get statusChanged() {
            let a =
                this.createSocketCount !== this.createSocketCountLastCheck ||
                this.createSocketErrorCount !==
                    this.createSocketErrorCountLastCheck ||
                this.closeSocketCount !== this.closeSocketCountLastCheck ||
                this.errorSocketCount !== this.errorSocketCountLastCheck ||
                this.timeoutSocketCount !== this.timeoutSocketCountLastCheck ||
                this.requestCount !== this.requestCountLastCheck
            return (
                a &&
                    ((this.createSocketCountLastCheck = this.createSocketCount),
                    (this.createSocketErrorCountLastCheck =
                        this.createSocketErrorCount),
                    (this.closeSocketCountLastCheck = this.closeSocketCount),
                    (this.errorSocketCountLastCheck = this.errorSocketCount),
                    (this.timeoutSocketCountLastCheck =
                        this.timeoutSocketCount),
                    (this.requestCountLastCheck = this.requestCount)),
                a
            )
        }
        getCurrentStatus() {
            return {
                createSocketCount: this.createSocketCount,
                createSocketErrorCount: this.createSocketErrorCount,
                closeSocketCount: this.closeSocketCount,
                errorSocketCount: this.errorSocketCount,
                timeoutSocketCount: this.timeoutSocketCount,
                requestCount: this.requestCount,
                freeSockets: R2(this.freeSockets),
                sockets: R2(this.sockets),
                requests: R2(this.requests)
            }
        }
    }
    function Gt(e) {
        return e.timeout || e._idleTimeout
    }
    function t9(e, a, t) {
        Re('%s create, timeout %sms', a[Pe], Gt(a))
        function r() {
            if (!a._httpMessage && a[Le] === 1) return
            a[r1]++,
                e.requestCount++,
                Re('%s(requests: %s, finished: %s) free', a[Pe], a[Le], a[r1])
            let s = e.getName(t)
            a.writable &&
                e.requests[s] &&
                e.requests[s].length &&
                (a[Le]++,
                Re(
                    '%s(requests: %s, finished: %s) will be reuse on agent free event',
                    a[Pe],
                    a[Le],
                    a[r1]
                ))
        }
        a.on('free', r)
        function i(s) {
            Re(
                '%s(requests: %s, finished: %s) close, isError: %s',
                a[Pe],
                a[Le],
                a[r1],
                s
            ),
                e.closeSocketCount++
        }
        a.on('close', i)
        function o() {
            let s = a.listeners('timeout').length,
                f = Gt(a),
                h = a._httpMessage,
                _ = (h && h.listeners('timeout').length) || 0
            Re(
                '%s(requests: %s, finished: %s) timeout after %sms, listeners %s, defaultTimeoutListenerCount %s, hasHttpRequest %s, HttpRequest timeoutListenerCount %s',
                a[Pe],
                a[Le],
                a[r1],
                f,
                s,
                C2,
                !!h,
                _
            ),
                Re.enabled &&
                    Re(
                        'timeout listeners: %s',
                        a
                            .listeners('timeout')
                            .map((g) => g.name)
                            .join(', ')
                    ),
                e.timeoutSocketCount++
            let w = e.getName(t)
            if (e.freeSockets[w] && e.freeSockets[w].indexOf(a) !== -1)
                a.destroy(),
                    e.removeSocket(a, t),
                    Re('%s is free, destroy quietly', a[Pe])
            else if (_ === 0) {
                let g = new Error('Socket timeout')
                ;(g.code = 'ERR_SOCKET_TIMEOUT'),
                    (g.timeout = f),
                    a.destroy(g),
                    e.removeSocket(a, t),
                    Re('%s destroy with timeout error', a[Pe])
            }
        }
        a.on('timeout', o)
        function m(s) {
            let f = a.listeners('error').length
            Re(
                '%s(requests: %s, finished: %s) error: %s, listenerCount: %s',
                a[Pe],
                a[Le],
                a[r1],
                s,
                f
            ),
                e.errorSocketCount++,
                f === 1 &&
                    (Re('%s emit uncaught error event', a[Pe]),
                    a.removeListener('error', m),
                    a.emit('error', s))
        }
        a.on('error', m)
        function u() {
            Re(
                '%s(requests: %s, finished: %s) agentRemove',
                a[Pe],
                a[Le],
                a[r1]
            ),
                a.removeListener('close', i),
                a.removeListener('error', m),
                a.removeListener('free', r),
                a.removeListener('timeout', o),
                a.removeListener('agentRemove', u)
        }
        a.on('agentRemove', u)
    }
    qo.exports = E2
    function R2(e) {
        let a = {}
        for (let t in e) a[t] = e[t].length
        return a
    }
})
var Oo = se((Y3, ko) => {
    'use strict'
    var N2 = Ae('https').Agent,
        p9 = P2(),
        { INIT_SOCKET: r9, CREATE_HTTPS_CONNECTION: Bo } = Er(),
        Yt = class extends p9 {
            constructor(a) {
                super(a),
                    (this.defaultPort = 443),
                    (this.protocol = 'https:'),
                    (this.maxCachedSessions = this.options.maxCachedSessions),
                    this.maxCachedSessions === void 0 &&
                        (this.maxCachedSessions = 100),
                    (this._sessionCache = { map: {}, list: [] })
            }
            createConnection(a, t) {
                let r = this[Bo](a, t)
                return this[r9](r, a), r
            }
        }
    Yt.prototype[Bo] = N2.prototype.createConnection
    ;['getName', '_getSession', '_cacheSession', '_evictSession'].forEach(
        function (e) {
            typeof N2.prototype[e] == 'function' &&
                (Yt.prototype[e] = N2.prototype[e])
        }
    )
    ko.exports = Yt
})
var Fo = se((K3, Nr) => {
    'use strict'
    Nr.exports = P2()
    Nr.exports.HttpsAgent = Oo()
    Nr.exports.constants = Er()
})
var Ho = se((Jt, Xt) => {
    'use strict'
    Object.defineProperty(Jt, '__esModule', { value: !0 })
    var zo = new WeakMap(),
        D2 = new WeakMap()
    function J(e) {
        let a = zo.get(e)
        return (
            console.assert(
                a != null,
                "'this' is expected an Event object, but got",
                e
            ),
            a
        )
    }
    function xo(e) {
        if (e.passiveListener != null) {
            typeof console < 'u' &&
                typeof console.error == 'function' &&
                console.error(
                    'Unable to preventDefault inside passive event listener invocation.',
                    e.passiveListener
                )
            return
        }
        e.event.cancelable &&
            ((e.canceled = !0),
            typeof e.event.preventDefault == 'function' &&
                e.event.preventDefault())
    }
    function rt(e, a) {
        zo.set(this, {
            eventTarget: e,
            event: a,
            eventPhase: 2,
            currentTarget: e,
            canceled: !1,
            stopped: !1,
            immediateStopped: !1,
            passiveListener: null,
            timeStamp: a.timeStamp || Date.now()
        }),
            Object.defineProperty(this, 'isTrusted', {
                value: !1,
                enumerable: !0
            })
        let t = Object.keys(a)
        for (let r = 0; r < t.length; ++r) {
            let i = t[r]
            i in this || Object.defineProperty(this, i, Mo(i))
        }
    }
    rt.prototype = {
        get type() {
            return J(this).event.type
        },
        get target() {
            return J(this).eventTarget
        },
        get currentTarget() {
            return J(this).currentTarget
        },
        composedPath() {
            let e = J(this).currentTarget
            return e == null ? [] : [e]
        },
        get NONE() {
            return 0
        },
        get CAPTURING_PHASE() {
            return 1
        },
        get AT_TARGET() {
            return 2
        },
        get BUBBLING_PHASE() {
            return 3
        },
        get eventPhase() {
            return J(this).eventPhase
        },
        stopPropagation() {
            let e = J(this)
            ;(e.stopped = !0),
                typeof e.event.stopPropagation == 'function' &&
                    e.event.stopPropagation()
        },
        stopImmediatePropagation() {
            let e = J(this)
            ;(e.stopped = !0),
                (e.immediateStopped = !0),
                typeof e.event.stopImmediatePropagation == 'function' &&
                    e.event.stopImmediatePropagation()
        },
        get bubbles() {
            return !!J(this).event.bubbles
        },
        get cancelable() {
            return !!J(this).event.cancelable
        },
        preventDefault() {
            xo(J(this))
        },
        get defaultPrevented() {
            return J(this).canceled
        },
        get composed() {
            return !!J(this).event.composed
        },
        get timeStamp() {
            return J(this).timeStamp
        },
        get srcElement() {
            return J(this).eventTarget
        },
        get cancelBubble() {
            return J(this).stopped
        },
        set cancelBubble(e) {
            if (!e) return
            let a = J(this)
            ;(a.stopped = !0),
                typeof a.event.cancelBubble == 'boolean' &&
                    (a.event.cancelBubble = !0)
        },
        get returnValue() {
            return !J(this).canceled
        },
        set returnValue(e) {
            e || xo(J(this))
        },
        initEvent() {}
    }
    Object.defineProperty(rt.prototype, 'constructor', {
        value: rt,
        configurable: !0,
        writable: !0
    })
    typeof window < 'u' &&
        typeof window.Event < 'u' &&
        (Object.setPrototypeOf(rt.prototype, window.Event.prototype),
        D2.set(window.Event.prototype, rt))
    function Mo(e) {
        return {
            get() {
                return J(this).event[e]
            },
            set(a) {
                J(this).event[e] = a
            },
            configurable: !0,
            enumerable: !0
        }
    }
    function d9(e) {
        return {
            value() {
                let a = J(this).event
                return a[e].apply(a, arguments)
            },
            configurable: !0,
            enumerable: !0
        }
    }
    function i9(e, a) {
        let t = Object.keys(a)
        if (t.length === 0) return e
        function r(i, o) {
            e.call(this, i, o)
        }
        r.prototype = Object.create(e.prototype, {
            constructor: { value: r, configurable: !0, writable: !0 }
        })
        for (let i = 0; i < t.length; ++i) {
            let o = t[i]
            if (!(o in e.prototype)) {
                let u =
                    typeof Object.getOwnPropertyDescriptor(a, o).value ==
                    'function'
                Object.defineProperty(r.prototype, o, u ? d9(o) : Mo(o))
            }
        }
        return r
    }
    function $o(e) {
        if (e == null || e === Object.prototype) return rt
        let a = D2.get(e)
        return (
            a == null &&
                ((a = i9($o(Object.getPrototypeOf(e)), e)), D2.set(e, a)),
            a
        )
    }
    function n9(e, a) {
        let t = $o(Object.getPrototypeOf(a))
        return new t(e, a)
    }
    function o9(e) {
        return J(e).immediateStopped
    }
    function l9(e, a) {
        J(e).eventPhase = a
    }
    function s9(e, a) {
        J(e).currentTarget = a
    }
    function Lo(e, a) {
        J(e).passiveListener = a
    }
    var Uo = new WeakMap(),
        Io = 1,
        jo = 2,
        Dr = 3
    function Ar(e) {
        return e !== null && typeof e == 'object'
    }
    function Kt(e) {
        let a = Uo.get(e)
        if (a == null)
            throw new TypeError(
                "'this' is expected an EventTarget object, but got another value."
            )
        return a
    }
    function m9(e) {
        return {
            get() {
                let t = Kt(this).get(e)
                for (; t != null; ) {
                    if (t.listenerType === Dr) return t.listener
                    t = t.next
                }
                return null
            },
            set(a) {
                typeof a != 'function' && !Ar(a) && (a = null)
                let t = Kt(this),
                    r = null,
                    i = t.get(e)
                for (; i != null; )
                    i.listenerType === Dr
                        ? r !== null
                            ? (r.next = i.next)
                            : i.next !== null
                            ? t.set(e, i.next)
                            : t.delete(e)
                        : (r = i),
                        (i = i.next)
                if (a !== null) {
                    let o = {
                        listener: a,
                        listenerType: Dr,
                        passive: !1,
                        once: !1,
                        next: null
                    }
                    r === null ? t.set(e, o) : (r.next = o)
                }
            },
            configurable: !0,
            enumerable: !0
        }
    }
    function A2(e, a) {
        Object.defineProperty(e, `on${a}`, m9(a))
    }
    function Wo(e) {
        function a() {
            Ge.call(this)
        }
        a.prototype = Object.create(Ge.prototype, {
            constructor: { value: a, configurable: !0, writable: !0 }
        })
        for (let t = 0; t < e.length; ++t) A2(a.prototype, e[t])
        return a
    }
    function Ge() {
        if (this instanceof Ge) {
            Uo.set(this, new Map())
            return
        }
        if (arguments.length === 1 && Array.isArray(arguments[0]))
            return Wo(arguments[0])
        if (arguments.length > 0) {
            let e = new Array(arguments.length)
            for (let a = 0; a < arguments.length; ++a) e[a] = arguments[a]
            return Wo(e)
        }
        throw new TypeError('Cannot call a class as a function')
    }
    Ge.prototype = {
        addEventListener(e, a, t) {
            if (a == null) return
            if (typeof a != 'function' && !Ar(a))
                throw new TypeError(
                    "'listener' should be a function or an object."
                )
            let r = Kt(this),
                i = Ar(t),
                m = (i ? !!t.capture : !!t) ? Io : jo,
                u = {
                    listener: a,
                    listenerType: m,
                    passive: i && !!t.passive,
                    once: i && !!t.once,
                    next: null
                },
                s = r.get(e)
            if (s === void 0) {
                r.set(e, u)
                return
            }
            let f = null
            for (; s != null; ) {
                if (s.listener === a && s.listenerType === m) return
                ;(f = s), (s = s.next)
            }
            f.next = u
        },
        removeEventListener(e, a, t) {
            if (a == null) return
            let r = Kt(this),
                o = (Ar(t) ? !!t.capture : !!t) ? Io : jo,
                m = null,
                u = r.get(e)
            for (; u != null; ) {
                if (u.listener === a && u.listenerType === o) {
                    m !== null
                        ? (m.next = u.next)
                        : u.next !== null
                        ? r.set(e, u.next)
                        : r.delete(e)
                    return
                }
                ;(m = u), (u = u.next)
            }
        },
        dispatchEvent(e) {
            if (e == null || typeof e.type != 'string')
                throw new TypeError('"event.type" should be a string.')
            let a = Kt(this),
                t = e.type,
                r = a.get(t)
            if (r == null) return !0
            let i = n9(this, e),
                o = null
            for (; r != null; ) {
                if (
                    (r.once
                        ? o !== null
                            ? (o.next = r.next)
                            : r.next !== null
                            ? a.set(t, r.next)
                            : a.delete(t)
                        : (o = r),
                    Lo(i, r.passive ? r.listener : null),
                    typeof r.listener == 'function')
                )
                    try {
                        r.listener.call(this, i)
                    } catch (m) {
                        typeof console < 'u' &&
                            typeof console.error == 'function' &&
                            console.error(m)
                    }
                else
                    r.listenerType !== Dr &&
                        typeof r.listener.handleEvent == 'function' &&
                        r.listener.handleEvent(i)
                if (o9(i)) break
                r = r.next
            }
            return Lo(i, null), l9(i, 0), s9(i, null), !i.defaultPrevented
        }
    }
    Object.defineProperty(Ge.prototype, 'constructor', {
        value: Ge,
        configurable: !0,
        writable: !0
    })
    typeof window < 'u' &&
        typeof window.EventTarget < 'u' &&
        Object.setPrototypeOf(Ge.prototype, window.EventTarget.prototype)
    Jt.defineEventAttribute = A2
    Jt.EventTarget = Ge
    Jt.default = Ge
    Xt.exports = Ge
    Xt.exports.EventTarget = Xt.exports.default = Ge
    Xt.exports.defineEventAttribute = A2
})
var Yo = se((ep, Zt) => {
    'use strict'
    Object.defineProperty(ep, '__esModule', { value: !0 })
    var V2 = Ho(),
        Q1 = class extends V2.EventTarget {
            constructor() {
                throw (
                    (super(),
                    new TypeError('AbortSignal cannot be constructed directly'))
                )
            }
            get aborted() {
                let a = Vr.get(this)
                if (typeof a != 'boolean')
                    throw new TypeError(
                        `Expected 'this' to be an 'AbortSignal' object, but got ${
                            this === null ? 'null' : typeof this
                        }`
                    )
                return a
            }
        }
    V2.defineEventAttribute(Q1.prototype, 'abort')
    function u9() {
        let e = Object.create(Q1.prototype)
        return V2.EventTarget.call(e), Vr.set(e, !1), e
    }
    function c9(e) {
        Vr.get(e) === !1 && (Vr.set(e, !0), e.dispatchEvent({ type: 'abort' }))
    }
    var Vr = new WeakMap()
    Object.defineProperties(Q1.prototype, { aborted: { enumerable: !0 } })
    typeof Symbol == 'function' &&
        typeof Symbol.toStringTag == 'symbol' &&
        Object.defineProperty(Q1.prototype, Symbol.toStringTag, {
            configurable: !0,
            value: 'AbortSignal'
        })
    var G1 = class {
            constructor() {
                Go.set(this, u9())
            }
            get signal() {
                return Qo(this)
            }
            abort() {
                c9(Qo(this))
            }
        },
        Go = new WeakMap()
    function Qo(e) {
        let a = Go.get(e)
        if (a == null)
            throw new TypeError(
                `Expected 'this' to be an 'AbortController' object, but got ${
                    e === null ? 'null' : typeof e
                }`
            )
        return a
    }
    Object.defineProperties(G1.prototype, {
        signal: { enumerable: !0 },
        abort: { enumerable: !0 }
    })
    typeof Symbol == 'function' &&
        typeof Symbol.toStringTag == 'symbol' &&
        Object.defineProperty(G1.prototype, Symbol.toStringTag, {
            configurable: !0,
            value: 'AbortController'
        })
    ep.AbortController = G1
    ep.AbortSignal = Q1
    ep.default = G1
    Zt.exports = G1
    Zt.exports.AbortController = Zt.exports.default = G1
    Zt.exports.AbortSignal = Q1
})
var e6 = se((xr, Zo) => {
    ;(function (e, a) {
        typeof xr == 'object' && typeof Zo < 'u'
            ? a(xr)
            : typeof define == 'function' && define.amd
            ? define(['exports'], a)
            : ((e = typeof globalThis < 'u' ? globalThis : e || self),
              a((e.WebStreamsPolyfill = {})))
    })(xr, function (e) {
        'use strict'
        function a() {}
        function t(p) {
            return (
                (typeof p == 'object' && p !== null) || typeof p == 'function'
            )
        }
        let r = a
        function i(p, d) {
            try {
                Object.defineProperty(p, 'name', { value: d, configurable: !0 })
            } catch {}
        }
        let o = Promise,
            m = Promise.prototype.then,
            u = Promise.reject.bind(o)
        function s(p) {
            return new o(p)
        }
        function f(p) {
            return s((d) => d(p))
        }
        function h(p) {
            return u(p)
        }
        function _(p, d, n) {
            return m.call(p, d, n)
        }
        function w(p, d, n) {
            _(_(p, d, n), void 0, r)
        }
        function g(p, d) {
            w(p, d)
        }
        function y(p, d) {
            w(p, void 0, d)
        }
        function C(p, d, n) {
            return _(p, d, n)
        }
        function R(p) {
            _(p, void 0, r)
        }
        let T = (p) => {
            if (typeof queueMicrotask == 'function') T = queueMicrotask
            else {
                let d = f(void 0)
                T = (n) => _(d, n)
            }
            return T(p)
        }
        function F(p, d, n) {
            if (typeof p != 'function')
                throw new TypeError('Argument is not a function')
            return Function.prototype.apply.call(p, d, n)
        }
        function K(p, d, n) {
            try {
                return f(F(p, d, n))
            } catch (l) {
                return h(l)
            }
        }
        let I = 16384
        class k {
            constructor() {
                ;(this._cursor = 0),
                    (this._size = 0),
                    (this._front = { _elements: [], _next: void 0 }),
                    (this._back = this._front),
                    (this._cursor = 0),
                    (this._size = 0)
            }
            get length() {
                return this._size
            }
            push(d) {
                let n = this._back,
                    l = n
                n._elements.length === I - 1 &&
                    (l = { _elements: [], _next: void 0 }),
                    n._elements.push(d),
                    l !== n && ((this._back = l), (n._next = l)),
                    ++this._size
            }
            shift() {
                let d = this._front,
                    n = d,
                    l = this._cursor,
                    c = l + 1,
                    v = d._elements,
                    b = v[l]
                return (
                    c === I && ((n = d._next), (c = 0)),
                    --this._size,
                    (this._cursor = c),
                    d !== n && (this._front = n),
                    (v[l] = void 0),
                    b
                )
            }
            forEach(d) {
                let n = this._cursor,
                    l = this._front,
                    c = l._elements
                for (
                    ;
                    (n !== c.length || l._next !== void 0) &&
                    !(
                        n === c.length &&
                        ((l = l._next),
                        (c = l._elements),
                        (n = 0),
                        c.length === 0)
                    );

                )
                    d(c[n]), ++n
            }
            peek() {
                let d = this._front,
                    n = this._cursor
                return d._elements[n]
            }
        }
        let Z = Symbol('[[AbortSteps]]'),
            ve = Symbol('[[ErrorSteps]]'),
            N = Symbol('[[CancelSteps]]'),
            D = Symbol('[[PullSteps]]'),
            x = Symbol('[[ReleaseSteps]]')
        function Q(p, d) {
            ;(p._ownerReadableStream = d),
                (d._reader = p),
                d._state === 'readable'
                    ? O(p)
                    : d._state === 'closed'
                    ? ge(p)
                    : $(p, d._storedError)
        }
        function ie(p, d) {
            let n = p._ownerReadableStream
            return je(n, d)
        }
        function q(p) {
            let d = p._ownerReadableStream
            d._state === 'readable'
                ? le(
                      p,
                      new TypeError(
                          "Reader was released and can no longer be used to monitor the stream's closedness"
                      )
                  )
                : E1(
                      p,
                      new TypeError(
                          "Reader was released and can no longer be used to monitor the stream's closedness"
                      )
                  ),
                d._readableStreamController[x](),
                (d._reader = void 0),
                (p._ownerReadableStream = void 0)
        }
        function M(p) {
            return new TypeError(
                'Cannot ' + p + ' a stream using a released reader'
            )
        }
        function O(p) {
            p._closedPromise = s((d, n) => {
                ;(p._closedPromise_resolve = d), (p._closedPromise_reject = n)
            })
        }
        function $(p, d) {
            O(p), le(p, d)
        }
        function ge(p) {
            O(p), Q2(p)
        }
        function le(p, d) {
            p._closedPromise_reject !== void 0 &&
                (R(p._closedPromise),
                p._closedPromise_reject(d),
                (p._closedPromise_resolve = void 0),
                (p._closedPromise_reject = void 0))
        }
        function E1(p, d) {
            $(p, d)
        }
        function Q2(p) {
            p._closedPromise_resolve !== void 0 &&
                (p._closedPromise_resolve(void 0),
                (p._closedPromise_resolve = void 0),
                (p._closedPromise_reject = void 0))
        }
        let G2 =
                Number.isFinite ||
                function (p) {
                    return typeof p == 'number' && isFinite(p)
                },
            N6 =
                Math.trunc ||
                function (p) {
                    return p < 0 ? Math.ceil(p) : Math.floor(p)
                }
        function D6(p) {
            return typeof p == 'object' || typeof p == 'function'
        }
        function Ye(p, d) {
            if (p !== void 0 && !D6(p))
                throw new TypeError(`${d} is not an object.`)
        }
        function qe(p, d) {
            if (typeof p != 'function')
                throw new TypeError(`${d} is not a function.`)
        }
        function A6(p) {
            return (
                (typeof p == 'object' && p !== null) || typeof p == 'function'
            )
        }
        function Y2(p, d) {
            if (!A6(p)) throw new TypeError(`${d} is not an object.`)
        }
        function d1(p, d, n) {
            if (p === void 0)
                throw new TypeError(`Parameter ${d} is required in '${n}'.`)
        }
        function Mr(p, d, n) {
            if (p === void 0) throw new TypeError(`${d} is required in '${n}'.`)
        }
        function $r(p) {
            return Number(p)
        }
        function K2(p) {
            return p === 0 ? 0 : p
        }
        function V6(p) {
            return K2(N6(p))
        }
        function Ur(p, d) {
            let l = Number.MAX_SAFE_INTEGER,
                c = Number(p)
            if (((c = K2(c)), !G2(c)))
                throw new TypeError(`${d} is not a finite number`)
            if (((c = V6(c)), c < 0 || c > l))
                throw new TypeError(
                    `${d} is outside the accepted range of 0 to ${l}, inclusive`
                )
            return !G2(c) || c === 0 ? 0 : c
        }
        function Hr(p, d) {
            if (!B1(p)) throw new TypeError(`${d} is not a ReadableStream.`)
        }
        function Da(p) {
            return new P1(p)
        }
        function X2(p, d) {
            p._reader._readRequests.push(d)
        }
        function Qr(p, d, n) {
            let c = p._reader._readRequests.shift()
            n ? c._closeSteps() : c._chunkSteps(d)
        }
        function op(p) {
            return p._reader._readRequests.length
        }
        function J2(p) {
            let d = p._reader
            return !(d === void 0 || !N1(d))
        }
        class P1 {
            constructor(d) {
                if (
                    (d1(d, 1, 'ReadableStreamDefaultReader'),
                    Hr(d, 'First parameter'),
                    k1(d))
                )
                    throw new TypeError(
                        'This stream has already been locked for exclusive reading by another reader'
                    )
                Q(this, d), (this._readRequests = new k())
            }
            get closed() {
                return N1(this) ? this._closedPromise : h(lp('closed'))
            }
            cancel(d = void 0) {
                return N1(this)
                    ? this._ownerReadableStream === void 0
                        ? h(M('cancel'))
                        : ie(this, d)
                    : h(lp('cancel'))
            }
            read() {
                if (!N1(this)) return h(lp('read'))
                if (this._ownerReadableStream === void 0)
                    return h(M('read from'))
                let d,
                    n,
                    l = s((v, b) => {
                        ;(d = v), (n = b)
                    })
                return (
                    vt(this, {
                        _chunkSteps: (v) => d({ value: v, done: !1 }),
                        _closeSteps: () => d({ value: void 0, done: !0 }),
                        _errorSteps: (v) => n(v)
                    }),
                    l
                )
            }
            releaseLock() {
                if (!N1(this)) throw lp('releaseLock')
                this._ownerReadableStream !== void 0 && q6(this)
            }
        }
        Object.defineProperties(P1.prototype, {
            cancel: { enumerable: !0 },
            read: { enumerable: !0 },
            releaseLock: { enumerable: !0 },
            closed: { enumerable: !0 }
        }),
            i(P1.prototype.cancel, 'cancel'),
            i(P1.prototype.read, 'read'),
            i(P1.prototype.releaseLock, 'releaseLock'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(P1.prototype, Symbol.toStringTag, {
                    value: 'ReadableStreamDefaultReader',
                    configurable: !0
                })
        function N1(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(p, '_readRequests')
                ? !1
                : p instanceof P1
        }
        function vt(p, d) {
            let n = p._ownerReadableStream
            ;(n._disturbed = !0),
                n._state === 'closed'
                    ? d._closeSteps()
                    : n._state === 'errored'
                    ? d._errorSteps(n._storedError)
                    : n._readableStreamController[D](d)
        }
        function q6(p) {
            q(p)
            let d = new TypeError('Reader was released')
            Z2(p, d)
        }
        function Z2(p, d) {
            let n = p._readRequests
            ;(p._readRequests = new k()),
                n.forEach((l) => {
                    l._errorSteps(d)
                })
        }
        function lp(p) {
            return new TypeError(
                `ReadableStreamDefaultReader.prototype.${p} can only be used on a ReadableStreamDefaultReader`
            )
        }
        let B6 = Object.getPrototypeOf(
            Object.getPrototypeOf(async function* () {}).prototype
        )
        class ei {
            constructor(d, n) {
                ;(this._ongoingPromise = void 0),
                    (this._isFinished = !1),
                    (this._reader = d),
                    (this._preventCancel = n)
            }
            next() {
                let d = () => this._nextSteps()
                return (
                    (this._ongoingPromise = this._ongoingPromise
                        ? C(this._ongoingPromise, d, d)
                        : d()),
                    this._ongoingPromise
                )
            }
            return(d) {
                let n = () => this._returnSteps(d)
                return this._ongoingPromise
                    ? C(this._ongoingPromise, n, n)
                    : n()
            }
            _nextSteps() {
                if (this._isFinished)
                    return Promise.resolve({ value: void 0, done: !0 })
                let d = this._reader,
                    n,
                    l,
                    c = s((b, S) => {
                        ;(n = b), (l = S)
                    })
                return (
                    vt(d, {
                        _chunkSteps: (b) => {
                            ;(this._ongoingPromise = void 0),
                                T(() => n({ value: b, done: !1 }))
                        },
                        _closeSteps: () => {
                            ;(this._ongoingPromise = void 0),
                                (this._isFinished = !0),
                                q(d),
                                n({ value: void 0, done: !0 })
                        },
                        _errorSteps: (b) => {
                            ;(this._ongoingPromise = void 0),
                                (this._isFinished = !0),
                                q(d),
                                l(b)
                        }
                    }),
                    c
                )
            }
            _returnSteps(d) {
                if (this._isFinished)
                    return Promise.resolve({ value: d, done: !0 })
                this._isFinished = !0
                let n = this._reader
                if (!this._preventCancel) {
                    let l = ie(n, d)
                    return q(n), C(l, () => ({ value: d, done: !0 }))
                }
                return q(n), f({ value: d, done: !0 })
            }
        }
        let ai = {
            next() {
                return ti(this) ? this._asyncIteratorImpl.next() : h(pi('next'))
            },
            return(p) {
                return ti(this)
                    ? this._asyncIteratorImpl.return(p)
                    : h(pi('return'))
            }
        }
        Object.setPrototypeOf(ai, B6)
        function k6(p, d) {
            let n = Da(p),
                l = new ei(n, d),
                c = Object.create(ai)
            return (c._asyncIteratorImpl = l), c
        }
        function ti(p) {
            if (
                !t(p) ||
                !Object.prototype.hasOwnProperty.call(p, '_asyncIteratorImpl')
            )
                return !1
            try {
                return p._asyncIteratorImpl instanceof ei
            } catch {
                return !1
            }
        }
        function pi(p) {
            return new TypeError(
                `ReadableStreamAsyncIterator.${p} can only be used on a ReadableSteamAsyncIterator`
            )
        }
        let ri =
            Number.isNaN ||
            function (p) {
                return p !== p
            }
        var Gr, Yr, Kr
        function bt(p) {
            return p.slice()
        }
        function di(p, d, n, l, c) {
            new Uint8Array(p).set(new Uint8Array(n, l, c), d)
        }
        let i1 = (p) => (
                typeof p.transfer == 'function'
                    ? (i1 = (d) => d.transfer())
                    : typeof structuredClone == 'function'
                    ? (i1 = (d) => structuredClone(d, { transfer: [d] }))
                    : (i1 = (d) => d),
                i1(p)
            ),
            D1 = (p) => (
                typeof p.detached == 'boolean'
                    ? (D1 = (d) => d.detached)
                    : (D1 = (d) => d.byteLength === 0),
                D1(p)
            )
        function ii(p, d, n) {
            if (p.slice) return p.slice(d, n)
            let l = n - d,
                c = new ArrayBuffer(l)
            return di(c, 0, p, d, l), c
        }
        function sp(p, d) {
            let n = p[d]
            if (n != null) {
                if (typeof n != 'function')
                    throw new TypeError(`${String(d)} is not a function`)
                return n
            }
        }
        function O6(p) {
            let d = { [Symbol.iterator]: () => p.iterator },
                n = (async function* () {
                    return yield* d
                })(),
                l = n.next
            return { iterator: n, nextMethod: l, done: !1 }
        }
        let Xr =
            (Kr =
                (Gr = Symbol.asyncIterator) !== null && Gr !== void 0
                    ? Gr
                    : (Yr = Symbol.for) === null || Yr === void 0
                    ? void 0
                    : Yr.call(Symbol, 'Symbol.asyncIterator')) !== null &&
            Kr !== void 0
                ? Kr
                : '@@asyncIterator'
        function ni(p, d = 'sync', n) {
            if (n === void 0)
                if (d === 'async') {
                    if (((n = sp(p, Xr)), n === void 0)) {
                        let v = sp(p, Symbol.iterator),
                            b = ni(p, 'sync', v)
                        return O6(b)
                    }
                } else n = sp(p, Symbol.iterator)
            if (n === void 0) throw new TypeError('The object is not iterable')
            let l = F(n, p, [])
            if (!t(l))
                throw new TypeError('The iterator method must return an object')
            let c = l.next
            return { iterator: l, nextMethod: c, done: !1 }
        }
        function F6(p) {
            let d = F(p.nextMethod, p.iterator, [])
            if (!t(d))
                throw new TypeError(
                    'The iterator.next() method must return an object'
                )
            return d
        }
        function x6(p) {
            return !!p.done
        }
        function L6(p) {
            return p.value
        }
        function I6(p) {
            return !(typeof p != 'number' || ri(p) || p < 0)
        }
        function oi(p) {
            let d = ii(p.buffer, p.byteOffset, p.byteOffset + p.byteLength)
            return new Uint8Array(d)
        }
        function Jr(p) {
            let d = p._queue.shift()
            return (
                (p._queueTotalSize -= d.size),
                p._queueTotalSize < 0 && (p._queueTotalSize = 0),
                d.value
            )
        }
        function Zr(p, d, n) {
            if (!I6(n) || n === 1 / 0)
                throw new RangeError(
                    'Size must be a finite, non-NaN, non-negative number.'
                )
            p._queue.push({ value: d, size: n }), (p._queueTotalSize += n)
        }
        function j6(p) {
            return p._queue.peek().value
        }
        function A1(p) {
            ;(p._queue = new k()), (p._queueTotalSize = 0)
        }
        function li(p) {
            return p === DataView
        }
        function W6(p) {
            return li(p.constructor)
        }
        function z6(p) {
            return li(p) ? 1 : p.BYTES_PER_ELEMENT
        }
        class aa {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get view() {
                if (!ed(this)) throw dd('view')
                return this._view
            }
            respond(d) {
                if (!ed(this)) throw dd('respond')
                if (
                    (d1(d, 1, 'respond'),
                    (d = Ur(d, 'First parameter')),
                    this._associatedReadableByteStreamController === void 0)
                )
                    throw new TypeError(
                        'This BYOB request has been invalidated'
                    )
                if (D1(this._view.buffer))
                    throw new TypeError(
                        "The BYOB request's buffer has been detached and so cannot be used as a response"
                    )
                fp(this._associatedReadableByteStreamController, d)
            }
            respondWithNewView(d) {
                if (!ed(this)) throw dd('respondWithNewView')
                if ((d1(d, 1, 'respondWithNewView'), !ArrayBuffer.isView(d)))
                    throw new TypeError(
                        'You can only respond with array buffer views'
                    )
                if (this._associatedReadableByteStreamController === void 0)
                    throw new TypeError(
                        'This BYOB request has been invalidated'
                    )
                if (D1(d.buffer))
                    throw new TypeError(
                        "The given view's buffer has been detached and so cannot be used as a response"
                    )
                hp(this._associatedReadableByteStreamController, d)
            }
        }
        Object.defineProperties(aa.prototype, {
            respond: { enumerable: !0 },
            respondWithNewView: { enumerable: !0 },
            view: { enumerable: !0 }
        }),
            i(aa.prototype.respond, 'respond'),
            i(aa.prototype.respondWithNewView, 'respondWithNewView'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(aa.prototype, Symbol.toStringTag, {
                    value: 'ReadableStreamBYOBRequest',
                    configurable: !0
                })
        class n1 {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get byobRequest() {
                if (!ta(this)) throw _t('byobRequest')
                return rd(this)
            }
            get desiredSize() {
                if (!ta(this)) throw _t('desiredSize')
                return _i(this)
            }
            close() {
                if (!ta(this)) throw _t('close')
                if (this._closeRequested)
                    throw new TypeError(
                        'The stream has already been closed; do not close it again!'
                    )
                let d = this._controlledReadableByteStream._state
                if (d !== 'readable')
                    throw new TypeError(
                        `The stream (in ${d} state) is not in the readable state and cannot be closed`
                    )
                wt(this)
            }
            enqueue(d) {
                if (!ta(this)) throw _t('enqueue')
                if ((d1(d, 1, 'enqueue'), !ArrayBuffer.isView(d)))
                    throw new TypeError('chunk must be an array buffer view')
                if (d.byteLength === 0)
                    throw new TypeError('chunk must have non-zero byteLength')
                if (d.buffer.byteLength === 0)
                    throw new TypeError(
                        "chunk's buffer must have non-zero byteLength"
                    )
                if (this._closeRequested)
                    throw new TypeError('stream is closed or draining')
                let n = this._controlledReadableByteStream._state
                if (n !== 'readable')
                    throw new TypeError(
                        `The stream (in ${n} state) is not in the readable state and cannot be enqueued to`
                    )
                cp(this, d)
            }
            error(d = void 0) {
                if (!ta(this)) throw _t('error')
                Be(this, d)
            }
            [N](d) {
                si(this), A1(this)
                let n = this._cancelAlgorithm(d)
                return up(this), n
            }
            [D](d) {
                let n = this._controlledReadableByteStream
                if (this._queueTotalSize > 0) {
                    wi(this, d)
                    return
                }
                let l = this._autoAllocateChunkSize
                if (l !== void 0) {
                    let c
                    try {
                        c = new ArrayBuffer(l)
                    } catch (b) {
                        d._errorSteps(b)
                        return
                    }
                    let v = {
                        buffer: c,
                        bufferByteLength: l,
                        byteOffset: 0,
                        byteLength: l,
                        bytesFilled: 0,
                        minimumFill: 1,
                        elementSize: 1,
                        viewConstructor: Uint8Array,
                        readerType: 'default'
                    }
                    this._pendingPullIntos.push(v)
                }
                X2(n, d), pa(this)
            }
            [x]() {
                if (this._pendingPullIntos.length > 0) {
                    let d = this._pendingPullIntos.peek()
                    ;(d.readerType = 'none'),
                        (this._pendingPullIntos = new k()),
                        this._pendingPullIntos.push(d)
                }
            }
        }
        Object.defineProperties(n1.prototype, {
            close: { enumerable: !0 },
            enqueue: { enumerable: !0 },
            error: { enumerable: !0 },
            byobRequest: { enumerable: !0 },
            desiredSize: { enumerable: !0 }
        }),
            i(n1.prototype.close, 'close'),
            i(n1.prototype.enqueue, 'enqueue'),
            i(n1.prototype.error, 'error'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(n1.prototype, Symbol.toStringTag, {
                    value: 'ReadableByteStreamController',
                    configurable: !0
                })
        function ta(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_controlledReadableByteStream'
                )
                ? !1
                : p instanceof n1
        }
        function ed(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_associatedReadableByteStreamController'
                )
                ? !1
                : p instanceof aa
        }
        function pa(p) {
            if (!Q6(p)) return
            if (p._pulling) {
                p._pullAgain = !0
                return
            }
            p._pulling = !0
            let n = p._pullAlgorithm()
            w(
                n,
                () => (
                    (p._pulling = !1),
                    p._pullAgain && ((p._pullAgain = !1), pa(p)),
                    null
                ),
                (l) => (Be(p, l), null)
            )
        }
        function si(p) {
            td(p), (p._pendingPullIntos = new k())
        }
        function ad(p, d) {
            let n = !1
            p._state === 'closed' && (n = !0)
            let l = mi(d)
            d.readerType === 'default' ? Qr(p, l, n) : Z6(p, l, n)
        }
        function mi(p) {
            let d = p.bytesFilled,
                n = p.elementSize
            return new p.viewConstructor(p.buffer, p.byteOffset, d / n)
        }
        function mp(p, d, n, l) {
            p._queue.push({ buffer: d, byteOffset: n, byteLength: l }),
                (p._queueTotalSize += l)
        }
        function ui(p, d, n, l) {
            let c
            try {
                c = ii(d, n, n + l)
            } catch (v) {
                throw (Be(p, v), v)
            }
            mp(p, c, 0, l)
        }
        function ci(p, d) {
            d.bytesFilled > 0 && ui(p, d.buffer, d.byteOffset, d.bytesFilled),
                Aa(p)
        }
        function fi(p, d) {
            let n = Math.min(p._queueTotalSize, d.byteLength - d.bytesFilled),
                l = d.bytesFilled + n,
                c = n,
                v = !1,
                b = l % d.elementSize,
                S = l - b
            S >= d.minimumFill && ((c = S - d.bytesFilled), (v = !0))
            let V = p._queue
            for (; c > 0; ) {
                let E = V.peek(),
                    B = Math.min(c, E.byteLength),
                    L = d.byteOffset + d.bytesFilled
                di(d.buffer, L, E.buffer, E.byteOffset, B),
                    E.byteLength === B
                        ? V.shift()
                        : ((E.byteOffset += B), (E.byteLength -= B)),
                    (p._queueTotalSize -= B),
                    hi(p, B, d),
                    (c -= B)
            }
            return v
        }
        function hi(p, d, n) {
            n.bytesFilled += d
        }
        function vi(p) {
            p._queueTotalSize === 0 && p._closeRequested
                ? (up(p), Ct(p._controlledReadableByteStream))
                : pa(p)
        }
        function td(p) {
            p._byobRequest !== null &&
                ((p._byobRequest._associatedReadableByteStreamController =
                    void 0),
                (p._byobRequest._view = null),
                (p._byobRequest = null))
        }
        function pd(p) {
            for (; p._pendingPullIntos.length > 0; ) {
                if (p._queueTotalSize === 0) return
                let d = p._pendingPullIntos.peek()
                fi(p, d) && (Aa(p), ad(p._controlledReadableByteStream, d))
            }
        }
        function M6(p) {
            let d = p._controlledReadableByteStream._reader
            for (; d._readRequests.length > 0; ) {
                if (p._queueTotalSize === 0) return
                let n = d._readRequests.shift()
                wi(p, n)
            }
        }
        function $6(p, d, n, l) {
            let c = p._controlledReadableByteStream,
                v = d.constructor,
                b = z6(v),
                { byteOffset: S, byteLength: V } = d,
                E = n * b,
                B
            try {
                B = i1(d.buffer)
            } catch (G) {
                l._errorSteps(G)
                return
            }
            let L = {
                buffer: B,
                bufferByteLength: B.byteLength,
                byteOffset: S,
                byteLength: V,
                bytesFilled: 0,
                minimumFill: E,
                elementSize: b,
                viewConstructor: v,
                readerType: 'byob'
            }
            if (p._pendingPullIntos.length > 0) {
                p._pendingPullIntos.push(L), Si(c, l)
                return
            }
            if (c._state === 'closed') {
                let G = new v(L.buffer, L.byteOffset, 0)
                l._closeSteps(G)
                return
            }
            if (p._queueTotalSize > 0) {
                if (fi(p, L)) {
                    let G = mi(L)
                    vi(p), l._chunkSteps(G)
                    return
                }
                if (p._closeRequested) {
                    let G = new TypeError(
                        'Insufficient bytes to fill elements in the given buffer'
                    )
                    Be(p, G), l._errorSteps(G)
                    return
                }
            }
            p._pendingPullIntos.push(L), Si(c, l), pa(p)
        }
        function U6(p, d) {
            d.readerType === 'none' && Aa(p)
            let n = p._controlledReadableByteStream
            if (id(n))
                for (; Ti(n) > 0; ) {
                    let l = Aa(p)
                    ad(n, l)
                }
        }
        function H6(p, d, n) {
            if ((hi(p, d, n), n.readerType === 'none')) {
                ci(p, n), pd(p)
                return
            }
            if (n.bytesFilled < n.minimumFill) return
            Aa(p)
            let l = n.bytesFilled % n.elementSize
            if (l > 0) {
                let c = n.byteOffset + n.bytesFilled
                ui(p, n.buffer, c - l, l)
            }
            ;(n.bytesFilled -= l), ad(p._controlledReadableByteStream, n), pd(p)
        }
        function bi(p, d) {
            let n = p._pendingPullIntos.peek()
            td(p),
                p._controlledReadableByteStream._state === 'closed'
                    ? U6(p, n)
                    : H6(p, d, n),
                pa(p)
        }
        function Aa(p) {
            return p._pendingPullIntos.shift()
        }
        function Q6(p) {
            let d = p._controlledReadableByteStream
            return d._state !== 'readable' || p._closeRequested || !p._started
                ? !1
                : !!((J2(d) && op(d) > 0) || (id(d) && Ti(d) > 0) || _i(p) > 0)
        }
        function up(p) {
            ;(p._pullAlgorithm = void 0), (p._cancelAlgorithm = void 0)
        }
        function wt(p) {
            let d = p._controlledReadableByteStream
            if (!(p._closeRequested || d._state !== 'readable')) {
                if (p._queueTotalSize > 0) {
                    p._closeRequested = !0
                    return
                }
                if (p._pendingPullIntos.length > 0) {
                    let n = p._pendingPullIntos.peek()
                    if (n.bytesFilled % n.elementSize !== 0) {
                        let l = new TypeError(
                            'Insufficient bytes to fill elements in the given buffer'
                        )
                        throw (Be(p, l), l)
                    }
                }
                up(p), Ct(d)
            }
        }
        function cp(p, d) {
            let n = p._controlledReadableByteStream
            if (p._closeRequested || n._state !== 'readable') return
            let { buffer: l, byteOffset: c, byteLength: v } = d
            if (D1(l))
                throw new TypeError(
                    "chunk's buffer is detached and so cannot be enqueued"
                )
            let b = i1(l)
            if (p._pendingPullIntos.length > 0) {
                let S = p._pendingPullIntos.peek()
                if (D1(S.buffer))
                    throw new TypeError(
                        "The BYOB request's buffer has been detached and so cannot be filled with an enqueued chunk"
                    )
                td(p),
                    (S.buffer = i1(S.buffer)),
                    S.readerType === 'none' && ci(p, S)
            }
            if (J2(n))
                if ((M6(p), op(n) === 0)) mp(p, b, c, v)
                else {
                    p._pendingPullIntos.length > 0 && Aa(p)
                    let S = new Uint8Array(b, c, v)
                    Qr(n, S, !1)
                }
            else id(n) ? (mp(p, b, c, v), pd(p)) : mp(p, b, c, v)
            pa(p)
        }
        function Be(p, d) {
            let n = p._controlledReadableByteStream
            n._state === 'readable' && (si(p), A1(p), up(p), Qi(n, d))
        }
        function wi(p, d) {
            let n = p._queue.shift()
            ;(p._queueTotalSize -= n.byteLength), vi(p)
            let l = new Uint8Array(n.buffer, n.byteOffset, n.byteLength)
            d._chunkSteps(l)
        }
        function rd(p) {
            if (p._byobRequest === null && p._pendingPullIntos.length > 0) {
                let d = p._pendingPullIntos.peek(),
                    n = new Uint8Array(
                        d.buffer,
                        d.byteOffset + d.bytesFilled,
                        d.byteLength - d.bytesFilled
                    ),
                    l = Object.create(aa.prototype)
                Y6(l, p, n), (p._byobRequest = l)
            }
            return p._byobRequest
        }
        function _i(p) {
            let d = p._controlledReadableByteStream._state
            return d === 'errored'
                ? null
                : d === 'closed'
                ? 0
                : p._strategyHWM - p._queueTotalSize
        }
        function fp(p, d) {
            let n = p._pendingPullIntos.peek()
            if (p._controlledReadableByteStream._state === 'closed') {
                if (d !== 0)
                    throw new TypeError(
                        'bytesWritten must be 0 when calling respond() on a closed stream'
                    )
            } else {
                if (d === 0)
                    throw new TypeError(
                        'bytesWritten must be greater than 0 when calling respond() on a readable stream'
                    )
                if (n.bytesFilled + d > n.byteLength)
                    throw new RangeError('bytesWritten out of range')
            }
            ;(n.buffer = i1(n.buffer)), bi(p, d)
        }
        function hp(p, d) {
            let n = p._pendingPullIntos.peek()
            if (p._controlledReadableByteStream._state === 'closed') {
                if (d.byteLength !== 0)
                    throw new TypeError(
                        "The view's length must be 0 when calling respondWithNewView() on a closed stream"
                    )
            } else if (d.byteLength === 0) throw new TypeError("The view's length must be greater than 0 when calling respondWithNewView() on a readable stream")
            if (n.byteOffset + n.bytesFilled !== d.byteOffset)
                throw new RangeError(
                    'The region specified by view does not match byobRequest'
                )
            if (n.bufferByteLength !== d.buffer.byteLength)
                throw new RangeError(
                    'The buffer of view has different capacity than byobRequest'
                )
            if (n.bytesFilled + d.byteLength > n.byteLength)
                throw new RangeError(
                    'The region specified by view is larger than byobRequest'
                )
            let c = d.byteLength
            ;(n.buffer = i1(d.buffer)), bi(p, c)
        }
        function yi(p, d, n, l, c, v, b) {
            ;(d._controlledReadableByteStream = p),
                (d._pullAgain = !1),
                (d._pulling = !1),
                (d._byobRequest = null),
                (d._queue = d._queueTotalSize = void 0),
                A1(d),
                (d._closeRequested = !1),
                (d._started = !1),
                (d._strategyHWM = v),
                (d._pullAlgorithm = l),
                (d._cancelAlgorithm = c),
                (d._autoAllocateChunkSize = b),
                (d._pendingPullIntos = new k()),
                (p._readableStreamController = d)
            let S = n()
            w(
                f(S),
                () => ((d._started = !0), pa(d), null),
                (V) => (Be(d, V), null)
            )
        }
        function G6(p, d, n) {
            let l = Object.create(n1.prototype),
                c,
                v,
                b
            d.start !== void 0 ? (c = () => d.start(l)) : (c = () => {}),
                d.pull !== void 0
                    ? (v = () => d.pull(l))
                    : (v = () => f(void 0)),
                d.cancel !== void 0
                    ? (b = (V) => d.cancel(V))
                    : (b = () => f(void 0))
            let S = d.autoAllocateChunkSize
            if (S === 0)
                throw new TypeError(
                    'autoAllocateChunkSize must be greater than 0'
                )
            yi(p, l, c, v, b, n, S)
        }
        function Y6(p, d, n) {
            ;(p._associatedReadableByteStreamController = d), (p._view = n)
        }
        function dd(p) {
            return new TypeError(
                `ReadableStreamBYOBRequest.prototype.${p} can only be used on a ReadableStreamBYOBRequest`
            )
        }
        function _t(p) {
            return new TypeError(
                `ReadableByteStreamController.prototype.${p} can only be used on a ReadableByteStreamController`
            )
        }
        function K6(p, d) {
            Ye(p, d)
            let n = p?.mode
            return {
                mode:
                    n === void 0 ? void 0 : X6(n, `${d} has member 'mode' that`)
            }
        }
        function X6(p, d) {
            if (((p = `${p}`), p !== 'byob'))
                throw new TypeError(
                    `${d} '${p}' is not a valid enumeration value for ReadableStreamReaderMode`
                )
            return p
        }
        function J6(p, d) {
            var n
            Ye(p, d)
            let l = (n = p?.min) !== null && n !== void 0 ? n : 1
            return { min: Ur(l, `${d} has member 'min' that`) }
        }
        function gi(p) {
            return new V1(p)
        }
        function Si(p, d) {
            p._reader._readIntoRequests.push(d)
        }
        function Z6(p, d, n) {
            let c = p._reader._readIntoRequests.shift()
            n ? c._closeSteps(d) : c._chunkSteps(d)
        }
        function Ti(p) {
            return p._reader._readIntoRequests.length
        }
        function id(p) {
            let d = p._reader
            return !(d === void 0 || !ra(d))
        }
        class V1 {
            constructor(d) {
                if (
                    (d1(d, 1, 'ReadableStreamBYOBReader'),
                    Hr(d, 'First parameter'),
                    k1(d))
                )
                    throw new TypeError(
                        'This stream has already been locked for exclusive reading by another reader'
                    )
                if (!ta(d._readableStreamController))
                    throw new TypeError(
                        'Cannot construct a ReadableStreamBYOBReader for a stream not constructed with a byte source'
                    )
                Q(this, d), (this._readIntoRequests = new k())
            }
            get closed() {
                return ra(this) ? this._closedPromise : h(vp('closed'))
            }
            cancel(d = void 0) {
                return ra(this)
                    ? this._ownerReadableStream === void 0
                        ? h(M('cancel'))
                        : ie(this, d)
                    : h(vp('cancel'))
            }
            read(d, n = {}) {
                if (!ra(this)) return h(vp('read'))
                if (!ArrayBuffer.isView(d))
                    return h(new TypeError('view must be an array buffer view'))
                if (d.byteLength === 0)
                    return h(
                        new TypeError('view must have non-zero byteLength')
                    )
                if (d.buffer.byteLength === 0)
                    return h(
                        new TypeError(
                            "view's buffer must have non-zero byteLength"
                        )
                    )
                if (D1(d.buffer))
                    return h(new TypeError("view's buffer has been detached"))
                let l
                try {
                    l = J6(n, 'options')
                } catch (E) {
                    return h(E)
                }
                let c = l.min
                if (c === 0)
                    return h(
                        new TypeError('options.min must be greater than 0')
                    )
                if (W6(d)) {
                    if (c > d.byteLength)
                        return h(
                            new RangeError(
                                "options.min must be less than or equal to view's byteLength"
                            )
                        )
                } else if (c > d.length) return h(new RangeError("options.min must be less than or equal to view's length"))
                if (this._ownerReadableStream === void 0)
                    return h(M('read from'))
                let v,
                    b,
                    S = s((E, B) => {
                        ;(v = E), (b = B)
                    })
                return (
                    Ri(this, d, c, {
                        _chunkSteps: (E) => v({ value: E, done: !1 }),
                        _closeSteps: (E) => v({ value: E, done: !0 }),
                        _errorSteps: (E) => b(E)
                    }),
                    S
                )
            }
            releaseLock() {
                if (!ra(this)) throw vp('releaseLock')
                this._ownerReadableStream !== void 0 && e4(this)
            }
        }
        Object.defineProperties(V1.prototype, {
            cancel: { enumerable: !0 },
            read: { enumerable: !0 },
            releaseLock: { enumerable: !0 },
            closed: { enumerable: !0 }
        }),
            i(V1.prototype.cancel, 'cancel'),
            i(V1.prototype.read, 'read'),
            i(V1.prototype.releaseLock, 'releaseLock'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(V1.prototype, Symbol.toStringTag, {
                    value: 'ReadableStreamBYOBReader',
                    configurable: !0
                })
        function ra(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(p, '_readIntoRequests')
                ? !1
                : p instanceof V1
        }
        function Ri(p, d, n, l) {
            let c = p._ownerReadableStream
            ;(c._disturbed = !0),
                c._state === 'errored'
                    ? l._errorSteps(c._storedError)
                    : $6(c._readableStreamController, d, n, l)
        }
        function e4(p) {
            q(p)
            let d = new TypeError('Reader was released')
            Ci(p, d)
        }
        function Ci(p, d) {
            let n = p._readIntoRequests
            ;(p._readIntoRequests = new k()),
                n.forEach((l) => {
                    l._errorSteps(d)
                })
        }
        function vp(p) {
            return new TypeError(
                `ReadableStreamBYOBReader.prototype.${p} can only be used on a ReadableStreamBYOBReader`
            )
        }
        function yt(p, d) {
            let { highWaterMark: n } = p
            if (n === void 0) return d
            if (ri(n) || n < 0) throw new RangeError('Invalid highWaterMark')
            return n
        }
        function bp(p) {
            let { size: d } = p
            return d || (() => 1)
        }
        function wp(p, d) {
            Ye(p, d)
            let n = p?.highWaterMark,
                l = p?.size
            return {
                highWaterMark: n === void 0 ? void 0 : $r(n),
                size:
                    l === void 0 ? void 0 : a4(l, `${d} has member 'size' that`)
            }
        }
        function a4(p, d) {
            return qe(p, d), (n) => $r(p(n))
        }
        function t4(p, d) {
            Ye(p, d)
            let n = p?.abort,
                l = p?.close,
                c = p?.start,
                v = p?.type,
                b = p?.write
            return {
                abort:
                    n === void 0
                        ? void 0
                        : p4(n, p, `${d} has member 'abort' that`),
                close:
                    l === void 0
                        ? void 0
                        : r4(l, p, `${d} has member 'close' that`),
                start:
                    c === void 0
                        ? void 0
                        : d4(c, p, `${d} has member 'start' that`),
                write:
                    b === void 0
                        ? void 0
                        : i4(b, p, `${d} has member 'write' that`),
                type: v
            }
        }
        function p4(p, d, n) {
            return qe(p, n), (l) => K(p, d, [l])
        }
        function r4(p, d, n) {
            return qe(p, n), () => K(p, d, [])
        }
        function d4(p, d, n) {
            return qe(p, n), (l) => F(p, d, [l])
        }
        function i4(p, d, n) {
            return qe(p, n), (l, c) => K(p, d, [l, c])
        }
        function Ei(p, d) {
            if (!Va(p)) throw new TypeError(`${d} is not a WritableStream.`)
        }
        function n4(p) {
            if (typeof p != 'object' || p === null) return !1
            try {
                return typeof p.aborted == 'boolean'
            } catch {
                return !1
            }
        }
        let o4 = typeof AbortController == 'function'
        function l4() {
            if (o4) return new AbortController()
        }
        class q1 {
            constructor(d = {}, n = {}) {
                d === void 0 ? (d = null) : Y2(d, 'First parameter')
                let l = wp(n, 'Second parameter'),
                    c = t4(d, 'First parameter')
                if ((Ni(this), c.type !== void 0))
                    throw new RangeError('Invalid type is specified')
                let b = bp(l),
                    S = yt(l, 1)
                T4(this, c, S, b)
            }
            get locked() {
                if (!Va(this)) throw Tp('locked')
                return qa(this)
            }
            abort(d = void 0) {
                return Va(this)
                    ? qa(this)
                        ? h(
                              new TypeError(
                                  'Cannot abort a stream that already has a writer'
                              )
                          )
                        : _p(this, d)
                    : h(Tp('abort'))
            }
            close() {
                return Va(this)
                    ? qa(this)
                        ? h(
                              new TypeError(
                                  'Cannot close a stream that already has a writer'
                              )
                          )
                        : Ke(this)
                        ? h(
                              new TypeError(
                                  'Cannot close an already-closing stream'
                              )
                          )
                        : Di(this)
                    : h(Tp('close'))
            }
            getWriter() {
                if (!Va(this)) throw Tp('getWriter')
                return Pi(this)
            }
        }
        Object.defineProperties(q1.prototype, {
            abort: { enumerable: !0 },
            close: { enumerable: !0 },
            getWriter: { enumerable: !0 },
            locked: { enumerable: !0 }
        }),
            i(q1.prototype.abort, 'abort'),
            i(q1.prototype.close, 'close'),
            i(q1.prototype.getWriter, 'getWriter'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(q1.prototype, Symbol.toStringTag, {
                    value: 'WritableStream',
                    configurable: !0
                })
        function Pi(p) {
            return new o1(p)
        }
        function s4(p, d, n, l, c = 1, v = () => 1) {
            let b = Object.create(q1.prototype)
            Ni(b)
            let S = Object.create(Ba.prototype)
            return Oi(b, S, p, d, n, l, c, v), b
        }
        function Ni(p) {
            ;(p._state = 'writable'),
                (p._storedError = void 0),
                (p._writer = void 0),
                (p._writableStreamController = void 0),
                (p._writeRequests = new k()),
                (p._inFlightWriteRequest = void 0),
                (p._closeRequest = void 0),
                (p._inFlightCloseRequest = void 0),
                (p._pendingAbortRequest = void 0),
                (p._backpressure = !1)
        }
        function Va(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_writableStreamController'
                )
                ? !1
                : p instanceof q1
        }
        function qa(p) {
            return p._writer !== void 0
        }
        function _p(p, d) {
            var n
            if (p._state === 'closed' || p._state === 'errored')
                return f(void 0)
            ;(p._writableStreamController._abortReason = d),
                (n = p._writableStreamController._abortController) === null ||
                    n === void 0 ||
                    n.abort(d)
            let l = p._state
            if (l === 'closed' || l === 'errored') return f(void 0)
            if (p._pendingAbortRequest !== void 0)
                return p._pendingAbortRequest._promise
            let c = !1
            l === 'erroring' && ((c = !0), (d = void 0))
            let v = s((b, S) => {
                p._pendingAbortRequest = {
                    _promise: void 0,
                    _resolve: b,
                    _reject: S,
                    _reason: d,
                    _wasAlreadyErroring: c
                }
            })
            return (p._pendingAbortRequest._promise = v), c || od(p, d), v
        }
        function Di(p) {
            let d = p._state
            if (d === 'closed' || d === 'errored')
                return h(
                    new TypeError(
                        `The stream (in ${d} state) is not in the writable state and cannot be closed`
                    )
                )
            let n = s((c, v) => {
                    let b = { _resolve: c, _reject: v }
                    p._closeRequest = b
                }),
                l = p._writer
            return (
                l !== void 0 && p._backpressure && d === 'writable' && vd(l),
                R4(p._writableStreamController),
                n
            )
        }
        function m4(p) {
            return s((n, l) => {
                let c = { _resolve: n, _reject: l }
                p._writeRequests.push(c)
            })
        }
        function nd(p, d) {
            if (p._state === 'writable') {
                od(p, d)
                return
            }
            ld(p)
        }
        function od(p, d) {
            let n = p._writableStreamController
            ;(p._state = 'erroring'), (p._storedError = d)
            let l = p._writer
            l !== void 0 && Vi(l, d), !v4(p) && n._started && ld(p)
        }
        function ld(p) {
            ;(p._state = 'errored'), p._writableStreamController[ve]()
            let d = p._storedError
            if (
                (p._writeRequests.forEach((c) => {
                    c._reject(d)
                }),
                (p._writeRequests = new k()),
                p._pendingAbortRequest === void 0)
            ) {
                yp(p)
                return
            }
            let n = p._pendingAbortRequest
            if (((p._pendingAbortRequest = void 0), n._wasAlreadyErroring)) {
                n._reject(d), yp(p)
                return
            }
            let l = p._writableStreamController[Z](n._reason)
            w(
                l,
                () => (n._resolve(), yp(p), null),
                (c) => (n._reject(c), yp(p), null)
            )
        }
        function u4(p) {
            p._inFlightWriteRequest._resolve(void 0),
                (p._inFlightWriteRequest = void 0)
        }
        function c4(p, d) {
            p._inFlightWriteRequest._reject(d),
                (p._inFlightWriteRequest = void 0),
                nd(p, d)
        }
        function f4(p) {
            p._inFlightCloseRequest._resolve(void 0),
                (p._inFlightCloseRequest = void 0),
                p._state === 'erroring' &&
                    ((p._storedError = void 0),
                    p._pendingAbortRequest !== void 0 &&
                        (p._pendingAbortRequest._resolve(),
                        (p._pendingAbortRequest = void 0))),
                (p._state = 'closed')
            let n = p._writer
            n !== void 0 && Ii(n)
        }
        function h4(p, d) {
            p._inFlightCloseRequest._reject(d),
                (p._inFlightCloseRequest = void 0),
                p._pendingAbortRequest !== void 0 &&
                    (p._pendingAbortRequest._reject(d),
                    (p._pendingAbortRequest = void 0)),
                nd(p, d)
        }
        function Ke(p) {
            return !(
                p._closeRequest === void 0 && p._inFlightCloseRequest === void 0
            )
        }
        function v4(p) {
            return !(
                p._inFlightWriteRequest === void 0 &&
                p._inFlightCloseRequest === void 0
            )
        }
        function b4(p) {
            ;(p._inFlightCloseRequest = p._closeRequest),
                (p._closeRequest = void 0)
        }
        function w4(p) {
            p._inFlightWriteRequest = p._writeRequests.shift()
        }
        function yp(p) {
            p._closeRequest !== void 0 &&
                (p._closeRequest._reject(p._storedError),
                (p._closeRequest = void 0))
            let d = p._writer
            d !== void 0 && fd(d, p._storedError)
        }
        function sd(p, d) {
            let n = p._writer
            n !== void 0 && d !== p._backpressure && (d ? V4(n) : vd(n)),
                (p._backpressure = d)
        }
        class o1 {
            constructor(d) {
                if (
                    (d1(d, 1, 'WritableStreamDefaultWriter'),
                    Ei(d, 'First parameter'),
                    qa(d))
                )
                    throw new TypeError(
                        'This stream has already been locked for exclusive writing by another writer'
                    )
                ;(this._ownerWritableStream = d), (d._writer = this)
                let n = d._state
                if (n === 'writable')
                    !Ke(d) && d._backpressure ? Cp(this) : ji(this), Rp(this)
                else if (n === 'erroring') hd(this, d._storedError), Rp(this)
                else if (n === 'closed') ji(this), D4(this)
                else {
                    let l = d._storedError
                    hd(this, l), Li(this, l)
                }
            }
            get closed() {
                return da(this) ? this._closedPromise : h(ia('closed'))
            }
            get desiredSize() {
                if (!da(this)) throw ia('desiredSize')
                if (this._ownerWritableStream === void 0)
                    throw St('desiredSize')
                return S4(this)
            }
            get ready() {
                return da(this) ? this._readyPromise : h(ia('ready'))
            }
            abort(d = void 0) {
                return da(this)
                    ? this._ownerWritableStream === void 0
                        ? h(St('abort'))
                        : _4(this, d)
                    : h(ia('abort'))
            }
            close() {
                if (!da(this)) return h(ia('close'))
                let d = this._ownerWritableStream
                return d === void 0
                    ? h(St('close'))
                    : Ke(d)
                    ? h(new TypeError('Cannot close an already-closing stream'))
                    : Ai(this)
            }
            releaseLock() {
                if (!da(this)) throw ia('releaseLock')
                this._ownerWritableStream !== void 0 && qi(this)
            }
            write(d = void 0) {
                return da(this)
                    ? this._ownerWritableStream === void 0
                        ? h(St('write to'))
                        : Bi(this, d)
                    : h(ia('write'))
            }
        }
        Object.defineProperties(o1.prototype, {
            abort: { enumerable: !0 },
            close: { enumerable: !0 },
            releaseLock: { enumerable: !0 },
            write: { enumerable: !0 },
            closed: { enumerable: !0 },
            desiredSize: { enumerable: !0 },
            ready: { enumerable: !0 }
        }),
            i(o1.prototype.abort, 'abort'),
            i(o1.prototype.close, 'close'),
            i(o1.prototype.releaseLock, 'releaseLock'),
            i(o1.prototype.write, 'write'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(o1.prototype, Symbol.toStringTag, {
                    value: 'WritableStreamDefaultWriter',
                    configurable: !0
                })
        function da(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(p, '_ownerWritableStream')
                ? !1
                : p instanceof o1
        }
        function _4(p, d) {
            let n = p._ownerWritableStream
            return _p(n, d)
        }
        function Ai(p) {
            let d = p._ownerWritableStream
            return Di(d)
        }
        function y4(p) {
            let d = p._ownerWritableStream,
                n = d._state
            return Ke(d) || n === 'closed'
                ? f(void 0)
                : n === 'errored'
                ? h(d._storedError)
                : Ai(p)
        }
        function g4(p, d) {
            p._closedPromiseState === 'pending' ? fd(p, d) : A4(p, d)
        }
        function Vi(p, d) {
            p._readyPromiseState === 'pending' ? Wi(p, d) : q4(p, d)
        }
        function S4(p) {
            let d = p._ownerWritableStream,
                n = d._state
            return n === 'errored' || n === 'erroring'
                ? null
                : n === 'closed'
                ? 0
                : Fi(d._writableStreamController)
        }
        function qi(p) {
            let d = p._ownerWritableStream,
                n = new TypeError(
                    "Writer was released and can no longer be used to monitor the stream's closedness"
                )
            Vi(p, n),
                g4(p, n),
                (d._writer = void 0),
                (p._ownerWritableStream = void 0)
        }
        function Bi(p, d) {
            let n = p._ownerWritableStream,
                l = n._writableStreamController,
                c = C4(l, d)
            if (n !== p._ownerWritableStream) return h(St('write to'))
            let v = n._state
            if (v === 'errored') return h(n._storedError)
            if (Ke(n) || v === 'closed')
                return h(
                    new TypeError(
                        'The stream is closing or closed and cannot be written to'
                    )
                )
            if (v === 'erroring') return h(n._storedError)
            let b = m4(n)
            return E4(l, d, c), b
        }
        let ki = {}
        class Ba {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get abortReason() {
                if (!md(this)) throw cd('abortReason')
                return this._abortReason
            }
            get signal() {
                if (!md(this)) throw cd('signal')
                if (this._abortController === void 0)
                    throw new TypeError(
                        'WritableStreamDefaultController.prototype.signal is not supported'
                    )
                return this._abortController.signal
            }
            error(d = void 0) {
                if (!md(this)) throw cd('error')
                this._controlledWritableStream._state === 'writable' &&
                    xi(this, d)
            }
            [Z](d) {
                let n = this._abortAlgorithm(d)
                return gp(this), n
            }
            [ve]() {
                A1(this)
            }
        }
        Object.defineProperties(Ba.prototype, {
            abortReason: { enumerable: !0 },
            signal: { enumerable: !0 },
            error: { enumerable: !0 }
        }),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(Ba.prototype, Symbol.toStringTag, {
                    value: 'WritableStreamDefaultController',
                    configurable: !0
                })
        function md(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_controlledWritableStream'
                )
                ? !1
                : p instanceof Ba
        }
        function Oi(p, d, n, l, c, v, b, S) {
            ;(d._controlledWritableStream = p),
                (p._writableStreamController = d),
                (d._queue = void 0),
                (d._queueTotalSize = void 0),
                A1(d),
                (d._abortReason = void 0),
                (d._abortController = l4()),
                (d._started = !1),
                (d._strategySizeAlgorithm = S),
                (d._strategyHWM = b),
                (d._writeAlgorithm = l),
                (d._closeAlgorithm = c),
                (d._abortAlgorithm = v)
            let V = ud(d)
            sd(p, V)
            let E = n(),
                B = f(E)
            w(
                B,
                () => ((d._started = !0), Sp(d), null),
                (L) => ((d._started = !0), nd(p, L), null)
            )
        }
        function T4(p, d, n, l) {
            let c = Object.create(Ba.prototype),
                v,
                b,
                S,
                V
            d.start !== void 0 ? (v = () => d.start(c)) : (v = () => {}),
                d.write !== void 0
                    ? (b = (E) => d.write(E, c))
                    : (b = () => f(void 0)),
                d.close !== void 0
                    ? (S = () => d.close())
                    : (S = () => f(void 0)),
                d.abort !== void 0
                    ? (V = (E) => d.abort(E))
                    : (V = () => f(void 0)),
                Oi(p, c, v, b, S, V, n, l)
        }
        function gp(p) {
            ;(p._writeAlgorithm = void 0),
                (p._closeAlgorithm = void 0),
                (p._abortAlgorithm = void 0),
                (p._strategySizeAlgorithm = void 0)
        }
        function R4(p) {
            Zr(p, ki, 0), Sp(p)
        }
        function C4(p, d) {
            try {
                return p._strategySizeAlgorithm(d)
            } catch (n) {
                return gt(p, n), 1
            }
        }
        function Fi(p) {
            return p._strategyHWM - p._queueTotalSize
        }
        function E4(p, d, n) {
            try {
                Zr(p, d, n)
            } catch (c) {
                gt(p, c)
                return
            }
            let l = p._controlledWritableStream
            if (!Ke(l) && l._state === 'writable') {
                let c = ud(p)
                sd(l, c)
            }
            Sp(p)
        }
        function Sp(p) {
            let d = p._controlledWritableStream
            if (!p._started || d._inFlightWriteRequest !== void 0) return
            if (d._state === 'erroring') {
                ld(d)
                return
            }
            if (p._queue.length === 0) return
            let l = j6(p)
            l === ki ? P4(p) : N4(p, l)
        }
        function gt(p, d) {
            p._controlledWritableStream._state === 'writable' && xi(p, d)
        }
        function P4(p) {
            let d = p._controlledWritableStream
            b4(d), Jr(p)
            let n = p._closeAlgorithm()
            gp(p),
                w(
                    n,
                    () => (f4(d), null),
                    (l) => (h4(d, l), null)
                )
        }
        function N4(p, d) {
            let n = p._controlledWritableStream
            w4(n)
            let l = p._writeAlgorithm(d)
            w(
                l,
                () => {
                    u4(n)
                    let c = n._state
                    if ((Jr(p), !Ke(n) && c === 'writable')) {
                        let v = ud(p)
                        sd(n, v)
                    }
                    return Sp(p), null
                },
                (c) => (n._state === 'writable' && gp(p), c4(n, c), null)
            )
        }
        function ud(p) {
            return Fi(p) <= 0
        }
        function xi(p, d) {
            let n = p._controlledWritableStream
            gp(p), od(n, d)
        }
        function Tp(p) {
            return new TypeError(
                `WritableStream.prototype.${p} can only be used on a WritableStream`
            )
        }
        function cd(p) {
            return new TypeError(
                `WritableStreamDefaultController.prototype.${p} can only be used on a WritableStreamDefaultController`
            )
        }
        function ia(p) {
            return new TypeError(
                `WritableStreamDefaultWriter.prototype.${p} can only be used on a WritableStreamDefaultWriter`
            )
        }
        function St(p) {
            return new TypeError(
                'Cannot ' + p + ' a stream using a released writer'
            )
        }
        function Rp(p) {
            p._closedPromise = s((d, n) => {
                ;(p._closedPromise_resolve = d),
                    (p._closedPromise_reject = n),
                    (p._closedPromiseState = 'pending')
            })
        }
        function Li(p, d) {
            Rp(p), fd(p, d)
        }
        function D4(p) {
            Rp(p), Ii(p)
        }
        function fd(p, d) {
            p._closedPromise_reject !== void 0 &&
                (R(p._closedPromise),
                p._closedPromise_reject(d),
                (p._closedPromise_resolve = void 0),
                (p._closedPromise_reject = void 0),
                (p._closedPromiseState = 'rejected'))
        }
        function A4(p, d) {
            Li(p, d)
        }
        function Ii(p) {
            p._closedPromise_resolve !== void 0 &&
                (p._closedPromise_resolve(void 0),
                (p._closedPromise_resolve = void 0),
                (p._closedPromise_reject = void 0),
                (p._closedPromiseState = 'resolved'))
        }
        function Cp(p) {
            ;(p._readyPromise = s((d, n) => {
                ;(p._readyPromise_resolve = d), (p._readyPromise_reject = n)
            })),
                (p._readyPromiseState = 'pending')
        }
        function hd(p, d) {
            Cp(p), Wi(p, d)
        }
        function ji(p) {
            Cp(p), vd(p)
        }
        function Wi(p, d) {
            p._readyPromise_reject !== void 0 &&
                (R(p._readyPromise),
                p._readyPromise_reject(d),
                (p._readyPromise_resolve = void 0),
                (p._readyPromise_reject = void 0),
                (p._readyPromiseState = 'rejected'))
        }
        function V4(p) {
            Cp(p)
        }
        function q4(p, d) {
            hd(p, d)
        }
        function vd(p) {
            p._readyPromise_resolve !== void 0 &&
                (p._readyPromise_resolve(void 0),
                (p._readyPromise_resolve = void 0),
                (p._readyPromise_reject = void 0),
                (p._readyPromiseState = 'fulfilled'))
        }
        function B4() {
            if (typeof globalThis < 'u') return globalThis
            if (typeof self < 'u') return self
            if (typeof global < 'u') return global
        }
        let bd = B4()
        function k4(p) {
            if (
                !(typeof p == 'function' || typeof p == 'object') ||
                p.name !== 'DOMException'
            )
                return !1
            try {
                return new p(), !0
            } catch {
                return !1
            }
        }
        function O4() {
            let p = bd?.DOMException
            return k4(p) ? p : void 0
        }
        function F4() {
            let p = function (n, l) {
                ;(this.message = n || ''),
                    (this.name = l || 'Error'),
                    Error.captureStackTrace &&
                        Error.captureStackTrace(this, this.constructor)
            }
            return (
                i(p, 'DOMException'),
                (p.prototype = Object.create(Error.prototype)),
                Object.defineProperty(p.prototype, 'constructor', {
                    value: p,
                    writable: !0,
                    configurable: !0
                }),
                p
            )
        }
        let x4 = O4() || F4()
        function zi(p, d, n, l, c, v) {
            let b = Da(p),
                S = Pi(d)
            p._disturbed = !0
            let V = !1,
                E = f(void 0)
            return s((B, L) => {
                let G
                if (v !== void 0) {
                    if (
                        ((G = () => {
                            let P =
                                    v.reason !== void 0
                                        ? v.reason
                                        : new x4('Aborted', 'AbortError'),
                                W = []
                            l ||
                                W.push(() =>
                                    d._state === 'writable'
                                        ? _p(d, P)
                                        : f(void 0)
                                ),
                                c ||
                                    W.push(() =>
                                        p._state === 'readable'
                                            ? je(p, P)
                                            : f(void 0)
                                    ),
                                Ce(() => Promise.all(W.map((X) => X())), !0, P)
                        }),
                        v.aborted)
                    ) {
                        G()
                        return
                    }
                    v.addEventListener('abort', G)
                }
                function We() {
                    return s((P, W) => {
                        function X(De) {
                            De ? P() : _(xa(), X, W)
                        }
                        X(!1)
                    })
                }
                function xa() {
                    return V
                        ? f(!0)
                        : _(S._readyPromise, () =>
                              s((P, W) => {
                                  vt(b, {
                                      _chunkSteps: (X) => {
                                          ;(E = _(Bi(S, X), void 0, a)), P(!1)
                                      },
                                      _closeSteps: () => P(!0),
                                      _errorSteps: W
                                  })
                              })
                          )
                }
                if (
                    (s1(
                        p,
                        b._closedPromise,
                        (P) => (l ? ke(!0, P) : Ce(() => _p(d, P), !0, P), null)
                    ),
                    s1(
                        d,
                        S._closedPromise,
                        (P) => (c ? ke(!0, P) : Ce(() => je(p, P), !0, P), null)
                    ),
                    Se(
                        p,
                        b._closedPromise,
                        () => (n ? ke() : Ce(() => y4(S)), null)
                    ),
                    Ke(d) || d._state === 'closed')
                ) {
                    let P = new TypeError(
                        'the destination writable stream closed before all data could be piped to it'
                    )
                    c ? ke(!0, P) : Ce(() => je(p, P), !0, P)
                }
                R(We())
                function F1() {
                    let P = E
                    return _(E, () => (P !== E ? F1() : void 0))
                }
                function s1(P, W, X) {
                    P._state === 'errored' ? X(P._storedError) : y(W, X)
                }
                function Se(P, W, X) {
                    P._state === 'closed' ? X() : g(W, X)
                }
                function Ce(P, W, X) {
                    if (V) return
                    ;(V = !0),
                        d._state === 'writable' && !Ke(d) ? g(F1(), De) : De()
                    function De() {
                        return (
                            w(
                                P(),
                                () => m1(W, X),
                                (La) => m1(!0, La)
                            ),
                            null
                        )
                    }
                }
                function ke(P, W) {
                    V ||
                        ((V = !0),
                        d._state === 'writable' && !Ke(d)
                            ? g(F1(), () => m1(P, W))
                            : m1(P, W))
                }
                function m1(P, W) {
                    return (
                        qi(S),
                        q(b),
                        v !== void 0 && v.removeEventListener('abort', G),
                        P ? L(W) : B(void 0),
                        null
                    )
                }
            })
        }
        class l1 {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get desiredSize() {
                if (!Ep(this)) throw Np('desiredSize')
                return wd(this)
            }
            close() {
                if (!Ep(this)) throw Np('close')
                if (!Oa(this))
                    throw new TypeError(
                        'The stream is not in a state that permits close'
                    )
                na(this)
            }
            enqueue(d = void 0) {
                if (!Ep(this)) throw Np('enqueue')
                if (!Oa(this))
                    throw new TypeError(
                        'The stream is not in a state that permits enqueue'
                    )
                return ka(this, d)
            }
            error(d = void 0) {
                if (!Ep(this)) throw Np('error')
                Ie(this, d)
            }
            [N](d) {
                A1(this)
                let n = this._cancelAlgorithm(d)
                return Pp(this), n
            }
            [D](d) {
                let n = this._controlledReadableStream
                if (this._queue.length > 0) {
                    let l = Jr(this)
                    this._closeRequested && this._queue.length === 0
                        ? (Pp(this), Ct(n))
                        : Tt(this),
                        d._chunkSteps(l)
                } else X2(n, d), Tt(this)
            }
            [x]() {}
        }
        Object.defineProperties(l1.prototype, {
            close: { enumerable: !0 },
            enqueue: { enumerable: !0 },
            error: { enumerable: !0 },
            desiredSize: { enumerable: !0 }
        }),
            i(l1.prototype.close, 'close'),
            i(l1.prototype.enqueue, 'enqueue'),
            i(l1.prototype.error, 'error'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(l1.prototype, Symbol.toStringTag, {
                    value: 'ReadableStreamDefaultController',
                    configurable: !0
                })
        function Ep(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_controlledReadableStream'
                )
                ? !1
                : p instanceof l1
        }
        function Tt(p) {
            if (!Mi(p)) return
            if (p._pulling) {
                p._pullAgain = !0
                return
            }
            p._pulling = !0
            let n = p._pullAlgorithm()
            w(
                n,
                () => (
                    (p._pulling = !1),
                    p._pullAgain && ((p._pullAgain = !1), Tt(p)),
                    null
                ),
                (l) => (Ie(p, l), null)
            )
        }
        function Mi(p) {
            let d = p._controlledReadableStream
            return !Oa(p) || !p._started
                ? !1
                : !!((k1(d) && op(d) > 0) || wd(p) > 0)
        }
        function Pp(p) {
            ;(p._pullAlgorithm = void 0),
                (p._cancelAlgorithm = void 0),
                (p._strategySizeAlgorithm = void 0)
        }
        function na(p) {
            if (!Oa(p)) return
            let d = p._controlledReadableStream
            ;(p._closeRequested = !0), p._queue.length === 0 && (Pp(p), Ct(d))
        }
        function ka(p, d) {
            if (!Oa(p)) return
            let n = p._controlledReadableStream
            if (k1(n) && op(n) > 0) Qr(n, d, !1)
            else {
                let l
                try {
                    l = p._strategySizeAlgorithm(d)
                } catch (c) {
                    throw (Ie(p, c), c)
                }
                try {
                    Zr(p, d, l)
                } catch (c) {
                    throw (Ie(p, c), c)
                }
            }
            Tt(p)
        }
        function Ie(p, d) {
            let n = p._controlledReadableStream
            n._state === 'readable' && (A1(p), Pp(p), Qi(n, d))
        }
        function wd(p) {
            let d = p._controlledReadableStream._state
            return d === 'errored'
                ? null
                : d === 'closed'
                ? 0
                : p._strategyHWM - p._queueTotalSize
        }
        function L4(p) {
            return !Mi(p)
        }
        function Oa(p) {
            let d = p._controlledReadableStream._state
            return !p._closeRequested && d === 'readable'
        }
        function $i(p, d, n, l, c, v, b) {
            ;(d._controlledReadableStream = p),
                (d._queue = void 0),
                (d._queueTotalSize = void 0),
                A1(d),
                (d._started = !1),
                (d._closeRequested = !1),
                (d._pullAgain = !1),
                (d._pulling = !1),
                (d._strategySizeAlgorithm = b),
                (d._strategyHWM = v),
                (d._pullAlgorithm = l),
                (d._cancelAlgorithm = c),
                (p._readableStreamController = d)
            let S = n()
            w(
                f(S),
                () => ((d._started = !0), Tt(d), null),
                (V) => (Ie(d, V), null)
            )
        }
        function I4(p, d, n, l) {
            let c = Object.create(l1.prototype),
                v,
                b,
                S
            d.start !== void 0 ? (v = () => d.start(c)) : (v = () => {}),
                d.pull !== void 0
                    ? (b = () => d.pull(c))
                    : (b = () => f(void 0)),
                d.cancel !== void 0
                    ? (S = (V) => d.cancel(V))
                    : (S = () => f(void 0)),
                $i(p, c, v, b, S, n, l)
        }
        function Np(p) {
            return new TypeError(
                `ReadableStreamDefaultController.prototype.${p} can only be used on a ReadableStreamDefaultController`
            )
        }
        function j4(p, d) {
            return ta(p._readableStreamController) ? z4(p) : W4(p)
        }
        function W4(p, d) {
            let n = Da(p),
                l = !1,
                c = !1,
                v = !1,
                b = !1,
                S,
                V,
                E,
                B,
                L,
                G = s((Se) => {
                    L = Se
                })
            function We() {
                return l
                    ? ((c = !0), f(void 0))
                    : ((l = !0),
                      vt(n, {
                          _chunkSteps: (Ce) => {
                              T(() => {
                                  c = !1
                                  let ke = Ce,
                                      m1 = Ce
                                  v || ka(E._readableStreamController, ke),
                                      b || ka(B._readableStreamController, m1),
                                      (l = !1),
                                      c && We()
                              })
                          },
                          _closeSteps: () => {
                              ;(l = !1),
                                  v || na(E._readableStreamController),
                                  b || na(B._readableStreamController),
                                  (!v || !b) && L(void 0)
                          },
                          _errorSteps: () => {
                              l = !1
                          }
                      }),
                      f(void 0))
            }
            function xa(Se) {
                if (((v = !0), (S = Se), b)) {
                    let Ce = bt([S, V]),
                        ke = je(p, Ce)
                    L(ke)
                }
                return G
            }
            function F1(Se) {
                if (((b = !0), (V = Se), v)) {
                    let Ce = bt([S, V]),
                        ke = je(p, Ce)
                    L(ke)
                }
                return G
            }
            function s1() {}
            return (
                (E = Rt(s1, We, xa)),
                (B = Rt(s1, We, F1)),
                y(
                    n._closedPromise,
                    (Se) => (
                        Ie(E._readableStreamController, Se),
                        Ie(B._readableStreamController, Se),
                        (!v || !b) && L(void 0),
                        null
                    )
                ),
                [E, B]
            )
        }
        function z4(p) {
            let d = Da(p),
                n = !1,
                l = !1,
                c = !1,
                v = !1,
                b = !1,
                S,
                V,
                E,
                B,
                L,
                G = s((P) => {
                    L = P
                })
            function We(P) {
                y(
                    P._closedPromise,
                    (W) => (
                        P !== d ||
                            (Be(E._readableStreamController, W),
                            Be(B._readableStreamController, W),
                            (!v || !b) && L(void 0)),
                        null
                    )
                )
            }
            function xa() {
                ra(d) && (q(d), (d = Da(p)), We(d)),
                    vt(d, {
                        _chunkSteps: (W) => {
                            T(() => {
                                ;(l = !1), (c = !1)
                                let X = W,
                                    De = W
                                if (!v && !b)
                                    try {
                                        De = oi(W)
                                    } catch (La) {
                                        Be(E._readableStreamController, La),
                                            Be(B._readableStreamController, La),
                                            L(je(p, La))
                                        return
                                    }
                                v || cp(E._readableStreamController, X),
                                    b || cp(B._readableStreamController, De),
                                    (n = !1),
                                    l ? s1() : c && Se()
                            })
                        },
                        _closeSteps: () => {
                            ;(n = !1),
                                v || wt(E._readableStreamController),
                                b || wt(B._readableStreamController),
                                E._readableStreamController._pendingPullIntos
                                    .length > 0 &&
                                    fp(E._readableStreamController, 0),
                                B._readableStreamController._pendingPullIntos
                                    .length > 0 &&
                                    fp(B._readableStreamController, 0),
                                (!v || !b) && L(void 0)
                        },
                        _errorSteps: () => {
                            n = !1
                        }
                    })
            }
            function F1(P, W) {
                N1(d) && (q(d), (d = gi(p)), We(d))
                let X = W ? B : E,
                    De = W ? E : B
                Ri(d, P, 1, {
                    _chunkSteps: (Ia) => {
                        T(() => {
                            ;(l = !1), (c = !1)
                            let ja = W ? b : v
                            if (W ? v : b)
                                ja || hp(X._readableStreamController, Ia)
                            else {
                                let i0
                                try {
                                    i0 = oi(Ia)
                                } catch (Td) {
                                    Be(X._readableStreamController, Td),
                                        Be(De._readableStreamController, Td),
                                        L(je(p, Td))
                                    return
                                }
                                ja || hp(X._readableStreamController, Ia),
                                    cp(De._readableStreamController, i0)
                            }
                            ;(n = !1), l ? s1() : c && Se()
                        })
                    },
                    _closeSteps: (Ia) => {
                        n = !1
                        let ja = W ? b : v,
                            Fp = W ? v : b
                        ja || wt(X._readableStreamController),
                            Fp || wt(De._readableStreamController),
                            Ia !== void 0 &&
                                (ja || hp(X._readableStreamController, Ia),
                                !Fp &&
                                    De._readableStreamController
                                        ._pendingPullIntos.length > 0 &&
                                    fp(De._readableStreamController, 0)),
                            (!ja || !Fp) && L(void 0)
                    },
                    _errorSteps: () => {
                        n = !1
                    }
                })
            }
            function s1() {
                if (n) return (l = !0), f(void 0)
                n = !0
                let P = rd(E._readableStreamController)
                return P === null ? xa() : F1(P._view, !1), f(void 0)
            }
            function Se() {
                if (n) return (c = !0), f(void 0)
                n = !0
                let P = rd(B._readableStreamController)
                return P === null ? xa() : F1(P._view, !0), f(void 0)
            }
            function Ce(P) {
                if (((v = !0), (S = P), b)) {
                    let W = bt([S, V]),
                        X = je(p, W)
                    L(X)
                }
                return G
            }
            function ke(P) {
                if (((b = !0), (V = P), v)) {
                    let W = bt([S, V]),
                        X = je(p, W)
                    L(X)
                }
                return G
            }
            function m1() {}
            return (E = Hi(m1, s1, Ce)), (B = Hi(m1, Se, ke)), We(d), [E, B]
        }
        function M4(p) {
            return t(p) && typeof p.getReader < 'u'
        }
        function $4(p) {
            return M4(p) ? H4(p.getReader()) : U4(p)
        }
        function U4(p) {
            let d,
                n = ni(p, 'async'),
                l = a
            function c() {
                let b
                try {
                    b = F6(n)
                } catch (V) {
                    return h(V)
                }
                let S = f(b)
                return C(S, (V) => {
                    if (!t(V))
                        throw new TypeError(
                            'The promise returned by the iterator.next() method must fulfill with an object'
                        )
                    if (x6(V)) na(d._readableStreamController)
                    else {
                        let B = L6(V)
                        ka(d._readableStreamController, B)
                    }
                })
            }
            function v(b) {
                let S = n.iterator,
                    V
                try {
                    V = sp(S, 'return')
                } catch (L) {
                    return h(L)
                }
                if (V === void 0) return f(void 0)
                let E
                try {
                    E = F(V, S, [b])
                } catch (L) {
                    return h(L)
                }
                let B = f(E)
                return C(B, (L) => {
                    if (!t(L))
                        throw new TypeError(
                            'The promise returned by the iterator.return() method must fulfill with an object'
                        )
                })
            }
            return (d = Rt(l, c, v, 0)), d
        }
        function H4(p) {
            let d,
                n = a
            function l() {
                let v
                try {
                    v = p.read()
                } catch (b) {
                    return h(b)
                }
                return C(v, (b) => {
                    if (!t(b))
                        throw new TypeError(
                            'The promise returned by the reader.read() method must fulfill with an object'
                        )
                    if (b.done) na(d._readableStreamController)
                    else {
                        let S = b.value
                        ka(d._readableStreamController, S)
                    }
                })
            }
            function c(v) {
                try {
                    return f(p.cancel(v))
                } catch (b) {
                    return h(b)
                }
            }
            return (d = Rt(n, l, c, 0)), d
        }
        function Q4(p, d) {
            Ye(p, d)
            let n = p,
                l = n?.autoAllocateChunkSize,
                c = n?.cancel,
                v = n?.pull,
                b = n?.start,
                S = n?.type
            return {
                autoAllocateChunkSize:
                    l === void 0
                        ? void 0
                        : Ur(l, `${d} has member 'autoAllocateChunkSize' that`),
                cancel:
                    c === void 0
                        ? void 0
                        : G4(c, n, `${d} has member 'cancel' that`),
                pull:
                    v === void 0
                        ? void 0
                        : Y4(v, n, `${d} has member 'pull' that`),
                start:
                    b === void 0
                        ? void 0
                        : K4(b, n, `${d} has member 'start' that`),
                type:
                    S === void 0 ? void 0 : X4(S, `${d} has member 'type' that`)
            }
        }
        function G4(p, d, n) {
            return qe(p, n), (l) => K(p, d, [l])
        }
        function Y4(p, d, n) {
            return qe(p, n), (l) => K(p, d, [l])
        }
        function K4(p, d, n) {
            return qe(p, n), (l) => F(p, d, [l])
        }
        function X4(p, d) {
            if (((p = `${p}`), p !== 'bytes'))
                throw new TypeError(
                    `${d} '${p}' is not a valid enumeration value for ReadableStreamType`
                )
            return p
        }
        function J4(p, d) {
            return Ye(p, d), { preventCancel: !!p?.preventCancel }
        }
        function Ui(p, d) {
            Ye(p, d)
            let n = p?.preventAbort,
                l = p?.preventCancel,
                c = p?.preventClose,
                v = p?.signal
            return (
                v !== void 0 && Z4(v, `${d} has member 'signal' that`),
                {
                    preventAbort: !!n,
                    preventCancel: !!l,
                    preventClose: !!c,
                    signal: v
                }
            )
        }
        function Z4(p, d) {
            if (!n4(p)) throw new TypeError(`${d} is not an AbortSignal.`)
        }
        function el(p, d) {
            Ye(p, d)
            let n = p?.readable
            Mr(n, 'readable', 'ReadableWritablePair'),
                Hr(n, `${d} has member 'readable' that`)
            let l = p?.writable
            return (
                Mr(l, 'writable', 'ReadableWritablePair'),
                Ei(l, `${d} has member 'writable' that`),
                { readable: n, writable: l }
            )
        }
        class we {
            constructor(d = {}, n = {}) {
                d === void 0 ? (d = null) : Y2(d, 'First parameter')
                let l = wp(n, 'Second parameter'),
                    c = Q4(d, 'First parameter')
                if ((_d(this), c.type === 'bytes')) {
                    if (l.size !== void 0)
                        throw new RangeError(
                            'The strategy for a byte stream cannot have a size function'
                        )
                    let v = yt(l, 0)
                    G6(this, c, v)
                } else {
                    let v = bp(l),
                        b = yt(l, 1)
                    I4(this, c, b, v)
                }
            }
            get locked() {
                if (!B1(this)) throw oa('locked')
                return k1(this)
            }
            cancel(d = void 0) {
                return B1(this)
                    ? k1(this)
                        ? h(
                              new TypeError(
                                  'Cannot cancel a stream that already has a reader'
                              )
                          )
                        : je(this, d)
                    : h(oa('cancel'))
            }
            getReader(d = void 0) {
                if (!B1(this)) throw oa('getReader')
                return K6(d, 'First parameter').mode === void 0
                    ? Da(this)
                    : gi(this)
            }
            pipeThrough(d, n = {}) {
                if (!B1(this)) throw oa('pipeThrough')
                d1(d, 1, 'pipeThrough')
                let l = el(d, 'First parameter'),
                    c = Ui(n, 'Second parameter')
                if (k1(this))
                    throw new TypeError(
                        'ReadableStream.prototype.pipeThrough cannot be used on a locked ReadableStream'
                    )
                if (qa(l.writable))
                    throw new TypeError(
                        'ReadableStream.prototype.pipeThrough cannot be used on a locked WritableStream'
                    )
                let v = zi(
                    this,
                    l.writable,
                    c.preventClose,
                    c.preventAbort,
                    c.preventCancel,
                    c.signal
                )
                return R(v), l.readable
            }
            pipeTo(d, n = {}) {
                if (!B1(this)) return h(oa('pipeTo'))
                if (d === void 0)
                    return h("Parameter 1 is required in 'pipeTo'.")
                if (!Va(d))
                    return h(
                        new TypeError(
                            "ReadableStream.prototype.pipeTo's first argument must be a WritableStream"
                        )
                    )
                let l
                try {
                    l = Ui(n, 'Second parameter')
                } catch (c) {
                    return h(c)
                }
                return k1(this)
                    ? h(
                          new TypeError(
                              'ReadableStream.prototype.pipeTo cannot be used on a locked ReadableStream'
                          )
                      )
                    : qa(d)
                    ? h(
                          new TypeError(
                              'ReadableStream.prototype.pipeTo cannot be used on a locked WritableStream'
                          )
                      )
                    : zi(
                          this,
                          d,
                          l.preventClose,
                          l.preventAbort,
                          l.preventCancel,
                          l.signal
                      )
            }
            tee() {
                if (!B1(this)) throw oa('tee')
                let d = j4(this)
                return bt(d)
            }
            values(d = void 0) {
                if (!B1(this)) throw oa('values')
                let n = J4(d, 'First parameter')
                return k6(this, n.preventCancel)
            }
            [Xr](d) {
                return this.values(d)
            }
            static from(d) {
                return $4(d)
            }
        }
        Object.defineProperties(we, { from: { enumerable: !0 } }),
            Object.defineProperties(we.prototype, {
                cancel: { enumerable: !0 },
                getReader: { enumerable: !0 },
                pipeThrough: { enumerable: !0 },
                pipeTo: { enumerable: !0 },
                tee: { enumerable: !0 },
                values: { enumerable: !0 },
                locked: { enumerable: !0 }
            }),
            i(we.from, 'from'),
            i(we.prototype.cancel, 'cancel'),
            i(we.prototype.getReader, 'getReader'),
            i(we.prototype.pipeThrough, 'pipeThrough'),
            i(we.prototype.pipeTo, 'pipeTo'),
            i(we.prototype.tee, 'tee'),
            i(we.prototype.values, 'values'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(we.prototype, Symbol.toStringTag, {
                    value: 'ReadableStream',
                    configurable: !0
                }),
            Object.defineProperty(we.prototype, Xr, {
                value: we.prototype.values,
                writable: !0,
                configurable: !0
            })
        function Rt(p, d, n, l = 1, c = () => 1) {
            let v = Object.create(we.prototype)
            _d(v)
            let b = Object.create(l1.prototype)
            return $i(v, b, p, d, n, l, c), v
        }
        function Hi(p, d, n) {
            let l = Object.create(we.prototype)
            _d(l)
            let c = Object.create(n1.prototype)
            return yi(l, c, p, d, n, 0, void 0), l
        }
        function _d(p) {
            ;(p._state = 'readable'),
                (p._reader = void 0),
                (p._storedError = void 0),
                (p._disturbed = !1)
        }
        function B1(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_readableStreamController'
                )
                ? !1
                : p instanceof we
        }
        function k1(p) {
            return p._reader !== void 0
        }
        function je(p, d) {
            if (((p._disturbed = !0), p._state === 'closed')) return f(void 0)
            if (p._state === 'errored') return h(p._storedError)
            Ct(p)
            let n = p._reader
            if (n !== void 0 && ra(n)) {
                let c = n._readIntoRequests
                ;(n._readIntoRequests = new k()),
                    c.forEach((v) => {
                        v._closeSteps(void 0)
                    })
            }
            let l = p._readableStreamController[N](d)
            return C(l, a)
        }
        function Ct(p) {
            p._state = 'closed'
            let d = p._reader
            if (d !== void 0 && (Q2(d), N1(d))) {
                let n = d._readRequests
                ;(d._readRequests = new k()),
                    n.forEach((l) => {
                        l._closeSteps()
                    })
            }
        }
        function Qi(p, d) {
            ;(p._state = 'errored'), (p._storedError = d)
            let n = p._reader
            n !== void 0 && (le(n, d), N1(n) ? Z2(n, d) : Ci(n, d))
        }
        function oa(p) {
            return new TypeError(
                `ReadableStream.prototype.${p} can only be used on a ReadableStream`
            )
        }
        function Gi(p, d) {
            Ye(p, d)
            let n = p?.highWaterMark
            return (
                Mr(n, 'highWaterMark', 'QueuingStrategyInit'),
                { highWaterMark: $r(n) }
            )
        }
        let Yi = (p) => p.byteLength
        i(Yi, 'size')
        class Dp {
            constructor(d) {
                d1(d, 1, 'ByteLengthQueuingStrategy'),
                    (d = Gi(d, 'First parameter')),
                    (this._byteLengthQueuingStrategyHighWaterMark =
                        d.highWaterMark)
            }
            get highWaterMark() {
                if (!Xi(this)) throw Ki('highWaterMark')
                return this._byteLengthQueuingStrategyHighWaterMark
            }
            get size() {
                if (!Xi(this)) throw Ki('size')
                return Yi
            }
        }
        Object.defineProperties(Dp.prototype, {
            highWaterMark: { enumerable: !0 },
            size: { enumerable: !0 }
        }),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(Dp.prototype, Symbol.toStringTag, {
                    value: 'ByteLengthQueuingStrategy',
                    configurable: !0
                })
        function Ki(p) {
            return new TypeError(
                `ByteLengthQueuingStrategy.prototype.${p} can only be used on a ByteLengthQueuingStrategy`
            )
        }
        function Xi(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_byteLengthQueuingStrategyHighWaterMark'
                )
                ? !1
                : p instanceof Dp
        }
        let Ji = () => 1
        i(Ji, 'size')
        class Ap {
            constructor(d) {
                d1(d, 1, 'CountQueuingStrategy'),
                    (d = Gi(d, 'First parameter')),
                    (this._countQueuingStrategyHighWaterMark = d.highWaterMark)
            }
            get highWaterMark() {
                if (!e0(this)) throw Zi('highWaterMark')
                return this._countQueuingStrategyHighWaterMark
            }
            get size() {
                if (!e0(this)) throw Zi('size')
                return Ji
            }
        }
        Object.defineProperties(Ap.prototype, {
            highWaterMark: { enumerable: !0 },
            size: { enumerable: !0 }
        }),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(Ap.prototype, Symbol.toStringTag, {
                    value: 'CountQueuingStrategy',
                    configurable: !0
                })
        function Zi(p) {
            return new TypeError(
                `CountQueuingStrategy.prototype.${p} can only be used on a CountQueuingStrategy`
            )
        }
        function e0(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_countQueuingStrategyHighWaterMark'
                )
                ? !1
                : p instanceof Ap
        }
        function al(p, d) {
            Ye(p, d)
            let n = p?.cancel,
                l = p?.flush,
                c = p?.readableType,
                v = p?.start,
                b = p?.transform,
                S = p?.writableType
            return {
                cancel:
                    n === void 0
                        ? void 0
                        : dl(n, p, `${d} has member 'cancel' that`),
                flush:
                    l === void 0
                        ? void 0
                        : tl(l, p, `${d} has member 'flush' that`),
                readableType: c,
                start:
                    v === void 0
                        ? void 0
                        : pl(v, p, `${d} has member 'start' that`),
                transform:
                    b === void 0
                        ? void 0
                        : rl(b, p, `${d} has member 'transform' that`),
                writableType: S
            }
        }
        function tl(p, d, n) {
            return qe(p, n), (l) => K(p, d, [l])
        }
        function pl(p, d, n) {
            return qe(p, n), (l) => F(p, d, [l])
        }
        function rl(p, d, n) {
            return qe(p, n), (l, c) => K(p, d, [l, c])
        }
        function dl(p, d, n) {
            return qe(p, n), (l) => K(p, d, [l])
        }
        class Vp {
            constructor(d = {}, n = {}, l = {}) {
                d === void 0 && (d = null)
                let c = wp(n, 'Second parameter'),
                    v = wp(l, 'Third parameter'),
                    b = al(d, 'First parameter')
                if (b.readableType !== void 0)
                    throw new RangeError('Invalid readableType specified')
                if (b.writableType !== void 0)
                    throw new RangeError('Invalid writableType specified')
                let S = yt(v, 0),
                    V = bp(v),
                    E = yt(c, 1),
                    B = bp(c),
                    L,
                    G = s((We) => {
                        L = We
                    })
                il(this, G, E, B, S, V),
                    ol(this, b),
                    b.start !== void 0
                        ? L(b.start(this._transformStreamController))
                        : L(void 0)
            }
            get readable() {
                if (!a0(this)) throw d0('readable')
                return this._readable
            }
            get writable() {
                if (!a0(this)) throw d0('writable')
                return this._writable
            }
        }
        Object.defineProperties(Vp.prototype, {
            readable: { enumerable: !0 },
            writable: { enumerable: !0 }
        }),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(Vp.prototype, Symbol.toStringTag, {
                    value: 'TransformStream',
                    configurable: !0
                })
        function il(p, d, n, l, c, v) {
            function b() {
                return d
            }
            function S(G) {
                return ml(p, G)
            }
            function V(G) {
                return ul(p, G)
            }
            function E() {
                return cl(p)
            }
            p._writable = s4(b, S, E, V, n, l)
            function B() {
                return fl(p)
            }
            function L(G) {
                return hl(p, G)
            }
            ;(p._readable = Rt(b, B, L, c, v)),
                (p._backpressure = void 0),
                (p._backpressureChangePromise = void 0),
                (p._backpressureChangePromise_resolve = void 0),
                qp(p, !0),
                (p._transformStreamController = void 0)
        }
        function a0(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_transformStreamController'
                )
                ? !1
                : p instanceof Vp
        }
        function t0(p, d) {
            Ie(p._readable._readableStreamController, d), yd(p, d)
        }
        function yd(p, d) {
            kp(p._transformStreamController),
                gt(p._writable._writableStreamController, d),
                gd(p)
        }
        function gd(p) {
            p._backpressure && qp(p, !1)
        }
        function qp(p, d) {
            p._backpressureChangePromise !== void 0 &&
                p._backpressureChangePromise_resolve(),
                (p._backpressureChangePromise = s((n) => {
                    p._backpressureChangePromise_resolve = n
                })),
                (p._backpressure = d)
        }
        class O1 {
            constructor() {
                throw new TypeError('Illegal constructor')
            }
            get desiredSize() {
                if (!Bp(this)) throw Op('desiredSize')
                let d =
                    this._controlledTransformStream._readable
                        ._readableStreamController
                return wd(d)
            }
            enqueue(d = void 0) {
                if (!Bp(this)) throw Op('enqueue')
                p0(this, d)
            }
            error(d = void 0) {
                if (!Bp(this)) throw Op('error')
                ll(this, d)
            }
            terminate() {
                if (!Bp(this)) throw Op('terminate')
                sl(this)
            }
        }
        Object.defineProperties(O1.prototype, {
            enqueue: { enumerable: !0 },
            error: { enumerable: !0 },
            terminate: { enumerable: !0 },
            desiredSize: { enumerable: !0 }
        }),
            i(O1.prototype.enqueue, 'enqueue'),
            i(O1.prototype.error, 'error'),
            i(O1.prototype.terminate, 'terminate'),
            typeof Symbol.toStringTag == 'symbol' &&
                Object.defineProperty(O1.prototype, Symbol.toStringTag, {
                    value: 'TransformStreamDefaultController',
                    configurable: !0
                })
        function Bp(p) {
            return !t(p) ||
                !Object.prototype.hasOwnProperty.call(
                    p,
                    '_controlledTransformStream'
                )
                ? !1
                : p instanceof O1
        }
        function nl(p, d, n, l, c) {
            ;(d._controlledTransformStream = p),
                (p._transformStreamController = d),
                (d._transformAlgorithm = n),
                (d._flushAlgorithm = l),
                (d._cancelAlgorithm = c),
                (d._finishPromise = void 0),
                (d._finishPromise_resolve = void 0),
                (d._finishPromise_reject = void 0)
        }
        function ol(p, d) {
            let n = Object.create(O1.prototype),
                l,
                c,
                v
            d.transform !== void 0
                ? (l = (b) => d.transform(b, n))
                : (l = (b) => {
                      try {
                          return p0(n, b), f(void 0)
                      } catch (S) {
                          return h(S)
                      }
                  }),
                d.flush !== void 0
                    ? (c = () => d.flush(n))
                    : (c = () => f(void 0)),
                d.cancel !== void 0
                    ? (v = (b) => d.cancel(b))
                    : (v = () => f(void 0)),
                nl(p, n, l, c, v)
        }
        function kp(p) {
            ;(p._transformAlgorithm = void 0),
                (p._flushAlgorithm = void 0),
                (p._cancelAlgorithm = void 0)
        }
        function p0(p, d) {
            let n = p._controlledTransformStream,
                l = n._readable._readableStreamController
            if (!Oa(l))
                throw new TypeError(
                    'Readable side is not in a state that permits enqueue'
                )
            try {
                ka(l, d)
            } catch (v) {
                throw (yd(n, v), n._readable._storedError)
            }
            L4(l) !== n._backpressure && qp(n, !0)
        }
        function ll(p, d) {
            t0(p._controlledTransformStream, d)
        }
        function r0(p, d) {
            let n = p._transformAlgorithm(d)
            return C(n, void 0, (l) => {
                throw (t0(p._controlledTransformStream, l), l)
            })
        }
        function sl(p) {
            let d = p._controlledTransformStream,
                n = d._readable._readableStreamController
            na(n)
            let l = new TypeError('TransformStream terminated')
            yd(d, l)
        }
        function ml(p, d) {
            let n = p._transformStreamController
            if (p._backpressure) {
                let l = p._backpressureChangePromise
                return C(l, () => {
                    let c = p._writable
                    if (c._state === 'erroring') throw c._storedError
                    return r0(n, d)
                })
            }
            return r0(n, d)
        }
        function ul(p, d) {
            let n = p._transformStreamController
            if (n._finishPromise !== void 0) return n._finishPromise
            let l = p._readable
            n._finishPromise = s((v, b) => {
                ;(n._finishPromise_resolve = v), (n._finishPromise_reject = b)
            })
            let c = n._cancelAlgorithm(d)
            return (
                kp(n),
                w(
                    c,
                    () => (
                        l._state === 'errored'
                            ? Fa(n, l._storedError)
                            : (Ie(l._readableStreamController, d), Sd(n)),
                        null
                    ),
                    (v) => (Ie(l._readableStreamController, v), Fa(n, v), null)
                ),
                n._finishPromise
            )
        }
        function cl(p) {
            let d = p._transformStreamController
            if (d._finishPromise !== void 0) return d._finishPromise
            let n = p._readable
            d._finishPromise = s((c, v) => {
                ;(d._finishPromise_resolve = c), (d._finishPromise_reject = v)
            })
            let l = d._flushAlgorithm()
            return (
                kp(d),
                w(
                    l,
                    () => (
                        n._state === 'errored'
                            ? Fa(d, n._storedError)
                            : (na(n._readableStreamController), Sd(d)),
                        null
                    ),
                    (c) => (Ie(n._readableStreamController, c), Fa(d, c), null)
                ),
                d._finishPromise
            )
        }
        function fl(p) {
            return qp(p, !1), p._backpressureChangePromise
        }
        function hl(p, d) {
            let n = p._transformStreamController
            if (n._finishPromise !== void 0) return n._finishPromise
            let l = p._writable
            n._finishPromise = s((v, b) => {
                ;(n._finishPromise_resolve = v), (n._finishPromise_reject = b)
            })
            let c = n._cancelAlgorithm(d)
            return (
                kp(n),
                w(
                    c,
                    () => (
                        l._state === 'errored'
                            ? Fa(n, l._storedError)
                            : (gt(l._writableStreamController, d),
                              gd(p),
                              Sd(n)),
                        null
                    ),
                    (v) => (
                        gt(l._writableStreamController, v),
                        gd(p),
                        Fa(n, v),
                        null
                    )
                ),
                n._finishPromise
            )
        }
        function Op(p) {
            return new TypeError(
                `TransformStreamDefaultController.prototype.${p} can only be used on a TransformStreamDefaultController`
            )
        }
        function Sd(p) {
            p._finishPromise_resolve !== void 0 &&
                (p._finishPromise_resolve(),
                (p._finishPromise_resolve = void 0),
                (p._finishPromise_reject = void 0))
        }
        function Fa(p, d) {
            p._finishPromise_reject !== void 0 &&
                (R(p._finishPromise),
                p._finishPromise_reject(d),
                (p._finishPromise_resolve = void 0),
                (p._finishPromise_reject = void 0))
        }
        function d0(p) {
            return new TypeError(
                `TransformStream.prototype.${p} can only be used on a TransformStream`
            )
        }
        ;(e.ByteLengthQueuingStrategy = Dp),
            (e.CountQueuingStrategy = Ap),
            (e.ReadableByteStreamController = n1),
            (e.ReadableStream = we),
            (e.ReadableStreamBYOBReader = V1),
            (e.ReadableStreamBYOBRequest = aa),
            (e.ReadableStreamDefaultController = l1),
            (e.ReadableStreamDefaultReader = P1),
            (e.TransformStream = Vp),
            (e.TransformStreamDefaultController = O1),
            (e.WritableStream = q1),
            (e.WritableStreamDefaultController = Ba),
            (e.WritableStreamDefaultWriter = o1)
    })
})
var t6 = se((y5, a6) => {
    if (!globalThis.DOMException)
        try {
            let { MessageChannel: e } = Ae('worker_threads'),
                a = new e().port1,
                t = new ArrayBuffer()
            a.postMessage(t, [t, t])
        } catch (e) {
            e.constructor.name === 'DOMException' &&
                (globalThis.DOMException = e.constructor)
        }
    a6.exports = globalThis.DOMException
})
function S9(e) {
    if (g9(e) !== 'object') return !1
    let a = Object.getPrototypeOf(e)
    return a == null
        ? !0
        : (a.constructor && a.constructor.toString()) === Object.toString()
}
var g9,
    p6,
    r6 = x1(() => {
        g9 = (e) => Object.prototype.toString.call(e).slice(8, -1).toLowerCase()
        p6 = S9
    })
var l6 = {}
n0(l6, { fileFromPath: () => N9, fileFromPathSync: () => P9, isFile: () => _2 })
import { statSync as T9, createReadStream as R9, promises as i6 } from 'fs'
import { basename as C9 } from 'path'
function o6(e, { mtimeMs: a, size: t }, r, i = {}) {
    let o
    p6(r) ? ([i, o] = [r, void 0]) : (o = r)
    let m = new F2({ path: e, size: t, lastModified: a })
    return (
        o || (o = m.name),
        new Qe([m], o, { ...i, lastModified: m.lastModified })
    )
}
function P9(e, a, t = {}) {
    let r = T9(e)
    return o6(e, r, a, t)
}
async function N9(e, a, t) {
    let r = await i6.stat(e)
    return o6(e, r, a, t)
}
var n6,
    d6,
    dt,
    Ra,
    ip,
    E9,
    F2,
    s6 = x1(() => {
        n6 = Et(t6(), 1)
        Ht()
        r6()
        y2()
        ;(d6 = function (e, a, t, r, i) {
            if (r === 'm') throw new TypeError('Private method is not writable')
            if (r === 'a' && !i)
                throw new TypeError(
                    'Private accessor was defined without a setter'
                )
            if (typeof a == 'function' ? e !== a || !i : !a.has(e))
                throw new TypeError(
                    'Cannot write private member to an object whose class did not declare it'
                )
            return r === 'a' ? i.call(e, t) : i ? (i.value = t) : a.set(e, t), t
        }),
            (dt = function (e, a, t, r) {
                if (t === 'a' && !r)
                    throw new TypeError(
                        'Private accessor was defined without a getter'
                    )
                if (typeof a == 'function' ? e !== a || !r : !a.has(e))
                    throw new TypeError(
                        'Cannot read private member from an object whose class did not declare it'
                    )
                return t === 'm'
                    ? r
                    : t === 'a'
                    ? r.call(e)
                    : r
                    ? r.value
                    : a.get(e)
            }),
            (E9 =
                'The requested file could not be read, typically due to permission problems that have occurred after a reference to a file was acquired.'),
            (F2 = class e {
                constructor(a) {
                    Ra.set(this, void 0),
                        ip.set(this, void 0),
                        d6(this, Ra, a.path, 'f'),
                        d6(this, ip, a.start || 0, 'f'),
                        (this.name = C9(dt(this, Ra, 'f'))),
                        (this.size = a.size),
                        (this.lastModified = a.lastModified)
                }
                slice(a, t) {
                    return new e({
                        path: dt(this, Ra, 'f'),
                        lastModified: this.lastModified,
                        size: t - a,
                        start: a
                    })
                }
                async *stream() {
                    let { mtimeMs: a } = await i6.stat(dt(this, Ra, 'f'))
                    if (a > this.lastModified)
                        throw new n6.default(E9, 'NotReadableError')
                    this.size &&
                        (yield* R9(dt(this, Ra, 'f'), {
                            start: dt(this, ip, 'f'),
                            end: dt(this, ip, 'f') + this.size - 1
                        }))
                }
                get [((Ra = new WeakMap()),
                (ip = new WeakMap()),
                Symbol.toStringTag)]() {
                    return 'File'
                }
            })
    })
var la = '0.3.2'
var o0 = !1,
    sa,
    Cd,
    Sl,
    Tl,
    Rl,
    Ed,
    Cl,
    xp,
    Pd,
    Nd,
    Dd,
    Lp,
    Ad
function l0(e, a = { auto: !1 }) {
    if (o0)
        throw new Error(
            `you must \`import 'groq-sdk/shims/${e.kind}'\` before importing anything else from groq-sdk`
        )
    if (sa)
        throw new Error(
            `can't \`import 'groq-sdk/shims/${e.kind}'\` after \`import 'groq-sdk/shims/${sa}'\``
        )
    ;(o0 = a.auto),
        (sa = e.kind),
        (Cd = e.fetch),
        (Sl = e.Request),
        (Tl = e.Response),
        (Rl = e.Headers),
        (Ed = e.FormData),
        (Cl = e.Blob),
        (xp = e.File),
        (Pd = e.ReadableStream),
        (Nd = e.getMultipartRequestOptions),
        (Dd = e.getDefaultAgent),
        (Lp = e.fileFromPath),
        (Ad = e.isFsReadStream)
}
var K1 = Et(G0(), 1)
Ht()
y2()
import { inspect as Hs } from 'util'
yr()
var So = (e) => e instanceof a1
br()
import { deprecate as Us } from 'util'
var To = Us(() => {},
'Constructor "entries" argument is not spec-compliant and will be removed in next major release.')
var t1 = function (e, a, t, r) {
        if (t === 'a' && !r)
            throw new TypeError('Private accessor was defined without a getter')
        if (typeof a == 'function' ? e !== a || !r : !a.has(e))
            throw new TypeError(
                'Cannot read private member from an object whose class did not declare it'
            )
        return t === 'm' ? r : t === 'a' ? r.call(e) : r ? r.value : a.get(e)
    },
    Tr,
    p1,
    g2,
    Rr = class {
        constructor(a) {
            Tr.add(this),
                p1.set(this, new Map()),
                a &&
                    (To(),
                    a.forEach(({ name: t, value: r, fileName: i }) =>
                        this.append(t, r, i)
                    ))
        }
        static [((p1 = new WeakMap()),
        (Tr = new WeakSet()),
        Symbol.hasInstance)](a) {
            return !!(
                a &&
                de(a.constructor) &&
                a[Symbol.toStringTag] === 'FormData' &&
                de(a.append) &&
                de(a.set) &&
                de(a.get) &&
                de(a.getAll) &&
                de(a.has) &&
                de(a.delete) &&
                de(a.entries) &&
                de(a.values) &&
                de(a.keys) &&
                de(a[Symbol.iterator]) &&
                de(a.forEach)
            )
        }
        append(a, t, r) {
            t1(this, Tr, 'm', g2).call(this, {
                name: a,
                fileName: r,
                append: !0,
                rawValue: t,
                argsLength: arguments.length
            })
        }
        set(a, t, r) {
            t1(this, Tr, 'm', g2).call(this, {
                name: a,
                fileName: r,
                append: !1,
                rawValue: t,
                argsLength: arguments.length
            })
        }
        get(a) {
            let t = t1(this, p1, 'f').get(String(a))
            return t ? t[0] : null
        }
        getAll(a) {
            let t = t1(this, p1, 'f').get(String(a))
            return t ? t.slice() : []
        }
        has(a) {
            return t1(this, p1, 'f').has(String(a))
        }
        delete(a) {
            t1(this, p1, 'f').delete(String(a))
        }
        *keys() {
            for (let a of t1(this, p1, 'f').keys()) yield a
        }
        *entries() {
            for (let a of this.keys()) {
                let t = this.getAll(a)
                for (let r of t) yield [a, r]
            }
        }
        *values() {
            for (let [, a] of this) yield a
        }
        [((g2 = function ({
            name: t,
            rawValue: r,
            append: i,
            fileName: o,
            argsLength: m
        }) {
            let u = i ? 'append' : 'set'
            if (m < 2)
                throw new TypeError(
                    `Failed to execute '${u}' on 'FormData': 2 arguments required, but only ${m} present.`
                )
            t = String(t)
            let s
            if (_2(r))
                s =
                    o === void 0
                        ? r
                        : new Qe([r], o, {
                              type: r.type,
                              lastModified: r.lastModified
                          })
            else if (So(r))
                s = new Qe([r], o === void 0 ? 'blob' : o, { type: r.type })
            else {
                if (o)
                    throw new TypeError(
                        `Failed to execute '${u}' on 'FormData': parameter 2 is not of type 'Blob'.`
                    )
                s = String(r)
            }
            let f = t1(this, p1, 'f').get(t)
            if (!f) return void t1(this, p1, 'f').set(t, [s])
            if (!i) return void t1(this, p1, 'f').set(t, [s])
            f.push(s)
        }),
        Symbol.iterator)]() {
            return this.entries()
        }
        forEach(a, t) {
            for (let [r, i] of this) a.call(t, i, r, this)
        }
        get [Symbol.toStringTag]() {
            return 'FormData'
        }
        [Hs.custom]() {
            return this[Symbol.toStringTag]
        }
    }
yr()
Ht()
var x2 = Et(Fo(), 1),
    u6 = Et(Yo(), 1)
import { ReadStream as D9 } from 'node:fs'
var Ko = 'abcdefghijklmnopqrstuvwxyz0123456789'
function f9() {
    let e = 16,
        a = ''
    for (; e--; ) a += Ko[(Math.random() * Ko.length) << 0]
    return a
}
var Xo = f9
var h9 = (e) => Object.prototype.toString.call(e).slice(8, -1).toLowerCase()
function v9(e) {
    if (h9(e) !== 'object') return !1
    let a = Object.getPrototypeOf(e)
    return a == null
        ? !0
        : (a.constructor && a.constructor.toString()) === Object.toString()
}
var q2 = v9
var b9 = (e) =>
        String(e).replace(/\r|\n/g, (a, t, r) =>
            (a === '\r' &&
                r[t + 1] !==
                    `
`) ||
            (a ===
                `
` &&
                r[t - 1] !== '\r')
                ? `\r
`
                : a
        ),
    B2 = b9
var w9 = (e) =>
        String(e)
            .replace(/\r/g, '%0D')
            .replace(/\n/g, '%0A')
            .replace(/"/g, '%22'),
    k2 = w9
var _9 = (e) => typeof e == 'function',
    R1 = _9
var Sa = (e) =>
    !!(
        e &&
        typeof e == 'object' &&
        R1(e.constructor) &&
        e[Symbol.toStringTag] === 'File' &&
        R1(e.stream) &&
        e.name != null &&
        e.size != null &&
        e.lastModified != null
    )
var Jo = (e) =>
    !!(
        e &&
        R1(e.constructor) &&
        e[Symbol.toStringTag] === 'FormData' &&
        R1(e.append) &&
        R1(e.getAll) &&
        R1(e.entries) &&
        R1(e[Symbol.iterator])
    )
var ap = function (e, a, t, r, i) {
        if (r === 'm') throw new TypeError('Private method is not writable')
        if (r === 'a' && !i)
            throw new TypeError('Private accessor was defined without a setter')
        if (typeof a == 'function' ? e !== a || !i : !a.has(e))
            throw new TypeError(
                'Cannot write private member to an object whose class did not declare it'
            )
        return r === 'a' ? i.call(e, t) : i ? (i.value = t) : a.set(e, t), t
    },
    ee = function (e, a, t, r) {
        if (t === 'a' && !r)
            throw new TypeError('Private accessor was defined without a getter')
        if (typeof a == 'function' ? e !== a || !r : !a.has(e))
            throw new TypeError(
                'Cannot read private member from an object whose class did not declare it'
            )
        return t === 'm' ? r : t === 'a' ? r.call(e) : r ? r.value : a.get(e)
    },
    qr,
    Y1,
    tp,
    Br,
    pp,
    Ta,
    rp,
    dp,
    kr,
    O2,
    y9 = { enableAdditionalHeaders: !1 },
    Or = class {
        constructor(a, t, r) {
            if (
                (qr.add(this),
                Y1.set(
                    this,
                    `\r
`
                ),
                tp.set(this, void 0),
                Br.set(this, void 0),
                pp.set(this, '-'.repeat(2)),
                Ta.set(this, new TextEncoder()),
                rp.set(this, void 0),
                dp.set(this, void 0),
                kr.set(this, void 0),
                !Jo(a))
            )
                throw new TypeError(
                    'Expected first argument to be a FormData instance.'
                )
            let i
            if (
                (q2(t) ? (r = t) : (i = t),
                i || (i = Xo()),
                typeof i != 'string')
            )
                throw new TypeError(
                    'Expected boundary argument to be a string.'
                )
            if (r && !q2(r))
                throw new TypeError(
                    'Expected options argument to be an object.'
                )
            ap(this, dp, a, 'f'),
                ap(this, kr, { ...y9, ...r }, 'f'),
                ap(this, tp, ee(this, Ta, 'f').encode(ee(this, Y1, 'f')), 'f'),
                ap(this, Br, ee(this, tp, 'f').byteLength, 'f'),
                (this.boundary = `form-data-boundary-${i}`),
                (this.contentType = `multipart/form-data; boundary=${this.boundary}`),
                ap(
                    this,
                    rp,
                    ee(this, Ta, 'f').encode(
                        `${ee(this, pp, 'f')}${this.boundary}${ee(
                            this,
                            pp,
                            'f'
                        )}${ee(this, Y1, 'f').repeat(2)}`
                    ),
                    'f'
                ),
                (this.contentLength = String(this.getContentLength())),
                (this.headers = Object.freeze({
                    'Content-Type': this.contentType,
                    'Content-Length': this.contentLength
                })),
                Object.defineProperties(this, {
                    boundary: { writable: !1, configurable: !1 },
                    contentType: { writable: !1, configurable: !1 },
                    contentLength: { writable: !1, configurable: !1 },
                    headers: { writable: !1, configurable: !1 }
                })
        }
        getContentLength() {
            let a = 0
            for (let [t, r] of ee(this, dp, 'f')) {
                let i = Sa(r) ? r : ee(this, Ta, 'f').encode(B2(r))
                ;(a += ee(this, qr, 'm', O2).call(this, t, i).byteLength),
                    (a += Sa(i) ? i.size : i.byteLength),
                    (a += ee(this, Br, 'f'))
            }
            return a + ee(this, rp, 'f').byteLength
        }
        *values() {
            for (let [a, t] of ee(this, dp, 'f').entries()) {
                let r = Sa(t) ? t : ee(this, Ta, 'f').encode(B2(t))
                yield ee(this, qr, 'm', O2).call(this, a, r),
                    yield r,
                    yield ee(this, tp, 'f')
            }
            yield ee(this, rp, 'f')
        }
        async *encode() {
            for (let a of this.values()) Sa(a) ? yield* a.stream() : yield a
        }
        [((Y1 = new WeakMap()),
        (tp = new WeakMap()),
        (Br = new WeakMap()),
        (pp = new WeakMap()),
        (Ta = new WeakMap()),
        (rp = new WeakMap()),
        (dp = new WeakMap()),
        (kr = new WeakMap()),
        (qr = new WeakSet()),
        (O2 = function (t, r) {
            let i = ''
            return (
                (i += `${ee(this, pp, 'f')}${this.boundary}${ee(
                    this,
                    Y1,
                    'f'
                )}`),
                (i += `Content-Disposition: form-data; name="${k2(t)}"`),
                Sa(r) &&
                    ((i += `; filename="${k2(r.name)}"${ee(this, Y1, 'f')}`),
                    (i += `Content-Type: ${
                        r.type || 'application/octet-stream'
                    }`)),
                ee(this, kr, 'f').enableAdditionalHeaders === !0 &&
                    (i += `${ee(this, Y1, 'f')}Content-Length: ${
                        Sa(r) ? r.size : r.byteLength
                    }`),
                ee(this, Ta, 'f').encode(`${i}${ee(this, Y1, 'f').repeat(2)}`)
            )
        }),
        Symbol.iterator)]() {
            return this.values()
        }
        [Symbol.asyncIterator]() {
            return this.encode()
        }
    }
import { Readable as A9 } from 'node:stream'
var Fr = class {
    constructor(a) {
        this.body = a
    }
    get [Symbol.toStringTag]() {
        return 'MultipartBody'
    }
}
var c6 = Et(e6(), 1),
    m6 = !1
async function V9(e, ...a) {
    let { fileFromPath: t } = await Promise.resolve().then(() => (s6(), l6))
    return (
        m6 ||
            (console.warn(
                `fileFromPath is deprecated; use fs.createReadStream(${JSON.stringify(
                    e
                )}) instead`
            ),
            (m6 = !0)),
        await t(e, ...a)
    )
}
var q9 = new x2.default({ keepAlive: !0, timeout: 5 * 60 * 1e3 }),
    B9 = new x2.default.HttpsAgent({ keepAlive: !0, timeout: 5 * 60 * 1e3 })
async function k9(e, a) {
    let t = new Or(e),
        r = A9.from(t),
        i = new Fr(r),
        o = { ...a.headers, ...t.headers, 'Content-Length': t.contentLength }
    return { ...a, body: i, headers: o }
}
function f6() {
    return (
        typeof AbortController > 'u' &&
            (globalThis.AbortController = u6.AbortController),
        {
            kind: 'node',
            fetch: K1.default,
            Request: K1.Request,
            Response: K1.Response,
            Headers: K1.Headers,
            FormData: Rr,
            Blob: a1,
            File: Qe,
            ReadableStream: c6.ReadableStream,
            getMultipartRequestOptions: k9,
            getDefaultAgent: (e) => (e.startsWith('https') ? B9 : q9),
            fileFromPath: V9,
            isFsReadStream: (e) => e instanceof D9
        }
    )
}
sa || l0(f6(), { auto: !0 })
var L2 = {}
n0(L2, {
    APIConnectionError: () => C1,
    APIConnectionTimeoutError: () => Ca,
    APIError: () => ue,
    APIUserAbortError: () => X1,
    AuthenticationError: () => nt,
    BadRequestError: () => it,
    ConflictError: () => st,
    GroqError: () => he,
    InternalServerError: () => ct,
    NotFoundError: () => lt,
    PermissionDeniedError: () => ot,
    RateLimitError: () => ut,
    UnprocessableEntityError: () => mt
})
var he = class extends Error {},
    ue = class e extends he {
        constructor(a, t, r, i) {
            super(`${e.makeMessage(a, t, r)}`),
                (this.status = a),
                (this.headers = i),
                (this.error = t)
        }
        static makeMessage(a, t, r) {
            let i = t?.message
                ? typeof t.message == 'string'
                    ? t.message
                    : JSON.stringify(t.message)
                : t
                ? JSON.stringify(t)
                : r
            return a && i
                ? `${a} ${i}`
                : a
                ? `${a} status code (no body)`
                : i || '(no status code or body)'
        }
        static generate(a, t, r, i) {
            if (!a) return new C1({ cause: Lr(t) })
            let o = t
            return a === 400
                ? new it(a, o, r, i)
                : a === 401
                ? new nt(a, o, r, i)
                : a === 403
                ? new ot(a, o, r, i)
                : a === 404
                ? new lt(a, o, r, i)
                : a === 409
                ? new st(a, o, r, i)
                : a === 422
                ? new mt(a, o, r, i)
                : a === 429
                ? new ut(a, o, r, i)
                : a >= 500
                ? new ct(a, o, r, i)
                : new e(a, o, r, i)
        }
    },
    X1 = class extends ue {
        constructor({ message: a } = {}) {
            super(void 0, void 0, a || 'Request was aborted.', void 0),
                (this.status = void 0)
        }
    },
    C1 = class extends ue {
        constructor({ message: a, cause: t }) {
            super(void 0, void 0, a || 'Connection error.', void 0),
                (this.status = void 0),
                t && (this.cause = t)
        }
    },
    Ca = class extends C1 {
        constructor({ message: a } = {}) {
            super({ message: a ?? 'Request timed out.' })
        }
    },
    it = class extends ue {
        constructor() {
            super(...arguments), (this.status = 400)
        }
    },
    nt = class extends ue {
        constructor() {
            super(...arguments), (this.status = 401)
        }
    },
    ot = class extends ue {
        constructor() {
            super(...arguments), (this.status = 403)
        }
    },
    lt = class extends ue {
        constructor() {
            super(...arguments), (this.status = 404)
        }
    },
    st = class extends ue {
        constructor() {
            super(...arguments), (this.status = 409)
        }
    },
    mt = class extends ue {
        constructor() {
            super(...arguments), (this.status = 422)
        }
    },
    ut = class extends ue {
        constructor() {
            super(...arguments), (this.status = 429)
        }
    },
    ct = class extends ue {}
var Ir = class e {
        constructor(a, t) {
            ;(this.iterator = a), (this.controller = t)
        }
        static fromSSEResponse(a, t) {
            let r = !1,
                i = new I2()
            async function* o() {
                if (!a.body)
                    throw (
                        (t.abort(),
                        new he(
                            'Attempted to iterate over a response with no body'
                        ))
                    )
                let u = new ft(),
                    s = h6(a.body)
                for await (let f of s)
                    for (let h of u.decode(f)) {
                        let _ = i.decode(h)
                        _ && (yield _)
                    }
                for (let f of u.flush()) {
                    let h = i.decode(f)
                    h && (yield h)
                }
            }
            async function* m() {
                if (r)
                    throw new Error(
                        'Cannot iterate over a consumed stream, use `.tee()` to split the stream.'
                    )
                r = !0
                let u = !1
                try {
                    for await (let s of o())
                        if (!u) {
                            if (s.data.startsWith('[DONE]')) {
                                u = !0
                                continue
                            }
                            if (s.event === null) {
                                let f
                                try {
                                    f = JSON.parse(s.data)
                                } catch (h) {
                                    throw (
                                        (console.error(
                                            'Could not parse message into JSON:',
                                            s.data
                                        ),
                                        console.error('From chunk:', s.raw),
                                        h)
                                    )
                                }
                                if (f && f.error)
                                    throw new ue(
                                        void 0,
                                        f.error,
                                        void 0,
                                        void 0
                                    )
                                yield f
                            }
                        }
                    u = !0
                } catch (s) {
                    if (s instanceof Error && s.name === 'AbortError') return
                    throw s
                } finally {
                    u || t.abort()
                }
            }
            return new e(m, t)
        }
        static fromReadableStream(a, t) {
            let r = !1
            async function* i() {
                let m = new ft(),
                    u = h6(a)
                for await (let s of u) for (let f of m.decode(s)) yield f
                for (let s of m.flush()) yield s
            }
            async function* o() {
                if (r)
                    throw new Error(
                        'Cannot iterate over a consumed stream, use `.tee()` to split the stream.'
                    )
                r = !0
                let m = !1
                try {
                    for await (let u of i()) m || (u && (yield JSON.parse(u)))
                    m = !0
                } catch (u) {
                    if (u instanceof Error && u.name === 'AbortError') return
                    throw u
                } finally {
                    m || t.abort()
                }
            }
            return new e(o, t)
        }
        [Symbol.asyncIterator]() {
            return this.iterator()
        }
        tee() {
            let a = [],
                t = [],
                r = this.iterator(),
                i = (o) => ({
                    next: () => {
                        if (o.length === 0) {
                            let m = r.next()
                            a.push(m), t.push(m)
                        }
                        return o.shift()
                    }
                })
            return [
                new e(() => i(a), this.controller),
                new e(() => i(t), this.controller)
            ]
        }
        toReadableStream() {
            let a = this,
                t,
                r = new TextEncoder()
            return new Pd({
                async start() {
                    t = a[Symbol.asyncIterator]()
                },
                async pull(i) {
                    try {
                        let { value: o, done: m } = await t.next()
                        if (m) return i.close()
                        let u = r.encode(
                            JSON.stringify(o) +
                                `
`
                        )
                        i.enqueue(u)
                    } catch (o) {
                        i.error(o)
                    }
                },
                async cancel() {
                    await t.return?.()
                }
            })
        }
    },
    I2 = class {
        constructor() {
            ;(this.event = null), (this.data = []), (this.chunks = [])
        }
        decode(a) {
            if ((a.endsWith('\r') && (a = a.substring(0, a.length - 1)), !a)) {
                if (!this.event && !this.data.length) return null
                let o = {
                    event: this.event,
                    data: this.data.join(`
`),
                    raw: this.chunks
                }
                return (
                    (this.event = null), (this.data = []), (this.chunks = []), o
                )
            }
            if ((this.chunks.push(a), a.startsWith(':'))) return null
            let [t, r, i] = O9(a, ':')
            return (
                i.startsWith(' ') && (i = i.substring(1)),
                t === 'event'
                    ? (this.event = i)
                    : t === 'data' && this.data.push(i),
                null
            )
        }
    },
    ft = class e {
        constructor() {
            ;(this.buffer = []), (this.trailingCR = !1)
        }
        decode(a) {
            let t = this.decodeText(a)
            if (
                (this.trailingCR && ((t = '\r' + t), (this.trailingCR = !1)),
                t.endsWith('\r') &&
                    ((this.trailingCR = !0), (t = t.slice(0, -1))),
                !t)
            )
                return []
            let r = e.NEWLINE_CHARS.has(t[t.length - 1] || ''),
                i = t.split(e.NEWLINE_REGEXP)
            return i.length === 1 && !r
                ? (this.buffer.push(i[0]), [])
                : (this.buffer.length > 0 &&
                      ((i = [this.buffer.join('') + i[0], ...i.slice(1)]),
                      (this.buffer = [])),
                  r || (this.buffer = [i.pop() || '']),
                  i)
        }
        decodeText(a) {
            if (a == null) return ''
            if (typeof a == 'string') return a
            if (typeof Buffer < 'u') {
                if (a instanceof Buffer) return a.toString()
                if (a instanceof Uint8Array) return Buffer.from(a).toString()
                throw new he(
                    `Unexpected: received non-Uint8Array (${a.constructor.name}) stream chunk in an environment with a global "Buffer" defined, which this library assumes to be Node. Please report this error.`
                )
            }
            if (typeof TextDecoder < 'u') {
                if (a instanceof Uint8Array || a instanceof ArrayBuffer)
                    return (
                        this.textDecoder ??
                            (this.textDecoder = new TextDecoder('utf8')),
                        this.textDecoder.decode(a)
                    )
                throw new he(
                    `Unexpected: received non-Uint8Array/ArrayBuffer (${a.constructor.name}) in a web platform. Please report this error.`
                )
            }
            throw new he(
                'Unexpected: neither Buffer nor TextDecoder are available as globals. Please report this error.'
            )
        }
        flush() {
            if (!this.buffer.length && !this.trailingCR) return []
            let a = [this.buffer.join('')]
            return (this.buffer = []), (this.trailingCR = !1), a
        }
    }
ft.NEWLINE_CHARS = new Set([
    `
`,
    '\r',
    '\v',
    '\f',
    '',
    '',
    '',
    '\x85',
    '\u2028',
    '\u2029'
])
ft.NEWLINE_REGEXP = /\r\n|[\n\r\x0b\x0c\x1c\x1d\x1e\x85\u2028\u2029]/g
function O9(e, a) {
    let t = e.indexOf(a)
    return t !== -1
        ? [e.substring(0, t), a, e.substring(t + a.length)]
        : [e, '', '']
}
function h6(e) {
    if (e[Symbol.asyncIterator]) return e
    let a = e.getReader()
    return {
        async next() {
            try {
                let t = await a.read()
                return t?.done && a.releaseLock(), t
            } catch (t) {
                throw (a.releaseLock(), t)
            }
        },
        async return() {
            let t = a.cancel()
            return a.releaseLock(), await t, { done: !0, value: void 0 }
        },
        [Symbol.asyncIterator]() {
            return this
        }
    }
}
var v6 = (e) =>
        e != null &&
        typeof e == 'object' &&
        typeof e.url == 'string' &&
        typeof e.blob == 'function',
    F9 = (e) =>
        e != null &&
        typeof e == 'object' &&
        typeof e.name == 'string' &&
        typeof e.lastModified == 'number' &&
        b6(e),
    b6 = (e) =>
        e != null &&
        typeof e == 'object' &&
        typeof e.size == 'number' &&
        typeof e.type == 'string' &&
        typeof e.text == 'function' &&
        typeof e.slice == 'function' &&
        typeof e.arrayBuffer == 'function',
    x9 = (e) => F9(e) || v6(e) || Ad(e)
async function z2(e, a, t = {}) {
    if (((e = await e), v6(e))) {
        let i = await e.blob()
        return (
            a ||
                (a =
                    new URL(e.url).pathname.split(/[\\/]/).pop() ??
                    'unknown_file'),
            new xp([i], a, t)
        )
    }
    let r = await L9(e)
    if ((a || (a = j9(e) ?? 'unknown_file'), !t.type)) {
        let i = r[0]?.type
        typeof i == 'string' && (t = { ...t, type: i })
    }
    return new xp(r, a, t)
}
async function L9(e) {
    let a = []
    if (
        typeof e == 'string' ||
        ArrayBuffer.isView(e) ||
        e instanceof ArrayBuffer
    )
        a.push(e)
    else if (b6(e)) a.push(await e.arrayBuffer())
    else if (W9(e)) for await (let t of e) a.push(t)
    else
        throw new Error(
            `Unexpected data type: ${typeof e}; constructor: ${
                e?.constructor?.name
            }; props: ${I9(e)}`
        )
    return a
}
function I9(e) {
    return `[${Object.getOwnPropertyNames(e)
        .map((t) => `"${t}"`)
        .join(', ')}]`
}
function j9(e) {
    return j2(e.name) || j2(e.filename) || j2(e.path)?.split(/[\\/]/).pop()
}
var j2 = (e) => {
        if (typeof e == 'string') return e
        if (typeof Buffer < 'u' && e instanceof Buffer) return String(e)
    },
    W9 = (e) =>
        e != null &&
        typeof e == 'object' &&
        typeof e[Symbol.asyncIterator] == 'function',
    M2 = (e) =>
        e &&
        typeof e == 'object' &&
        e.body &&
        e[Symbol.toStringTag] === 'MultipartBody'
var np = async (e) => {
        let a = await w6(e.body)
        return Nd(a, e)
    },
    w6 = async (e) => {
        let a = new Ed()
        return (
            await Promise.all(
                Object.entries(e || {}).map(([t, r]) => W2(a, t, r))
            ),
            a
        )
    }
var W2 = async (e, a, t) => {
    if (t !== void 0) {
        if (t == null)
            throw new TypeError(
                `Received null for "${a}"; to pass null in FormData, you must use the string 'null'`
            )
        if (
            typeof t == 'string' ||
            typeof t == 'number' ||
            typeof t == 'boolean'
        )
            e.append(a, String(t))
        else if (x9(t)) {
            let r = await z2(t)
            e.append(a, r)
        } else if (Array.isArray(t))
            await Promise.all(t.map((r) => W2(e, a + '[]', r)))
        else if (typeof t == 'object')
            await Promise.all(
                Object.entries(t).map(([r, i]) => W2(e, `${a}[${r}]`, i))
            )
        else
            throw new TypeError(
                `Invalid value given to form, expected a string, number, boolean, object, Array, File or Blob but got ${t} instead`
            )
    }
}
var M9 = function (e, a, t, r, i) {
        if (r === 'm') throw new TypeError('Private method is not writable')
        if (r === 'a' && !i)
            throw new TypeError('Private accessor was defined without a setter')
        if (typeof a == 'function' ? e !== a || !i : !a.has(e))
            throw new TypeError(
                'Cannot write private member to an object whose class did not declare it'
            )
        return r === 'a' ? i.call(e, t) : i ? (i.value = t) : a.set(e, t), t
    },
    $9 = function (e, a, t, r) {
        if (t === 'a' && !r)
            throw new TypeError('Private accessor was defined without a getter')
        if (typeof a == 'function' ? e !== a || !r : !a.has(e))
            throw new TypeError(
                'Cannot read private member from an object whose class did not declare it'
            )
        return t === 'm' ? r : t === 'a' ? r.call(e) : r ? r.value : a.get(e)
    },
    jr
async function R6(e) {
    let { response: a } = e
    if (e.options.stream)
        return (
            ht('response', a.status, a.url, a.headers, a.body),
            e.options.__streamClass
                ? e.options.__streamClass.fromSSEResponse(a, e.controller)
                : Ir.fromSSEResponse(a, e.controller)
        )
    if (a.status === 204) return null
    if (e.options.__binaryResponse) return a
    let t = a.headers.get('content-type')
    if (
        t?.includes('application/json') ||
        t?.includes('application/vnd.api+json')
    ) {
        let o = await a.json()
        return ht('response', a.status, a.url, a.headers, o), o
    }
    let i = await a.text()
    return ht('response', a.status, a.url, a.headers, i), i
}
var Wr = class e extends Promise {
        constructor(a, t = R6) {
            super((r) => {
                r(null)
            }),
                (this.responsePromise = a),
                (this.parseResponse = t)
        }
        _thenUnwrap(a) {
            return new e(this.responsePromise, async (t) =>
                a(await this.parseResponse(t))
            )
        }
        asResponse() {
            return this.responsePromise.then((a) => a.response)
        }
        async withResponse() {
            let [a, t] = await Promise.all([this.parse(), this.asResponse()])
            return { data: a, response: t }
        }
        parse() {
            return (
                this.parsedPromise ||
                    (this.parsedPromise = this.responsePromise.then(
                        this.parseResponse
                    )),
                this.parsedPromise
            )
        }
        then(a, t) {
            return this.parse().then(a, t)
        }
        catch(a) {
            return this.parse().catch(a)
        }
        finally(a) {
            return this.parse().finally(a)
        }
    },
    zr = class {
        constructor({
            baseURL: a,
            maxRetries: t = 2,
            timeout: r = 6e4,
            httpAgent: i,
            fetch: o
        }) {
            ;(this.baseURL = a),
                (this.maxRetries = $2('maxRetries', t)),
                (this.timeout = $2('timeout', r)),
                (this.httpAgent = i),
                (this.fetch = o ?? Cd)
        }
        authHeaders(a) {
            return {}
        }
        defaultHeaders(a) {
            return {
                Accept: 'application/json',
                'Content-Type': 'application/json',
                'User-Agent': this.getUserAgent(),
                ...G9(),
                ...this.authHeaders(a)
            }
        }
        validateHeaders(a, t) {}
        defaultIdempotencyKey() {
            return `stainless-node-retry-${a3()}`
        }
        get(a, t) {
            return this.methodRequest('get', a, t)
        }
        post(a, t) {
            return this.methodRequest('post', a, t)
        }
        patch(a, t) {
            return this.methodRequest('patch', a, t)
        }
        put(a, t) {
            return this.methodRequest('put', a, t)
        }
        delete(a, t) {
            return this.methodRequest('delete', a, t)
        }
        methodRequest(a, t, r) {
            return this.request(
                Promise.resolve(r).then((i) => ({ method: a, path: t, ...i }))
            )
        }
        getAPIList(a, t, r) {
            return this.requestAPIList(t, { method: 'get', path: a, ...r })
        }
        calculateContentLength(a) {
            if (typeof a == 'string') {
                if (typeof Buffer < 'u')
                    return Buffer.byteLength(a, 'utf8').toString()
                if (typeof TextEncoder < 'u')
                    return new TextEncoder().encode(a).length.toString()
            }
            return null
        }
        buildRequest(a) {
            let { method: t, path: r, query: i, headers: o = {} } = a,
                m = M2(a.body)
                    ? a.body.body
                    : a.body
                    ? JSON.stringify(a.body, null, 2)
                    : null,
                u = this.calculateContentLength(m),
                s = this.buildURL(r, i)
            'timeout' in a && $2('timeout', a.timeout)
            let f = a.timeout ?? this.timeout,
                h = a.httpAgent ?? this.httpAgent ?? Dd(s),
                _ = f + 1e3
            typeof h?.options?.timeout == 'number' &&
                _ > (h.options.timeout ?? 0) &&
                (h.options.timeout = _),
                this.idempotencyHeader &&
                    t !== 'get' &&
                    (a.idempotencyKey ||
                        (a.idempotencyKey = this.defaultIdempotencyKey()),
                    (o[this.idempotencyHeader] = a.idempotencyKey))
            let w = this.buildHeaders({
                options: a,
                headers: o,
                contentLength: u
            })
            return {
                req: {
                    method: t,
                    ...(m && { body: m }),
                    headers: w,
                    ...(h && { agent: h }),
                    signal: a.signal ?? null
                },
                url: s,
                timeout: f
            }
        }
        buildHeaders({ options: a, headers: t, contentLength: r }) {
            let i = {}
            r && (i['content-length'] = r)
            let o = this.defaultHeaders(a)
            return (
                T6(i, o),
                T6(i, t),
                M2(a.body) && sa !== 'node' && delete i['content-type'],
                this.validateHeaders(i, t),
                i
            )
        }
        async prepareOptions(a) {}
        async prepareRequest(a, { url: t, options: r }) {}
        parseHeaders(a) {
            return a
                ? Symbol.iterator in a
                    ? Object.fromEntries(Array.from(a).map((t) => [...t]))
                    : { ...a }
                : {}
        }
        makeStatusError(a, t, r, i) {
            return ue.generate(a, t, r, i)
        }
        request(a, t = null) {
            return new Wr(this.makeRequest(a, t))
        }
        async makeRequest(a, t) {
            let r = await a
            t == null && (t = r.maxRetries ?? this.maxRetries),
                await this.prepareOptions(r)
            let { req: i, url: o, timeout: m } = this.buildRequest(r)
            if (
                (await this.prepareRequest(i, { url: o, options: r }),
                ht('request', o, r, i.headers),
                r.signal?.aborted)
            )
                throw new X1()
            let u = new AbortController(),
                s = await this.fetchWithTimeout(o, i, m, u).catch(Lr)
            if (s instanceof Error) {
                if (r.signal?.aborted) throw new X1()
                if (t) return this.retryRequest(r, t)
                throw s.name === 'AbortError' ? new Ca() : new C1({ cause: s })
            }
            let f = U9(s.headers)
            if (!s.ok) {
                if (t && this.shouldRetry(s)) {
                    let C = `retrying, ${t} attempts remaining`
                    return (
                        ht(`response (error; ${C})`, s.status, o, f),
                        this.retryRequest(r, t, f)
                    )
                }
                let h = await s.text().catch((C) => Lr(C).message),
                    _ = Y9(h),
                    w = _ ? void 0 : h
                throw (
                    (ht(
                        `response (error; ${
                            t
                                ? '(error; no more retries left)'
                                : '(error; not retryable)'
                        })`,
                        s.status,
                        o,
                        f,
                        w
                    ),
                    this.makeStatusError(s.status, _, w, f))
                )
            }
            return { response: s, options: r, controller: u }
        }
        requestAPIList(a, t) {
            let r = this.makeRequest(t, null)
            return new U2(this, r, a)
        }
        buildURL(a, t) {
            let r = X9(a)
                    ? new URL(a)
                    : new URL(
                          this.baseURL +
                              (this.baseURL.endsWith('/') && a.startsWith('/')
                                  ? a.slice(1)
                                  : a)
                      ),
                i = this.defaultQuery()
            return (
                Z9(i) || (t = { ...i, ...t }),
                typeof t == 'object' &&
                    t &&
                    !Array.isArray(t) &&
                    (r.search = this.stringifyQuery(t)),
                r.toString()
            )
        }
        stringifyQuery(a) {
            return Object.entries(a)
                .filter(([t, r]) => typeof r < 'u')
                .map(([t, r]) => {
                    if (
                        typeof r == 'string' ||
                        typeof r == 'number' ||
                        typeof r == 'boolean'
                    )
                        return `${encodeURIComponent(t)}=${encodeURIComponent(
                            r
                        )}`
                    if (r === null) return `${encodeURIComponent(t)}=`
                    throw new he(
                        `Cannot stringify type ${typeof r}; Expected string, number, boolean, or null. If you need to pass nested query parameters, you can manually encode them, e.g. { query: { 'foo[key1]': value1, 'foo[key2]': value2 } }, and please open a GitHub issue requesting better support for your use case.`
                    )
                })
                .join('&')
        }
        async fetchWithTimeout(a, t, r, i) {
            let { signal: o, ...m } = t || {}
            o && o.addEventListener('abort', () => i.abort())
            let u = setTimeout(() => i.abort(), r)
            return this.getRequestClient()
                .fetch.call(void 0, a, { signal: i.signal, ...m })
                .finally(() => {
                    clearTimeout(u)
                })
        }
        getRequestClient() {
            return { fetch: this.fetch }
        }
        shouldRetry(a) {
            let t = a.headers.get('x-should-retry')
            return t === 'true'
                ? !0
                : t === 'false'
                ? !1
                : a.status === 408 ||
                  a.status === 409 ||
                  a.status === 429 ||
                  a.status >= 500
        }
        async retryRequest(a, t, r) {
            let i,
                o = r?.['retry-after-ms']
            if (o) {
                let u = parseFloat(o)
                Number.isNaN(u) || (i = u)
            }
            let m = r?.['retry-after']
            if (m && !i) {
                let u = parseFloat(m)
                Number.isNaN(u)
                    ? (i = Date.parse(m) - Date.now())
                    : (i = u * 1e3)
            }
            if (!(i && 0 <= i && i < 60 * 1e3)) {
                let u = a.maxRetries ?? this.maxRetries
                i = this.calculateDefaultRetryTimeoutMillis(t, u)
            }
            return await J9(i), this.makeRequest(a, t - 1)
        }
        calculateDefaultRetryTimeoutMillis(a, t) {
            let o = t - a,
                m = Math.min(0.5 * Math.pow(2, o), 8),
                u = 1 - Math.random() * 0.25
            return m * u * 1e3
        }
        getUserAgent() {
            return `${this.constructor.name}/JS ${la}`
        }
    },
    _6 = class {
        constructor(a, t, r, i) {
            jr.set(this, void 0),
                M9(this, jr, a, 'f'),
                (this.options = i),
                (this.response = t),
                (this.body = r)
        }
        hasNextPage() {
            return this.getPaginatedItems().length
                ? this.nextPageInfo() != null
                : !1
        }
        async getNextPage() {
            let a = this.nextPageInfo()
            if (!a)
                throw new he(
                    'No next page expected; please check `.hasNextPage()` before calling `.getNextPage()`.'
                )
            let t = { ...this.options }
            if ('params' in a && typeof t.query == 'object')
                t.query = { ...t.query, ...a.params }
            else if ('url' in a) {
                let r = [
                    ...Object.entries(t.query || {}),
                    ...a.url.searchParams.entries()
                ]
                for (let [i, o] of r) a.url.searchParams.set(i, o)
                ;(t.query = void 0), (t.path = a.url.toString())
            }
            return await $9(this, jr, 'f').requestAPIList(this.constructor, t)
        }
        async *iterPages() {
            let a = this
            for (yield a; a.hasNextPage(); )
                (a = await a.getNextPage()), yield a
        }
        async *[((jr = new WeakMap()), Symbol.asyncIterator)]() {
            for await (let a of this.iterPages())
                for (let t of a.getPaginatedItems()) yield t
        }
    },
    U2 = class extends Wr {
        constructor(a, t, r) {
            super(t, async (i) => new r(a, i.response, await R6(i), i.options))
        }
        async *[Symbol.asyncIterator]() {
            let a = await this
            for await (let t of a) yield t
        }
    },
    U9 = (e) =>
        new Proxy(Object.fromEntries(e.entries()), {
            get(a, t) {
                let r = t.toString()
                return a[r.toLowerCase()] || a[r]
            }
        })
var H9 = () => {
    if (typeof Deno < 'u' && Deno.build != null)
        return {
            'X-Stainless-Lang': 'js',
            'X-Stainless-Package-Version': la,
            'X-Stainless-OS': g6(Deno.build.os),
            'X-Stainless-Arch': y6(Deno.build.arch),
            'X-Stainless-Runtime': 'deno',
            'X-Stainless-Runtime-Version': Deno.version
        }
    if (typeof EdgeRuntime < 'u')
        return {
            'X-Stainless-Lang': 'js',
            'X-Stainless-Package-Version': la,
            'X-Stainless-OS': 'Unknown',
            'X-Stainless-Arch': `other:${EdgeRuntime}`,
            'X-Stainless-Runtime': 'edge',
            'X-Stainless-Runtime-Version': process.version
        }
    if (
        Object.prototype.toString.call(typeof process < 'u' ? process : 0) ===
        '[object process]'
    )
        return {
            'X-Stainless-Lang': 'js',
            'X-Stainless-Package-Version': la,
            'X-Stainless-OS': g6(process.platform),
            'X-Stainless-Arch': y6(process.arch),
            'X-Stainless-Runtime': 'node',
            'X-Stainless-Runtime-Version': process.version
        }
    let e = Q9()
    return e
        ? {
              'X-Stainless-Lang': 'js',
              'X-Stainless-Package-Version': la,
              'X-Stainless-OS': 'Unknown',
              'X-Stainless-Arch': 'unknown',
              'X-Stainless-Runtime': `browser:${e.browser}`,
              'X-Stainless-Runtime-Version': e.version
          }
        : {
              'X-Stainless-Lang': 'js',
              'X-Stainless-Package-Version': la,
              'X-Stainless-OS': 'Unknown',
              'X-Stainless-Arch': 'unknown',
              'X-Stainless-Runtime': 'unknown',
              'X-Stainless-Runtime-Version': 'unknown'
          }
}
function Q9() {
    if (typeof navigator > 'u' || !navigator) return null
    let e = [
        { key: 'edge', pattern: /Edge(?:\W+(\d+)\.(\d+)(?:\.(\d+))?)?/ },
        { key: 'ie', pattern: /MSIE(?:\W+(\d+)\.(\d+)(?:\.(\d+))?)?/ },
        { key: 'ie', pattern: /Trident(?:.*rv\:(\d+)\.(\d+)(?:\.(\d+))?)?/ },
        { key: 'chrome', pattern: /Chrome(?:\W+(\d+)\.(\d+)(?:\.(\d+))?)?/ },
        { key: 'firefox', pattern: /Firefox(?:\W+(\d+)\.(\d+)(?:\.(\d+))?)?/ },
        {
            key: 'safari',
            pattern:
                /(?:Version\W+(\d+)\.(\d+)(?:\.(\d+))?)?(?:\W+Mobile\S*)?\W+Safari/
        }
    ]
    for (let { key: a, pattern: t } of e) {
        let r = t.exec(navigator.userAgent)
        if (r) {
            let i = r[1] || 0,
                o = r[2] || 0,
                m = r[3] || 0
            return { browser: a, version: `${i}.${o}.${m}` }
        }
    }
    return null
}
var y6 = (e) =>
        e === 'x32'
            ? 'x32'
            : e === 'x86_64' || e === 'x64'
            ? 'x64'
            : e === 'arm'
            ? 'arm'
            : e === 'aarch64' || e === 'arm64'
            ? 'arm64'
            : e
            ? `other:${e}`
            : 'unknown',
    g6 = (e) => (
        (e = e.toLowerCase()),
        e.includes('ios')
            ? 'iOS'
            : e === 'android'
            ? 'Android'
            : e === 'darwin'
            ? 'MacOS'
            : e === 'win32'
            ? 'Windows'
            : e === 'freebsd'
            ? 'FreeBSD'
            : e === 'openbsd'
            ? 'OpenBSD'
            : e === 'linux'
            ? 'Linux'
            : e
            ? `Other:${e}`
            : 'Unknown'
    ),
    S6,
    G9 = () => S6 ?? (S6 = H9()),
    Y9 = (e) => {
        try {
            return JSON.parse(e)
        } catch {
            return
        }
    },
    K9 = new RegExp('^(?:[a-z]+:)?//', 'i'),
    X9 = (e) => K9.test(e),
    J9 = (e) => new Promise((a) => setTimeout(a, e)),
    $2 = (e, a) => {
        if (typeof a != 'number' || !Number.isInteger(a))
            throw new he(`${e} must be an integer`)
        if (a < 0) throw new he(`${e} must be a positive integer`)
        return a
    },
    Lr = (e) => (e instanceof Error ? e : new Error(e))
var H2 = (e) => {
    if (typeof process < 'u') return process.env?.[e]?.trim() ?? void 0
    if (typeof Deno < 'u') return Deno.env?.get?.(e)?.trim()
}
function Z9(e) {
    if (!e) return !0
    for (let a in e) return !1
    return !0
}
function e3(e, a) {
    return Object.prototype.hasOwnProperty.call(e, a)
}
function T6(e, a) {
    for (let t in a) {
        if (!e3(a, t)) continue
        let r = t.toLowerCase()
        if (!r) continue
        let i = a[t]
        i === null ? delete e[r] : i !== void 0 && (e[r] = i)
    }
}
function ht(e, ...a) {
    typeof process < 'u' &&
        process.env.DEBUG === 'true' &&
        console.log(`Groq:DEBUG:${e}`, ...a)
}
var a3 = () =>
        'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (e) => {
            let a = (Math.random() * 16) | 0
            return (e === 'x' ? a : (a & 3) | 8).toString(16)
        }),
    C6 = () =>
        typeof window < 'u' &&
        typeof window.document < 'u' &&
        typeof navigator < 'u'
var Ne = class {
    constructor(a) {
        this._client = a
    }
}
var Ea = class extends Ne {
    create(a, t) {
        return this._client.post('/openai/v1/chat/completions', {
            body: a,
            ...t,
            stream: a.stream ?? !1
        })
    }
}
Ea || (Ea = {})
var J1 = class extends Ne {
    constructor() {
        super(...arguments), (this.completions = new Ea(this._client))
    }
}
;(function (e) {
    e.Completions = Ea
})(J1 || (J1 = {}))
var Z1 = class extends Ne {
    retrieve(a, t) {
        return this._client.get(`/openai/v1/models/${a}`, t)
    }
    list(a) {
        return this._client.get('/openai/v1/models', a)
    }
    delete(a, t) {
        return this._client.delete(`/openai/v1/models/${a}`, {
            ...t,
            headers: { Accept: '*/*', ...t?.headers }
        })
    }
}
Z1 || (Z1 = {})
var Pa = class extends Ne {
    create(a, t) {
        return this._client.post(
            '/openai/v1/audio/transcriptions',
            np({ body: a, ...t })
        )
    }
}
Pa || (Pa = {})
var Na = class extends Ne {
    create(a, t) {
        return this._client.post(
            '/openai/v1/audio/translations',
            np({ body: a, ...t })
        )
    }
}
Na || (Na = {})
var ea = class extends Ne {
    constructor() {
        super(...arguments),
            (this.transcriptions = new Pa(this._client)),
            (this.translations = new Na(this._client))
    }
}
;(function (e) {
    ;(e.Transcriptions = Pa), (e.Translations = Na)
})(ea || (ea = {}))
var E6,
    oe = class extends zr {
        constructor({
            baseURL: a = H2('GROQ_BASE_URL'),
            apiKey: t = H2('GROQ_API_KEY'),
            ...r
        } = {}) {
            if (t === void 0)
                throw new he(
                    "The GROQ_API_KEY environment variable is missing or empty; either provide it, or instantiate the Groq client with an apiKey option, like new Groq({ apiKey: 'My API Key' })."
                )
            let i = { apiKey: t, ...r, baseURL: a || 'https://api.groq.com' }
            if (!i.dangerouslyAllowBrowser && C6())
                throw new he(
                    'This is disabled by default, as it risks exposing your secret API credentials to attackers.\nIf you understand the risks and have appropriate mitigations in place,\nyou can set the `dangerouslyAllowBrowser` option to `true`, e.g.,\n\nnew Groq({ dangerouslyAllowBrowser: true })'
                )
            super({
                baseURL: i.baseURL,
                timeout: i.timeout ?? 6e4,
                httpAgent: i.httpAgent,
                maxRetries: i.maxRetries,
                fetch: i.fetch
            }),
                (this.chat = new J1(this)),
                (this.audio = new ea(this)),
                (this.models = new Z1(this)),
                (this._options = i),
                (this.apiKey = t)
        }
        defaultQuery() {
            return this._options.defaultQuery
        }
        defaultHeaders(a) {
            return {
                ...super.defaultHeaders(a),
                ...this._options.defaultHeaders
            }
        }
        authHeaders(a) {
            return { Authorization: `Bearer ${this.apiKey}` }
        }
    }
E6 = oe
oe.Groq = E6
oe.GroqError = he
oe.APIError = ue
oe.APIConnectionError = C1
oe.APIConnectionTimeoutError = Ca
oe.APIUserAbortError = X1
oe.NotFoundError = lt
oe.ConflictError = st
oe.RateLimitError = ut
oe.BadRequestError = it
oe.AuthenticationError = nt
oe.InternalServerError = ct
oe.PermissionDeniedError = ot
oe.UnprocessableEntityError = mt
var {
    GroqError: s8,
    APIError: m8,
    APIConnectionError: u8,
    APIConnectionTimeoutError: c8,
    APIUserAbortError: f8,
    NotFoundError: h8,
    ConflictError: v8,
    RateLimitError: b8,
    BadRequestError: w8,
    AuthenticationError: _8,
    InternalServerError: y8,
    PermissionDeniedError: g8,
    UnprocessableEntityError: S8
} = L2
;(function (e) {
    ;(e.toFile = z2),
        (e.fileFromPath = Lp),
        (e.Chat = J1),
        (e.Audio = ea),
        (e.Models = Z1)
})(oe || (oe = {}))
var P6 = oe
var n3 = { l3: 'llama3-8b-8192', l370: 'llama3-70b-8192', gemma: 'gemma-7b-it' }
function C8(e) {
    async function a({ start: t, end: r, prompt: i, model: o = 'l3' }) {
        return await new P6({ apiKey: e }).chat.completions.create({
            messages: [
                { role: 'user', content: i },
                { role: 'assistant', content: t }
            ],
            model: n3[o],
            temperature: 0.9,
            max_tokens: 2e3,
            stop: [r]
        }) //.choices[0]?.message?.content) || ''
    }
    return { llm: a }
}
export { C8 as default }
/*! Bundled license information:

web-streams-polyfill/dist/ponyfill.mjs:
  (**
   * @license
   * web-streams-polyfill v4.0.0-beta.3
   * Copyright 2021 Mattias Buelens, Diwank Singh Tomer and other contributors.
   * This code is released under the MIT license.
   * SPDX-License-Identifier: MIT
   *)

formdata-node/lib/esm/blobHelpers.js:
  (*! Based on fetch-blob. MIT License. Jimmy Wärting <https://jimmy.warting.se/opensource> & David Frank *)

formdata-node/lib/esm/Blob.js:
  (*! Based on fetch-blob. MIT License. Jimmy Wärting <https://jimmy.warting.se/opensource> & David Frank *)

humanize-ms/index.js:
  (*!
   * humanize-ms - index.js
   * Copyright(c) 2014 dead_horse <dead_horse@qq.com>
   * MIT Licensed
   *)

web-streams-polyfill/dist/ponyfill.es2018.js:
  (**
   * @license
   * web-streams-polyfill v3.3.3
   * Copyright 2024 Mattias Buelens, Diwank Singh Tomer and other contributors.
   * This code is released under the MIT license.
   * SPDX-License-Identifier: MIT
   *)

node-domexception/index.js:
  (*! node-domexception. MIT License. Jimmy Wärting <https://jimmy.warting.se/opensource> *)
*/
